...
/Users/tomcranstoun/Documents/GitHub/MX-hub/mx-outputs/audit/2026-05-26/mx.allabout.network/mx-allabout-network-report.md - 9 placeholder leaks at lines: 83(116), 800([X]), 801([X]), 802([X]), 803([X]), 804([X]), 805([X]), 806([X])…. Each one indicates an infill handler that did not match its template anchor, or a handler that returned empty for an expected field.
...
/Users/tomcranstoun/Documents/GitHub/MX-hub/mx-outputs/audit/2026-05-26/mx.allabout.network/mx-allabout-network-report.md - 17 placeholder leaks at lines: 79(116), 79([X]), 79([X]), 79([X]), 79([X]), 79([X]), 79([X]), 79([X])…. Each one indicates an infill handler that did not match its template anchor, or a handler that returned empty for an expected field.
...
Section sanity: mx-allabout-network-report.md: 1 issue(s)
  ...
  leaked-template-marker (1):
    line 83 - section "Warnings (rule violations)"
      Unprocessed template marker "[IF SDQ_SCORE >= 76: "Across the 116" survived infill - every IF/ELSE/ENDIF must be consumed before the report ships.
