...
Every automated gate ran to completion; this section surfaces 3 findings (1 error, 2 warnings) for the human reviewer to read, accept, or rebut before sign-off. Each entry names the gate that raised it, the severity, and the supporting evidence.

### Errors (I/O or structural shortfalls)

| # | Gate | Category | Finding | Recorded |
|---|------|----------|---------|----------|
| 1 | check-report-coherence.js | placeholder-leak | 34 unresolved placeholder token(s) leaked into the final report: NUMBER_OF_PAGES_AUDITED, X, N, PLACEHOLDER | 2026-05-27T06:14:50Z |

<details><summary>Error detail (1)</summary>

**1. check-report-coherence.js - 34 unresolved placeholder token(s) leaked into the final report: NUMBER_OF_PAGES_AUDITED, X, N, PLACEHOLDER**

/Users/tomcranstoun/Documents/GitHub/MX-hub/mx-outputs/audit/2026-05-26/mx.allabout.network/mx-allabout-network-report.md - 34 placeholder leaks at lines: 80([NUMBER_OF_PAGES_AUDITED]), 80([X]), 80([X]), 80([X]), 80([X]), 80([X]), 80([X]), 80([X])…. Each one indicates an infill handler that did not match its template anchor, or a handler that returned empty for an expected field.
...
| # | Gate | Category | Finding | Recorded |
|---|------|----------|---------|----------|
| 1 | sample-vs-total-scope | scope-mis-statements | Scope mis-statements remain after auto-repair: 1 | 2026-05-27T06:13:41Z |
| 2 | check-recommendation-consistency.js | missing-in-engagement | Priority 2 not represented in Engagement phases | 2026-05-27T06:14:51Z |

<details><summary>Warning detail (2)</summary>
...
check-report-scope: /Users/tomcranstoun/Documents/GitHub/MX-hub/mx-outputs/audit/2026-05-26/mx.allabout.network/mx-allabout-network-report.md
  ...
  [sampling-inside-sitewide-section] line 565
    section: ## Discovery Files  (line 536)
...
**2. check-recommendation-consistency.js - Priority 2 not represented in Engagement phases**
...
We found no llms.txt reachable on mx.allabout.network, meaning machines that consult this discovery file to understand the site's scope, content inventory, and permitted uses arrive at a dead end. We recommend adding llms.txt with a site description, page inventory, and content policy to give machines a structured entry point.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We find no llms-full.txt reachable on mx.allabout.network, with the endpoint returning a 404 across every discovery check. Whether adding it would be worthwhile depends on the actual depth of content across the full site, which the audited sample alone does not resolve.
