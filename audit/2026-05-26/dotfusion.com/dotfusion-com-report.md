...
reportId: "dotfusion-com-WEB-AUDIT-20260526"
...
mx:
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-05-26/dotfusion.com/dotfusion-com-report.pdf"
...
**Date:** 26 May 2026\
**Report ID:** dotfusion-com-WEB-AUDIT-20260526
...
Every automated gate ran to completion; this section surfaces 7 findings (1 error, 5 warnings, 1 info) for the human reviewer to read, accept, or rebut before sign-off. Each entry names the gate that raised it, the severity, and the supporting evidence.
...
<details><summary>Warning detail (5)</summary>
...
<details><summary>Info detail (1)</summary>
...
We audited seven pages of dotfusion.com and found a site that presents well for human visitors. Performance stands as the strongest dimension across the audited set, and an SEO score of 86/100 confirms that the technical foundations search engines depend on are firmly in place. The content structure is coherent, the Next.js platform delivers pages efficiently, and the overall experience reflects a team that has invested seriously in what human visitors see and feel.

Before turning to machine-readiness, we want to name accessibility as a Priority 1 compliance item. Across the audited set, we recorded 68 raw instances of WCAG AA issues spanning 38 distinct issue types, with 68 instances flagged as critical by Pa11y. Of those instances, 43 trace to 9 recurring template patterns, which means a single corrective edit per pattern has the potential to resolve a large share of the raw instance count in one pass. That ratio makes the remediation effort proportionate and the compliance trajectory achievable. The headline opportunity beyond accessibility is machine readiness. Discovery Readiness scores 25/100 across the audited set, and while Structured Data Quality and Machine Suitability scores already clear the numeric thresholds for Citation-ready status, dotfusion.com currently sits at Level 1 (Discoverable) because no MX-namespaced governance metadata was detected on the audited pages. Machines can discover and parse the content today, but they cannot yet cite it as an attested source. The path to Level 2 is concrete: add full MX fields, governance, and provenance metadata so agents can cite as well as discover, and raise Machine Suitability above 60 and Discovery Readiness above 40.
...
Across the audited set, dotfusion.com delivers strong performance and SEO, with accessibility the clearest area for further development.
...
Across the audited set, machines can discover and parse dotfusion.com's content reliably, though the current MX Readiness Level of 1 (Discoverable) means citation capability as an attested source remains an opportunity still to unlock.
...
Across the audited set, dotfusion.com demonstrates strong foundations of technical care, with SEO scoring well, a meaningful range of structured data types already deployed, and HTTPS with HSTS in place as a starting point for security. These strengths provide the groundwork for the targeted improvements that follow.
...
We have prioritised the findings below by the degree to which each gap blocks machines from discovering, parsing, and ultimately acting on content across the audited set. Discovery gaps lead because they constrain everything downstream; structured data and catalogue visibility follow because they determine how much of dotfusion.com's offering machines can surface and interpret.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Unlabelled header anchors present on all 7 audited pages, WCAG 4.1.2 | Compliance Risk | High | Low | Screen reader users may miss navigation links in the site header |
| 2 | reCAPTCHA textarea has no accessible name or label, WCAG 4.1.2 and 1.3.1, vendor: cdn-ca.aglty.io | Compliance Risk | High | Medium | Screen reader users may miss the CAPTCHA field and are less likely to complete contact forms |
| 3 | Unlabelled form inputs (#name, #company) on 5 of 7 audited pages, WCAG 4.1.2 | Compliance Risk | High | Low | Screen reader users may miss the name and company fields and risk missing their purpose |
| 4 | Semantic structure score 49/100, worst page: /services/contentful-development-agency (23 of 45 bare divs) | Compliance Risk | Medium | Medium | Assistive technologies and machines are less likely to interpret page structure correctly |
| 5 | Security headers: only 2 of 5 present (HTTPS, HSTS) across all 7 audited URLs | Cross-cutting | High | Low | Browsers and security scanners may flag the absence of CSP, X-Frame-Options, and X-Content-Type-Options |
| 6 | Discovery Readiness: 25/100 (Needs Improvement), llms-full.txt, agent-card.json, ai.txt, and humans.txt absent | AI Opportunity | High | Medium | Machines may miss structured permission signals and reduce agent confidence in citing dotfusion.com |
| 7 | MX Stack Completeness: 44/100 (Could Be Better), MX governance fields absent from page frontmatter | AI Opportunity | Medium | Medium | Machines are less likely to attribute content to the correct audience, status, or canonical source |
| 8 | Structured Data Quality: 63/100 (Good), no sameAs or legalName on Organisation entities | AI Opportunity | Medium | Low | Machines may treat dotfusion.com organisation identity as unverified and reduces agent confidence in cross-source attribution |

---

**Priority 1: Unlabelled Header Anchors, WCAG 4.1.2 (7 of 7 Audited Pages)**

**Bucket:** Compliance Risk

**Finding:** Across the audited set, at least one anchor element in the site header carries a valid href but supplies no link content, text, image alt text, or aria-label on any of the audited pages. Screen reader users encounter a link with no announced purpose, which constitutes a WCAG 4.1.2 breach. Because the pattern originates in the shared header template, a single template edit resolves all instances across the audited set.

**What to change and why:**
- Supply a descriptive accessible name for each affected anchor in the site header, either as visible text, an aria-label, or an aria-labelledby reference. This directly addresses WCAG 4.1.2 (Name, Role, Value) and removes the most widespread single template gap in the audited set.
- Review the header template for any icon-only or logo-only links that may share the same pattern; resolving them together prevents recurrence on pages outside the current sample.
- After remediation, re-run Pa11y against the header component to confirm zero remaining instances of this selector pattern before wider deployment.

**Effort:** Low

---

**Priority 2: reCAPTCHA Textarea Has No Accessible Name or Label, WCAG 4.1.2 and 1.3.1, Vendor cdn-ca.aglty.io (7 of 7 Audited Pages)**

**Bucket:** Compliance Risk

**Finding:** Across the audited set, the reCAPTCHA textarea rendered as `#g-recaptcha-response-100000` carries no name available to an accessibility API and no associated label element, breaching both WCAG 4.1.2 and WCAG 1.3.1. This element is injected at runtime by the third-party SDK hosted at cdn-ca.aglty.io and does not exist in dotfusion.com's own template; a theme edit alone will not resolve it. Screen reader users attempting to complete any contact form may miss the CAPTCHA field entirely.

**What to change and why:**
- Engage the vendor responsible for the SDK hosted at cdn-ca.aglty.io to ship an updated version that supplies a proper accessible name and label for the injected textarea. This is the cleanest fix path and addresses both WCAG 4.1.2 and 1.3.1 at the source.
- As an interim measure, apply a DOM-observer patch that adds the missing accessible name to the `#g-recaptcha-response-100000` element after injection; this resolves the audit finding immediately while the vendor fix is in progress.
- Document the patch in the codebase with a clear comment referencing the vendor ticket, so it can be removed cleanly once the SDK upgrade is confirmed.

**Effort:** Medium

---

**Priority 3: Unlabelled Form Inputs (#name and #company), WCAG 4.1.2 (5 of 7 Audited Pages)**

**Bucket:** Compliance Risk

**Finding:** On 5 of the 7 audited pages, the `#name` and `#company` form inputs each lack an accessible name available to an accessibility API. There is no associated label element, title attribute, aria-label, or aria-labelledby present on either field. Screen reader users may miss the purpose of these fields and risk missing what data to enter, constituting a WCAG 4.1.2 breach. Because these inputs appear in a shared form component, a single template correction is expected to resolve both fields across all affected pages in the audited set.

**What to change and why:**
- Add an explicit accessible name to both the `#name` and `#company` inputs using a label element, title attribute, aria-label, or aria-labelledby reference. This directly satisfies WCAG 4.1.2 and removes the barrier for screen reader users completing contact or enquiry forms.
- Audit any other form fields in the same shared component for similar gaps; resolving them in one pass avoids piecemeal rework.
- Verify the fix with Pa11y against at least two of the five affected pages before deployment to confirm the selector pattern no longer triggers.

**Effort:** Low

---

**Priority 4: Semantic Structure Score 49/100, Worst Page /services/contentful-development-agency (23 of 45 Bare Divs)**

**Bucket:** Compliance Risk

**Finding:** We score semantic structure at 49/100 across the audited set. The worst-performing page in our sample, https://dotfusion.com/services/contentful-development-agency, carries 23 bare divs out of 45 total measured elements. Where this structural pattern is shared across the audited set via a common template, the concern extends beyond a single page. Assistive technologies rely on semantic elements to convey document structure; a high proportion of bare divs means screen reader users and machines are less likely to interpret sections, headings, and landmarks correctly.

**What to change and why:**
- Review the layout structure on https://dotfusion.com/services/contentful-development-agency and replace bare divs that serve as section containers, navigation regions, or content groupings with appropriate semantic elements. This improves how assistive technologies and machines traverse the page, directly moving the semantic structure score.
- Where the pattern originates in a shared template component, applying the fix there will improve structure across every page that uses the same component, not only the worst-case page.
- After updating the template, re-measure the bare-div ratio on the /services/contentful-development-agency page specifically to confirm the score moves above the 70/100 threshold.

**Effort:** Medium

---

**Priority 5: Security Headers, Only 2 of 5 Present Across All 7 Audited URLs (HTTPS, HSTS)**

**Bucket:** Cross-cutting

**Finding:** Across all 7 audited URLs, only HTTPS and HSTS are present from the expected set of five security headers. The three absent headers mean browsers receive no Content Security Policy instruction, no X-Frame-Options directive, and no X-Content-Type-Options declaration from dotfusion.com. Security scanners and enterprise procurement tools flag these gaps, and their absence reduces the protective signal available to both browsers and automated security assessors.

**What to change and why:**
- Add a Content Security Policy header to define which sources may load scripts, styles, and other resources; its absence is the most consequential gap for browser-level protection and is frequently flagged in client-side security audits.
- Add X-Frame-Options to prevent the pages from being embedded in third-party frames without consent; this mitigates clickjacking risk and is a straightforward server or CDN configuration change.
- Add X-Content-Type-Options with the value nosniff to prevent browsers from MIME-type sniffing responses; this is a low-effort, high-signal header that security scanners check by default.

**Effort:** Low

---

**Priority 6: Discovery Readiness 25/100 (Needs Improvement), llms-full.txt, agent-card.json, ai.txt, and humans.txt Absent**

**Bucket:** AI Opportunity

**Finding:** We score Discovery Readiness at 25/100 across the audited set. While llms.txt is present, the four remaining well-known discovery artefacts (llms-full.txt, agent-card.json, ai.txt, and humans.txt) are absent from dotfusion.com. Machines that follow the discovery chain beyond the base llms.txt file find no extended content index, no declared agent capabilities, no crawl permission policy, and no human team attribution. This reduces agent confidence in the completeness and authority of the site as a citation source.

**What to change and why:**
- Add llms-full.txt to provide machines with a richer content index beyond what the base llms.txt surface carries; this directly extends machine-readable discoverability and is the next logical step given llms.txt is already present. Note that whether the full 148-page inventory warrants the extended file is a judgement call the audited sample alone cannot resolve; we recommend reviewing content depth across the full URL set before authoring the file.
- Add agent-card.json to declare the capabilities and identity of any agent surfaces associated with dotfusion.com; machines encountering this file gain a structured permission and identity signal that increases confidence in the source.
- Add ai.txt to communicate crawl and usage permissions to machine clients explicitly; its absence means automated systems must infer permission rather than read it, which reduces agent confidence.
- Add humans.txt to provide team attribution metadata; while lower in urgency than the other three artefacts, its presence completes the standard discovery set and contributes to the MX Stack Completeness score alongside the other missing artefacts.

**Effort:** Medium

---

**Priority 7: MX Stack Completeness 44/100 (Could Be Better), MX Governance Fields Absent from Page Frontmatter**

**Bucket:** AI Opportunity

**Finding:** We score MX Stack Completeness at 44/100 across the audited set. Beyond the discovery artefact gaps addressed in Priority 6, the pages we audited carry no MX governance fields in their frontmatter: canonicalUri, contentType, audience, and status are all absent. Machines that parse page metadata to determine the authoritative URL, content type, intended audience, and publication status must infer or omit these signals, making dotfusion.com content less reliably attributable in agent-generated answers.

**What to change and why:**
- Add canonicalUri to page frontmatter so machines and indexers receive an unambiguous pointer to the authoritative version of each page; this reduces the risk of fragmented attribution across duplicate or near-duplicate URLs.
- Add contentType to frontmatter to allow machines to classify pages correctly (for example, as a service page, case study, or resource); classification improves the relevance signal machines assign when selecting sources.
- Add audience to frontmatter so machines can match pages to the correct reader segment; its absence means agents may present dotfusion.com content in contexts misaligned with the intended audience.
- Add status to frontmatter to signal whether a page is current, archived, or in draft; machines that cannot read a status field may surface outdated content with the same confidence as current content.

**Effort:** Medium

---

**Priority 8: Structured Data Quality 63/100 (Good), No sameAs or legalName on Organisation Entities**

**Bucket:** AI Opportunity

**Finding:** We score Structured Data Quality at 63/100 across the audited set. The schema vocabulary is broad, with Organisation, WebSite, BreadcrumbList, and a range of service and FAQ types already present. The gap we identify is on the Organisation entities: neither sameAs nor legalName properties are populated. Without sameAs, machines cannot cross-reference dotfusion.com against authoritative external sources such as Wikidata or Companies House entries. Without legalName, the formal identity of the business is unavailable to machines attempting to verify or attribute the organisation.

**What to change and why:**
- Add sameAs to the existing Organisation entity, linking to any authoritative external profiles (such as a Wikidata entry, a LinkedIn company page, or a recognised business registry record); this is the primary mechanism machines use to confirm that multiple data sources refer to the same real-world entity, directly improving cross-source attribution confidence.
- Add legalName to the existing Organisation entity to supply the formal registered name of the business alongside the trading name; machines that encounter both properties are better positioned to confirm organisational identity during verification passes.
- Both properties are low-effort additions to a schema block that already exists; the structural lift is minimal and the improvement to machine-readable identity verification is direct.
...
- **sameAs links on Organisation**: adding `sameAs` properties to the Organisation entities already present across the audited set connects dotfusion.com to its Wikidata, LinkedIn, or other public authority records, giving machines an attested identity anchor they can carry into citations and knowledge-graph entries.

- **potentialAction on Organisation**: attaching a `potentialAction` (such as a contact or quote-request action) to the existing Organisation entities advertises dotfusion.com's contact capabilities directly to machines, enabling them to surface interaction pathways without parsing page prose.
...
The robots.txt file is present and declares no disallow paths, meaning machines face no crawl restrictions across any part of dotfusion.com. It also announces one sitemap reference, giving crawlers a clear entry point to the full URL inventory.
...
We grade dotfusion.com's sitemap as Partial: it covers 148 URLs and carries both lastmod and changefreq values, yet the absence of priority attributes leaves machines without the relative crawl-weighting signals that help them allocate attention across the full URL set.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We find llms.txt present and carrying a site description, which gives machines a useful starting point, but the file currently lacks both a page inventory and a content policy, and we recommend extending it to include both so machines can navigate and respect usage boundaries with confidence.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We find no llms-full.txt on dotfusion.com, with the endpoint returning a 404 and the file absent from both the sitemap and the homepage head. Whether adding it would be worthwhile depends on the depth of content across the full 148-page inventory, a question the audited sample alone cannot resolve.
...
| Probed at | 2026-05-26T20:25:01.956Z |
...
| ---------------- | ------ | ---------------------- |
| Truncation Risk | Fail · 2/7 | 2 page(s) flag for truncation risk; 2 of them exceed the 250 KB hard ceiling, the rest place main content too far into the document. Agents with limited fetch windows may stop reading before reaching the main content. **Data:** Largest page: 471 KB. Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows. See dotfusion-com-pipeline-truncation-risk-pages.csv (2 pages). |
| SPA Shell | Pass · 7/7 | Served HTML matches rendered HTML - no JavaScript is required for content. Server-side agents see the same content a browser does. **Data:** Max gap score: 8. 0 means served and rendered match. |
| Soft 404 | Fail · 0/7 | The server returns HTTP 200 for non-existent paths rather than a proper HTTP 404 status, as confirmed by the Error Page Test. Agents and crawlers that rely on status codes to determine resource existence may be misled. **Data:** Soft-404 behaviour detected on non-existent paths; see Error Page Test section. |
| Boilerplate Burial | Pass · 7/7 | Navigation and chrome do not dominate the page; main content is reachable without wading through overhead. **Data:** Highest boilerplate-to-content ratio: 0.05. Threshold: < 10 (and < 80 KB of inline head bytes). |
...
| Body Content Ratio | Fail · N/M | Prose content averages only 9% of served bytes. Scripts, styles, and images dominate; agents get little signal per byte. **Data:** Average: 9%. Threshold: 30%. |
| Inline Tag Bloat | Pass · 7/7 | No `<style>` or `<script>` block exceeds the 500-byte threshold on any page. Head stays lean for agents that read head-first. **Data:** 0 element(s) > 500 bytes. Largest single-page inline CSS block: 0 B. Largest single-page inline JS block: 0 B. |
...
Across the audited set, pipeline survivability scores at 97/100. The resilience checks we flag are Truncation Risk, which affects two of the seven pages we reviewed, and the soft-404 behaviour documented in the Error Page Test. When content is liable to truncation, machines reading those pages may receive an incomplete picture, losing context that influences how they interpret or relay the content. Addressing the length and structure of content on those two pages would close the truncation gap and bring full resilience consistency across the audited set.
...
On the worst-performing page we sampled, https://dotfusion.com/services/contentful-development-agency, the rendered surface shows 23 of 45 elements as bare divs (51%), which means machines lose structural context and fall back on positional inference to determine meaning. The pattern is surface-wide rather than deeply chained, a shallow deepest chain of 4 alongside a high bare ratio typically points to a utility-class framework where layout is assembled from generic containers rather than semantically typed components. The most direct first move is to wrap the obvious landmarks (header, nav, main, footer, aside) so that structural intent is declared explicitly, which would bring the bare-div ratio down without requiring any restructuring of the existing layout.
...
Accessibility legislation has converged on ISO 14289-1 (PDF/UA) as the technical baseline across major markets, with the EAA (Directive (EU) 2019/882, in force 28 June 2025) representing the most precisely codified instance of a global alignment that extends equally to Section 508, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent frameworks in Australia and Canada. The machine-readability concern is equally independent: an untagged or image-based PDF is opaque to search crawlers, AI systems, and automated pipelines in the same way that unsemantic markup is, whereas a tagged PDF with a proper structure tree exposes text, entities, and hierarchy that machines can read and act upon.
...
1. **Address Priority 1 findings**: we recommend resolving the 68 WCAG 2.1 AA accessibility issues identified across the audited pages, prioritising the 9 recurring template patterns that account for 43 of those instances (regulatory exposure)
2. **Review Priority 2-3 findings**: we recommend working through the Discovery Readiness gaps (currently 25/100) and MX Stack Completeness gaps (currently 44/100), including adding the absent llms-full.txt, agent-card.json, and ai.txt artefacts, as these compound over time
3. **Consider optional enhancements**: we recommend reviewing the optional patterns listed in Priority 6-8 and the Optional Enhancements section, including sameAs and legalName on Organisation entities, which offer an early-mover opportunity as agent-mediated discovery matures
...
Across the audited set, https://dotfusion.com performs with confidence on SEO, scoring 86/100, with strong foundations that serve human visitors well. Discovery Readiness at 25/100 and Structured Data at 63/100 represent the clearest opportunities to extend that reach to machines, through improved discovery artefacts and richer metadata. We invite the https://dotfusion.com team to work through the findings that follow and take the next step toward a more machine-ready presence.
...
- **`/services/contentful-development-agency`**: SEO 82 · A11y 70 · Back 85 · Served 100 · Rendered 100
...
Across the audited set, we sampled 89 images in total. The format breakdown sits at 38 SVG, 27 PNG, 3 WebP, and 21 images in other or unrecognised formats, with no JPEG images present. Alt-text coverage reaches 62 of 89 images (69.7%), leaving 27 images without descriptive text. For a developer audience, those 27 gaps are worth addressing methodically: decorative images need an empty alt attribute to be correctly ignored by screen readers, while informative images need meaningful descriptions.

On loading strategy, 19 images carry an explicit loading="lazy" attribute and none carry loading="eager". The remaining 70 images have no loading attribute set at all. That last group is worth calling out separately: the absence of a loading attribute is not equivalent to loading="eager". Without an explicit declaration, the browser applies its own heuristics, typically loading images above the fold eagerly and deferring others based on viewport estimates, but the behaviour is not guaranteed to match what a developer might assume. Explicit attribution across the full image set gives the browser a clear instruction and removes the ambiguity, particularly for images that sit near the fold where heuristics are least reliable.
