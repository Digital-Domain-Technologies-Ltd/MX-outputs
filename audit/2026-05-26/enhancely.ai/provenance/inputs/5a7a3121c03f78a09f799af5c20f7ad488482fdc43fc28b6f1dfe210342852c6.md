...
- Pages audited: 7 of 51 in sitemap - sample run
- Date: 2026-05-26
- Scores: SEO 84/100 | Accessibility 89/100 | Discovery Readiness 40/100 | Structured Data 87/100
- Scope: 7 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): Question (37), Answer (37), ListItem (34), BreadcrumbList (14), Organization (14), Person (12), WebPage (7), WebSite (7), FAQPage (7), Article (4), BlogPosting (3), ImageObject (1)
...
- Accessibility: 16 distinct WCAG AA issue types (53 raw instances, 53 marked critical by Pa11y) - 48 of these instances trace to 11 recurring template patterns (single theme edit per pattern fixes all instances)
...
- Next-level action: Score thresholds for Citation-ready are met (MSC 68, SDQ 87, DR 40), but no MX-namespaced governance metadata was detected on the audited pages. Add MX governance fields (mx:status, mx:contentType, mx:audience, canonicalUri, and provenance markers) to at least one published page to unlock Citation-ready. Without those fields a machine can discover the page but cannot cite it as an attested source.
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- Performance: 3632ms avg (Good)
- Accessibility: 89/100 (Excellent)
- SEO: 84/100 (Excellent)
- Accessibility issues: 16 distinct WCAG codes across 53 raw instances (48 instances trace to 11 template patterns)
-->
...
- Accessibility: 89/100 (16 distinct WCAG codes, 53 raw instances) - 48 of these instances trace to 11 template-level patterns
- Security headers: 3/5 present (HTTPS, X-Frame-Options, X-Content-Type-Options); 0 of 7 audited URLs carry all five
- Structured Data Quality: 87/100
- Consistency: 53%
-->
...
Div Soup (Semantic Structure): rendered score 49/100, band high. When the rendered score is below 70, include this as a numbered Priority row. The figures 44 bare divs out of 103 total describe the WORST-CASE page in the audited set (https://www.enhancely.ai/blog/schema-markup-explained-the-invisible-code-that-makes-your-website-visible-to-ai), not a site-wide average. The finding title and the Finding paragraph must scope to that worst page; do NOT present these numbers as if they hold across every audited page. The title should include the rendered score, the worst-page URL, and the bare-divs-of-total figure (for example, a title naming the score, the page path, and the figure such as "95 of 153 bare divs"). In the Finding paragraph, name the worst page URL explicitly and note that the figures cited come from it specifically; if the band still applies because most pages share the template, say so.

Separate Priority blocks with `---`.

FACTS:
- Pa11y data for this audit:  Of these, 48 (91%) trace to 11 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.

Pa11y findings for this audit (use ONLY these for specific recommendations):
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.67:1. Recommendation:  change background to #000. | selector: `html > body > footer > div:nth-child(1) > div:nth-child(1) >…` | 6 instances on 6 pages | affects: low-vision users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.67:1. Recommendation:  change background to #000. | selector: `html > body > footer > div:nth-child(1) > div:nth-child(1) >…` | 6 instances on 6 pages | affects: low-vision users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.67:1. Recommendation:  change background to #000. | selector: `html > body > footer > div:nth-child(2) > span:nth-child(1)` | 6 instances on 6 pages | affects: low-vision users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.67:1. Recommendation:  change background to #000. | selector: `html > body > footer > div:nth-child(2) > span:nth-child(2)` | 6 instances on 6 pages | affects: low-vision users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.7:1. Recommendation:  change background to #1f1f1f. | selector: `#content > section:nth-child(1) > div > div > div:nth-child(…` | 6 instances on 6 pages | affects: low-vision users
- Div Soup rendered score: 49/100, band high
- Div Soup bare divs: 44 of 103 (figures from the WORST page only)
- Div Soup worst page URL: https://www.enhancely.ai/blog/schema-markup-explained-the-invisible-code-that-makes-your-website-visible-to-ai
- Div Soup audited page count: 7

When a Pa11y finding row in  Of these, 48 (91%) trace to 11 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 48 (91%) trace to 11 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.67:1. Recommendation:  change background to #000. | selector: `html > body > footer > div:nth-child(2) > span:nth-child(1)` | 6 instances on 6 pages | affects: low-vision users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.67:1. Recommendation:  change background to #000. | selector: `html > body > footer > div:nth-child(2) > span:nth-child(2)` | 6 instances on 6 pages | affects: low-vision users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.7:1. Recommendation:  change background to #1f1f1f. | selector: `#content > section:nth-child(1) > div > div > div:nth-child(…` | 6 instances on 6 pages | affects: low-vision users above.
...
| Organization | 7 | 100% | 100% | ImageObject |
| Person | 6 | 100% | 100% | - |
| WebPage | 7 | 100% | 100% | BreadcrumbList, Organization, WebSite, Person |
| WebSite | 7 | 100% | 25% | Organization |
| FAQPage | 7 | 100% | 100% | - |
| Article | 4 | 92% | 100% | Organization, WebPage, Person |
| BlogPosting | 3 | 100% | 100% | Person, Organization, WebPage |
...
- Rendered score: 49, band high (worst page: https://www.enhancely.ai/blog/schema-markup-explained-the-invisible-code-that-makes-your-website-visible-to-ai)
- Bare divs (rendered): 44 of 103 (43%)
- Deepest bare chain (rendered): 5
- Top bare selectors (rendered): `div.k-block.k-block__text` (59), `div.mobile-sub` (42), `div.max-w-[1440px].mx-auto` (41), `div.faq-a.text-[0.92rem]` (22), `div.flyout-panel` (21)
...
2 of the five standard security headers are absent across every audited response: HSTS (Strict-Transport-Security), Content-Security-Policy (CSP). Adding these at the origin-server or CDN edge closes the corresponding attack surfaces without touching application code.
...
| Full Optimization | Catalogue Visibility, Discovery Readiness, Semantic Structure, MX Stack Completeness, Performance, Security headers, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
WRITE:
2-3 sentences summarising the audit findings for Enhancely (https://www.enhancely.ai). Always refer to the site by its domain (https://www.enhancely.ai), never by the consultancy name. The highest-scoring dimension is Structured Data at 87/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 84/100; Structured Data: 87/100; Discovery Readiness: 40/100
...
- **`/blog/e-e-a-t-for-the-ai-era-how-chatgpt-co-recognize-your-brand-as-a-trustworthy-source`**: SEO 83 · A11y 90 · Back 70 · Served 100 · Rendered 100
...
- With alt text: 98 (74.8%)
- Missing alt text: 33
- loading="lazy": 41
- loading="eager": 0
- no loading attribute: 90
- JS Lazy Pattern instances: 41
- Double Lazy instances: 41
