...
We identified a URL variant cluster across the audited set where the sitemap includes both https://www.dkd.de/de and https://www.dkd.de/de/ as separate entries, producing nine crawled URLs before deduplication against eight unique canonical pages, an inflation factor of 1.1×. Machines that do not normalise URLs will fetch the same content multiple times, wasting token budget and risking contradictory findings; we recommend consolidating sitemap.xml to one canonical form per resource and adding a canonical link tag to each page.
