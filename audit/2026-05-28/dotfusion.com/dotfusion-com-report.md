...
reportId: "dotfusion-com-WEB-AUDIT-20260528"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-05-28/dotfusion.com/dotfusion-com-report.md
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-05-28/dotfusion.com/dotfusion-com-report.pdf"
...
**Date:** 28 May 2026\
**Report ID:** dotfusion-com-WEB-AUDIT-20260528
...
We audited seven pages of dotfusion.com and found a site that presents well to human visitors. The SEO foundations are solid, scoring 86/100 in the Excellent band, which means search crawlers can discover, index, and rank the content with confidence. The Next.js platform delivers a technically coherent base, and the breadth of structured markup already in place across the audited set reflects genuine investment in content quality.

The headline opportunity is machine readiness. Before we turn to that, we want to name accessibility as a Priority 1 compliance item: we recorded 68 instances of WCAG AA issues across 6 distinct issue types, and 45 of those instances trace to 9 recurring template patterns, meaning a single theme-level edit per pattern can clear the majority of raw instances without touching each page individually. That is a concentrated remediation effort with wide impact. On the machine-readiness side, dotfusion.com currently sits at MX Readiness Level 1 (Discoverable), which means machines can find and parse the content but cannot yet cite the site as an attested source. The numeric scores for Structured Data Quality and Discovery Readiness already approach the thresholds for Level 2, yet the actual lever is not raising those scores further; it is adding MX governance fields, including mx:status, mx:contentType, mx:audience, canonicalUri, and provenance markers, none of which we detected across the audited set. Add full MX fields, governance, and provenance metadata so agents can cite as well as discover, and raise Machine Suitability above 60 and Discovery Readiness above 40.

Because dotfusion.com is built on Next.js, rendering behaviour can influence what machines read at fetch time versus what a browser assembles after script execution. Regardless of that rendering context, Schema.org JSON-LD is the highest-leverage asset available, because every agent can read it directly from the served response without executing JavaScript. The groundwork is there; the next step is extending that structured layer with the governance signals that transform discoverability into citation readiness.
...
Across the audited set, dotfusion.com delivers a strong experience for human visitors in performance and SEO, with accessibility representing the clearest area for improvement.
...
Across the audited set, machines can discover and parse dotfusion.com pages with high pipeline reliability, though structured data depth and discovery readiness leave meaningful room to grow before citation-ready status is within reach.

| Dimension | Score | Rating | Grade | vs Peers |
|-----------|-------|--------|-------|----------|
| Discovery Readiness | 25/100 | Needs Improvement | D | D (median) |
| Structured Data Quality | 63/100 | Good | B | C (median) |
| MX Stack Completeness | 44/100 | Could Be Better | C | C (median) |
...
Across the audited set, dotfusion.com demonstrates a strong foundations to build on, with SEO scoring at 86/100 and a rich structured data vocabulary already in place across multiple schema types. These strengths give the groundwork for the targeted improvements we outline in the sections ahead.

| Dimension | Score | Highlights |
|-----------|-------|------------|
| Performance | Good | Good - 3154ms average load time |
...
We present the findings below as prioritised opportunities, ordered by downstream impact across the audited set of 7 pages. Discovery gaps lead the sequence because they block machines from locating and indexing content before any other signal can take effect.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Unlabelled Header Anchors, WCAG 4.1.2 (7 pages, 7 instances) | Compliance Risk | High | Low | Screen reader users may miss navigation links and fail to traverse the page header |
| 2 | Unlabelled reCAPTCHA Textarea, WCAG 4.1.2 and 1.3.1 (7 pages, 7 instances each) | Compliance Risk | High | Low | Screen reader users may be unable to identify or complete the reCAPTCHA field |
| 3 | Unlabelled Form Inputs, WCAG 4.1.2 (name and company fields, 5 pages, 5 instances each) | Compliance Risk | High | Low | Screen reader users risk missing the purpose of contact form fields and may be unable to complete them |
| 4 | Semantic Structure 49/100, dotfusion.com/services/contentful-development-agency (23 of 45 bare divs) | Compliance Risk | Medium | Medium | Screen reader users and machines are less likely to interpret page structure correctly on layout-heavy templates |
| 5 | Security Headers Incomplete (2 of 5 present, missing X-Frame-Options, CSP, X-Content-Type-Options) | Cross-cutting | High | Low | Browsers and security scanners may flag pages as vulnerable; trust signals for enterprise buyers are reduced |
| 6 | Discovery Readiness 25/100, missing llms-full.txt, agent-card.json, ai.txt, humans.txt | AI Opportunity | High | Medium | Machines may miss structured discovery signals and are less likely to cite dotfusion.com as an attested source |
| 7 | MX Stack Completeness 44/100, missing discovery artefacts and MX governance fields | AI Opportunity | Medium | Medium | Machines may have reduced confidence in content provenance and agent-routing accuracy |

---

**Priority 1: Unlabelled Header Anchors, WCAG 4.1.2**

**Bucket:** Compliance Risk

**Finding:** Across the audited set, each page carries at least one anchor element in the site header that has a valid href attribute but supplies no discernible link content. Screen reader users encounter these links without any announcement of their purpose, which constitutes a shortfall of WCAG 4.1.2 (Name, Role, Value). Because the selector traces to the site header template, a single theme edit resolves all seven instances.

**What to change and why:**

- Add descriptive accessible text (visible or visually hidden) to each header anchor that currently has no link content; this gives assistive technology a name to announce and satisfies WCAG 4.1.2, moving the Accessibility score toward the 80+ band.
- Audit the header partial for any additional icon-only or image-only links that may share the same gap; resolving the template once removes all instances across the audited set in a single deployment.
- After the fix, rerun Pa11y against the header selector to confirm zero remaining WCAG 4.1.2 violations in that component.

**Effort:** Low

---

**Priority 2: Unlabelled reCAPTCHA Textarea, WCAG 4.1.2 and 1.3.1**

**Bucket:** Compliance Risk

**Finding:** The reCAPTCHA response field (selector `#g-recaptcha-response-100000`) appears on all seven audited pages and carries two distinct WCAG violations: the textarea element has no name available to an accessibility API (WCAG 4.1.2), and the form field has no label of any kind (WCAG 1.3.1). This element is injected at runtime by the reCAPTCHA SDK hosted at cdn-ca.aglty.io and does not exist in the site's own template; a theme edit alone will not resolve it.

**What to change and why:**

- Engage the vendor responsible for the SDK hosted at cdn-ca.aglty.io to ship an updated SDK version that includes a valid accessible name on the reCAPTCHA textarea; this is the cleanest path and resolves the WCAG 4.1.2 and 1.3.1 violations at source.
- Where an SDK upgrade is not immediately available, implement a DOM-observer patch that adds the missing label or aria-label to the injected field after rendering; this is a documented interim remediation path and prevents screen reader users from encountering an unnamed field in the interim.
- Verify after either fix that assistive technology correctly announces the reCAPTCHA field purpose, confirming both WCAG 4.1.2 and 1.3.1 are satisfied.

**Effort:** Low

---

**Priority 3: Unlabelled Form Inputs, WCAG 4.1.2 (name and company fields)**

**Bucket:** Compliance Risk

**Finding:** The `#name` and `#company` input fields appear on five of the seven audited pages without an accessible name available to an accessibility API, violating WCAG 4.1.2. Screen reader users risk being unable to identify the purpose of these contact form fields and may not be able to complete the form. Because both selectors are template-level elements, resolving the labelling in the shared form component addresses all five affected pages in one change.

**What to change and why:**

- Associate a label element (or add an aria-label or aria-labelledby attribute) with each of the `#name` and `#company` inputs; this is the direct remediation for WCAG 4.1.2 and gives assistive technology a reliable name to announce for each field.
- Review all other inputs in the same form partial for equivalent labelling gaps while the component is open; fixing one template pass is more efficient than returning for individual field fixes.
- Retest the form with a screen reader after deployment to confirm that each field announces its purpose correctly, which also supports the audit-trail evidence that WCAG 4.1.2 is satisfied.

**Effort:** Low

---

**Priority 4: Semantic Structure 49/100, dotfusion.com/services/contentful-development-agency (23 of 45 bare divs)**

**Bucket:** Compliance Risk

**Finding:** We score Semantic Structure at 49/100, a band that warrants attention. At its most pronounced, the page at https://dotfusion.com/services/contentful-development-agency carries 23 bare divs out of 45 total elements audited on that page. Where services pages share a common layout template, the pattern is likely present across similarly structured pages in the audited set, though the cited figures apply specifically to that URL. A high ratio of non-semantic container elements reduces the structural signal available to both assistive technology and machines attempting to parse content hierarchy.

**What to change and why:**

- Review the layout template used by https://dotfusion.com/services/contentful-development-agency and identify which bare div containers carry meaningful structural roles (sections, articles, navigation landmarks, figures) that could be expressed with semantic elements; replacing them improves WCAG 1.3.1 compliance and raises the Semantic Structure score.
- Prioritise landmark regions (header, main, nav, footer, section, article) in the conversion pass; these carry the greatest benefit for screen reader users who rely on landmark navigation to move efficiently through a page.
- Machines read semantic structure as a proxy for content confidence; improving the element ratio on service pages directly strengthens the structural signal that agents use when parsing Dotfusion's content, which also supports MX Stack Completeness.
...
**Finding:** Across the audited set, only two of the five standard security headers are present (HTTPS and HSTS). X-Frame-Options, Content-Security-Policy, and X-Content-Type-Options are absent from every audited URL. Enterprise buyers in particular regard a complete security-header set as a baseline trust signal, and their procurement tooling may flag pages without them.

**What to change and why:**

- Add the X-Frame-Options header at the server or CDN layer to prevent clickjacking; this is a low-implementation-effort change that removes a common automated-scanner flag.
- Add the X-Content-Type-Options header to prevent MIME-type sniffing; browsers that sniff content types outside the declared value may execute scripts unexpectedly, and the header closes that vector with a single server-level directive.
- Add a Content-Security-Policy header scoped to the domains dotfusion.com already loads assets from; a starter policy that permits known first-party and third-party origins (including cdn-ca.aglty.io) is a meaningful improvement even before a strict mode is adopted, and it moves the security-header count from 2/5 to 5/5.

**Effort:** Low

---

**Priority 6: Discovery Readiness 25/100, missing llms-full.txt, agent-card.json, ai.txt, humans.txt**

**Bucket:** AI Opportunity

**Finding:** We score Discovery Readiness at 25/100 (Needs Improvement). While llms.txt is present, the four remaining well-known discovery artefacts, llms-full.txt, agent-card.json, ai.txt, and humans.txt, are absent from dotfusion.com. Machines crawling for structured discovery signals find only partial guidance; without the fuller artefact set, agents are less likely to treat dotfusion.com as a citation-ready source.

**What to change and why:**

- Publish llms-full.txt to give machines a richer content index beyond the summary that llms.txt provides; this directly raises Discovery Readiness and increases the likelihood that agents surface Dotfusion content when answering queries about the services it offers.
- Publish agent-card.json to declare the capabilities and contact surface that autonomous agents should use when interacting programmatically with dotfusion.com; this is the artefact most directly tied to agent-routing accuracy.
- Add ai.txt to declare content-use permissions explicitly for machine consumers; without it, agents must infer permission from robots.txt, which is not designed for that purpose.
- Add humans.txt to establish editorial provenance; while its direct SEO impact is limited, it contributes to MX governance and supports the content-provenance signals that raise MX Stack Completeness from 44/100.

**Effort:** Medium

---

**Priority 7: MX Stack Completeness 44/100, missing discovery artefacts and MX governance fields**

**Bucket:** AI Opportunity

**Finding:** MX Stack Completeness scores at 44/100 (Could Be Better), reflecting gaps across two contributing categories: the discovery artefact layer (addressed in Priority 6) and MX governance fields in page frontmatter. Specifically, the fields canonicalUri, contentType, audience, and status are absent from audited pages. Without these fields, machines have reduced confidence in content provenance and are less likely to route queries accurately to Dotfusion's content.

**What to change and why:**

- Add canonicalUri to page frontmatter so that machines can resolve the authoritative URL for each page unambiguously; this removes a common source of agent-routing errors when content appears at multiple paths or is syndicated.
- Add contentType and audience fields to frontmatter so that machines can classify what each page is (a service page, a case study, a resource) and who it addresses; structured classification improves agent confidence when matching user queries to Dotfusion's content.
- Add the status field to frontmatter to indicate whether content is current, archived, or in draft; agents that cannot determine content status are less likely to surface it as a reliable answer, and the field is low-cost to implement once a vocabulary is agreed.
- Completing Priority 6 (the four missing discovery artefacts) simultaneously with these frontmatter fields delivers compound improvement to MX Stack Completeness rather than incremental gains from either category alone.
...
- **`sameAs` links on Organisation entities**: adding `sameAs` properties pointing to Wikidata, LinkedIn, or other authoritative external identifiers allows machines to disambiguate Dotfusion from other entities bearing similar names, strengthening citation confidence when agents resolve brand references.

- **`potentialAction` on Organisation**: attaching a `potentialAction` entry to the existing Organisation markup advertises contact or service-request capabilities directly to machines, enabling agent-driven task routing without requiring a human to navigate to a contact page first.

- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt: declaring content-use policy via Content-Signal directives gives machines explicit, machine-readable guidance on how they may use dotfusion.com content, reducing ambiguity for AI crawlers that otherwise infer permissions from silence.
...
**Slowest.** The slowest page is `https://dotfusion.com/services`. A first-time visitor sees the cold-cache cost: the crawler recorded 4824 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three cache-busted re-probes that followed returned 232ms, 110ms, 341ms, giving a returning-visitor median of **232 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://dotfusion.com/`. A first-time visitor sees the cold-cache cost: the crawler recorded 2571 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three cache-busted re-probes that followed returned 122ms, 148ms, 139ms, giving a returning-visitor median of **139 ms**. **Returning-visitor verdict: Healthy**.
...
The robots.txt file is present and declares no disallow paths, leaving all machines free to crawl without restriction. The file also announces one sitemap reference, giving crawlers a direct route to the full URL index.
...
The sitemap declares 148 URLs and earns a Partial grade; it carries both lastmod and changefreq values, which gives machines useful freshness signals, but the absence of priority attributes leaves crawlers without any weighting guidance across the full URL set.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found llms.txt present on dotfusion.com, and the file carries a site description, which gives machines a basic orientation to the host. However, it lacks both a page inventory and a content policy, so machines querying the file for a structured content index or usage guidance will find neither; we recommend extending the file to include both sections.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms-full.txt at dotfusion.com, with the endpoint returning a 404 and no reference to it appearing in the sitemap or the homepage head. We recommend publishing llms-full.txt to give machines a richer content index beyond the summary that llms.txt provides; this directly raises Discovery Readiness and increases the likelihood that agents surface Dotfusion content when answering queries about the services it offers.
...
Across the audited set, resilience is strong at 97/100, with Truncation Risk the one area we see an opportunity to strengthen: two of the seven pages we reviewed carry enough content volume that machines fetching them may receive incomplete extracts rather than the full text. Resolving this gives machines a cleaner, uninterrupted read of those pages and is the single fix with the most immediate effect on machine comprehension across the audited set.
...
On the worst-performing page in the audited set, https://dotfusion.com/services/contentful-development-agency, rendered markup shows 23 of 45 elements as bare divs (51%), giving machines little structural signal to work with; without semantic role information, they fall back on positional inference to determine meaning. The pattern here is surface-wide rather than deeply nested, a shallow chain depth of 4 alongside a high bare ratio points to a utility-class framework where layout and spacing concerns drive the markup, leaving semantic intent untyped at the component level. The most immediate gain comes from wrapping the obvious page-level landmarks, header, nav, main, footer, and aside, in their correct elements and assigning descriptive class names to the remaining generic containers, which would bring the bare-div ratio down without requiring any change to the visual layout.
...
Across the audited set, PDF handling sits at the intersection of two independent concerns: accessibility legislation has converged globally on ISO 14289-1 (PDF/UA) as the shared technical baseline, with the EU's EAA (Directive (EU) 2019/882, in force 28 June 2025) as the most precisely codified instance, alongside Section 508, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent frameworks in Australia and Canada. Equally, an untagged or image-based PDF is invisible to machines in the same way that unsemantic HTML is; only a properly tagged PDF with a valid structure tree allows search crawlers, AI systems, and automated pipelines to extract text, entities, and structure reliably.
...
Across the audited set, https://dotfusion.com performs well where human visitors are concerned, with SEO scoring 86/100, a clear strength in the foundations that search crawlers rely on. Discovery Readiness at 25/100 and Structured Data at 63/100 represent the most immediate opportunities, pointing to gaps in how machines find, parse, and contextualise the content they encounter. We invite the team to read on and work through our recommendations to close those gaps.
...
Across the audited set, we examined 87 images in total. The format breakdown leans heavily on SVG (38 images) and PNG (27 images), with WebP accounting for just 3 images and 19 images in other or unrecognised formats. Alt-text coverage stands at 62 images (71.3%), leaving 25 without an alt attribute; that gap warrants attention both for accessibility and for machine-readable content signals.

On loading strategy, 19 images carry an explicit `loading="lazy"` attribute, and none are marked `loading="eager"`. The remaining 68 images have no loading attribute set at all. That absence is worth distinguishing from eager loading: without an explicit attribute, the browser applies its own heuristics, typically loading above-the-fold images immediately and deferring others, but the behaviour is not guaranteed and varies across browsers. Explicitly declaring either `lazy` or `eager` on every image hands control back to the team rather than leaving it to the browser's discretion.
