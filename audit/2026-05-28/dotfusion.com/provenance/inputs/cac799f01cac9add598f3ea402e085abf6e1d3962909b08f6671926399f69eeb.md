...
- Accessibility: 6 distinct WCAG AA issue types (68 raw instances, 68 marked critical by Pa11y) - 45 of these instances trace to 9 recurring template patterns (single theme edit per pattern fixes all instances)
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- Performance: 3154ms avg (Good)
- Accessibility: 73/100 (Good)
- SEO: 86/100 (Excellent)
- Accessibility issues: 6 distinct WCAG codes across 68 raw instances (45 instances trace to 9 template patterns)
-->
...
- Accessibility: 73/100 (6 distinct WCAG codes, 68 raw instances) - 45 of these instances trace to 9 template-level patterns
...
When a Pa11y finding row in  Of these, 41 (60%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 4 (6%) trace to 2 vendor-injected DOM patterns from cdn-ca.aglty.io; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 41 (60%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 4 (6%) trace to 2 vendor-injected DOM patterns from cdn-ca.aglty.io; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Accessibility: 73/100 (Good) - 68 issues - 45 trace to 9 template pattern(s)
- SEO: 86/100 (Excellent)
- Security headers: 2/5 present (HTTPS, HSTS); 0 of 7 audited URLs carry all five
- Heading quality: 96/100
- Pipeline Survivability: 97/100
- Served HTML: 100/100, Rendered HTML: n/a/100
- Performance: 3154ms average load time
