...
- Site: Dkd (www.dkd.de)
- Pages audited: 7 (full audit)
- Date: 2026-05-28
- Scores: SEO 92/100 | Accessibility 89/100 | Discovery Readiness 25/100 | Structured Data 65/100
- Scope: 7 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): ListItem (13), BreadcrumbList (7), WebSite (1)
...
- Accessibility: 4 distinct WCAG AA issue types (520 raw instances, 520 marked critical by Pa11y) - 412 of these instances trace to 65 recurring template patterns (single theme edit per pattern fixes all instances)
- AI Suitability (served): 78/100
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- Performance: 1844ms avg (Excellent)
- Accessibility: 89/100 (Excellent)
- SEO: 92/100 (Excellent)
- Accessibility issues: 4 distinct WCAG codes across 520 raw instances (412 instances trace to 65 template patterns)
-->
...
- Accessibility: 89/100 (4 distinct WCAG codes, 520 raw instances) - 412 of these instances trace to 65 template-level patterns
- Security headers: 4/5 present (HTTPS, HSTS, CSP, X-Content-Type-Options); 0 of 7 audited URLs carry all five
- Structured Data Quality: 65/100
- Consistency: 64%
-->
...
Div Soup (Semantic Structure): rendered score 53/100, band medium. When the rendered score is below 70, include this as a numbered Priority row. The figures 26 bare divs out of 48 total describe the WORST-CASE page in the audited set (https://www.dkd.de/de/kontakt), not a site-wide average. The finding title and the Finding paragraph must scope to that worst page; do NOT present these numbers as if they hold across every audited page. The title should include the rendered score, the worst-page URL, and the bare-divs-of-total figure (for example, a title naming the score, the page path, and the figure such as "95 of 153 bare divs"). In the Finding paragraph, name the worst page URL explicitly and note that the figures cited come from it specifically; if the band still applies because most pages share the template, say so.

Separate Priority blocks with `---`.

FACTS:
- Pa11y data for this audit:  Of these, 412 (79%) trace to 65 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.

Pa11y findings for this audit (use ONLY these for specific recommendations):
- **WCAG 4.1.1**: Duplicate id attribute value "accessibility" found on the web page. | selector: `#accessibility` | 7 instances on 7 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "account" found on the web page. | selector: `#account` | 7 instances on 7 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "alarm" found on the web page. | selector: `#alarm` | 7 instances on 7 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "article" found on the web page. | selector: `#article` | 7 instances on 7 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "bell" found on the web page. | selector: `#bell` | 7 instances on 7 pages | affects: all assistive tech users
- Div Soup rendered score: 53/100, band medium
- Div Soup bare divs: 26 of 48 (figures from the WORST page only)
- Div Soup worst page URL: https://www.dkd.de/de/kontakt
- Div Soup audited page count: 7

When a Pa11y finding row in  Of these, 412 (79%) trace to 65 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 412 (79%) trace to 65 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- **WCAG 4.1.1**: Duplicate id attribute value "alarm" found on the web page. | selector: `#alarm` | 7 instances on 7 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "article" found on the web page. | selector: `#article` | 7 instances on 7 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "bell" found on the web page. | selector: `#bell` | 7 instances on 7 pages | affects: all assistive tech users above.
...
- Accessibility: 89/100 (Excellent) - 520 issues - 412 trace to 65 template pattern(s)
- SEO: 92/100 (Excellent)
- Security headers: 4/5 present (HTTPS, HSTS, CSP, X-Content-Type-Options); 0 of 7 audited URLs carry all five
- Heading quality: 92/100
- Pipeline Survivability: 78/100
- Served HTML: 78/100, Rendered HTML: n/a/100
- Performance: 1844ms average load time
...
- Pages audited: 7
- Pipeline Survivability score: 78
...
- Rendered score: 53, band medium (worst page: https://www.dkd.de/de/kontakt)
- Bare divs (rendered): 26 of 48 (54%)
- Deepest bare chain (rendered): 3
- Top bare selectors (rendered): `div.text-columns__column` (78), `div.blog-card__info` (55), `div.text-columns.text-columns--` (28), `div.wrap.wrap--master` (7), `div.text-columns.text-columns--above` (4)
...
| Full Optimization | Catalogue Visibility, Discovery Readiness, Semantic Structure, MX Stack Completeness, Structured Data, Security headers, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
- PNG: 37
- JPEG: 33
- Other / unrecognised formats: 14
- With alt text: 83 (98.8%)
- Missing alt text: 1
- loading="lazy": 84
- loading="eager": 0
