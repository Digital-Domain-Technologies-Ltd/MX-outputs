...
- Date: 2026-05-28
- Scores: SEO 77/100 | Accessibility 90/100 | Discovery Readiness 25/100 | Structured Data 0/100
...
- Accessibility: 14 distinct WCAG AA issue types (30 raw instances, 30 marked critical by Pa11y) - 14 of these instances trace to 2 recurring template patterns (single theme edit per pattern fixes all instances)
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- Performance: 954ms avg (Excellent)
- Accessibility: 90/100 (Excellent)
- SEO: 77/100 (Excellent)
- Accessibility issues: 14 distinct WCAG codes across 30 raw instances (14 instances trace to 2 template patterns)
-->
...
- Accessibility: 90/100 (14 distinct WCAG codes, 30 raw instances) - 14 of these instances trace to 2 template-level patterns
...
Div Soup (Semantic Structure): rendered score 21/100, band high. When the rendered score is below 70, include this as a numbered Priority row. The figures 228 bare divs out of 310 total describe the WORST-CASE page in the audited set (https://typo3.org/association), not a site-wide average. The finding title and the Finding paragraph must scope to that worst page; do NOT present these numbers as if they hold across every audited page. The title should include the rendered score, the worst-page URL, and the bare-divs-of-total figure (for example, a title naming the score, the page path, and the figure such as "95 of 153 bare divs"). In the Finding paragraph, name the worst page URL explicitly and note that the figures cited come from it specifically; if the band still applies because most pages share the template, say so.

Separate Priority blocks with `---`.

FACTS:
- Pa11y data for this audit:  Of these, 7 (23%) trace to 1 recurring template-level pattern; a single theme edit per pattern resolves all instances site-wide. A further 7 (23%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.09:1. Recommendation:  change background to #c25700. | selector: `#uc-btn-accept-banner` | 7 instances on 7 pages | affects: low-vision users
...
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.09:1. Recommendation:  change background to #c25700. | selector: `#c1012 > div > div > div > div > div:nth-child(2) > p > a:nt…` | 1 instance on 1 page | affects: low-vision users
- Div Soup rendered score: 21/100, band high
- Div Soup bare divs: 228 of 310 (figures from the WORST page only)
- Div Soup worst page URL: https://typo3.org/association
- Div Soup audited page count: 7

When a Pa11y finding row in  Of these, 7 (23%) trace to 1 recurring template-level pattern; a single theme edit per pattern resolves all instances site-wide. A further 7 (23%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 7 (23%) trace to 1 recurring template-level pattern; a single theme edit per pattern resolves all instances site-wide. A further 7 (23%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Rendered score: 21, band high (worst page: https://typo3.org/association)
