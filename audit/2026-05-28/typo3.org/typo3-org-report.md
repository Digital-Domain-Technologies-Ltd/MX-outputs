...
| MX Stack Completeness | **47**/100 | `########----------` **(!)** |
| Agent Readability | **68**/100 | `############------` |
| Pipeline Survivability | **93**/100 | `#################-` |

We audited seven pages of typo3.org on 2026-05-28, scoring each across ten dimensions covering human and machine readiness. The human picture is strong. Performance stands as the strongest dimension across the audited set, and an SEO score of 77/100 places search foundations in the Excellent band, meaning that visitors who arrive via organic search land on pages that are well-structured, load promptly, and present content clearly. The TYPO3 CMS platform delivers a coherent, professional experience that reflects the maturity of the project behind it.

Before turning to machine readiness, we want to name accessibility as a Priority 1 compliance item. Across the audited set, we recorded 30 raw instances of WCAG AA issues spanning 14 distinct issue types, and Pa11y marks all 30 as critical. The remediation picture is more tractable than the raw count implies: 14 of those instances trace to two recurring template patterns, meaning that two theme-level edits would clear a meaningful share of the critical findings. Addressing these categories is the appropriate first action, both to extend the experience to all users and to satisfy WCAG AA obligations. With that compliance work underway, the headline opportunity is machine readiness. typo3.org sits at MX Readiness Level 1 (Discoverable), which means machines can discover and parse the audited pages but cannot yet cite them as an attested source. Structured Data Quality scores 0/100 and Discovery Readiness scores 25/100, and while those are the scores that draw the eye, they are not the lever that moves the readiness level. The path to Level 2 (Citation-ready) runs through adding full MX governance fields, including mx:status, mx:contentType, mx:audience, canonicalUri, and provenance markers, alongside raising MSC above 60 and DR above 40.

The groundwork is genuinely there for a rapid advance. Because Schema.org JSON-LD is readable by every machine regardless of how a page is rendered, it offers the highest-leverage starting point for structured enrichment, and MX governance metadata can be layered in at the same time. The gap between where typo3.org sits today and a citation-ready posture is a matter of deliberate, targeted additions rather than any fundamental rebuild.
...
Across the audited set, typo3.org delivers a strong human experience, with Performance and Accessibility both sitting in the Excellent band, and SEO at 77/100 offering the clearest opportunity for further gain.
...
Across the audited set of 7 pages, machines can discover and parse typo3.org at a functional level, though the combination of a Discovery Readiness score of 25/100, a Structured Data Quality score of 0/100, and an MX Readiness Level of 1 (Discoverable) means agents cannot yet cite these pages as an attested source.
...
| MX Stack Completeness | 47/100 | Could Be Better | C | C (median) |
...
**Evidence:** MX Stack Completeness 47/100 | Structured Data Quality 0/100 | Discovery Readiness 25/100 | Consistency 36%
...
Across the audited set, typo3.org demonstrates a strong foundations of technical hygiene and user-facing quality, with SEO and accessibility scores that reflect deliberate, sustained investment in both discoverability and inclusive design. These strengths provide the groundwork for the targeted improvements that follow.

| Dimension | Score | Highlights |
|-----------|-------|------------|
| Performance | Excellent | Excellent - 954ms average load time |
...
| Consistency | 36% | 36% - same metadata patterns across every page |
...
We present the findings below as a set of prioritised opportunities, ordered by the degree to which each gap constrains what machines can do with typo3.org. Discovery gaps lead the table because they sit upstream of everything else: until machines can reliably find and index the content, the returns from structured data, accessibility, and schema work remain limited.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Contrast Ratio 3.09:1 on Usercentrics Consent Banner, WCAG 1.4.3 (7 pages) | Compliance Risk | High | Medium | Low-vision users may miss consent controls and be unable to exercise their data-rights choices |
| 2 | Contrast Ratio 3.09:1 on Page Body Text, WCAG 1.4.3 (1 page) | Compliance Risk | High | Low | Low-vision users on that page risk missing body content and in-text links |
| 3 | Iframe Missing Title Attribute, WCAG 2.4.1, Usercentrics Cross-Domain Consent iframe (7 pages) | Compliance Risk | High | Low | Sighted keyboard users may miss the purpose of the consent-sharing frame and be unable to navigate past it meaningfully |
| 4 | Semantic Structure 21/100, https://typo3.org/association (228 of 310 bare divs) | Compliance Risk | Medium | Medium | Screen-reader users on that page are less likely to navigate the page structure efficiently; machines may assign lower confidence to content hierarchy |
| 5 | Discovery Readiness 25/100, five machine-discovery artefacts absent (llms.txt, llms-full.txt, agent-card.json, ai.txt, humans.txt) | AI Opportunity | High | Medium | Machines may miss typo3.org as an authoritative source; citation eligibility remains unverified |
| 6 | Structured Data Quality 0/100, no schema markup detected across the audited set | AI Opportunity | High | Medium | Machines are less likely to attribute structured facts to typo3.org; rich-result eligibility is absent |
| 7 | Security Headers Incomplete, 2 of 5 present (Content-Security-Policy, X-Frame-Options, Permissions-Policy absent) | Cross-cutting | Medium | Medium | Browsers and security-scanning tools may flag unmitigated clickjacking and injection vectors |

---

**Priority 1: Contrast Ratio 3.09:1 on Usercentrics Consent Banner, WCAG 1.4.3 (7 pages)**

**Bucket:** Compliance Risk

**Finding:** Across the audited set, the accept button rendered by the Usercentrics consent SDK (`#uc-btn-accept-banner`) presents a contrast ratio of 3.09:1 against its background, below the 4.5:1 threshold required by WCAG 1.4.3. The finding appears on every one of the 7 audited pages because Usercentrics injects this element at runtime; it is not part of the typo3.org template. Because consent acceptance is a legally consequential interaction under data-protection frameworks, low-vision users who cannot perceive the button are effectively excluded from exercising an informed choice.

**What to change and why:**
- Engage Usercentrics to adjust the consent banner's button background to meet a minimum contrast ratio of 4.5:1, as the consent SDK controls the rendered colour values and a theme edit to typo3.org's own template will not reach this element. Resolving this through the vendor moves all 7 instances simultaneously and addresses WCAG 1.4.3 across the audited set.
- If an SDK update from Usercentrics is not available within the required remediation window, a DOM-observer patch that modifies the button's background after the SDK injects the element is a viable interim measure; document this patch in the site's accessibility audit trail as a temporary compensating control, not a permanent fix.
- Record the vendor's response and any interim patch in your accessibility conformance record so that an audit trail demonstrating due diligence exists should a regulatory review arise.

**Effort:** Medium

---

**Priority 2: Contrast Ratio 3.09:1 on Page Body Text, WCAG 1.4.3 (1 page)**

**Bucket:** Compliance Risk

**Finding:** On one audited page, body-copy text and in-text link elements within `#c1012` present a contrast ratio of 3.09:1, below the WCAG 1.4.3 minimum of 4.5:1. Three selectors on that page share the same contrast shortfall, suggesting a single styling rule governs both the paragraph text and the anchor elements within it. Low-vision users reading that page risk missing substantive content and any calls to action embedded in the affected text.

**What to change and why:**
- Adjust the background colour applied to the `#c1012` content block so that text and link elements within it achieve at least 4.5:1 contrast. Because three selectors share the same contrast shortfall, a single change to the parent block's background is likely to resolve all three instances together, addressing WCAG 1.4.3 for that page.
- Verify that the corrected contrast value holds across all interactive states of the in-text links (default, visited, hover, focus), as each state may inherit or override the parent background independently.
- Once corrected, add this selector to your regression test suite so that future template or content changes do not reintroduce the contrast gap.

**Effort:** Low

---

**Priority 3: Iframe Missing Title Attribute, WCAG 2.4.1, Usercentrics Cross-Domain Consent iframe (7 pages)**

**Bucket:** Compliance Risk

**Finding:** Across the audited set, an iframe element with the selector `#cross-domain-consent-sharing-iframe` carries no title attribute. WCAG 2.4.1 requires that frames be given a descriptive title so that keyboard and assistive-technology users can identify the frame's purpose and choose whether to enter it. This element is injected at runtime by the Usercentrics consent-management SDK (hosted at `app.usercentrics.eu`) and is absent from the typo3.org template; sighted keyboard users on all 7 audited pages are therefore unable to determine the frame's purpose from keyboard navigation alone.

**What to change and why:**
- Engage Usercentrics to ship a fix in their consent SDK that adds a descriptive, non-empty title attribute to `#cross-domain-consent-sharing-iframe` at the point of injection. Because this element originates in the vendor's SDK and not in typo3.org's own template, a theme or partial edit on the typo3.org side will not reach it; the remediation must come from the vendor.
- If the vendor fix is delayed, a DOM-observer patch that detects the injected iframe and programmatically sets a descriptive title attribute after injection is a viable interim measure. Document this patch as a temporary compensating control in your accessibility audit trail.
- Confirm with Usercentrics that the title value they add accurately describes the frame's function (cross-domain consent sharing) so that the attribute satisfies WCAG 2.4.1's requirement for meaningful identification, not merely a non-empty string.

**Effort:** Low

---

**Priority 4: Semantic Structure 21/100, https://typo3.org/association (228 of 310 bare divs)**

**Bucket:** Compliance Risk

**Finding:** We score Semantic Structure at 21/100 across the audited set. The figures that drive this band come specifically from https://typo3.org/association, where 228 of 310 total elements are bare, non-semantic divs. If the remaining audited pages share the same template structure, the band is likely representative of the template's default output rather than content specific to that page. A DOM built predominantly from unsemantic containers gives screen-reader users fewer structural landmarks to navigate by, and reduces the confidence machines can assign when parsing content hierarchy for indexing or summarisation purposes.

**What to change and why:**
- Review the template responsible for https://typo3.org/association and identify layout containers that wrap distinct content regions (navigation, main content, sidebars, footers, complementary sections) with the intent of replacing them with the appropriate sectioning elements. Doing so improves structural navigation for screen-reader users, addressing the practical outcome that WCAG 1.3.1 and 2.4.6 are designed to protect.
- Prioritise landmark regions first, as assistive technologies expose these to users as navigation shortcuts. Even a partial conversion of top-level containers on the shared template would propagate across every page that uses it, reducing the bare-div ratio beyond the single worst-case page.
- After the initial conversion, rerun the Semantic Structure check against https://typo3.org/association to verify the score moves out of the high-concern band; use the resulting score as the baseline for ongoing template review.

**Effort:** Medium

---

**Priority 5: Discovery Readiness 25/100, five machine-discovery artefacts absent (llms.txt, llms-full.txt, agent-card.json, ai.txt, humans.txt)**

**Bucket:** AI Opportunity

**Finding:** We score Discovery Readiness at 25/100 (Needs Improvement). None of the five well-known machine-discovery artefacts are reachable on typo3.org: llms.txt, llms-full.txt, agent-card.json, ai.txt, and humans.txt are all absent. Without these artefacts, machines have no authoritative declaration of typo3.org's content scope, permitted use, or identity, meaning citation eligibility as an attested source remains unverified. For a project of typo3.org's standing in the open-source CMS space, the absence of these signals is a material gap as machine-mediated discovery becomes a primary access path.

**What to change and why:**
- Add llms.txt and llms-full.txt to declare the site's content scope and permitted machine-use terms. These files are read by machines during discovery to determine whether and how to index or summarise content; their absence means machines must infer permissions, reducing confidence in any citation that results.
- Add agent-card.json to provide structured identity metadata for typo3.org as a machine-readable endpoint. This artefact is read by agent orchestration layers to verify the source before attributing claims; without it, automated pipelines are less likely to treat typo3.org as a verified source.
- Add ai.txt and humans.txt to complete the discovery artefact set. ai.txt signals machine-access policy in a format read by AI crawlers; humans.txt provides team and project attribution that supports provenance verification.
- Together, these additions directly improve the MX Stack Completeness score (currently 47/100, Could Be Better) by addressing the discovery-artefact category, one of the two primary categories depressing that composite.

**Effort:** Medium

---

**Priority 6: Structured Data Quality 0/100, no schema markup detected across the audited set**

**Bucket:** AI Opportunity

**Finding:** We score Structured Data Quality at 0/100 (Needs Improvement) across the audited set. We detected no schema markup on any of the 7 audited pages. Without structured data, machines have no machine-readable statements of entity type, authorship, dates, or relationships to verify against external knowledge sources; across the audited set, typo3.org content is unlikely to surface as a rich result in search or be cited with high confidence in an agent response. This also directly depresses the MX Stack Completeness score (47/100, Could Be Better) through the structured data coverage category.

**What to change and why:**
- Add schema markup to the page templates that govern the highest-value content types across the audited set: at minimum, Organisation or SoftwareApplication for the project's identity pages, and BreadcrumbList for navigational context. Machines use these entity declarations to build a knowledge graph of the source; their presence is prerequisite to structured-fact attribution.
- Populate schema fields that support provenance: name, url, description, and sameAs links to authoritative external references (Wikidata, GitHub, official registries). The sameAs property is the primary signal machines use to verify that the described entity corresponds to a known external record, unlocking attested-source status.
- Once markup is in place, validate it against Schema.org vocabulary and confirm that machines can read it from the rendered DOM. Structured data that is present in the source but absent from the rendered page does not contribute to Structured Data Quality scores.
- Addressing this finding moves the Structured Data Quality score from 0/100 and improves the structured data coverage category within MX Stack Completeness, the two largest remaining gaps in the machine-readiness profile.

**Effort:** Medium

---

**Priority 7: Security Headers Incomplete, 2 of 5 present (Content-Security-Policy, X-Frame-Options, Permissions-Policy absent)**

**Bucket:** Cross-cutting

**Finding:** We detect only 2 of the 5 expected security response headers on typo3.org: HTTPS and X-Content-Type-Options are present, but HSTS, Content-Security-Policy, X-Frame-Options, and Permissions-Policy are absent. Security-scanning tools and browsers enforce or flag the absence of these headers; their omission means the site carries unmitigated exposure to clickjacking and cross-site injection vectors. Because these are response headers applied at the server or CDN layer, the fix is independent of CMS template work and can be deployed without a content release.

**What to change and why:**
- Add a Content-Security-Policy header to declare the permitted sources of scripts, styles, and frames. This is the primary defence against cross-site scripting injection; its absence means browsers apply no resource-load restrictions and security scanners will flag typo3.org as unprotected on this vector.
- Add an X-Frame-Options header to prevent the site's pages from being embedded in third-party frames without permission, directly mitigating clickjacking risk. This is a server-level or CDN-level configuration change and does not require a template edit.
- Add a Permissions-Policy header to declare which browser features (camera, microphone, geolocation, and similar) the site's pages may request. Its absence leaves feature-access policy undefined, which security and privacy audits treat as an unacknowledged exposure surface.
...
- **sameAs links on Organisation**: Adding `sameAs` properties to an Organisation entity pointing to Typo3's Wikidata and GitHub profiles would allow machines to cross-reference typo3.org with authoritative external records, strengthening entity disambiguation across knowledge graphs.

- **BreadcrumbList on deep pages**: Pages sitting several levels below the homepage currently rely on URL path structure alone for navigational context; a `BreadcrumbList` entity on those pages would give machines an explicit, parseable hierarchy to follow when constructing answers about site structure or content location.

- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt: Declaring content-use policy via Content-Signal directives would give machines a clear, machine-readable signal about how content across the audited set may be used, filling a governance gap that currently leaves AI agents to infer permissions without explicit instruction.
...
**Slowest.** The slowest page is `https://typo3.org/association/membership`. A first-time visitor sees the cold-cache cost: the crawler recorded 1596 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three cache-busted re-probes that followed returned 353ms (HTTP 404), 257ms (HTTP 404), 159ms (HTTP 404); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: Indeterminate**.

**Median-load control.** The median-load control page is `https://typo3.org/`. A first-time visitor sees the cold-cache cost: the crawler recorded 846 ms on its initial fetch. **First-visit verdict: Healthy**. Three cache-busted re-probes that followed returned 80ms (HTTP 404), 164ms (HTTP 404), 234ms (HTTP 404); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: Indeterminate**.
...
We find no robots.txt present on typo3.org, meaning machines receive no crawl directives and no sitemap reference to guide their indexing.
...
The sitemap declares 76 URLs and earns a Partial grade; it carries both lastmod and priority attributes, yet the absence of changefreq values leaves crawlers without explicit guidance on how frequently each URL is expected to change.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms.txt at the site root, meaning machines that query typo3.org for a structured content index encounter no description, no page inventory, and no content policy. We recommend adding an llms.txt file to give machines a clear entry point into the site's content and scope.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms-full.txt at typo3.org; the endpoint returns a 404 and no reference to it appears in the sitemap or the homepage head. Whether adding it would be worthwhile depends on the site's full content depth, which our audited sample does not yet measure, so we recommend treating this as a conditional next step once coverage of the remaining pages has been assessed.
...
Across the 7 pages we audited, structured data is limited. Machines cannot reliably extract entity data from these pages. Adding Schema.org JSON-LD with required properties is the highest-impact improvement; we would identify the same gaps across the rest of the estate in a wider audit.
...
All detected markers are present in the served HTML on the pages we audited. Server-side and browser-based agents see the same signals on the sampled pages; we would confirm whether the same pattern holds across the rest of the estate in a wider audit.
...
Partially Compatible; the audited stages are still developing, and Search & Compare, Price Understanding, and Purchase Confidence are not applicable for this site type.
...
- **Truncation Risk** - Fail · 1/7
  ...
  - *Data:* Largest page: 230 KB. Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows. Page: https://typo3.org/association/contact
- **SPA Shell** - Pass · 7/7
...
Across the audited set, Pipeline Survivability scores 93/100, with two checks worth addressing: Truncation Risk and Inline Tag Bloat. Inline Tag Bloat affects the majority of the pages we sampled, and when machines encounter dense inline markup, they may extract content less cleanly or truncate meaningful text before reaching it. Reducing inline tag density across the audited set offers the largest single opportunity to improve how reliably machines read and reproduce content from typo3.org.
...
|--------|--------------|----------------|--------------------|
| Rendered HTML | 21/100 (high) | 228 bare divs · 74% ratio · depth 7 | `div.frame-group-container` (136), `div.frame-group-inner` (132), `div.frame-container.frame-container-default` (132), `div.frame-inner` (132), `div.contentcontainer-column` (77) |

On the worst page we sampled, https://typo3.org/association, 228 of 310 elements on the rendered surface are bare divs (74%), meaning machines lose structural context and must rely on positional inference to determine meaning. The pattern is simultaneously deep and surface-wide: a deepest bare chain of 7 combined with a high bare-div ratio points to an untyped component framework where layout wrappers such as `div.frame-group-container`, `div.frame-group-inner`, `div.frame-container.frame-container-default`, and `div.frame-inner` are stacked concentrically, each adding nesting without contributing semantic signal. To reduce the bare-div ratio without restructuring the visual layout, replace the outermost wrappers with appropriate landmark elements (header, nav, main, footer) and assign meaningful class names to the remaining containers.
...
4 of the five standard security headers are absent on every audited response: HSTS (Strict-Transport-Security), Content-Security-Policy (CSP), X-Frame-Options, and Permissions-Policy. Adding them at the origin-server or CDN edge closes the corresponding attack surfaces without touching application code.
...
We found 5 identical inline fragment(s) repeated across multiple pages, totalling 863 KB redundant bytes. Extracting these to external CSS or JS files would reduce page weight, improve cacheability, and simplify maintenance.

| Type | Bytes per fragment | Appears on N pages | Preview |
|------|-------------------:|-------------------:|---------|
| js | 373 | 7 | (function(w,d,s,l,i){w[l]=w[l]\|\|[];w[l].push({'gtm.start':   |
| css | 218748 | 5 | @keyframes ucOpacity{0%{opacity:0}100%{opacity:1}}.usercentr |
| css | 773 | 5 | .usercentrics-button .uc-corner-modal .uc-corner-modal-conte |
| css | 652 | 5 | #usercentrics-button .uc-banner-content {       background-c |
| css | 151 | 5 | #uc-corner-modal a[href], #uc-centre-modal a[href], #uc-bann |
...
Accessibility legislation has converged on ISO 14289-1 (PDF/UA) as the structural baseline across major markets, with the EAA (Directive (EU) 2019/882, in force 28 June 2025) being the most precisely codified instance of a global alignment that spans Section 508, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent frameworks in Australia and Canada. The machine-readability concern is equally pressing and independent: an untagged or image-based PDF is opaque to machines in the same way that unsemantic HTML is, and only a properly tagged PDF with a complete structure tree gives search crawlers, AI systems, and automated pipelines a surface from which to extract text, entities, and structure.
...
1. **Address Priority 1 findings**: address the 30 WCAG 2.1 AA accessibility issues identified (regulatory exposure)
...
Across the audited set, SEO leads the scorecard at 77/100, reflecting a strong foundations for search visibility on https://typo3.org. Structured Data and Discovery Readiness present the clearest opportunities, scoring 0/100 and 25/100 respectively, meaning machines can do relatively little with the content they find. We invite the team to review the findings that follow and take the first steps toward closing those gaps.
...
| Cross-Page Consistency | 36% | Could Be Better |

---

## Appendix A: Pages Audited

- **`/ (nav)`**: SEO 84 · A11y 90 · Back 45 · Served 80 · Rendered 85
...
Across the audited set, we catalogued 71 images in total. SVG is the dominant format at 52 images, followed by 14 PNG, 3 WebP, and 2 JPEG. Alt-text coverage stands at 51 images (71.8%), leaving 20 without descriptive text. Those 20 gaps are worth resolving: screen readers and machines alike rely on alt text to understand non-decorative images, and missing attributes reduce both accessibility scores and the quality of structured content signals.

On loading strategy, 22 images carry the explicit `loading="lazy"` attribute, and none are marked `loading="eager"`. The remaining 49 images have no loading attribute at all. That third state is worth noting separately: a missing attribute does not behave identically to eager loading. The browser applies its own heuristic, factoring in the image's position in the document and viewport size, and the result can vary across browsers and devices. For images below the fold, adding `loading="lazy"` gives the browser an unambiguous instruction; for images in the initial viewport, an explicit `loading="eager"` makes the intent equally clear. Working through those 49 un-attributed images and tagging each one deliberately would tighten control over page load sequencing across the audited set.
