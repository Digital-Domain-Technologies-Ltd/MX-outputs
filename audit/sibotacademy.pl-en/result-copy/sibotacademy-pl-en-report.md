...
reportId: "sibotacademy-pl-en-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://sibotacademy.pl/en --pages 10"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/sibotacademy.pl-en/sibotacademy-pl-en-report.md
  ...
  runbook: "Executive audit report for Sibotacademy. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/sibotacademy.pl-en/sibotacademy-pl-en-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/sibotacademy.pl-en/sibotacademy-pl-en-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/sibotacademy.pl-en/result-copy/sibotacademy-pl-en-report.pdf"
...
**Date:** 16 June 2026\
**Report ID:** sibotacademy-pl-en-WEB-AUDIT-20260616
...
Your site reads as aimed at a child audience, and your page text names an age band (`7-14`) that is not in machine-readable structured data, so an agent has to infer it; declaring `schema:typicalAgeRange` or `mx:intendedAgeRange` would let an agent read the band directly.

Across the audited set, your content delivers a smooth and engaging experience for visitors. The foundation of performance is solid, with page loads that keep users on track and reduce friction. SEO foundations are also strong, scoring 87/100, ensuring that search engines can surface your learning resources effectively.

The headline opportunity we want to draw attention to is the machine-readiness gap in governance metadata, but first we recommend addressing the single WCAG AA issue type that appears in 35 instances; fixing this one category will clear all critical errors flagged by Pa11y. Across the audited set, no MX governance markers were detected, keeping the site at Level 1 Discoverable. Adding full MX fields-mx:status, mx:contentType, mx:audience, canonicalUri and provenance markers-will give machines the structured context they need for accurate comprehension and lift the site to Level 2 Governed. Raising the MSC above 60 and Discovery Readiness above 40 will support that transition.
...
We find that across the audited set, the pages deliver an excellent experience for human visitors, with performance averaging 896 ms and high accessibility (95/100) and SEO (87/100) scores.
...
We find that across the audited set, machines can discover and parse your content with a Discovery Readiness of 32/100, Structured Data Quality of 52/100, and MX Stack Completeness of 59/100, yet they lack governance context as shown by an MX Readiness Level of 1 (Discoverable) while Pipeline Survivability remains high at 97/100.
...
Across the audited set, your pages show a strong baseline with an SEO score of 87/100 and an Accessibility rating of 95/100, giving us a strong foundations to build on. These strengths provide the groundwork for targeted improvements.
...
- **sameAs**: Add `sameAs` URLs to your Organisation entities (e.g., LinkedIn, Wikipedia) so machines can verify the entity’s identity across platforms.  
- **potentialAction**: Attach a `potentialAction` to your Organisation schema (such as a contact action) to expose machine-readable contact capabilities that agents can surface directly in search or voice interfaces.  
...
Across the audited set, two resilience checks require attention: Inline Tag Bloat and SPA Shell. Inline Tag Bloat appears on about a third of the pages and can cause machines to misinterpret or skip content because of excessive small tags; SPA Shell means the page relies on JavaScript to load its main content, so bots that do not execute JS may miss it entirely. Fixing Inline Tag Bloat first-by consolidating or removing redundant tags-will give machines a clearer view of the page and has the biggest impact.
...
We find that across the audited set, your pages use semantic HTML effectively, giving machines reliable structural cues.
...
We note that accessibility legislation across major markets converges on ISO 14289-1 (PDF/UA) as the technical baseline; the EAA Directive (EU) 2019/882, Section 508 of the US Rehabilitation Act, UK Public Sector Bodies Accessibility Regulations 2018, and equivalent Australian and Canadian statutes all resolve to this same structural artefact.  
...
Our audit of https://sibotacademy.pl shows a strong SEO foundation, scoring 87/100 and enabling machines to discover and index the content efficiently. However, Discovery Readiness at 32/100 and Structured Data at 52/100 reveal significant opportunities for improving machine comprehension and visibility. We invite you to address these gaps to enhance your site’s performance across human visitors and automated agents.
...
**Scope:** 11 pages examined | Platform: Unknown Platform | Frameworks: **Next.js** (medium confidence)  -  JS framework; **Tailwind CSS** (medium confidence)  -  CSS framework | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (7 directives)
