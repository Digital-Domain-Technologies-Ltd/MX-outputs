...
reportId: "mx-allabout-network-WEB-AUDIT-20260531"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-05-31/mx.allabout.network/mx-allabout-network-report.md
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-05-31/mx.allabout.network/mx-allabout-network-report.pdf"
...
**Date:** 31 May 2026\
**Report ID:** mx-allabout-network-WEB-AUDIT-20260531
...
Across the audited set, we find that agents can discover and parse the content, read structured data with high quality, and, thanks to a MX Readiness Level of 5 and present governance markers, can cite the pages as attested sources.
...
We’ve found that across the audited set the site demonstrates a strong foundations in SEO, accessibility, security, and structured data, with scores of 91/100, 100/100, 5/5 headers, and 92/100 respectively. These strengths lay the groundwork for the targeted improvements that follow, enabling machines to parse and index content more effectively.
...
We identified a set of opportunities across the audited set, prioritised by impact: discovery readiness leads because it blocks downstream processes, followed by structured data quality, catalogue visibility, and MX stack completeness. These priorities reflect the relative scores and the direct influence each area has on machine discoverability and user experience.
...
We found a robots.txt file that declares a single sitemap reference. It disallows three paths, allowing all other agents to crawl the rest of the site.
...
We found the sitemap complete, declaring 115 URLs. It includes lastmod, changefreq, and priority attributes.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms.txt on the host; the file is missing a description, page inventory, and content policy, so machines that query the host for a structured content index receive no information. We recommend adding a well‑formed llms.txt to provide machines with a concise description, a page inventory, and a content policy.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms‑full.txt at the root of mx.allabout.network; the file is missing, not referenced in the sitemap or homepage, and returned a 404 status. Adding llms‑full.txt would be advisable if the site hosts a substantial amount of content beyond the pages we audited, but the current audit sample does not confirm such depth.
...
| Probed at | 2026-05-31T12:28:43.436Z |
...
| Metric | Count |
|--------|-------|
| AI crawler hits (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, …) | 1401 |
| AI referral hits (human visits from chat surfaces) | 1 |
| Total AI-attributable visits | 1402 |
...
We identified a single page with inline tag bloat, a resilience check that can make it harder for machines to parse or render efficiently. By moving inline styles and scripts to external files on that page, we can keep the site resilient and improve machine readability. This small change is an opportunity to strengthen the overall resilience of the audited set.
...
We note that accessibility legislation worldwide converges on ISO 14289‑1 (PDF/UA) as the technical baseline, with the EAA Directive (EU) 2019/882 in force 28 June 2025 serving as a precisely codified example, while an untagged PDF remains invisible to machines. Machines such as search crawlers, AI systems, and automated pipelines cannot extract text, entities, or structure from a scanned or image‑based PDF, but a tagged PDF with a proper structure tree becomes machine‑readable in the same way semantic HTML is.
...
We found 407 images across the audited set, of which 404 are WebP, 3 are SVG, and none are PNG, JPEG, or other formats. Alt text is present on 346 images (85.0%), leaving 61 images without alt text.  

We observed that 271 images use loading="lazy", 136 use loading="eager", and none lack a loading attribute, so every image has an explicit loading strategy. Because no image omits the loading attribute, we do not have any cases where the browser defaults to eager loading.
