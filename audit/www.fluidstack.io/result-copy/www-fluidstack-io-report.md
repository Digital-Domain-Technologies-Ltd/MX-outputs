---
title: "Www Fluidstack: Website Analysis & Machine Readiness"
author: "Tom Cranstoun"
created: "2026-06-16"
modified: "2026-06-16"
client: "Www Fluidstack"
clientSlug: "www-fluidstack-io"
clientUrl: "https://fluidstack.io"
reportId: "www-fluidstack-io-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://www.fluidstack.io --pages 10 --date 2026-06-16"
description: "Executive audit report reviewing accessibility, performance, SEO, structured data, and AI agent compatibility for Www Fluidstack"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/www.fluidstack.io/www-fluidstack-io-report.md
  maintainer: info@cognovamx.com
  stability: stable
  partOf: mx-audit
  purpose: "Executive machine-readiness audit for Www Fluidstack covering accessibility, performance, SEO, structured data, and AI agent compatibility."
  x-mx-contextProvides: ["web audit findings for Www Fluidstack", "WCAG accessibility assessment", "AI agent compatibility scores", "SEO and structured data analysis", "machine readiness recommendations"]
  ...
  runbook: "Executive audit report for Www Fluidstack. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/www.fluidstack.io/www-fluidstack-io-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/www.fluidstack.io/www-fluidstack-io-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/www.fluidstack.io/result-copy/www-fluidstack-io-report.pdf"
    description: "Generate PDF audit report for Www Fluidstack"
  ...
  x-mx-provenance:
    sidecar: "www-fluidstack-io-report.provenance.ai.json"
    frameworks: [EU-AI-Act, UK-ICO-AI-guidance, NIST-AI-RMF, Colorado-AI-Act]
    companion: "www-fluidstack-io-report.provenance.deterministic.json"
...
**Date:** 16 June 2026\
**Report ID:** www-fluidstack-io-WEB-AUDIT-20260616
...
- **Probes hit network errors or timeouts reading the site** (-2): 1 probe error (1 timeout)
- **Bare-div nesting (div soup) across most pages** (-8): 11 of 11 pages are heavy with bare-div nesting

We found that the audited pages deliver a strong human experience, with accessibility scoring 100/100 and zero WCAG AA issue types across all eleven sampled pages. The site’s clean, responsive design and clear content hierarchy support users of all abilities, while its solid SEO foundation-72/100-ensures that search engines can surface the brand’s key messages without friction.

The headline opportunity we want to draw attention to is the machine-readiness gap in structured data and governance metadata. Accessibility remains a Priority 1 compliance item for machines, but the lowest score across the audited set is Structured Data Quality at 0/100. Adding full MX fields-mx:status, mx:contentType, mx:audience, canonicalUri, provenance markers-and raising MSC above 60 and DR above 40 will lift the site from Level 1 (Discoverable) to Level 2 (Governed), giving machines the structured context they need for accurate comprehension.
...
Across the audited set, we find that the site delivers a strong experience for human visitors, with fast loading times and consistent accessibility, while SEO remains solid but offers room for further optimisation.
...
We find that across the audited set, discovery readiness is 25/100 and structured data quality is 0/100, meaning machines can discover and parse but lack governance context; MX stack completeness at 49/100 indicates partial coverage of governance metadata, while pipeline survivability at 74/100 reflects resilience in content delivery.
...
| MX Stack Completeness | 49/100 | Could Be Better | C | B (median) |
| Pipeline Survivability | 74/100 | Good | B | A (median) |
...
**Evidence:** MX Stack Completeness 49/100 | Structured Data Quality 0/100 | Discovery Readiness 25/100 | Consistency 43%
...
We see that across the audited set, your pages deliver a strong foundations with strong SEO performance (72 / 100) and consistent accessibility compliance (100 / 100), setting the groundwork for further enhancements. Additionally, key security headers are present on several pages, reinforcing the base for robust protection.
...
|-----------|-------|------------|
| Performance | Good | Good  -  1928ms average load time |
...
| Heading Quality | 61 | Good  -  headings present and machine-parseable |
| Consistency | 43% | 43%  -  same metadata patterns across every page |
...
| 4 | Semantic Structure (Naked Containers) 22/100 | Compliance Risk | Medium | Medium | machines lose structural context and infer page regions by position |
| 5 | Security headers absent: CSP, X-Frame-Options | Cross-cutting | Medium | Low | Missing security headers increase exposure to content injection and clickjacking |
| 6 | No Schema.org structured data (SDQ 0/100) | Machine Readability Opportunity | Medium | Medium | Agents must infer entity type and facts from prose rather than declared structured data |
| 7 | No llms.txt published | Machine Readability Opportunity | Medium | Low | Agents have no machine-curated index of site content or declared access policy |
...
**Finding:** 51 of 51 images (100%) on the audited set carry no alt text, so their content is unavailable to assistive technology and to machines reading the page.
...
**Priority 4: Semantic Structure (Naked Containers) 22/100**

**Bucket:** Compliance Risk

**Finding:** Rendered semantic-structure score 22/100: containers carry no role, ARIA landmark, or descriptive class, so machines fall back on positional inference to determine meaning. The worst page ([/blog/fluidstack-expands-u-s-investment-by-relocating-global-headquarters-to-new-york-city](https://fluidstack.io/blog/fluidstack-expands-u-s-investment-by-relocating-global-headquarters-to-new-york-city)) carries 72 bare divs of 115.
...
**Priority 5: Security headers absent: CSP, X-Frame-Options**

**Bucket:** Cross-cutting

**Finding:** Security headers absent: CSP, X-Frame-Options (Site-wide). Missing security headers increase exposure to content injection and clickjacking

**What to change and why:**

- Add the missing CSP and X-Frame-Options response headers at the server or CDN edge; each is a one-line directive that applies across the audited set once configured.
...
**Priority 6: No Schema.org structured data (SDQ 0/100)**
...
**Priority 7: No llms.txt published**
...
- **Content-Signal directives**: adding content-use policy declarations in robots.txt helps machines understand how the content may be reused.  
- **BreadcrumbList**: embedding a simple BreadcrumbList JSON-LD on deep pages gives agents explicit navigation context, improving search presentation.
...
| Check | Result |
|-------|--------|
| URL probed | https://www.fluidstack.io |
...
Across the 12 pages we checked, 23 distinct issue patterns reduce what reaches the tree. 1 of these repeat across pages with the same structure, which marks them as template-level: one change in the shared component clears the finding everywhere it appears.

**Interactive element nested inside another interactive element** (WCAG 2.1 4.1.2)

A button or link sits inside another button or link. The accessibility tree cannot represent one control inside another: readers expose one, the other, or an unusable merge, and keyboard focus order becomes unpredictable. Seen on 3 pages, for example `/blog/fluidstack-awarded-clustermax-gold-by-semianalysis`.
...
A container is wired to respond to clicks but carries no role and is not a native control, so it does not exist as an interactive element in the accessibility tree. A screen reader user cannot reach it; an agent reading the tree or the markup cannot tell it is actionable. Seen on `/jobs`.
...
An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Seen on `/jobs`.
...
The page declares no main landmark. Landmark navigation is how tree consumers skip repeated chrome and land on the content; without it, every visit starts from the top of everything. Repeats on 12 of 12 pages with the same structure (a template-level pattern).
...
The link's accessible name is a generic phrase or a bare URL. In the accessibility tree, and in any links-only projection of the page, every such link reads identically: the destination is unknowable without the surrounding layout, which machines and screen reader users do not have. Seen on 3 pages, for example `/blog/fluidstack-awarded-clustermax-gold-by-semianalysis`.

*The fix:* Name the destination in the link text itself in the template ("View pricing plans" rather than "click here").

**Link whose accessible name carries no destination meaning** (WCAG 2.1 2.4.4)

The link's accessible name is a generic phrase or a bare URL. In the accessibility tree, and in any links-only projection of the page, every such link reads identically: the destination is unknowable without the surrounding layout, which machines and screen reader users do not have. Seen on 2 pages, for example `/legal/cookie-policy`.

*The fix:* Name the destination in the link text itself in the template ("View pricing plans" rather than "click here").

**Link whose accessible name carries no destination meaning** (WCAG 2.1 2.4.4)

The link's accessible name is a generic phrase or a bare URL. In the accessibility tree, and in any links-only projection of the page, every such link reads identically: the destination is unknowable without the surrounding layout, which machines and screen reader users do not have. Seen on `/blog/fluidstack-awarded-clustermax-gold-by-semianalysis`.

*The fix:* Name the destination in the link text itself in the template ("View pricing plans" rather than "click here").

The full set, one row per pattern with every affected page counted, is recorded in the `www-fluidstack-io-accessibility-tree.csv` sidecar alongside this report.
...
**Slowest.** The slowest page is `https://fluidstack.io/legal/terms-of-use`. A first-time visitor sees the cold-cache cost: the crawler recorded 2849 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three fresh re-probes that followed returned 255ms, 235ms, 290ms, giving a returning-visitor median of **255 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://fluidstack.io/legal/cookie-policy`. A first-time visitor sees the cold-cache cost: the crawler recorded 1605 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three fresh re-probes that followed returned 216ms, 253ms, 212ms, giving a returning-visitor median of **216 ms**. **Returning-visitor verdict: Healthy**.
...
Sitemap: https://fluidstack.io/sitemap.xml
```

*The full `robots.txt` (4 lines) is preserved alongside this report as `www-fluidstack-io-robots-txt.txt`.*
...
The sitemap declares 145 URLs and grades Minimal. Lastmod is absent, so machines have no change signal to scheduleule re-crawls against. The sitemap omits changefreq and priority. Google dropped both as ranking signals in 2017, but non-Google crawlers and AI agents still read changefreq as a re-crawl cadence hint and priority as a relative-importance signal, so adding them is a low-effort way to broaden machine compatibility.

This was a full crawl: the audit reached every page it could discover, and 1 of them is absent from the sitemap (which lists 145). The unlisted pages: `/blog`. The full set is recorded in the `www-fluidstack-io-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.
...
| Attribute | Value |
|-----------|-------|
| Origin | https://www.fluidstack.io |
...
|-------|-------------------|-------------|-------|---------------------------------------------------|
| 1 | Discovery | Fail | 44 | Issues: no <main> |
...
Stage 1 (Discovery) is the weakest link in the agent journey. Because each stage depends on the previous one, this gap affects all downstream stages. Addressing Issues: no <main> is the highest-priority improvement.
...
- **Truncation Risk** - Fail · 5/11
  - *Means:* 5 page(s) flag for truncation risk; 1 of them exceed the 250 KB hard ceiling, the rest place main content too far into the document. Agents with limited fetch windows may stop reading before reaching the main content.
  - *Data:* Largest page: 301 KB ([/legal/terms-conditions](https://fluidstack.io/legal/terms-conditions)). Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows. See www-fluidstack-io-pipeline-truncation-risk-pages.csv (5 pages).
...
- **Boilerplate Burial** - Fail · 11/11
  ...
  - *Data:* Highest boilerplate-to-content ratio: 19.49. Threshold: < 10 (and < 80 KB of inline head bytes). See www-fluidstack-io-pipeline-boilerplate-burial-pages.csv (11 pages).
...
- **HTTP Content Negotiation (Vary)** - Fail · 11/11
  ...
  - *Data:* 11 page(s) advertise format negotiation. See www-fluidstack-io-pipeline-http-content-negotiation-(vary)-pages.csv (11 pages).
...
- **Body Content Ratio** - Fail · N/A
  - *Means:* Prose content averages only 12% of served bytes. Scripts, styles, and images dominate; agents get little signal per byte.
  - *Data:* Average: 12%. Threshold: 30%.
- **Inline Tag Bloat** - Fail · 11/11
  ...
  - *Data:* 64 element(s) > 500 bytes. Largest single-page inline CSS block: 130351 B. Largest single-page inline JS block: 60052 B. See www-fluidstack-io-pipeline-inline-tag-bloat-pages.csv (11 pages).
...
We found that Boilerplate Burial affects every audited page, meaning machines may overlook essential content and metadata because it is hidden within non-rendered sections. Addressing this by moving key elements into visible markup would give agents full access to the information they need. Other resilience checks-Truncation Risk, Content Negotiation, and Inline Tag Bloat-also present opportunities for improvement that can further enhance machine comprehension.
...
|--------|--------------|----------------|--------------------|
| Served and rendered | 22/100 (high) | 72 bare divs (63% of containers, depth 9) | `div.ssr-variant.hidden-fb0mta` (14), `div.framer-1FBmV.framer-oeudnf` (14), `div.ssr-variant.hidden-umv7vp` (14), `div.framer-iop2g.framer-XLzhT` (14), `div.ssr-variant` (12) |

**Worst page (served and rendered are identical):** [/blog/fluidstack-expands-u-s-investment-by-relocating-global-headquarters-to-new-york-city](https://fluidstack.io/blog/fluidstack-expands-u-s-investment-by-relocating-global-headquarters-to-new-york-city)

Across the audited set, the worst-case page at https://fluidstack.io/blog/fluidstack-expands-u-s-investment-by-relocating-global-headquarters-to-new-york-city shows 72 of 115 elements are bare divs (63 %). This high ratio means machines lose structural context and must rely on positional inference to determine meaning. The soup is structural, with a deepest bare chain of nine levels-typical of untyped component frameworks or late-stage JavaScript injection that nests many empty wrappers. A low-cost first step is to wrap the main landmarks-header, nav, main, footer, aside-with semantic tags and assign meaningful class names to remaining divs; this reduces the bare-div ratio without altering layout.
...
- **`/jobs`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/legal/terms-of-use`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/legal/terms-conditions`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/legal/data-protection`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/legal/privacy-policy`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/legal/cookie-policy`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/legal/non-circumvention`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/legal/acceptable-use`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/blog/fluidstack-expands-u-s-investment-by-relocating-global-headquarters-to-new-york-city`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/blog/fluidstack-selected-by-anthropic-to-deliver-custom-data-centers-in-the-us`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
- **`/blog/fluidstack-awarded-clustermax-gold-by-semianalysis`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type Yes
...
| Lighthouse heading compliance | 18% | 9 |

**Overall Consistency:** 43%
...
We found 13 identical inline fragment(s) repeated across multiple pages, totalling 1025 KB redundant bytes. Extracting these to external CSS or JS files would reduce page weight, improve cacheability, and simplify maintenance.
...
|------|-------------------:|-------------------:|------------------------------------------------------------------|
| css | 18303 | 11 | /* cyrillic-ext */ @font-face {   font-family: 'Geist Mono'; |
| js | 1140 | 11 | (()=>{function u(){function n(t,e,i){let r=document.createEl |
| js | 801 | 11 | !function(){var w="framer_variant";function u(a,r){let e=r.i |
| js | 342 | 11 | (function(w,d,s,l,i){w[l]=w[l]\|\|[];w[l].push({'gtm.start': n |
| js | 216 | 11 | try{if(localStorage.getItem("__framer_force_showing_editorba |
| js | 109 | 11 | typeof document<"u"&&(window.process={...window.process,env: |
| css | 94 | 10 | :root body { background: var(--token-77602712-ae1e-4ad2-a07d |
| css | 45 | 10 | html body { background: rgb(255, 255, 255); } |
| css | 101650 | 7 | html,body,#main{box-sizing:border-box;margin:0;padding:0}:ro |
| css | 632 | 7 | @media(min-width: 1440px){.hidden-sba7fc{display:none!import |

*Showing the top 10 of 12 duplicate fragments by occurrence count. The full inventory (every fragment with its hash and the page URLs that carry it) is preserved alongside this report as `www-fluidstack-io-inline-code-duplicates.csv`.*
...
We note that accessibility legislation worldwide has converged on ISO 14289-1 (PDF/UA) as the technical baseline for documents; the European Accessibility Act is a highly detailed example of this alignment.  
We also recognise that an untagged PDF remains invisible to machines-search crawlers, AI systems and automated pipelines cannot extract text, entities or structure from scanned or image-based PDFs, whereas a tagged PDF with a proper structure tree becomes machine-readable just like semantic HTML.
...
Analysis of text patterns across audited pages found content reaching Hints at AI on the AI-tells scale (3 of 11 pages scored). Machines do not consistently cite or label AI-generated content; this observation describes what the analysis found, not a conclusion about authorship. The full per-page breakdown is in `ai-tells.json` in the results directory.
...
We found that accessibility is a clear strength, scoring 100/100 across the audited set of 11 pages on https://fluidstack.io.  
Key opportunities lie in structured data and discovery readiness, which scored 0/100 and 25/100 respectively, limiting how effectively machines can understand and surface your content.  
Addressing these gaps improves machine comprehension of fluidstack.io.
...
| SEO (all pages) | 72/100 | Good |
| SEO (content pages) | 73/100 | Good |
...
| Heading Quality | 61/100 | Good |
| Agent Readability | 54/100 | Good |
| Pipeline Survivability | 74/100 | Good |
| Cross-Page Consistency | 43% | Could Be Better |
...
- **`/ (nav)`**: SEO 61 · A11y 100 · Back 50 · Served 90 · Rendered 90
- **`/legal/terms-of-use`**: SEO 69 · A11y 100 · Back 50 · Served 79 · Rendered 79
- **`/legal/terms-conditions`**: SEO 74 · A11y 100 · Back 50 · Served 82 · Rendered 82
- **`/legal/data-protection`**: SEO 74 · A11y 100 · Back 50 · Served 81 · Rendered 81
- **`/legal/privacy-policy`**: SEO 74 · A11y 100 · Back 60 · Served 80 · Rendered 80
- **`/legal/cookie-policy`**: SEO 74 · A11y 100 · Back 60 · Served 80 · Rendered 80
- **`/legal/non-circumvention`**: SEO 74 · A11y 100 · Back 50 · Served 79 · Rendered 79
- **`/legal/acceptable-use`**: SEO 74 · A11y 100 · Back 50 · Served 80 · Rendered 80
- **`/blog/fluidstack-expands-u-s-investment-by-relocating-global-headquarters-to-new-york-city`**: SEO 66 · A11y 100 · Back 50 · Served 89 · Rendered 89
- **`/blog/fluidstack-selected-by-anthropic-to-deliver-custom-data-centers-in-the-us`**: SEO 68 · A11y 100 · Back 50 · Served 74 · Rendered 74
- **`/blog/fluidstack-awarded-clustermax-gold-by-semianalysis`**: SEO 79 · A11y 100 · Back 50 · Served 74 · Rendered 74
...
Per page, internal links range from 1 to 8, averaging 6 across 12 pages. That is sparser than typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 77 |
...
We reviewed 51 images across the audited set: 22 PNG and 29 in other or unidentified formats. 0 of 51 (0.0%) carry alt text, leaving 51 without it. Each missing alt attribute is a place where a screen-reader user or a machine reading the page gets no description of what the image shows.

On loading strategy, 11 images are marked `loading="lazy"` and 0 `loading="eager"`, while 40 carry no loading attribute at all. No attribute is not the same as eager: the browser decides for itself when to fetch, which removes the explicit control that lazy and eager give you. Setting an explicit attribute on those images makes the fetch behaviour predictable for browsers and machines alike.
...
**Scope:** 11 pages examined | Platform: Unknown Platform | Frameworks: **Cloudflare CDN** (low confidence) -  CDN / delivery | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (3 directives)

**Measurement completeness:** Reading the site timed out or errored 1 time during this audit, at:

- timeout reading https://www.fluidstack.io/.well-known/acme-challenge/

Each is a point where a machine reading the site under a time budget would abandon the read and miss content, so this is not a neutral event. Agent Readability is reduced by 2 points for this run.
