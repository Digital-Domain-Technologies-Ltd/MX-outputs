...
reportId: "mx-allabout-network-WEB-AUDIT-20260601"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-01/mx.allabout.network/mx-allabout-network-report.md
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-06-01/mx.allabout.network/mx-allabout-network-report.pdf"
...
**About scope.** We scope findings to the 128 pages we crawled. Where a finding is structural -- a missing security header, a soft 404 pattern, an llms.txt transport problem -- we say so explicitly, as structural issues typically hold across the estate. Contact <info@cognovamx.com> to scope ongoing monitoring or implementation work.
...
We find that across the audited set, mx.allabout.network delivers a strong experience for human visitors, with excellent performance, accessibility, and SEO scores.
...
Across the audited set, we find that agents can discover and parse the content, read it, and, with a MX Readiness Level of 5 and full governance markers, can cite the pages as attested sources, supported by Discovery Readiness of 87/100, Structured Data Quality of 92/100, MX Stack Completeness of 95/100, and Pipeline Survivability of 100/100.
...
Across the 128 pages we audited, we see a strong foundations: SEO scores of 91/100, consistent accessibility at 100/100, comprehensive security headers on 99 % of URLs, and structured‑data quality of 92/100. These strengths give us confidence that the groundwork is in place for the enhancements that follow.

| Dimension | Score | Highlights |
|-----------|-------|------------|
| Performance | Excellent | Excellent - 414ms average load time |
| SEO (content pages) | 91 | Excellent - titles, meta descriptions, canonical URLs in place |
| Security | 5/5 | 5/5 headers present; 133 of 134 URLs carry all five |
...
- Accessibility is compliant across the audited set: Pa11y reports 100/100 with zero WCAG 2.1 AA errors on 128 pages.
...
We summarise key opportunities across the audited set, prioritised by impact on machine discoverability, catalogue visibility, and data quality. Discovery readiness leads because it unlocks all downstream machine interactions; catalogue visibility follows as it directly affects product discovery, with structured data quality and MX stack completeness completing the set.
...
**Slowest.** The slowest page is `https://mx.allabout.network/blog/the-new-web-government-public-sector.html`. A first-time visitor sees the cold-cache cost: the crawler recorded 1085 ms on its initial fetch. **First-visit verdict: Healthy**. Three cache-busted re-probes that followed returned 302ms, 467ms, 320ms, giving a returning-visitor median of **320 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://mx.allabout.network/books/index.html`. A first-time visitor sees the cold-cache cost: the crawler recorded 368 ms on its initial fetch. **First-visit verdict: Healthy**. Three cache-busted re-probes that followed returned 317ms, 418ms, 295ms, giving a returning-visitor median of **317 ms**. **Returning-visitor verdict: Healthy**.
...
We found a robots.txt file that declares a single sitemap reference and disallows three paths, allowing all other agents to crawl the rest of the site. The file permits all user agents except for the three disallowed paths, and it includes a reference to the sitemap for comprehensive indexing.

### sitemap.xml

| Attribute | Present | Assessment |
|-----------|---------|------------|
| `<loc>` URLs | 116 entries | Matches crawl count |
...
We found the sitemap to be complete, declaring 116 URLs and including lastmod, changefreq, and priority values. The most significant attribute is the inclusion of priority values, which helps search engines gauge relative importance.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms.txt on the host, so machines that query the host for a structured content index receive no description, page inventory, or content policy. We recommend adding an llms.txt that includes a site description, page inventory, and content policy.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms‑full.txt; the file is missing, not referenced in the sitemap or homepage, and returned a 404 status. Given the 128 pages we audited represent the full estate, we recommend adding llms‑full.txt and serving it as text/html, diverging from the llmstxt.org specification.
...
| Probed at | 2026-06-01T19:32:41.931Z |
...
| WebPage | 100 | 100% | 100% | Website, WebSite, Organisation, Person |
| BreadcrumbList | 98 | 100% | 100% | - |
| Offer | 17 | 97% | 100% | Organisation, Service, PriceSpecification, Country |
| BlogPosting | 65 | 100% | 100% | Person, Organisation, Reference, Website |
| ImageObject | 41 | 100% | 100% | - |
| Organisation | 44 | 100% | 100% | Person, ContactPoint, Reference, Website |
| Person | 42 | 100% | 100% | Organisation, PropertyValue |
...
|------|------|----------|----------|----------------|
| /audit | Offer | required | price | Offer has no price - agents cannot compare against alternatives |
| /learn/mx-for-pdfs.html | Offer | required | price | Offer has no price - agents cannot compare against alternatives |
| /services/eaa | Offer | required | price | Offer has no price - agents cannot compare against alternatives |
| /audit/ | Offer | required | price | Offer has no price - agents cannot compare against alternatives |
...
| Metric | Count |
|--------|-------|
| AI crawler hits (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, …) | 1499 |
| AI referral hits (human visits from chat surfaces) | 1 |
| Total AI-attributable visits | 1500 |

**Top referring AI surfaces:**

| Agent | Event Type | Hits |
|-------|-----------|------|
| amazonbot | crawler | 535 |
| chatgpt | crawler | 268 |
| claude | crawler | 216 |
| meta-ai | crawler | 192 |
| bytespider | crawler | 146 |
| ccbot | crawler | 86 |
...
- **Truncation Risk** - Pass · 126/126
  - *Means:* Every page is well under the 250 KB hard ceiling at which some server-side agents stop reading, and main content sits within the 50-100 KB offset windows. The largest page is 93 KB.
  - *Data:* Largest page: 93 KB. Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows.
- **SPA Shell** - Pass · 126/126
...
- **Boilerplate Burial** - Pass · 126/126
...
- **Tabbed Disclosure** - Pass · 126/126
...
- **Broken Code Fences** - Pass · 126/126
...
- **HTTP Content Negotiation (Vary)** - Pass · 126/126
...
- **Cross-Host Redirect** - Pass · 126/126
...
We found a single page with inline tag bloat, which can make it harder for machines to parse the content quickly. Moving the inline styles or scripts to external files will keep the page resilient and improve machine readability. This small change is the biggest opportunity to strengthen the site’s resilience.
...
| HSTS | Yes (133/134) | Forces HTTPS, prevents downgrade attacks |
| Content-Security-Policy | Yes (133/134) | Prevents XSS and injection attacks |
| X-Frame-Options | Yes (133/134) | Prevents clickjacking |
| X-Content-Type-Options | Yes (133/134) | Prevents MIME-type sniffing |

Header coverage is uneven across audited responses: HSTS (Strict-Transport-Security) on 133/134 audited responses, Content-Security-Policy (CSP) on 133/134 audited responses, X-Frame-Options on 133/134 audited responses, X-Content-Type-Options on 133/134 audited responses. The header set is configured per route or per virtual host rather than uniformly at the origin or CDN edge; a single server-config change brings the missing responses in line with the strongest baseline already in place.

**Coverage:** 133 of 134 audited URLs carry all five headers (`/about` is the exception).
...
HTTPS: 134/134 | HSTS: 133/134 | CSP: 133/134 | X-Frame-Options: 133/134 | X-Content-Type-Options: 133/134
...
| Canonical URL | 100% | - |
| Exactly 1 H1 | 100% | - |
| Code examples present | 17% | 105 |
| Self-contained sections | 100% | - |
| Error/troubleshooting docs | 6% | 118 |
...
| Check | Result | Notes |
|-------|--------|-------|
| Organisation name parity | Pass | Organisation name appears consistently across all 126 audited pages |
| Canonical URL duplicates | Pass | No duplicate canonical URLs detected across the 126-page audited set |
...
We found that https://mx.allabout.network achieves a perfect Accessibility score of 100/100, demonstrating a strong foundation for inclusive user experiences. Key opportunities lie in AI Suitability, which currently scores 0/100, and Discovery Readiness at 87/100, both of which limit the site’s potential for machine discoverability and AI integration. We invite you to prioritise these areas to unlock greater machine engagement and broaden the site’s reach.
...
*Showing the first 25 of 128 audited pages; the remaining 103 are in `mx-allabout-network-pages-audited.csv` next to this report.*
...
Across the audited set, we examined 411 images. The format distribution is dominated by WebP, with 408 images, and a small SVG presence of 3 images. No PNG or JPEG images were found. Alt
...
**Scope:** 128 pages analysed | Platform: GitHub Pages | Analysis method: Hybrid (automated + manual verification) | robots.txt: Found
