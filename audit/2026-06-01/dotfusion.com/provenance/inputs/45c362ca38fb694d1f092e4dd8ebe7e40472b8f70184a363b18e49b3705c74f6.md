...
We find that the site delivers a strong experience for human visitors, with good performance, good accessibility, and excellent SEO across the audited set.
...
We find that across the audited set, agents can discover and parse the content, but cannot yet cite it as an attested source, with discovery readiness at 20/100, structured data quality at 44/100, MX stack completeness at 43/100, and pipeline survivability at 95/100.
...
Across the audited set, we see a strong foundations in SEO, accessibility, and structured data that sets the groundwork for the improvements that follow. These strengths demonstrate the groundwork for the enhancements we’ll explore next.
...
We identified key opportunities across the audited set of 12 pages, prioritised by impact on machine discoverability and data quality. The priority order reflects that discovery readiness gaps most directly block downstream machine processing, followed by catalogue visibility, MX stack completeness, and structured data quality.
...
We found a robots.txt file that allows all agents, with no disallowed paths, and it references a sitemap. The sitemap reference ensures that crawlers can discover the site’s content structure.
...
We assessed the sitemap and found it carries 148 URLs, includes lastmod and changefreq values, but lacks priority attributes. This combination earned the sitemap a Partial grade.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found llms.txt present and it includes a site description, but it lacks a page inventory and a content policy. Our recommendation diverges from the llmstxt.org specification: we recommend adding a page inventory and content policy, and serving the file as text/html.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms‑full.txt; the file is missing from the root, not referenced in the sitemap or homepage head, and the 404 status confirms its absence. Our recommendation diverges from the llmstxt.org specification: we recommend text/html, conditional on the site's actual content depth, which the audit's sample does not yet measure.
...
Partially Compatible; Purchase Confidence is N/A.
...
We found that five of the twelve pages we examined risk truncation, which means machines may miss part of the content when they crawl or index the page.  
This truncation can lead to incomplete or misleading information being presented to users or in search results.  
The biggest opportunity is to adjust the fetch offset or reduce page size so that the full content is available, strengthening the reliability of machine reading.
...
We note that accessibility legislation across the EU, US, UK, Australia and Canada converges on ISO 14289‑1 (PDF/UA) as the technical baseline, with the EAA Directive (EU 2019/882, effective 28 June 2025) serving as the most precisely codified example of this global alignment.  
...
| Full Optimisation | Discovery Readiness, Semantic Structure, Catalogue Visibility, MX Stack Completeness, Structured Data, Performance, Security headers, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
We found that https://dotfusion.com scores 84/100 in SEO, demonstrating strong optimisation for search crawlers. Key opportunities lie in Discovery Readiness, which sits at 20/100, and Structured Data, at 44/100, both of which limit machines from fully recognising and citing the content. We invite the team to prioritise enhancing discovery markers and enriching structured data to unlock greater machine visibility and engagement.
...
Across the audited set we examined 127 images. The format mix is dominated by SVG, with 60 instances, followed by PNG at 38, WebP at 3, and 26 images in other or unrecognised formats. Alt text is present on 99 images, which is 78.0 % of the total, leaving 28 images without alt text.  

When it comes to loading strategy, 24 images are explicitly set to load lazily, while none are marked eager. The remaining 103 images lack a loading attribute, meaning the browser will decide how to fetch them. We note that the absence of a loading attribute is not equivalent to eager loading; it simply defers the decision to the browser.
