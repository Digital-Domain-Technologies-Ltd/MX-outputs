...
reportId: "dotfusion-com-WEB-AUDIT-20260601"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-01/dotfusion.com/dotfusion-com-report.md
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-06-01/dotfusion.com/dotfusion-com-report.pdf"
...
**Date:** 1 June 2026\
**Report ID:** dotfusion-com-WEB-AUDIT-20260601
...
- Pages audited: 12 of 148 in sitemap - sample run
- Date: 2026-06-01
- Scores: SEO 84/100 | Accessibility 75/100 | Discovery Readiness 20/100 | Structured Data 44/100
...
- Accessibility: 6 distinct WCAG AA issue types (97 raw instances, 97 marked critical by Pa11y) - 65 of these instances trace to 7 recurring template patterns (single theme edit per pattern fixes all instances)
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 12
- Performance: 2800ms avg (Good)
- Accessibility: 75/100 (Good)
- SEO: 84/100 (Excellent)
- Accessibility issues: 6 distinct WCAG codes across 97 raw instances (65 instances trace to 7 template patterns)
-->
...
|-----------|-------|--------|-------|----------|
| Discovery Readiness | 20/100 | Needs Improvement | D | D (median) |
| Structured Data Quality | 44/100 | Could Be Better | C | C (median) |
| MX Stack Completeness | 43/100 | Could Be Better | C | C (median) |
...
- SEO (content pages only): 84/100
- Accessibility: 75/100 (6 distinct WCAG codes, 97 raw instances) - 65 of these instances trace to 7 template-level patterns
...
|-----------|-------|------------|
| Performance | Good | Good - 2800ms average load time |
...
Div Soup (Semantic Structure): rendered score 31/100, band high. When the rendered score is below 70, include this as a numbered Priority row. The figures 269 bare divs out of 369 total describe the WORST-CASE page in the audited set (https://dotfusion.com/about), not a site-wide average. The finding title and the Finding paragraph must scope to that worst page; do NOT present these numbers as if they hold across every audited page. The title should include the rendered score, the worst-page URL, and the bare-divs-of-total figure (for example, a title naming the score, the page path, and the figure such as "95 of 153 bare divs"). In the Finding paragraph, name the worst page URL explicitly and note that the figures cited come from it specifically; if the band still applies because most pages share the template, say so.

Separate Priority blocks with `---`.

FACTS:
- Pa11y data for this audit:  Of these, 65 (67%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.

Pa11y findings for this audit (use ONLY these for specific recommendations):
- **WCAG 4.1.2**: Anchor element found with a valid href attribute, but no link content has been supplied. | selector: `#site-header > div:nth-child(2) > div > div > div:nth-child(…` | 12 instances on 12 pages | affects: screen reader users
- **WCAG 4.1.2**: This textarea element does not have a name available to an accessibility API. Valid names are: label element, title , aria-label , aria-labelledby . | selector: `#g-recaptcha-response-100000` | 12 instances on 12 pages | affects: screen reader users
- **WCAG 1.3.1**: This form field should be labelled in some way. Use the label element (either with a "for" attribute or wrapped around the form field), or "title", "aria-label" or "aria-labelledby" attributes as appropriate. | selector: `#g-recaptcha-response-100000` | 12 instances on 12 pages | affects: screen reader users
- **WCAG 4.1.2**: This stringinput element does not have a name available to an accessibility API. Valid names are: element content, aria-label , aria-labelledby . | selector: `#name` | 8 instances on 8 pages | affects: screen reader users
- **WCAG 4.1.2**: This stringinput element does not have a name available to an accessibility API. Valid names are: element content, aria-label , aria-labelledby . | selector: `#company` | 8 instances on 8 pages | affects: screen reader users
- Div Soup rendered score: 31/100, band high
- Div Soup bare divs: 269 of 369 (figures from the WORST page only)
- Div Soup worst page URL: https://dotfusion.com/about
- Div Soup audited page count: 12

When a Pa11y finding row in  Of these, 65 (67%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 65 (67%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- **WCAG 1.3.1**: This form field should be labelled in some way. Use the label element (either with a "for" attribute or wrapped around the form field), or "title", "aria-label" or "aria-labelledby" attributes as appropriate. | selector: `#g-recaptcha-response-100000` | 12 instances on 12 pages | affects: screen reader users
- **WCAG 4.1.2**: This stringinput element does not have a name available to an accessibility API. Valid names are: element content, aria-label , aria-labelledby . | selector: `#name` | 8 instances on 8 pages | affects: screen reader users
- **WCAG 4.1.2**: This stringinput element does not have a name available to an accessibility API. Valid names are: element content, aria-label , aria-labelledby . | selector: `#company` | 8 instances on 8 pages | affects: screen reader users above.
...
FACTS (do not change any number, percentage, URL, page count, or name):
- Pages audited: 12 - scope all Priority descriptions to "the audited page" (singular) or "the audited set" (plural); do NOT say "site-wide" or "across the site"
...
**Slowest.** The slowest page is `https://dotfusion.com/services`. A first-time visitor sees the cold-cache cost: the crawler recorded 4824 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three cache-busted re-probes that followed returned 259ms, 226ms, 310ms, giving a returning-visitor median of **259 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://dotfusion.com/services/contentful-development-agency`. A first-time visitor sees the cold-cache cost: the crawler recorded 2554 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three cache-busted re-probes that followed returned 327ms, 266ms, 248ms, giving a returning-visitor median of **266 ms**. **Returning-visitor verdict: Healthy**.
...
Across the 12 pages we audited, structured data is limited. Machines cannot reliably extract entity data from these pages. Adding Schema.org JSON-LD with required properties is the highest-impact improvement, and a wider audit identifies the same gaps across the rest of the estate.
...
- Rendered score: 31, band high (worst page: https://dotfusion.com/about)
...
| Full Optimization | Discovery Readiness, Semantic Structure, Catalogue Visibility, MX Stack Completeness, Structured Data, Performance, Security headers, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
WRITE:
2-3 sentences summarising the audit findings for Dotfusion (https://dotfusion.com). Always refer to the site by its domain (https://dotfusion.com), never by the consultancy name. The highest-scoring dimension is SEO at 84/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 84/100; Structured Data: 44/100; Discovery Readiness: 20/100
...
- **`/about`**: SEO 87 · A11y 80 · Back 55 · Served 100 · Rendered 100
- **`/contact-us`**: SEO 67 · A11y 70 · Back 55 · Served 100 · Rendered 100
- **`/privacy`**: SEO 83 · A11y 80 · Back 55 · Served 100 · Rendered 100
...
- PNG: 38
- JPEG: 0
- Other / unrecognised formats: 26
- With alt text: 99 (78.0%)
- Missing alt text: 28
- loading="lazy": 24
- loading="eager": 0
- no loading attribute: 103
