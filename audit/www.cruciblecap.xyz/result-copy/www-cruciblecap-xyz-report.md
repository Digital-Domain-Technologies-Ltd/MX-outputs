---
title: "Www Cruciblecap: Website Analysis & Machine Readiness"
author: "Tom Cranstoun"
created: "2026-06-16"
modified: "2026-06-16"
client: "Www Cruciblecap"
clientSlug: "www-cruciblecap-xyz"
clientUrl: "https://www.cruciblecap.xyz"
reportId: "www-cruciblecap-xyz-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://www.cruciblecap.xyz --pages 10 --date 2026-06-16"
description: "Executive audit report reviewing accessibility, performance, SEO, structured data, and AI agent compatibility for Www Cruciblecap"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/www.cruciblecap.xyz/www-cruciblecap-xyz-report.md
  maintainer: info@cognovamx.com
  stability: stable
  partOf: mx-audit
  purpose: "Executive machine-readiness audit for Www Cruciblecap covering accessibility, performance, SEO, structured data, and AI agent compatibility."
  x-mx-contextProvides: ["web audit findings for Www Cruciblecap", "WCAG accessibility assessment", "AI agent compatibility scores", "SEO and structured data analysis", "machine readiness recommendations"]
  ...
  runbook: "Executive audit report for Www Cruciblecap. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/www.cruciblecap.xyz/www-cruciblecap-xyz-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/www.cruciblecap.xyz/www-cruciblecap-xyz-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-16/www.cruciblecap.xyz/www-cruciblecap-xyz-report.pdf"
    description: "Generate PDF audit report for Www Cruciblecap"
  ...
  x-mx-provenance:
    sidecar: "www-cruciblecap-xyz-report.provenance.ai.json"
    frameworks: [EU-AI-Act, UK-ICO-AI-guidance, NIST-AI-RMF, Colorado-AI-Act]
    companion: "www-cruciblecap-xyz-report.provenance.deterministic.json"
...
**Date:** 16 June 2026\
**Report ID:** www-cruciblecap-xyz-WEB-AUDIT-20260616
...
**About sample scope.** Findings throughout this report describe what we observed on the 3 pages we crawled. Verdicts scoped to the sample should not be extrapolated to the full estate without a wider audit; where a finding is structural (a missing security header, a soft 404 pattern, an llms.txt transport problem) we say so.
...
| MX Stack Completeness | **36**/100 | `######------------` **(!)** |
| Agent Readability | **46**/100 | `########----------` **(!)** |
...
- **Origin server is slow for an agent fetch** (-5): origin verdict slow, slowest median 920ms
- **Bare-div nesting (div soup) across most pages** (-8): 3 of 3 pages are heavy with bare-div nesting
- **Pages are consistently heavy to load** (-5): median page load 14704ms across 3 pages

Across the audited set, the site delivers a highly accessible experience with zero WCAG AA violations and clear navigation, ensuring every visitor can interact with the content without barriers. The clean layout and concise copy support user understanding, while the solid SEO foundation-reflected in a 57/100 score-helps users discover the pages through search engines.

The headline opportunity is to enrich the content with structured data and MX governance metadata. Adding Schema.org JSON-LD blocks will give machines the context needed for accurate comprehension, and embedding MX fields such as mx:status, mx:contentType, mx:audience, canonicalUri, and provenance markers will lift the site from a Not-Ready state to a Governed level. This next step aligns with the high AI Suitability score of 81/100, ensuring that the already well-served HTML can be fully understood by machines once the governance layer is in place.
...
We find that across the audited set, the site delivers a solid experience for human visitors, with excellent accessibility and good SEO, though page load times could be improved.
...
|-----------|--------|-------|----------|
| UX / Navigation | Needs Improvement | D | - |
...
We find that across the audited set, machines can discover and parse content but lack governance context, as shown by a Discovery Readiness score of 10/100, Structured Data Quality of 0/100, MX Stack Completeness of 36/100, Pipeline Survivability of 92/100, and an overall MX Readiness Level of 0 (Not Ready).
...
| MX Stack Completeness | 36/100 | Could Be Better | C | B (median) |
...
**Evidence:** MX Stack Completeness 36/100 | Structured Data Quality 0/100 | Discovery Readiness 10/100 | Consistency 17%
...
Across the audited set, your pages demonstrate a strong foundations with perfect accessibility scores and respectable search visibility. These strengths lay the groundwork for targeted improvements that will further enhance machine comprehension and user experience.
...
|-----------|-------|------------|
| SEO (content pages) | 60 | Good  -  titles, meta descriptions, canonical URLs in place |
| Security | 2/5 | 2/5 headers present (CSP, X-Frame-Options, X-Content-Type-Options absent); 0 of 2 URLs carry all five |
| Heading Quality | 58 | Good  -  headings present and machine-parseable |
| Consistency | 17% | 17%  -  same metadata patterns across every page |
| Agent access | 8/8 | every tested agent receives HTTP 200 |

**Positive patterns observed:**

- Accessibility is compliant across the audited set: Pa11y reports 100/100 with zero WCAG 2.1 AA errors on 3 pages.
...
| 4 | Semantic Structure (Naked Containers) 33/100 | Compliance Risk | Medium | Medium | machines lose structural context and infer page regions by position |
| 5 | Slow Origin Response | Cross-cutting | Medium | High | first-time visitors and cold-cache agents wait for the first byte |
| 6 | Security headers absent: CSP, X-Frame-Options, X-Content-Type-Options | Cross-cutting | Medium | Low | Missing security headers increase exposure to content injection and clickjacking |
| 7 | No Schema.org structured data (SDQ 0/100) | Machine Readability Opportunity | Medium | Medium | Agents must infer entity type and facts from prose rather than declared structured data |
...
**Finding:** 60 of 60 images (100%) on the audited set carry no alt text, so their content is unavailable to assistive technology and to machines reading the page.
...
**Priority 4: Semantic Structure (Naked Containers) 33/100**

**Bucket:** Compliance Risk

**Finding:** Rendered semantic-structure score 33/100: containers carry no role, ARIA landmark, or descriptive class, so machines fall back on positional inference to determine meaning. The worst page ([/portfolio](https://www.cruciblecap.xyz/portfolio)) carries 616 bare divs of 780.
...
**Finding:** The slowest audited page took 22823 ms on a first-time (cold-cache) fetch, well above the healthy origin-response band, so a first-time visitor or an agent arriving on a cold cache waits noticeably for the first byte (returning visitors with a warm cache are served in about 920 ms).
...
**Priority 6: Security headers absent: CSP, X-Frame-Options, X-Content-Type-Options**
...
**Priority 7: No Schema.org structured data (SDQ 0/100)**
...
- **potentialAction**: Adding a potentialAction on an Organisation entity would let machines discover and invoke your contact capabilities directly from the structured data, and it would be placed on any page that lists your organisation’s contact information.
...
| Check | Result |
|-------|--------|
| URL probed | https://www.cruciblecap.xyz |
| HTTP status | 200 |
...
**Slowest.** The slowest page is `https://www.cruciblecap.xyz/`. A first-time visitor sees the cold-cache cost: the crawler recorded 22823 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three fresh re-probes that followed returned 928ms, 783ms, 920ms, giving a returning-visitor median of **920 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://www.cruciblecap.xyz`. A first-time visitor sees the cold-cache cost: the crawler recorded 14704 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three fresh re-probes that followed returned 706ms, 1851ms, 778ms, giving a returning-visitor median of **778 ms**. **Returning-visitor verdict: Healthy**.
...
We found that internal links contain multiple URL variants-trailing-slash and hash-fragment forms-for the same canonical page, which are identical at the HTTP level. Because machines that do not normalise URLs will treat each variant as a separate resource, they waste token budget and risk generating contradictory or inflated findings; we recommend adding a <link rel="canonical"> tag to each affected URL and, when publishing a sitemap.xml, listing only one canonical form per resource.
...
| Attribute | Value |
|-----------|-------|
| Origin | https://www.cruciblecap.xyz |
...
Across the 3 pages we audited, structured data is limited. Machines cannot reliably extract entity data from these pages. Adding Schema.org JSON-LD with required properties is the highest-impact improvement.
...
- **Truncation Risk** - Pass · 3/3
  - *Means:* Every page is well under the 250 KB hard ceiling at which some server-side agents stop reading, and main content sits within the 50-100 KB offset windows. The largest page is 57 KB ([/portfolio](https://www.cruciblecap.xyz/portfolio)).
  - *Data:* Largest page: 57 KB ([/portfolio](https://www.cruciblecap.xyz/portfolio)). Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows.
- **SPA Shell** - Pass · 3/3
...
- **Boilerplate Burial** - Pass · 3/3
  - *Means:* Navigation and chrome do not dominate the page; main content is reachable without wading through overhead.
  - *Data:* Highest boilerplate-to-content ratio: 0.07. Threshold: < 10 (and < 80 KB of inline head bytes).
- **Tabbed Disclosure** - Pass · 3/3
...
- **HTTP Content Negotiation (Vary)** - Pass · 3/3
...
- **Body Content Ratio** - Pass · N/A
  - *Means:* Actual prose content averages 67% of served bytes, well above the 30% threshold. Pages are content-heavy, not overhead-heavy.
  - *Data:* Average: 67%. Threshold: 30%.
- **Inline Tag Bloat** - Fail · 3/3
  ...
  - *Data:* 10 element(s) > 500 bytes. Largest single-page inline CSS block: 1631 B. Largest single-page inline JS block: 3474 B. See www-cruciblecap-xyz-pipeline-inline-tag-bloat-pages.csv (3 pages).
...
Across the audited set every page shows Inline Tag Bloat, meaning large amounts of CSS or JavaScript are embedded directly in the HTML. This bloat can cause machines to spend extra time parsing and executing code, which may delay or even prevent them from extracting key content quickly. The most impactful fix is to move those inline styles and scripts into external files, reducing page size and improving machine readability.
...
|--------|--------------|----------------|--------------------|
| Served and rendered | 33/100 (high) | 616 bare divs (79% of containers, depth 4) | `div` (241), `div.lil-dot-guy` (84), `div.wrapportfolioitemdetail.industry` (60), `div._16px-square` (60), `div.wrapportfolioitemdetail` (60) |

**Worst page (served and rendered are identical):** [/portfolio](https://www.cruciblecap.xyz/portfolio)

We recorded a bare-div ratio of 616 of 780 (79 %) on the worst page https://www.cruciblecap.xyz/portfolio, meaning machines lose structural context and fall back to positional inference for meaning.  
The soup shows deep chains up to four divs nested, indicating a source pipeline that nests many generic elements-typical of drag-and-drop builders or untyped component frameworks.  
The cheapest first move is to wrap the obvious landmarks (header, nav, main, footer, aside) and give remaining sections meaningful class names so the bare-div ratio drops without restructuring layout.
...
**Coverage:** 0 of 2 audited URLs carry all five headers; see the Security Headers appendix for the full exception list.

- **`/`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type No
- **`/portfolio`**: HTTPS Yes · HSTS Yes · CSP No · X-Frame No · X-Content-Type No

HTTPS: 2/2 | HSTS: 2/2 | CSP: 0/2 | X-Frame-Options: 0/2 | X-Content-Type-Options: 0/2
...
|----------------------------------|----------|--------------------|
| Schema.org JSON-LD | 0% | 3 |
| MX governance tags | 0% | 3 |
| Open Graph tags | 33% |  -  |
| Twitter Card tags | 33% |  -  |
| Skip link | 0% | `/portfolio` |
| llms.txt link tag | 0% | `/portfolio` |
| Canonical URL | 0% | `/portfolio` |
| Exactly 1 H1 | 0% | 3 |
| Code examples present | 0% | 3 |
| Self-contained sections | 100% |  -  |
| Error/troubleshooting docs | 0% | 3 |
| Lighthouse heading compliance | 0% | 3 |

**Overall Consistency:** 17%

Some pages in the 3-page sample are missing metadata patterns that others carry. Machines hitting different pages get different quality data. The Missing Pages column shows where to focus on the sampled pages.
...
We found 11 identical inline fragment(s) repeated across multiple pages, totalling 15 KB redundant bytes. Extracting these to external CSS or JS files would reduce page weight, improve cacheability, and simplify maintenance.
...
|------|-------------------:|-------------------:|------------------------------------------------------------------|
| css | 3720 | 3 | @font-face{font-family:semplicitapro;src:url(https://use.typ |
| css | 1431 | 3 | /* Antialiased font smoothing  */  * {   -webkit-font-smooth |
| js | 1074 | 3 | const dotSize = 2.25 const spacing = dotSize * 8 const areaO |
| js | 199 | 3 | WebFont.load({  google: {    families: ["Open Sans:300,300it |
| js | 181 | 3 | !function(o,c){var n=c.documentElement,t=" w-mod-";n.classNa |
| css | 58 | 3 | .wf-force-outline-none[tabindex="-1"]:focus{outline:none;} |
| css | 58 | 3 | .tk-semplicitapro{font-family:"semplicitapro",sans-serif;} |
| js | 30 | 3 | try{Typekit.load();}catch(e){} |
| js | 1358 | 2 | $(document).ready(function(){      var textBox = $('.text_bo |
| js | 621 | 2 | const lenis = new Lenis({  duration: 1.25,  easing: (t) => M |

*Showing the top 10 of 11 duplicate fragments by occurrence count. The full inventory (every fragment with its hash and the page URLs that carry it) is preserved alongside this report as `www-cruciblecap-xyz-inline-code-duplicates.csv`.*
...
We note that accessibility legislation worldwide converges on ISO 14289-1 (PDF/UA) as the technical baseline, with the European Accessibility Act providing a particularly detailed codification among many jurisdictions. We also observe that an untagged PDF remains invisible to machines-search crawlers and AI systems cannot extract text or structure from scanned or image-based PDFs-whereas a tagged PDF equipped with a proper structure tree becomes machine-readable in the same way semantic HTML is.

We linked no PDFs from the 3-page sample we crawled, and the sitemap declares no `.pdf` URLs either. This is a statement about what we sampled and what the sitemap reports, not a verdict about the wider document estate: PDFs do not appear in this count if they sit behind login forms, are linked only from uncrawled pages, are stored in unlinked directories, are kept out of the sitemap, or are hosted on third-party domains.
...
Analysis of text patterns across audited pages found content reaching Probably AI on the AI-tells scale (3 of 3 pages scored). Machines do not consistently cite or label AI-generated content; this observation describes what the analysis found, not a conclusion about authorship. The full per-page breakdown is in `ai-tells.json` in the results directory.
...
Accessibility scores are 100/100 across the audited set, giving humans full access to content on https://www.cruciblecap.xyz. However, structured data (0/100) and discovery readiness (10/100) are low-scoring opportunities that limit machine comprehension of your content. We recommend addressing these gaps to unlock better visibility and richer AI-driven interactions.
...
| SEO (all pages) | 57/100 | Good |
| SEO (content pages) | 60/100 | Good |
| MX Stack Completeness | 36/100 | Could Be Better |
...
- **`/ (nav)`**: SEO 56 · A11y 100 · Back 45 · Served 83 · Rendered 84
- **`/portfolio`**: SEO 60 · A11y 100 · Back 45 · Served 80 · Rendered 79
- **`/ (nav)`**: SEO 56 · A11y 100 · Back 45 · Served 83 · Rendered 84

Pages marked (nav) are navigational: they route visitors to content rather than containing it, and are excluded from the SEO content average. Content-pages SEO average: 60/100.

**URL deduplication note:** 3 crawled URLs resolved to 2 unique pages after canonicalisation (inflation factor 1.5×). The following URL clusters were treated as the same page: https://www.cruciblecap.xyz/ (2 variants: https://www.cruciblecap.xyz, https://www.cruciblecap.xyz/). Fragment suffixes (such as `#` and `#/`) and trailing-slash variants are treated as identical resources by HTTP servers and search engines; this audit deduplicated them before scoring to avoid inflating per-page counts.

---

## Appendix B: Link Inventory

We recorded every same-host internal link found on each audited page. The total is dominated by the HTML sitemap page (`/`), which lists 4 same-host URLs across the full site. The remaining 1 pages average 2 links each -- normal for a site with a large navigation menu and footer. External links are not tracked; this inventory covers same-host `<a href>` links only. Link status was not probed; for a dedicated broken-link audit, run a rate-limited crawler on the link set at a time that suits the site.

Per page, internal links range from 2 to 4, averaging 3 across 2 pages. That is sparser than typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 6 |
| -- `/` page only | 4 |
| -- remaining 1 pages | 2 |
| External links (not tracked) | -- |
| Anchor-only (`#fragment`) links | 0 |
| mailto / tel links | 0 |
| URL variant links (same canonical) | 3     |

---

## Appendix C: Image Efficiency

We reviewed 60 images across the audited set: 48 SVG and 12 PNG. 0 of 60 (0.0%) carry alt text, leaving 60 without it. Each missing alt attribute is a place where a screen-reader user or a machine reading the page gets no description of what the image shows.

On loading strategy, 60 images are marked `loading="lazy"` and 0 `loading="eager"`, so every image on the audited pages declares its fetch behaviour explicitly rather than leaving the browser to guess.
...
**Scope:** 3 pages examined | Platform: Unknown Platform | Frameworks: **Foundation** (low confidence) -  CSS framework | Analysis method: Hybrid (automated + manual verification) | robots.txt: Absent (HTTP 404)
