...
- Pages audited: 22 of 148 in sitemap - sample run
- Date: 2026-06-05
- Scores: SEO 81/100 | Accessibility 73/100 | Discovery Readiness 22/100 | Structured Data 58/100
- Scope: 22 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): Question (56), Answer (56), ListItem (53), Service (36), Offer (29), Organization (15), BreadcrumbList (13), Audience (12), Country (10), CreativeWork (9), FAQPage (7), OfferCatalog (5), PostalAddress (3), WebSite (3), ContactPoint (2), WebPage (2), Thing (2), ImageObject (1), EducationalOccupationalCredential (1), ProfessionalService (1), GeoCoordinates (1), OpeningHoursSpecification (1), ItemList (1), AggregateRating (1)
...
SCOPE: 22 pages were audited. Scope all claims to "the audited set" - do not say "site-wide" unless cross-page consistency is explicitly confirmed in the facts.
ACCESSIBILITY NOTE: There are 7 distinct WCAG AA issue types across 517 raw instances. If Accessibility is not grade A, do NOT call it the "strongest" dimension in Paragraph 1. Acknowledge the accessibility opportunity in Paragraph 2 as a Priority 1 compliance item before the MX opportunity. Reference the DISTINCT count when discussing remediation effort (one fix per category clears many instances); reference the RAW count when discussing how many elements need touching.
...
- Accessibility: 7 distinct WCAG AA issue types (517 raw instances, 517 marked critical by Pa11y) - 397 of these instances trace to 38 recurring template patterns (single theme edit per pattern fixes all instances)
...
- Lowest machine-readiness score: 22/100
-->
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 22
- Performance: 12208ms avg (Could Be Better)
- Accessibility: 73/100 (Good)
- SEO: 81/100 (Excellent)
- Accessibility issues: 7 distinct WCAG codes across 517 raw instances (397 instances trace to 38 template patterns)
-->
...
- SEO (content pages only): 80/100
- Accessibility: 73/100 (7 distinct WCAG codes, 517 raw instances) - 397 of these instances trace to 38 template-level patterns
- Security headers: 2/5 present (HTTPS, HSTS); 0 of 22 audited URLs carry all five
- Structured Data Quality: 58/100
- Consistency: 38%
-->
...
| Offer | 5 | 0% | 0% | Service |
| Organization | 13 | 100% | 100% | ImageObject, EducationalOccupationalCredential, PostalAddress, OfferCatalog |
| BreadcrumbList | 13 | 100% | 100% | - |
| Audience | 2 | 100% | 100% | - |
| Country | 5 | 100% | 100% | - |
| CreativeWork | 9 | 100% | 100% | Organization |
| FAQPage | 6 | 100% | 100% | - |
...
- Pages affected by that check: 15 of 22
...
- Top bare selectors (rendered): `div.object-cover.w-full` (675), `div.slick-slide.slick-cloned` (488), `div` (241), `div.slick-slide` (204), `div.flex.items-center` (57)
- Pages contributing to the Div Soup aggregation: 22
...
WRITE:
2-3 sentences summarising the audit findings for Dotfusion (https://dotfusion.com). Always refer to the site by its domain (https://dotfusion.com), never by the consultancy name. The highest-scoring dimension is SEO at 81/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 81/100; Structured Data: 58/100; Discovery Readiness: 22/100
- Human visitors: served well
- Machine opportunity: structured data, discovery, metadata

DO_NOT:
- Say "site-wide" unless consistency was explicitly confirmed. 22 pages were audited; scope claims to "the audited set".
-->
