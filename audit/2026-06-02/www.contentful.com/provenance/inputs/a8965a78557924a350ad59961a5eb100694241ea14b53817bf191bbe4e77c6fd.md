...
Contentful's website serves its human audience well. SEO foundations across the audited set are excellent at 94/100, titles and meta descriptions are in place across product and navigational pages, and Schema.org JSON-LD is present in the served HTML of every audited page -- meaning every machine that fetches the raw HTML receives structured data without needing to execute JavaScript. The Organization entity carries `sameAs` links to Wikidata (Q18348837), Wikipedia, LinkedIn, GitHub, and YouTube, providing external authority anchors that machines can use to verify the publisher's identity. The homepage and product pages are clearly structured with proper landmark elements and a single H1 on each page.
...
| 6 | No llms.txt, no discovery link tags in page heads | Machine Readability Opportunity | High | Low | Machines receive no structured content index; growing AI-search gap |
| 7 | Schema Maturity Level 1: WebSite missing recommended properties | Machine Readability Opportunity | Med | Low | Machines lack attribution, publishing context, visual identity |
| 8 | JSON-LD in body (not head) on several audited pages | Machine Readability Opportunity | Med | Low | Head-only parsers may miss structured data |
...
**Priority 6: No llms.txt, No Discovery Link Tags in Page Heads**
...
- For a content estate of Contentful's scale (3,358 URLs in the sitemap), publishing an llms-full.txt companion file is worth scoping. This concatenates the full text of every published page into a single file so machines can ingest the corpus in one fetch rather than crawling each page individually.
...
- Move JSON-LD blocks into the `<head>` section. Structured data in `<head>` is reachable by machines that parse only the head for metadata. The current placement at body offsets of 18,000-22,000 bytes means these machines may not reach it on pages where they apply a token-budget limit.
- For product pages, consider adding `SoftwareApplication` schema to describe each product's capabilities, pricing model, and platform requirements. This enriches the machine's understanding beyond "this is a website" to "this is a specific software product."

**Effort:** Low (template and JSON-LD changes)

---

**Priority 8: JSON-LD Emitted Inside body on Several Audited Pages**
...
Single load-time measurements can mislead. A page that returns in a few hundred milliseconds on the crawler's second visit may be served from a warm CDN edge; the same page on a genuine cold visit could spend several seconds at the origin before the first byte arrives. To separate the two experiences, this section probes the slowest page from the crawl and a median-load control with three cache-busted GETs each, then compares those measurements against the crawler's original cold-cache baseline.
...
**Slowest.** The slowest page is `https://www.contentful.com/products/ai-actions/`. A first-time visitor sees the cold-cache cost: the crawler recorded 6,814ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three cache-busted re-probes that followed returned a returning-visitor median of **727ms**. **Returning-visitor verdict: Healthy**.
...
*Reference: the [IANA Well-Known URIs registry](https://www.iana.org/assignments/well-known-uris/well-known-uris.xhtml) lists the full set of registered `/.well-known/` paths and their RFCs.*
...
We identified structured data property gaps across the audited set. Each row names a single missing property on a single entity with a short note on why it matters to machines.

The full per-entity list is delivered alongside this report as a sidecar CSV: [`www-contentful-com-structured-data-findings.csv`](www-contentful-com-structured-data-findings.csv). The rows describe individual Schema.org property gaps on specific entities; most of them share a small number of underlying patterns, shown below ranked by instance count.
...
**What we mean by provenance gap.** A provenance gap is the structural distance between a page that *describes* a claim and a page that *evidences* it. Schema markup tells a machine what an entity is: a Product, an Article, an Organization: but it cannot tell a machine who made the assertion, when, or whether the claim is supported by anything outside any single page. The Provenance Gap concept and its full taxonomy are documented at <https://mx.allabout.network/blog/the-provenance-gap.html>.

### Templated clusters

No templated clusters detected at the audited scale.
...
Scoring a machine's metadata is not the same as scoring whether a machine can read each page at all. Pipeline Survivability (PS) runs eleven reading-resilience checks on every audited page. Each one asks whether a page survives a known agent-reading risk: truncation by the agent's fetch tool, JavaScript-only content, soft 404s, broken code fences, content negotiation drift, cross-host redirects, generic headings, content that begins too far into the document, or overhead-heavy pages where scripts, styles, and images outweigh actual content.

- **Truncation Risk** - Fail - 12/12
  - *Means:* Every page in the audited set exceeds the 250 KB threshold at which some machines stop reading. A machine with a limited fetch window may reach the metadata in the head but stop before the main content -- meaning the structured data and titles are seen but the body text is not.
  - *Data:* All 12 pages exceed the 250 KB ceiling. See www-contentful-com-pipeline-truncation-risk-pages.csv.
- **SPA Shell** - Pass - 12/12
  ...
  - *Data:* Max gap score: 3. 0 means served and rendered match.
- **Soft 404** - Pass - 12/12
...
- **Inline Tag Bloat** - Fail - 12/12
  - *Means:* Every page in the audited set carries large inline `<script>` blocks. The contact/sales page is the extreme case; machines that read the page as a document must parse through substantial inline JavaScript before reaching content. Externalising these blocks to separate `.js` files lets machines skip them during cheap head-first fetches.
  - *Data:* Multiple elements over 500 bytes per page. See www-contentful-com-pipeline-inline-tag-bloat-pages.csv.
...
The two failing checks -- Truncation Risk and Inline Tag Bloat -- both point to the same root cause: page payloads dominated by JavaScript. All 12 audited pages exceed the 250 KB truncation threshold, and all 12 carry large inline script blocks. Moving repeated analytics and consent scripts to external files (they appear identically across all pages) would reduce both the inline bloat and the total page size in one pass. The body content ratio of 17% reflects this imbalance; improving it would directly lift Pipeline Survivability toward the 90+ range.
...
|--------|--------------|----------------|--------------------|------------|
| Rendered HTML | 34/100 (high) | 290 bare divs (51% of containers, depth 7) | `div` (240), `div.grid-helper-col.col-span-1` (144), `div.tooltip_root__EulUk` (95), `div.button_markup_root__sACGr.button_markup_secondary__F_qxS` (68), `div.round-icon.round_icon_container__Jmlwp` (57) | <https://www.contentful.com/products/personalization/> |
...
### AI Judgment -- Review Required

The following finding requires human review before the report ships. It is derived from LLM analysis of observed patterns and cannot be verified deterministically.

**Personalisation layer serving test-variant H1s to automated fetches.** During manual verification, the homepage was fetched directly and the H1 read "Sorry, content chaos -- your time's up". A separate run returned "Lorem ipsum dolor sit amet... whatever". Neither is the production headline that the Puppeteer-rendered audit score reflects (SEO 94/100 is consistent with real content). Contentful appears to be running A/B or personalisation experiments that serve placeholder H1 content to automated user-agents, including the crawlers used by this audit. Automated agents that fetch raw HTML without browser rendering may receive these test variants rather than production content. This warrants investigation to confirm that test variants are not inadvertently exposed to real machines at inference time.
...
2. **Review Priority 2-5 findings**: CSP, skip link, alt text, performance improvements that compound over time
...
SEO is the standout strength across the audited set: 94/100 overall and 93/100 on content pages, with titles, meta descriptions, and canonical URLs consistently in place. JSON-LD is present in the served HTML of every audited page, all 8 tested machines return HTTP 200 at inference time, and the Organization entity carries strong linked-data signals including Wikidata (Q18348837). The headline opportunities are accessibility (279 WCAG 2.1 AA issues with 14 template-level patterns to resolve), a missing Content Security Policy across all audited pages, and a Discovery Readiness score of 39/100 driven by the absence of llms.txt, discovery link tags, and MX governance fields. Addressing the accessibility and CSP items closes the immediate compliance exposure; adding llms.txt and MX governance metadata moves the site from MX Readiness Level 1 (Discoverable) to Level 2 (Citation-ready). We are ready to scope either engagement.
...
**Link inventory:** We record every internal link found on every audited page with its URL, anchor text, and link type. We do not probe link status: a dedicated, rate-limited broken-link crawler handles that separately. Appendix B is a link inventory, not a broken-link list.

**Scope:** 12 pages analysed | Platform: Next.js | Analysis method: Hybrid (automated + manual verification) | robots.txt: Found

**Measurement completeness:** Some probes may encounter network errors (HTTP 499 responses) or timeouts that halt data collection for that check. These are recorded in the audit findings as informational notes. Network timeouts are expected behaviour; a machine reading under time constraints stops processing just as a human reviewer would if time ran short.
...
The deterministic evidence chain lives at `www-contentful-com-report.provenance.deterministic.json`. It records every rule-driven step: gate verdicts, CSV checks, regex matches, render steps, probe results, and the closing PDF conformance verdict. This chain is designed as evidence for EAA Directive 2019/882 accessibility conformance.
