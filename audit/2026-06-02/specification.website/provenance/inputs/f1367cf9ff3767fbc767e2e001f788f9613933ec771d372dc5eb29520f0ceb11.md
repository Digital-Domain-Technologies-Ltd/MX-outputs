...
Across the 11 audited pages, SEO leads at 93/100 with Excellent ratings on performance, accessibility, and security. The site is machine-accessible today: 100/100 served HTML, all AI crawlers admitted, Content-Signal directives in robots.txt, and a full llms.txt and llms-full.txt discovery pair. The headline opportunity is the governance layer: score thresholds for Citation-ready are already met, and adding MX governance fields to published pages is the one step needed to move from Discoverable to Citation-ready. Contact us to scope the implementation.
...
The pipeline scores 91/100 with two checks flagged. Inline Tag Bloat fires on all 11 pages: each carries a 918-byte inline JavaScript block used for prerendering hints. Moving this to an external file would let agents skip it during a cheap fetch, reducing bytes-before-body on every page. Content negotiation (Vary: Accept) fires on three pages -- these are the pages that serve Markdown when asked, which is a positive capability and not a problem; the Vary header is correct behaviour for content-negotiating servers. The rest of the checks are clean.
...
Partially Compatible for this site type: Stages 1 and 2 pass, Stage 2 at Partial, and Stages 3 and 4 pass on the metrics that apply to a non-commerce specification site. Stage 5 (Purchase Confidence) does not apply -- the site has no transaction or checkout pages.
...
| 4 | Schema maturity at Level 1, recommended properties absent | Machine Readability Opportunity | Low | Low | Agents have less context for author, publisher, and dates |
...
- The breadcrumb separator is marked `aria-hidden="true"`, which is correct. Do not remove that attribute; it correctly hides the purely visual separator from screen reader users.
...
**Finding:** `/spec/foundations` has a heading level jump from H1 directly to H3 (no H2 between them). Lighthouse flags this as a heading-order shortfall. Screen readers and machines that build a document outline from the heading hierarchy will see a gap in the structure. Every other audited page passes Lighthouse heading compliance.
...
- Add `<link rel="canonical" href="https://specification.website/[path]">` if not already present (the audit found canonical URLs in the marker reachability table, so this may already be covered -- verify against the specific governance `canonicalUri` field, which is distinct from the HTML `<link rel="canonical">`).
...
**Priority 4: Schema Maturity Level 1, Recommended Properties Absent**
...
**What to change and why:**
- Add `image` to the WebSite entity with the site logo URL. This enables machines to visually identify the source.
- Add `author` and `publisher` entities, linked by `@id`, to the WebSite block. A single `Organization` or `Person` entity defined once and referenced from each page covers the whole site.
...
We found three areas to strengthen, ordered by regulatory exposure first, then by reach across the audited set.
