...
reportId: "crowdfavorite-com-WEB-AUDIT-20260602"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-02/crowdfavorite.com/crowdfavorite-com-report.md
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-06-02/crowdfavorite.com/crowdfavorite-com-report.pdf"
...
**Date:** 2 June 2026\
**Report ID:** crowdfavorite-com-WEB-AUDIT-20260602
...
Across the audited set, SEO is Excellent at 96/100. Performance sits at Could Be Better (35/100, 3569ms average); accessibility has room to strengthen from 74/100.
...
Across the audited set, we find that machines can discover and parse the content with a Discovery Readiness of 40/100 and Structured Data Quality of 90/100, yet they lack governance context as MX Readiness Level 1 indicates discoverable only, with MX Stack Completeness at 73/100 and Pipeline Survivability at 63/100.
...
We found a strong foundations across the audited set, with SEO scoring 96/100 and content pages at 95/100, accessibility at 74/100, and structured data quality at 90/100. The consistent presence of all five security headers on 11 of 12 URLs and a 62 % consistency score further reinforce the groundwork for the improvements that follow.

| Dimension | Score | Highlights |
|-----------|-------|------------|
| Performance | Could Be Better | 35/100 - 3569ms average; JS-heavy pages driving the gap |
...
We found four areas to strengthen, ordered by compliance exposure first, then machine readability. The 548 accessibility issues (Pa11y: automated WCAG 2.1 AA testing) break down into three fixable patterns across the audited set -- one template, one vendor SDK, one image authoring gap.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Duplicate ID attributes, WCAG 4.1.1 (391 instances on homepage) | Compliance Risk | High | Low | ARIA and AT navigation unreliable; duplicate IDs break the accessibility tree |
| 2 | Images missing alt text (55 instances) and image links missing alt (28 instances) | Compliance Risk | High | Low | Screen reader users cannot interpret images or understand image-link destinations |
| 3 | Complianz consent close button missing accessible name, WCAG 4.1.2 (11 instances) | Compliance Risk | Medium | Low (vendor SDK) | Screen reader users cannot close the consent banner |
| 4 | Discovery artefacts absent: llms-full.txt, agent-card.json | Machine Readability Opportunity | Medium | Low | Machines that crawl discovery files for corpus ingestion find an incomplete picture |

---

**Priority 1: Duplicate ID Attributes, WCAG 4.1.1 (391 Instances, Homepage)**

**Bucket:** Compliance Risk

**Finding:** Pa11y recorded 391 duplicate ID failures (WCAG 4.1.1 F77) on the homepage. The top offenders are `#module-contact-request-container` (13 duplicates), `#lets-talk` (6 duplicates), and `#module-contact-tell` (6 duplicates). These IDs are generated by a module or widget rendered multiple times in the page template. Duplicate IDs break the HTML spec, corrupt the accessibility tree, and make ARIA attributes unreliable -- any `aria-labelledby`, `aria-describedby`, or internal anchor pointing at these IDs will resolve to the wrong element or none at all.

**What to change and why:**

- Find the page template component that renders `#module-contact-request-container`, `#lets-talk`, and `#module-contact-tell`. Each must appear exactly once per page. Convert repeated instances to use unique IDs (suffix with an index: `module-contact-request-container-1`, `module-contact-request-container-2`) or remove the ID from the repeated instances if ARIA does not reference it.
- Run Pa11y or any WCAG 4.1.1 checker after the fix to confirm the count drops to zero. The 391 instances are dominated by these three IDs -- a single template edit covers the bulk of the finding.
- WCAG 4.1.1 (Level A) is the lowest conformance level; failures here are a prerequisite for any higher-level accessibility claim.

**Effort:** Low

---

**Priority 2: Missing Alt Text on Images (55 Instances) and Image Links (28 Instances)**

**Bucket:** Compliance Risk

**Finding:** Pa11y found 55 images missing `alt` attributes (WCAG 1.1.1 H37) and 28 image-only links where the wrapped image has no alt text (WCAG 1.1.1 H30). Both classes are on the homepage. A sample context: `<img decoding="async" src="https://crowdfavorite.com/wp-content/uploads/2023/06/...">` -- WordPress content images uploaded without alt text, and `<a href="https://crowdfavorite.com/insights/weve-been-here-before/"><img ...></a>` -- thumbnail links where the image carries the entire link purpose for screen reader users.

**What to change and why:**

- Add `alt` attributes to all content images. For editorial images, the alt should describe what is shown; for decorative images, use `alt=""` (empty, not absent) to signal the image is decorative.
- For image-only links (`<a><img></a>` with no text), the image `alt` is the only text a screen reader user hears for the link. Write alt text that describes the link destination, not the image itself: "Read: We've Been Here Before" rather than a description of the thumbnail.
- Both categories are WCAG 1.1.1 Level A -- the minimum accessibility floor for perceivable content.

**Effort:** Low

---

**Priority 3: Complianz Consent Close Button Missing Accessible Name, WCAG 4.1.2 (11 Instances)**

**Bucket:** Compliance Risk

**Finding:** Pa11y flagged `<div class="cmplz-close" tabindex="0" role="button" aria-label=" ">` -- the Complianz (CMPLZ) cookie consent banner close button. The `aria-label=" "` value is a whitespace-only string, which is treated as absent. Screen reader users cannot close the consent banner. The selector is `#cmplz-cookiebanner-container > div > div:nth-child(1) > div:nth-child(3)` -- the element is injected by the Complianz SDK, not the site template.

**What to change and why:**

- The element is rendered by the Complianz consent management SDK at runtime. The fix path is to engage Complianz (via their plugin settings or a support request) to ship a corrected `aria-label` such as "Close cookie consent" -- not a site-template or theme edit.
- As an interim measure, a small DOM-observer script can add the correct `aria-label` after the banner mounts: `document.querySelector('.cmplz-close')?.setAttribute('aria-label', 'Close cookie consent banner')`. This resolves the WCAG 4.1.2 Level A violation immediately while the SDK fix is pending.

**Effort:** Low (vendor SDK path: DOM-observer interim fix is trivial; full fix requires Complianz SDK update)

---

**Priority 4: Discovery Artefacts Absent: llms-full.txt, agent-card.json**

**Bucket:** Machine Readability Opportunity

**Finding:** The site publishes `llms.txt` (positive) but not `llms-full.txt` or `/.well-known/agent-card.json`. The `llms-full.txt` convention (popularised by Fern and Mintlify) concatenates the full published corpus into one file so machines can ingest the whole site in a single fetch -- approximately 10x fewer tokens than page-by-page crawling. The `agent-card.json` (Google A2A protocol) describes the site's service capabilities to agentic workflows. Neither is required, but both are low-effort and raise Discovery Readiness from 40/100.

**What to change and why:**

- Add `llms-full.txt` at the domain root containing the full text of every published page, one `## [Page Title](URL)` section per page. Serve it as `text/html` with content in a `<pre>` block and list it in `sitemap.xml` so Common Crawl ingests it.
- Add `/.well-known/agent-card.json` describing the site's capabilities (read-only informational site, content types, contact endpoint). Minimal JSON following the A2A spec; the agent-card.json for specification.website is a good reference model.
...
We found a robots.txt file that disallows four paths and does not reference a sitemap. The file permits all other agents to crawl the remaining content.
...
We graded the sitemap as Partial. The most significant attribute is the lack of changefreq and priority, which remain useful signals for non‑Google crawlers and machines, while the sitemap does include lastmod dates for all 379 URLs.



### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found llms.txt present and it includes a site description, but it lacks a page inventory and a content policy; we recommend adding those elements and using text/html content‑type.
...
Across the 11 pages we audited, structured data is strong. Machines extract entity data from these pages reliably and the typed vocabulary survives the read; whether machines use the content for attribution is their decision -- governance metadata (see the MX Readiness Level section) gives them the structured context to do so accurately. A full-estate audit confirms whether the same pattern holds across the rest of the site.
...
Partially Compatible for this site type; Purchase Confidence does not apply – the site has no transaction or checkout pages.
...
We found that every audited page shows a truncation risk, meaning machines may receive incomplete content.  
This can cause search crawlers and other automated bots to miss key information or misinterpret the page.  
Fixing the truncation risk - by ensuring the full page is delivered - offers the biggest opportunity to strengthen machine readability and overall resilience.
...
We observed No content variance was detected across cache‑busted fetches in this audit run.  
An unknown personalisation/A/B testing vendor was detected on pages across the audited set.  
Machines arrive cold on every fetch, with no cookies or session continuity, so each visit may return a different experiment variant.  
This variability can cause sitemap‑sweeping training corpus builders to ingest contradictory facts, masking the publisher’s canonical content.  
We recommend that the publisher configure the personalisation layer to serve a consistent canonical version to machines, or provide a crawler‑friendly variant that excludes experiment‑driven content.

---

## Content Consistency

We found consistent metadata patterns across the audited set, with no organisation‑name or canonical‑URL divergence flagged by the consistency check.
...
We note that accessibility legislation worldwide has converged on ISO 14289‑1 (PDF/UA) as the technical baseline, with the EAA Directive (EU) 2019/882 - effective 28 June 2025 - providing the most precisely codified example, while Section 508, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent Australian and Canadian statutes all resolve to the same structural artefact.  
We also recognise that an untagged PDF remains invisible to machines, as search crawlers, AI systems and automated pipelines cannot extract text, entities or structure from a scanned or image‑based PDF, whereas a tagged PDF with a proper structure tree is machine‑readable in the same way that semantic HTML is.
...
| https://crowdfavorite.com/wp-content/uploads/2022/12/Giant-Health-Floor-Plan-Crowd-Favourite.pdf | https://crowdfavorite.com/campaign/gianthealth | No |
...
We found that https://crowdfavorite.com excels in SEO, scoring 96/100, which positions it strongly for search visibility across the audited set.  
Key opportunities lie in Discovery Readiness, which scored 40/100, and in Accessibility, which remains an area for improvement across the audited set
...
Across the audited set we examined 568 images. The format distribution is 1 WebP, 112 SVG, 330 PNG, 76 JPEG, and 49 images in other or unrecognised formats. Alt text is present on 397 images, which is 69.9 % of the total, leaving 171 images without alt text.

We found that 260 images use the loading="lazy" attribute, 0 images use loading="eager", and 308 images have no loading attribute set. The absence of a loading attribute means the browser will decide how to load the image, which is not the same as eager loading. This mix indicates an opportunity to standardise the loading strategy across the audited set.
...
**Platform detection:** We fingerprint the hosting platform from HTTP response headers and HTML signatures. Detected platform: **Drupal**. The main audit uses Drupal-specific rate limits from our platform knowledge base. Requests are paced at that platform's known-safe speed, with exponential backoff and retry (up to 4 attempts) on rate-limit responses.
...
**Scope:** 11 pages analysed | Platform: Drupal | Analysis method: Hybrid (automated + manual verification) | robots.txt: Found
