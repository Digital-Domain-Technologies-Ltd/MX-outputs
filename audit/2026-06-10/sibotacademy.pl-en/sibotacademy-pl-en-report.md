...
reportId: "sibotacademy-pl-en-WEB-AUDIT-20260610"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-10/sibotacademy.pl-en/sibotacademy-pl-en-report.md
  runbook: "Executive audit report for Sibotacademy. Focus on the highest-leverage MX opportunities surfaced by the audit. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-10/sibotacademy.pl-en/sibotacademy-pl-en-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-10/sibotacademy.pl-en/sibotacademy-pl-en-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-10/sibotacademy.pl-en/sibotacademy-pl-en-report.pdf"
...
**Date:** 10 June 2026\
**Report ID:** sibotacademy-pl-en-WEB-AUDIT-20260610
...
Your site reads as aimed at a child audience, and your page text names an age band (`7-14`) that is not in machine-readable structured data, so an agent has to infer it; declaring `schema:typicalAgeRange` or `mx:intendedAgeRange` would let an agent read the band directly. Your site runs on **Vercel** (detected from multiple platform signals).

Across the audited set, your learning platform delivers a fast, responsive experience that keeps learners engaged. Performance is excellent, with page loads that feel instant on both desktop and mobile. The clear layout of course listings and intuitive navigation further support users in finding content quickly.

The next step for your audience is to ensure every learner interaction is accessible. We identified one distinct WCAG AA issue type affecting 35 elements; addressing this single category will resolve all critical instances. Beyond accessibility, the headline opportunity lies in enriching machine comprehension. Adding full MX governance fields-mx:status, mx:contentType, mx:audience, canonicalUri, provenance markers-will lift your pages from Discoverable to Governed, raising MSC above 60 and Discovery Readiness above 40.

\clearpage

<!-- AUDIT-DELTA:START -->
## Change Since Our 9 June 2026 Audit

We last audited sibotacademy.pl on 9 June 2026. The table compares that audit with the current one across the headline measures. Every headline score is unchanged from that audit, so the recommendations from that audit remain open.

| Measure | 9 June 2026 | 10 June 2026 | Change |
|---------|------:|------:|:-------|
...
The Human Experience subtable summarises how we evaluate the audited pages for visitors. Across the audited set, the site delivers a strong experience for human visitors, with excellent performance, accessibility and SEO scores.
...
Across the audited set, machines can discover and parse your content-Discovery Readiness 32/100 and Structured Data Quality 52/100-but lack governance context, as shown by an MX Readiness Level of 1 (Discoverable), despite an MX Stack Completeness of 59/100 and Pipeline Survivability of 97/100.
...
Across the audited set, your pages demonstrate a strong foundations for digital excellence. High scores in SEO (87/100) and Accessibility (95/100), together with a strong structured-data implementation, lay the groundwork for further optimisation.
...
**Finding:** Intended audience not machine-declared (child audience inferred from content) (across the audited set). Agents must infer the intended audience from the prose; a declared signal removes the ambiguity and supports age-appropriate routing
...
**Finding:** Age-assurance signal not machine-detectable (child audience) (across the audited set). An agent cannot tell whether age assurance is in place; where one is operated, it is not exposed in a way a machine can read
...
**Finding:** Consent signal not machine-detectable on first visit (child audience) (across the audited set). An agent cannot confirm whether a consent mechanism is presented before processing
...
We identified two resilience checks that warrant attention across the audited set - a SPA shell and inline tag bloat.  
The inline tag bloat, affecting three of eleven pages, can slow or confuse machines as they parse heavy markup, potentially causing missed content or delayed indexing.  
Simplifying the markup on those pages by moving scripts and styles to external files offers the greatest opportunity to improve machine comprehension and overall resilience.
...
Across the audited set, the worst page at https://sibotacademy.pl/en/worlds shows a bare-div ratio of 81 % in the served markup; this deprives machines of reliable structural cues and forces them to rely on positional inference to determine meaning.  
The soup is structural: a deepest bare chain of eight levels indicates that many nested divs are inserted by a drag-and-drop builder or an untyped component framework, producing deep nesting rather than shallow surface-wide clutter.  
The cheapest first move is to wrap the obvious landmarks-header, nav, main, footer, aside-and give remaining elements meaningful class names; this will lower the bare-div ratio without restructuring the layout.
...
We observed no content variance across cache-busted fetches in this audit run. Machines arrive cold on every fetch - no cookies, no cohort or session continuity - so each visit can trigger a different experiment variant. This stateless behaviour means that a crawler building a knowledge graph may record contradictory facts from the same URL, and sitemap-sweeping training corpus builders could ingest inconsistent content across successive crawls. The publisher’s canonical message is therefore obscured by the experiment layer.

We recommend adding bypass rules in your personalisation platform to serve canonical content when the User-Agent matches known machine visitors such as ClaudeBot, GPTBot or CCBot. Reference the experiment URLs in llms.txt with a note that content varies by session so machines are warned not to treat any single fetch as canonical. Consider serving a stable, non-personalised version of key pages to bot User-Agents via CDN routing rules.

---

## Content Consistency

We examined metadata across the audited set to assess how consistently your pages present brand information, canonical URLs, and structured data. Consistency ensures that machines interpret each page as part of a unified content strategy, reducing duplicate signals and improving crawl efficiency. Our analysis focused on title tags, meta descriptions, canonical links, and entity values.
...
We note that accessibility legislation worldwide has converged on ISO 14289-1 (PDF/UA) as the technical baseline for PDFs, with the European Accessibility Act providing a highly detailed example of this alignment.  
We also observe that an untagged PDF remains invisible to machines-search crawlers, AI systems and automated pipelines cannot extract text or structure from a scanned or image-based file, whereas a tagged PDF with a proper structure tree is machine-readable in the same way semantic HTML is.
...
We found that https://sibotacademy.pl excels in SEO, scoring 87/100 across the audited set, which positions your content strongly for search visibility. However, discovery readiness and structured data lag behind at 32/100 and 52/100 respectively, limiting how effectively machines can locate and interpret your pages. Addressing these gaps will unlock greater machine discoverability and richer data presentation.
...
**Platform detection:** We fingerprint the hosting platform from HTTP response headers, HTML signatures, asset paths, and class patterns. Platform identification is probabilistic -- a site can obscure or mimic platform signals. We report the result as: **Vercel** (high confidence - multiple fingerprint signals). The main audit uses Vercel-specific rate limits from our platform knowledge base. Requests are paced at that platform's known-safe speed, with exponential backoff and retry (up to 4 attempts) on rate-limit responses.

**Frameworks detected:** **Next.js** (medium confidence) - JS framework; **Tailwind CSS** (medium confidence) - CSS framework; **Vercel** (low confidence) - CDN / delivery. Framework detection scans JS component frameworks, CSS utility libraries, CMS plugins and page builders, and CDN/delivery layers from cached HTML. Confidence is high (3+ signals), medium (2 signals), or low (1 signal, treat as a hint). Low-confidence detections are noted but do not influence scoring.
...
**Scope:** 11 pages examined | Platform: Vercel | Frameworks: **Next.js** (medium confidence) - JS framework; **Tailwind CSS** (medium confidence) - CSS framework; **Vercel** (low confidence) - CDN / delivery | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (7 directives)
