...
Across the audited set, your content delivers a fast and engaging experience that resonates with visitors. Performance leads the scorecard as the strongest human-experience dimension, giving users instant access to learning materials and interactive elements. The clear layout, concise course descriptions, and consistent branding reinforce trust and ease of navigation.

The headline opportunity we want to draw attention to is enhancing machine comprehension through governance metadata. We identified a single WCAG AA issue type that appears in 35 instances; addressing it will lift accessibility to full compliance. Beyond accessibility, the next step for your digital presence is to add full MX fields-mx:status, mx:contentType, mx:audience, canonicalUri, and provenance markers-to each page. This will elevate your MX Readiness Level from Discoverable to Governed, raising MSC above 60 and Discovery Readiness above 40.
...
Human Experience - this subtable summarises key metrics that matter to visitors.  
Across the audited set, we deliver an excellent experience for human visitors, with performance at 896 ms average, accessibility at 95/100, and SEO at 87/100.
...
We find that across the audited set, machines can discover and parse the content but currently lack structured governance context, reflected in a Discovery Readiness score of 32/100, Structured Data Quality of 52/100, MX Stack Completeness of 59/100, Pipeline Survivability of 97/100, and an overall MX Readiness Level of 1 (Discoverable).
...
Across the audited set, we see strong foundations in search visibility and user experience, with an SEO score of 87/100 and accessibility at 95/100, while also establishing a solid groundwork with a 52/100 structured data quality score. These achievements lay a solid groundwork for further enhancements.
...
- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt to declare content-use policy for machines, enabling agents to understand how they may reuse or transform your content.
...
We found that three of the audited pages contain excessive inline scripts or styles, a condition we call Inline Tag Bloat.  
This bloat can slow down or even block machines from parsing the page content quickly, reducing their ability to index and serve the information.  
Moving those inline tags into external files would give the biggest improvement in how reliably and swiftly machines read the site, while addressing the smaller SPA Shell issue would further strengthen resilience.
...
We find that your pages employ semantic HTML effectively, giving machines clear structural cues for accurate interpretation.
...
Google Optimise, an A/B testing platform, was detected on eight URLs: https://sibotacademy.pl/en/index.html, https://sibotacademy.pl/en/parent/sign-in, https://sibotacademy.pl/en/privacy-policy.html, https://sibotacademy.pl/en, https://sibotacademy.pl/en/forparents.html and six additional pages.  

Machines arrive cold on every fetch - no cookies or session data are persisted, so cohort assignment is random or time-based. Consequently, each request may return a different experiment variant. This stateless behaviour means that a bot building a knowledge graph could record contradictory facts across visits to the same URL. Sitemap-sweeping training corpus builders are also affected; each crawl sweep may ingest different variants of the same page, embedding conflicting information into training data. The publisher’s canonical content - what the business actually says - is therefore masked by the experiment layer.

Mitigations: add bypass rules in your personalisation platform to serve canonical content when the User-Agent matches known machine visitors such as ClaudeBot, GPTBot, CCBot, etc.; reference experiment URLs in llms.txt with a note that content varies by session so machines are warned not to treat any single fetch as canonical; consider serving a stable, non-personalised version of key pages to bot User-Agents via CDN routing rules.
...
We observe that accessibility legislation worldwide has converged on ISO 14289-1 (PDF/UA) as the technical baseline, with the European Accessibility Act-Directive (EU) 2019/882, in force 28 June 2025-providing the most precise codification while other jurisdictions such as the United States Section 508, the United Kingdom Public Sector Bodies Accessibility Regulations 2018, and equivalent Australian and Canadian laws all resolve to the same structural artefact.  
...
We scored SEO at 87/100 on the audited set of https://sibotacademy.pl, showing strong optimisation that helps machines discover and rank content. Opportunities lie in structured data (52/100) and discovery readiness (32/100), where richer metadata and better crawlability would enhance machine comprehension. We recommend prioritising these improvements to unlock further visibility and engagement.
