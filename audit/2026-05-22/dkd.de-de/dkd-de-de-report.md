...
We audited seven pages of www.dkd.de and found a property that is clearly built with human visitors in mind. The content is well-structured, the information architecture is coherent, and the SEO foundations are in excellent shape, scoring 92/100 across the audited set. Performance stands out as the strongest dimension of the human experience, giving visitors a fast and responsive interaction from the first page load.

The priority item we want to bring to your attention before discussing machine readiness is accessibility. Across the audited set, we recorded 527 raw instances flagged as critical, spanning 14 distinct WCAG AA issue types. The remediation picture is more manageable than the raw count suggests: 408 of those instances trace to 64 recurring template patterns, meaning a single theme-level correction per pattern has the potential to resolve the majority of flagged elements in one pass. Addressing these issues is a Priority 1 compliance matter, and we recommend treating it as a near-term workstream independent of any other improvements. From there, the headline opportunity is machine readiness. Discovery Readiness scores 25/100 and Catalogue Visibility sits at 10/100, the lowest score across the audited set, which tells us that while human visitors can find and use www.dkd.de with confidence, the machines that increasingly mediate discovery, citation, and agent-driven referrals have very limited visibility into the property.

The groundwork is there to change that picture substantially. The TYPO3 CMS platform is capable of serving well-formed structured data, and the Schema.org JSON-LD already present across the audited set gives agents a reliable surface to read regardless of how any given runtime processes the page. Building on that foundation, the next step is to extend the structured data vocabulary, improve the discovery artefacts, and ensure that the signals machines rely on are as well-considered as the signals www.dkd.de already provides to its human audience.
...
Across the audited set, www.dkd.de delivers a strong human experience, with Performance, Accessibility and SEO all sitting in the Excellent band, though the volume of recurring accessibility instances points to template-level patterns as the primary area for improvement.
...
Machines visiting www.dkd.de across the audited set can reliably process and pass on page content once they reach it, though their ability to discover, contextualise, and act on that content is constrained by the gap between a strong Pipeline Survivability and the lower Discovery Readiness and MX Stack Completeness scores.
...
**Evidence:** MX Stack Completeness 49/100 | Structured Data Quality 65/100 | Discovery Readiness 25/100 | Consistency 89%
...
Across the audited set, www.dkd.de demonstrates a strong technical foundation, with SEO sitting at 92/100, Accessibility at 90/100, and cross-page Consistency at 89%. These scores reflect deliberate, careful work and provide the groundwork for the targeted improvements this report outlines.
...
| Agent access | 0/7 | 0 of 7 AI user-agents receive HTTP 200; ClaudeBot (Anthropic), GPTBot (OpenAI), ChatGPT-User (OpenAI), and 4 more blocked |
...
We have prioritised the findings below by the degree to which each gap constrains what machines can discover, interpret, and act on across the audited set. Discovery gaps lead the order because they block everything downstream: a machine that cannot reliably locate or index content cannot benefit from the structured data, metadata, or agent-facing signals we examine in the sections that follow.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Duplicate ID Attributes, WCAG 4.1.1 (5 IDs repeated across all 7 audited pages) | Compliance Risk | High | Low | Assistive tech users may miss or misread labelled regions when screen readers encounter non-unique IDs |
| 2 | Semantic Structure 53/100, Div Soup at https://www.dkd.de/de/kontakt (26 of 48 bare divs) | Compliance Risk | Medium | Medium | Users relying on structural navigation are less likely to move efficiently through content-dense pages |
| 3 | Discovery Readiness 25/100, No llms.txt or Agent Card Detected | AI Opportunity | High | Low | Machines risk missing the full scope of www.dkd.de content and may not attribute it correctly in agent answers |
| 4 | Structured Data Quality 65/100, Missing Organisation and Service Schema | AI Opportunity | Medium | Medium | Machines may not confidently link www.dkd.de to specific services and entity relationships, reducing citation eligibility |

---

**Priority 1: Duplicate ID Attributes, WCAG 4.1.1 (5 IDs Repeated Across All 7 Audited Pages)**

**Bucket:** Compliance Risk

**Finding:** Across the audited set, five ID values — `accessibility`, `account`, `alarm`, `article`, and `bell` — each appear more than once per page, and all five recur across all seven audited pages. WCAG 4.1.1 (Parsing) requires that every ID attribute be unique within a document; when duplicates exist, the accessibility tree becomes ambiguous. Screen readers and other assistive technologies rely on unique IDs to resolve landmark references, form associations, and ARIA relationships. Because 77% of the total issues traced to recurring template-level patterns, these five IDs appear to originate in shared template components, meaning a targeted fix at the template level would resolve all instances across the audited set in a single change.

**What to change and why:**

- **Ensure each ID is unique per page, not just per component.** When the same icon or region component is rendered more than once in a document (for instance, once in the header and once in a sticky navigation), a static ID baked into the component will collide. Scoping IDs to their rendered instance — or replacing presentational IDs with class attributes where no programmatic reference is needed — directly addresses WCAG 4.1.1 and removes the ambiguity that causes assistive technologies to resolve the wrong target.
- **Audit ARIA attributes and form label associations that reference these five IDs.** Any `aria-labelledby`, `aria-describedby`, or `<label for>` that points to a duplicated ID may silently wire to the wrong element in the DOM. Confirming those associations after deduplication ensures that the accessible name tree reflects the intended structure and prevents regressions in WCAG 1.3.1 (Info and Relationships).
- **Update the template component or shared partial that generates these IDs.** Because the same five IDs appear on every one of the seven audited pages, the source is almost certainly a shared layout component. A single corrective edit at that level propagates the fix across every page that uses the template, rather than requiring per-page remediation.

**Effort:** Low

---

**Priority 2: Semantic Structure 53/100, Div Soup at https://www.dkd.de/de/kontakt (26 of 48 Bare Divs)**

**Bucket:** Compliance Risk

**Finding:** At https://www.dkd.de/de/kontakt, the worst-performing page in the audited set, we record a rendered semantic structure score of 53/100 (medium band). The figures cited — 26 bare divs out of 48 total elements — come from that specific page and should not be read as a uniform average across the audited set. That said, the score falls below 70 across the set as a whole, which suggests the template driving this page is a shared pattern rather than an isolated outlier; other pages using the same layout are likely to carry a similar structural profile. Bare divs contribute no semantic information to the accessibility tree, leaving users who rely on structural navigation — screen reader users scanning by landmark or heading, for instance — with fewer reliable entry points into the content.

**What to change and why:**

- **Replace structural wrapper divs with appropriate landmark elements.** Containers that delimit a navigation region, a main content area, a complementary sidebar, or a page footer each have a native HTML counterpart. Substituting the correct element for a generic div makes the region discoverable by landmark navigation without any additional ARIA, directly improving WCAG 1.3.1 (Info and Relationships) and raising the semantic structure score.
- **Reserve divs for purely presentational grouping.** Where a div wraps only layout concerns (spacing, grid columns) and carries no meaning, it can remain a div; however, any container that groups related content for a user should carry a role that conveys that relationship. Drawing the line between presentational and structural grouping is the practical decision the development team needs to make here.
- **Review the shared template responsible for the contact-page layout.** Because the low score traces to a template-level pattern, a structural review of the contact-page component will address the root cause rather than requiring page-by-page edits. Pages sharing the same template will benefit from the same change.

**Effort:** Medium

---

**Priority 3: Discovery Readiness 25/100, No llms.txt or Agent Card Detected**

**Bucket:** AI Opportunity

**Finding:** www.dkd.de scores 25/100 on Discovery Readiness, placing it in the Needs Improvement band. We did not detect an `llms.txt` file or an agent card across the audited set. Without these machine-readable entry points, machines visiting www.dkd.de have no structured signal about which content is authoritative, which sections are in scope for indexing or citation, and how the entity relates to its services. The MX Stack Completeness score of 49/100 reinforces that the overall machine-facing infrastructure has significant gaps. These gaps reduce the likelihood that www.dkd.de content is attributed accurately in agent-generated answers.

**What to change and why:**

- **Publish an `llms.txt` file at the root of www.dkd.de.** This file provides machines with a concise, structured summary of the site's purpose, authoritative content sections, and preferred citation scope. Improving the Discovery Readiness score from 25/100 requires this foundation. We note that our recommendation diverges from the llmstxt.org specification — we recommend serving the file as `text/html` to maximise compatibility with the broadest range of crawlers.
- **Publish an agent card.** An agent card expresses the structured identity of www.dkd.de as an entity: its name, primary language, content scope, and points of contact for automated access. Without it, machines must infer these properties from unstructured content, which reduces agent confidence in attribution and increases the risk of misidentification in multi-source answers.
- **Align the MX Stack Completeness posture with the llms.txt and agent card additions.** The 49/100 MX Stack score reflects a broader gap in machine-facing metadata. Publishing the two artefacts above will move that score and widen the surface area available to machines navigating www.dkd.de for citation purposes.

**Effort:** Low

---

**Priority 4: Structured Data Quality 65/100, Missing Organisation and Service Schema**

**Bucket:** AI Opportunity

**Finding:** The Structured Data Quality score of 65/100 (Good band) reflects the presence of `ListItem`, `BreadcrumbList`, and `WebSite` types across the audited set, but we do not detect `Organisation` or `Service` schema among the deployed types. These two types are the primary signals by which machines link a domain to a legal entity and to the specific services that entity offers. Without them, machines processing www.dkd.de content must infer entity identity and service scope from natural-language text alone, which reduces the confidence of structured citations and may cause the brand to surface less consistently in agent-driven responses.

**What to change and why:**

- **Add an `Organisation` schema block to the root page.** This type carries canonical properties — legal name, founding information, contact details, geographic scope — that allow machines to unambiguously identify www.dkd.de as a distinct entity. Including `sameAs` links to authoritative external profiles (such as industry directories or knowledge-graph entries) strengthens that identification further and raises the Structured Data Quality score.
- **Add `Service` schema to pages that describe discrete offerings.** Each service page is an opportunity to tell machines precisely what the offering is, who it is for, and how it relates to the parent organisation. Without this, the relationship between www.dkd.de and its individual services exists only in prose, and machines are less likely to surface the correct service in response to specific queries.
- **Validate new schema blocks against the Pipeline Survivability score.** The current Pipeline Survivability of 78/100 indicates that machine-readable content survives rendering well but is not yet fully optimised. Structured data added in a way that depends on client-side rendering may not be visible to all machines; ensuring the new types appear in served HTML protects the investment and keeps the score moving upward.
...
- **sameAs links on Organization**: adding `sameAs` properties to the `WebSite` entity already present across the audited set, pointing to Wikidata, LinkedIn, or equivalent authority sources, gives machines a reliable cross-reference to resolve dkd.de's identity unambiguously when composing agent answers.

- **potentialAction on Organization**: declaring a `potentialAction` (such as a contact or search action) on the `WebSite` or a parent `Organization` entity lets machines advertise what a user can do directly from an agent surface, turning a passive brand mention into an actionable entry point.
...
We found a robots.txt file in place, carrying 21 disallow paths that shape what machines are permitted to crawl, alongside a single sitemap reference to assist discovery. The breadth of the disallow list warrants a careful review to confirm that no indexable content of value is inadvertently excluded from machine access.
...
We did not surface data sufficient to assess the sitemap for www.dkd.de in this audit; a deeper review would extend that lens.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We did not detect an llms.txt file at www.dkd.de, meaning machines have no structured entry point through which to discover a site description, page inventory, or content policy. We recommend adding one to give crawlers and AI agents a clear, machine-readable summary of what www.dkd.de offers and which content is available for indexing.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We did not find a llms-full.txt file at www.dkd.de; the endpoint returns a 404 with no sitemap reference or homepage link element pointing to one. For a content-heavy site, adding a llms-full.txt gives machines a single, structured view of the full page corpus, reducing the guesswork involved in deep content discovery.
...
Across the audited set, www.dkd.de is **Not Compatible**: both assessed MX Journey stages fail, and Search & Compare, Price Understanding, and Purchase Confidence are not applicable to this site type.
...
| ---------------- | ------ | ----- | ------------- | ---- |
| Truncation Risk | Fail | 7/7 | 7 page(s) exceed the 250 KB threshold. Agents with limited fetch windows may stop reading before reaching the main content. | Largest page: 232 KB. Threshold: 250 KB. See dkd-de-de-pipeline-truncation-risk-pages.csv (7 pages). |
...
Across the audited set, pipeline survivability sits at 78/100, with Truncation Risk appearing on every one of the seven pages reviewed, meaning machines attempting to read or index this content face a consistent signal that text may be cut short before it is fully processed. Content Negotiation and Inline Tag Bloat add further friction, reducing the confidence with which automated readers can cleanly extract and interpret what www.dkd.de publishes. Addressing the truncation pattern first would have the largest effect, as resolving it across all seven pages in one pass removes the most pervasive barrier to reliable machine consumption.
...
On the worst-performing page, https://www.dkd.de/de/kontakt, rendered output carries a bare-div ratio of 54% (26 of 48 elements), meaning machines lose structural context and fall back on positional inference to determine meaning. The pattern here is surface-wide rather than structural: the deepest bare chain reaches only 3 levels, yet the high bare ratio across named selectors points to a component framework that applies class names tied to presentation rather than semantics, leaving the underlying element type doing little communicative work. The most direct first move is wrapping the obvious landmarks, such as header, nav, main, footer and aside, and ensuring the remaining components carry semantically descriptive class names, which would reduce the bare-div ratio without requiring any restructuring of the existing layout.
...
Across the audited set, we surface two distinct concerns around PDF accessibility: globally, accessibility legislation has converged on ISO 14289-1 (PDF/UA) as the shared technical baseline, with the European Accessibility Act (Directive (EU) 2019/882, in force 28 June 2025) as the most precisely codified instance alongside equivalent obligations under Section 508, the UK Public Sector Bodies Accessibility Regulations 2018, and comparable frameworks in Australia and Canada. In parallel, an untagged PDF is opaque to machines entirely; where a tagged PDF with a proper structure tree is readable to search crawlers, AI systems, and automated pipelines in much the same way semantic HTML is, a scanned or image-based document offers those machines nothing to extract.
...
Across the audited set, https://www.dkd.de performs with genuine confidence in SEO, recording 92/100, a result that reflects well-considered metadata and crawlability for human-facing search. The key opportunities lie in Discovery Readiness at 25/100 and Structured Data at 65/100, where the groundwork for machines to read, interpret, and cite the content accurately remains to be established. We invite the team at https://www.dkd.de to review the findings in detail and work with us to close those gaps.
...
We audited 84 images across the audited set, finding a format mix of 37 PNG and 33 JPEG files. Neither WebP nor SVG appears anywhere in the pages we reviewed, which means the format portfolio consists entirely of raster formats without a modern compressed alternative. Alt-text coverage is strong: 83 of 84 images carry descriptive text, leaving a single image without an attribute to address.

Every one of the 84 images carries an explicit loading="lazy" attribute, and none are set to loading="eager" or left without a loading attribute at all. That last point is worth noting: omitting the attribute is not equivalent to setting eager; it leaves the decision to the browser's own heuristics, which can produce inconsistent above-the-fold behaviour. Across the audited set that ambiguity does not arise, since every image is explicitly declared.
