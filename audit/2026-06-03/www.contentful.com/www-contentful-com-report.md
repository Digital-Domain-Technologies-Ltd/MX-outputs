...
Across the audited set we find a strong foundations that serves our human visitors well. Our SEO is excellent, scoring 94/100., which means search engines can easily surface and index its content, giving users quick access to relevant information. This strong SEO base reflects thoughtful keyword strategy, clean URLs, and consistent metadata - all of which enhance the overall user experience.

The headline opportunity we want to draw attention to is accessibility, a Priority 1 compliance item that directly impacts how many visitors can engage with the site. We identified 11 distinct WCAG AA issue types across 279 raw instances; 156 of those are marked critical by Pa11y and trace back to just 14 recurring template patterns. Addressing each pattern will clear a large portion of these issues, improving usability for all users. Following that, the next natural step is to elevate machine comprehension: add full MX fields, governance, and provenance metadata so machines have the structured context they need for accurate comprehension. Raising the MSC above 60 and Discovery Readiness above 40 will move the site from Discoverable (Level 1) to Governed (Level 2), enabling richer interaction with AI agents and search crawlers.
...
Across the audited set, we find that the experience for human visitors is strong, though page load times could be improved.
...
Across the audited set, machines can discover and parse content with a Discovery Readiness score of 39/100, yet they lack governance context because MX Readiness Level is 1 (Discoverable), while Structured Data Quality stands at 61/100, MX Stack Completeness at 66/100, and Pipeline Survivability at 79/100.
...
Across the audited set, we find a strong foundations in SEO - 94/100 overall and 97/100 on content pages - and strong accessibility performance with 77/100. These strengths provide the groundwork for targeted improvements in structured data quality and consistency.
...
Across the audited set of 12 pages, we highlighted key opportunities for strengthening machine comprehension and discoverability, prioritising them by impact. Catalogue visibility gaps rank highest as they block downstream agent interactions, followed by discovery readiness which limits initial access, and then structured data quality and MX stack completeness.
...
We found a robots.txt file that permits all user agents to crawl the site, as it contains no disallow directives. The file also announces a single sitemap reference, confirming that the sitemap is publicly discoverable.
...
We assessed the sitemap and graded it Partial; it declares 3359 URLs, includes lastmod dates but omits changefreq and priority, which presents an opportunity to improve machine compatibility beyond Google.
...
We found no llms.txt at the root of www.contentful.com. Machines that query the host for a structured content index receive no description, page inventory or content policy; our recommendation diverges from the llmstxt.org specification: we recommend adding an llms.txt file with text/html content type.
...
We found no llms‑full.txt; it is absent from the sitemap and not referenced in the homepage head. Because our audit sampled only 12 pages, we cannot determine whether a full‑site llms‑full.txt would be useful for content‑heavy sites.
...
All detected markers are present in the served HTML on the pages we audited. Server-side and browser-based agents see the same signals on the sampled pages. A full-estate audit confirms whether the same pattern holds across the rest of the site.
---
...
**Not Compatible for this site type. Purchase Confidence does not apply - no transaction or checkout pages were detected.**

Stage 2 (Citation) is the weakest link in the agent journey. Because each stage depends on the previous one, this gap affects all downstream stages. Addressing 33 is the highest-priority improvement.
...
We found that every audited page shows a truncation risk, meaning machines may receive only part of the content they need to understand.  
This could lead to incomplete indexing or inaccurate responses from LLMs and search crawlers.  
Addressing truncation - by reducing page size or ensuring full delivery - offers the greatest chance to strengthen resilience and give machines complete data.
...
We recorded a bare‑div ratio of 51 % on the rendered surface of the worst‑case page https://www.contentful.com/products/personalization/, meaning that more than half of the visible elements are generic divs and machines lose reliable structural context, forcing them to rely on positional inference.  
The soup is surface‑wide with a moderate deepest chain of 7, indicating that the source pipeline likely relies on drag‑and‑drop builders or untyped component frameworks that inject many shallow div wrappers rather than semantic markup.  
Our cheapest first move is to wrap the obvious landmarks - header, nav, main, footer, aside - with their respective semantic tags and assign meaningful class names to remaining elements so the bare‑div ratio drops without restructuring the layout.
...
The personalisation layer - implemented by Ninetailed, Google Optimise, Optimizely, Visual Website Optimizer (VWO), Kameleoon, Dynamic Yield and Unbounce Smart Traffic - is active on https://www.contentful.com/products/ai-actions/, https://www.contentful.com/products/studio/, https://www.contentful.com/contact/sales/, https://www.contentful.com/products/platform/, https://www.contentful.com/products/personalization/ and seven additional pages.  
Machines arrive cold on every fetch, with no session cookie or cohort data persisted between visits; consequently each request may be served a different experiment variant. This stateless behaviour means that a crawler building a knowledge graph could record contradictory facts across successive page visits, and a sitemap‑sweeping training corpus builder might ingest multiple divergent versions of the same URL during separate crawl sweeps, embedding inconsistent information into its training data.  
To mitigate these machine‑experience implications we recommend adding bypass rules in your personalisation platform to serve canonical content when the User‑Agent matches known machine visitors such as ClaudeBot, GPTBot or CCBot. Additionally, reference experiment URLs in llms.txt with a note that content varies by session so machines are warned not to treat any single fetch as canonical. Finally, consider serving a stable, non‑personalised version of key pages to bot User‑Agents via CDN routing rules.

---

## Content Consistency

We find consistent metadata patterns across the audited set, with no organisation‑name or canonical‑URL divergence flagged by the consistency check.
...
We note that accessibility legislation worldwide converges on ISO 14289‑1 (PDF/UA) as the technical baseline, with the European Accessibility Act serving as a highly detailed example of this global alignment. We also observe that an untagged PDF is invisible to machines, while a tagged PDF equipped with a structure tree becomes machine-readable in the same way semantic HTML is.
...
| Full Optimisation | Catalogue Visibility, Semantic Structure, Discovery Readiness, Structured Data, MX Stack Completeness, Heading Quality, Performance, Security headers, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
We found that https://www.contentful.com excels in SEO with a score of 94/100, indicating strong visibility for human visitors and search engines alike. However, the audit highlights opportunities in discovery readiness (39/100) and structured data (61/100), where richer metadata could enhance machine comprehension and content discoverability across the audited set. We invite you to prioritise these areas to unlock greater reach and engagement.
...
Across the audited set we examined 890 images. The format distribution is as follows: WebP 1, SVG 598, PNG 264, JPEG 15 and 12 in other or unrecognised formats. Alt‑text coverage is strong - 868 of 890 images (97.5%) carry alt text, leaving 22 without.

Loading strategy is predominantly lazy: 840 images have loading="lazy", 14 are marked eager, and 36 lack a loading attribute entirely. The absence of the attribute means browsers will not treat those images as eager; they will defer until needed, which differs from an explicit eager setting.
