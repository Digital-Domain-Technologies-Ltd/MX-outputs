...
- Site: Www Contentful (www.contentful.com)
- Pages audited: 12 of 3359 in sitemap - sample run
- Date: 2026-06-03
- Scores: SEO 94/100 | Accessibility 77/100 | Discovery Readiness 39/100 | Structured Data 61/100
- Scope: 12 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): WebSite (13), Organization (13)
...
- Accessibility: 11 distinct WCAG AA issue types (279 raw instances, 279 marked critical by Pa11y) - 156 of these instances trace to 14 recurring template patterns (single theme edit per pattern fixes all instances)
- AI Suitability (served): 99/100
- Structured Data Quality: 61/100
- Discovery Readiness: 39/100
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 12
- Performance: 3489ms avg (Could Be Better)
- Accessibility: 77/100 (Excellent)
- SEO: 94/100 (Excellent)
- Accessibility issues: 11 distinct WCAG codes across 279 raw instances (156 instances trace to 14 template patterns)
-->
...
- SEO (content pages only): 97/100
- Accessibility: 77/100 (11 distinct WCAG codes, 279 raw instances) - 156 of these instances trace to 14 template-level patterns
- Security headers: 4/5 present (HTTPS, HSTS, X-Frame-Options, X-Content-Type-Options); 0 of 12 audited URLs carry all five
- Structured Data Quality: 61/100
- Consistency: 56%
-->
...
Div Soup (Semantic Structure): rendered score 34/100, band high. When the rendered score is below 70, include this as a numbered Priority row. The figures 290 bare divs out of 566 total describe the WORST-CASE page in the audited set (https://www.contentful.com/products/personalization/), not a site-wide average. The finding title and the Finding paragraph must scope to that worst page; do NOT present these numbers as if they hold across every audited page. The title should include the rendered score, the worst-page URL, and the bare-divs-of-total figure (for example, a title naming the score, the page path, and the figure such as "95 of 153 bare divs"). In the Finding paragraph, name the worst page URL explicitly and note that the figures cited come from it specifically; if the band still applies because most pages share the template, say so.

Separate Priority blocks with `---`.

FACTS:
- Pa11y data for this audit:  Of these, 144 (52%) trace to 13 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 12 (4%) trace to 1 vendor-injected DOM pattern from ad.ipredictive.com; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.

Pa11y findings for this audit (use ONLY these for specific recommendations):
- **WCAG 4.1.2**: Anchor element found with a valid href attribute, but no link content has been s | 195 total instances on 11 pages | sample selector: `html > body > div:nth-child(10) > footer > div > div > div:n` (+2 more selectors) | affects: screen reader users
- **WCAG 4.1.1**: Duplicate id attribute value "accordion-header-0" found on the web page. | 15 total instances on 2 pages | sample selector: `#accordion-header-0` (+2 more selectors) | affects: all assistive tech users
- **WCAG 1.1.1**: Img element missing an alt attribute. Use the alt attribute to specify a short t | 13 total instances on 12 pages | sample selector: `html > body > img` (+1 more selectors) | affects: screen reader users | third-party element, host: `ad.ipredictive.com`, NOT in site template
- **WCAG 2.4.1**: Iframe element requires a non-empty title attribute that identifies the frame. | 12 total instances on 12 pages | sample selector: `html > body > iframe` | affects: sighted keyboard users
- **WCAG 1.3.1**: Not all th elements in this table contain an id attribute. These cells should co | 5 total instances on 1 page | sample selector: `#pricing-table-section-compare-platform-plans > div > div:nt` (+2 more selectors) | affects: screen reader users
- **WCAG 2.4.2**: A title should be provided for the document, using a non-empty title element in  | 1 total instance on 1 page | sample selector: `html > head` | affects: screen reader users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a con | 38 total instances on 1 page | sample selector: `#5d8fba12-12d8-4e2a-b037-30af14a60c19 > div:nth-child(3) > b` (+2 more selectors) | affects: low-vision users
- Div Soup rendered score: 34/100, band high
- Div Soup bare divs: 290 of 566 (figures from the WORST page only)
- Div Soup worst page URL: https://www.contentful.com/products/personalization/
- Div Soup audited page count: 12

When a Pa11y finding row in  Of these, 144 (52%) trace to 13 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 12 (4%) trace to 1 vendor-injected DOM pattern from ad.ipredictive.com; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 144 (52%) trace to 13 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 12 (4%) trace to 1 vendor-injected DOM pattern from ad.ipredictive.com; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- **WCAG 1.3.1**: Not all th elements in this table contain an id attribute. These cells should co | 5 total instances on 1 page | sample selector: `#pricing-table-section-compare-platform-plans > div > div:nt` (+2 more selectors) | affects: screen reader users
- **WCAG 2.4.2**: A title should be provided for the document, using a non-empty title element in  | 1 total instance on 1 page | sample selector: `html > head` | affects: screen reader users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a con | 38 total instances on 1 page | sample selector: `#5d8fba12-12d8-4e2a-b037-30af14a60c19 > div:nth-child(3) > b` (+2 more selectors) | affects: low-vision users above.
...
| 2 | Citation | Fail | 33 | Schema.org: WebSite, Organization, WebSite (100% required properties) |
...
- Checks flagged on any page: Truncation Risk, Body Content Ratio, Inline Tag Bloat
...
- Rendered score: 34, band high (worst page: https://www.contentful.com/products/personalization/)
- Bare divs (rendered): 290 of 566 (51%)
- Deepest bare chain (rendered): 7
- Top bare selectors (rendered): `div` (242), `div.grid-helper-col.col-span-1` (144), `div.tooltip_root__EulUk` (95), `div.button_markup_root__sACGr.button_markup_secondary__F_qxS` (68), `div.round-icon.round_icon_container__Jmlwp` (57)
...
- Ninetailed, Google Optimize, Optimizely, Visual Website Optimizer (VWO), Kameleoon, Dynamic Yield, Unbounce Smart Traffic (personalisation) was detected on https://www.contentful.com/products/ai-actions/, https://www.contentful.com/products/studio/, https://www.contentful.com/contact/sales/, https://www.contentful.com/products/platform/, https://www.contentful.com/products/personalization/ and 7 more
...
- Vendor(s): Ninetailed, Google Optimize, Optimizely, Visual Website Optimizer (VWO), Kameleoon, Dynamic Yield, Unbounce Smart Traffic
- Category: personalisation
- Pages affected: https://www.contentful.com/products/ai-actions/, https://www.contentful.com/products/studio/, https://www.contentful.com/contact/sales/, https://www.contentful.com/products/platform/, https://www.contentful.com/products/personalization/ and 7 more
...
WRITE:
2-3 sentences summarising the audit findings for Www Contentful (https://www.contentful.com). Always refer to the site by its domain (https://www.contentful.com), never by the consultancy name. The highest-scoring dimension is SEO at 94/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 94/100; Structured Data: 61/100; Discovery Readiness: 39/100
...
Pattern:
- Paragraph 1: Total images audited, format distribution (WebP / SVG / PNG / JPEG / Other), alt-text coverage (count and percentage, name the number missing if any). Complete the alt-text coverage sentence fully using 868 and 97.5 data - for example: "868 of 890 images (97.5%) carry alt text" followed by the missing count if 22 is greater than zero. Never leave the alt-text sentence incomplete. The format counts MUST sum to the total - when the Other-format count is non-zero, name it explicitly so the breakdown closes (e.g. "[N] images in other or unrecognised formats"). When it is zero, omit the Other clause entirely.
- Paragraph 2: Loading strategy. Distinguish the three states: `loading="lazy"`, `loading="eager"`, and no attribute set (the browser guesses). No attribute is NOT the same as eager: make that point if 36 is greater than zero.
...
- PNG: 264
- JPEG: 15
- Other / unrecognised formats: 12
- With alt text: 868 (97.5%)
- Missing alt text: 22
- loading="lazy": 840
- loading="eager": 14
- no loading attribute: 36
