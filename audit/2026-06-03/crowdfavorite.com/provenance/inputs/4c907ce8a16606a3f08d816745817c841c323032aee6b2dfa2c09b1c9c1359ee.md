...
Across the audited set we found that Crowdfavorite delivers a strong human experience, with SEO foundations scoring 96/100 – an excellent level that positions content well in search results and supports users in finding relevant information quickly. The site’s clear navigation and engaging product listings help visitors discover new projects easily, reinforcing brand trust and encouraging repeat engagement.

We identified 11 distinct WCAG AA issue types, totaling 548 critical instances; addressing one fix per category would resolve many elements. The headline opportunity is to elevate the site’s governance metadata. Score thresholds for Governed are met (MSC 73, SDQ 90, DR 40), but no MX‑namespaced governance metadata was detected across the audited set. Adding mx:status, mx:contentType, mx:audience, canonicalUri and provenance markers to at least one published page will unlock Governed level and give machines the context needed for accurate comprehension.
...
We found that across the audited set, Crowdfavorite delivers a generally positive experience for human visitors, though page load times could be improved.
...
Across the audited set, we find that machines can discover and parse content with a structured data quality of 90/100, yet they lack governance context as reflected by an MX readiness level of 1 (Discoverable), a discovery readiness score of 40/100, MX stack completeness of 73/100, and pipeline survivability of 63/100.
...
Across the audited set, we observe a strong foundations in SEO (96/100), accessibility (74/100 with 11 distinct WCAG codes), security headers (5/5 present on 11 of 12 URLs), and structured data quality (90/100). These strengths provide the groundwork for the improvements that follow.
...
Across the audited set, the At a Glance table highlights four key opportunities—structured data quality, commerce visibility, MX stack completeness, and discovery readiness—ranked by their influence on how machines interpret and surface content. We prioritised them by impact: commerce visibility first because it directly limits shopping agents, followed by discovery readiness which blocks downstream processing, then MX stack completeness and structured data quality.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Duplicate ID attribute value "module-contact-request-container" | Compliance Risk | High | Medium | may miss for assistive tech users |
| 2 | Img element missing alt text in link | Compliance Risk | High | Medium | screen reader users might miss linked content |
| 3 | Role button without name | Compliance Risk | High | Medium | screen reader users may not identify the button |
| 4 | Iframe missing title attribute | Compliance Risk | High | Medium | sighted keyboard users may not know frame purpose |
| 5 | Role presentation with semantic content | Compliance Risk | High | Low | screen reader users may misinterpret structure |
| 6 | Insufficient contrast | Compliance Risk | High | Medium | low‑vision users might miss or misread text |
| 7 | Missing discovery artefacts (llms-full.txt, agent-card.json, ai.txt, humans.txt) | Machine Readability Opportunity | Medium | Medium | agents may not discover content |

**Priority 1: Duplicate ID Attribute Value 'module-contact-request-container', WCAG 4.1.1**

Finding  
Duplicate id attribute value "module-contact-request-container" appears 391 times on a single page, affecting all assistive tech users.

What to change and why  
- Remove or rename duplicate IDs so each element has a unique identifier; this satisfies WCAG 4.1.1 and ensures screen readers can reference elements correctly.  
- Update the theme template once to eliminate the pattern site‑wide; this reduces maintenance overhead.  

Effort: Medium  

---  

**Priority 2: Img Element Missing Alt Text in Link, WCAG 1.1.1**

Finding  
83 instances across ten pages where an image is the sole content of a link and lacks alt text, impacting screen reader users.

What to change and why  
- Add descriptive alt attributes to each linked image; this meets WCAG 1.1.1 and provides context for assistive technology.  
- Apply a global template rule or CMS validation to enforce alt presence on image links.  

Effort: Medium  

---  

**Priority 3: Role Button Without Name, WCAG 4.1.2**

Finding  
19 instances across eleven pages where elements with role="button" lack an accessible name, affecting screen reader users.

What to change and why  
- Provide an aria‑label or visible text for each button; this satisfies WCAG 4.1.2 and improves discoverability.  
- Add a template‑level rule to ensure all role="button" elements have names.  

Effort: Medium  

---  

**Priority 4: Iframe Missing Title Attribute, WCAG 2.4.1**

Finding  
Five instances on three pages where iframes lack a non‑empty title attribute, impacting sighted keyboard users.

What to change and why  
- Add a descriptive title to each iframe; this meets WCAG 2.4.1 and clarifies frame purpose.  
- Update the embedding code or CMS field to include titles automatically.  

Effort: Medium  

---  

**Priority 5: Role Presentation With Semantic Content, WCAG 1.3.1**

Finding  
Two instances on one page where elements with role="presentation" contain semantic child elements, affecting screen reader users.

What to change and why  
- Remove the role="presentation" or replace it with a more appropriate role; this aligns with WCAG 1.3.1.  
- Adjust the template so that only non‑semantic wrappers use presentation roles.  

Effort: Low  

---  

**Priority 6: Insufficient Contrast, WCAG 1.4.3**

Finding  
Forty‑eight instances across four pages where elements fail to meet contrast requirements, impacting low‑vision users.

What to change and why  
- Increase foreground or background colour contrast to satisfy WCAG 1.4.3; this improves readability for low‑vision users.  
- Apply a global CSS rule or design system update to enforce contrast thresholds.  

Effort: Medium  

---  

**Priority 7: Missing Discovery Artefacts (llms-full.txt, agent-card.json, ai.txt, humans.txt)**

Finding  
llms-full.txt, agent-card.json, ai.txt and humans.txt are absent from the host, reducing discovery readiness.

What to change and why  
- Create and publish llms‑full.txt with machine‑readable content metadata; this enhances agent discoverability.  
- Add an agent-card.json file containing structured data about the site; this supports richer search results.  
- Provide ai.txt and humans.txt files to signal AI‑friendly and human‑friendly content respectively; these are recognised by some agents.  
- Host all four artefacts under /.well‑known/ so they are easily located by crawlers.  
...
We found a robots.txt file that does not reference a sitemap.  
The file disallows four paths, allowing all user agents to crawl the remaining content.
...
We assessed Crowdfavorite’s sitemap and gave it a Partial grade. The file declares all 379 URLs and includes lastmod dates, but lacks changefreq and priority entries; while Google no longer uses these tags, non‑Google crawlers and machines still interpret them as re‑crawl cadence hints and relative importance signals, so adding them would be low effort and broaden machine compatibility.
...
We found that our sitemap lists multiple URL forms—trailing‑slash and hash‑fragment variations—for the same canonical resource, inflating the crawl set by about 10 % (12 URLs versus 11 unique pages). Because machines that skip URL normalisation will treat each variant as a distinct page, they waste token budget and risk generating contradictory or inflated findings; we recommend consolidating the sitemap to a single canonical form per resource and adding a <link rel="canonical"> tag on each page.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

The llms.txt is present and contains a host description, but it does not include a page inventory or a content policy. We recommend adding both a page inventory and a content policy to provide machines with a complete structured content index.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms‑full.txt; it is absent from the root, returns 404, and is not referenced in the sitemap or homepage head. Because our sample does not cover the full content depth, we recommend adding a text/html version only if the site hosts sufficient content to justify such an artefact.
...
Partially Compatible for this site type. Purchase Confidence does not apply – no transaction or checkout pages were detected.
...
Across the audited set, every page shows a truncation risk, meaning that when machines crawl or index the pages they may receive only part of the content.  
The most impactful fix is to eliminate this truncation by ensuring the full page loads before any cutoff; doing so will give machines complete information and improve their ability to understand and use the content.  
Other resilience checks such as boilerplate burial, inline tag bloat, and delayed content start also present opportunities for strengthening overall robustness.
...
On the worst page (https://crowdfavorite.com/campaign/digital-summit-atlanta-2022) we observed a bare‑div ratio of 57 % (1024 of 1812 elements), which forces machines to abandon semantic cues and rely on positional inference to infer meaning.  
The soup is surface‑wide, with a high bare‑div ratio but shallow chains (deepest chain length 5), indicating that the source pipeline likely relies on drag‑and‑drop builders or untyped component frameworks that inject many divs without semantic markup.  
Our cheapest first move is to wrap key landmarks—header, nav, main, footer, aside—in their appropriate semantic elements and give remaining divs descriptive class names; this will lower the bare‑div ratio without restructuring the layout.
...
We observed No content variance was detected across cache-busted fetches in this audit run. However, an unknown vendor implementing personalisation/A/B testing is present on pages in the audited set. Machines arrive cold on every fetch – no cookies or session continuity – so each request may be served a different experiment variant. This stateless behaviour means that bots crawling the sitemap can ingest contradictory facts from the same URL across crawls, obscuring the publisher’s canonical content for machine consumption. We recommend ensuring that content variants are either served via distinct URLs or that a machine‑readable indicator of the canonical version is provided so that training corpora and search engines receive a consistent representation.
...
| Phase | Scope | Outcome |
|-------|-------|---------|
| Critical Fixes | No regulatory compliance findings on the audited surface | No regulatory blockers — focus shifts straight to improvement |
...
SEO remains a clear strength for https://crowdfavorite.com, scoring 96/100 across the audited set.  
Discovery readiness is an opportunity, with a score of 40/100 indicating room to improve machine discoverability and metadata exposure.  
We invite you to act on these findings to strengthen your digital presence and unlock further machine‑readable value.
...
Across the audited set we examined 568 images. The format mix is dominated by PNG (330), followed by SVG (112), JPEG (76), WebP (1) and other/unidentified formats (49). Alt text is present on 397 of those images, which is 69.9 % of the total; 171 images lack alt text.

In terms of loading strategy, 260 images are explicitly set to lazy load, none are marked eager, and 308 have no loading attribute at all. The absence of a loading attribute means the browser will decide how to fetch them, which is not equivalent to an eager load.
