---
title: "Oxide: Website Analysis & Machine Readiness"
author: "Tom Cranstoun"
created: "2026-06-16"
modified: "2026-06-16"
client: "Oxide"
clientSlug: "oxide-computer"
clientUrl: "https://oxide.computer"
reportId: "oxide-computer-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://oxide.computer --pages 10 --date 2026-06-16"
description: "Executive audit report reviewing accessibility, performance, SEO, structured data, and AI agent compatibility for Oxide"
tags: [web-audit, accessibility, wcag-aa, ai-agents, seo, performance, mx, executive-report]
performanceScore: 70
accessibilityScore: 100
seoScore: 87
llmSuitabilityScore: 96
totalIssues: 0
pagesAudited: 12
version: "1.0"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/oxide.computer/oxide-computer-report.md
  maintainer: info@cognovamx.com
  stability: stable
  partOf: mx-audit
  purpose: "Executive machine-readiness audit for Oxide covering accessibility, performance, SEO, structured data, and AI agent compatibility."
  x-mx-contextProvides: ["web audit findings for Oxide", "WCAG accessibility assessment", "AI agent compatibility scores", "SEO and structured data analysis", "machine readiness recommendations"]
  ...
  runbook: "Executive audit report for Oxide. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/oxide.computer/oxide-computer-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/oxide.computer/oxide-computer-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/oxide.computer/result-copy/oxide-computer-report.pdf"
    description: "Generate PDF audit report for Oxide"
  ...
  x-mx-provenance:
    sidecar: "oxide-computer-report.provenance.ai.json"
    frameworks: [EU-AI-Act, UK-ICO-AI-guidance, NIST-AI-RMF, Colorado-AI-Act]
    companion: "oxide-computer-report.provenance.deterministic.json"
...
**Date:** 16 June 2026\
**Report ID:** oxide-computer-WEB-AUDIT-20260616
...
| SEO | **87**/100 | `################--` |
| Served-HTML Structure | **96**/100 | `#################-` |
| MX Stack Completeness | **50**/100 | `#########---------` |
| Agent Readability | **48**/100 | `#########---------` **(!)** |
...
- **Bare-div nesting (div soup) across most pages** (-8): 12 of 12 pages are heavy with bare-div nesting

Across the audited set, your brand delivers a strong human experience. The SEO foundations are solid, earning an excellent score of 87/100, which means visitors find and engage with content quickly and reliably. Clear navigation and concise copy reinforce trust and ease of use for every user who lands on your pages.

The headline opportunity is to elevate machine comprehension by adding MX governance metadata. Across the audited set, no mx:status, mx:contentType, mx:audience, canonicalUri or provenance markers are present, keeping your MX Readiness Level at Not Ready. By embedding these fields you move toward a Governed level where machines can accurately attribute and contextualise your content.

We also noted that structured data quality is 0/100; introducing Schema.org JSON-LD would give every agent a reliable source of context regardless of rendering differences, aligning with the MX governance path and strengthening discoverability for automated agents.
...
We find that across the audited set, the site delivers a strong experience for human visitors, with excellent accessibility and SEO scores and good average performance.
...
We find that across the audited set, machines can discover and parse content with limited governance context-Discovery Readiness 10/100 and Structured Data Quality 0/100-while MX Stack Completeness stands at 50/100 and Pipeline Survivability is high at 92/100, indicating that full governance for accurate comprehension has not yet been achieved.
...
| MX Stack Completeness | 50/100 | Could Be Better | C | B (median) |
...
**Evidence:** MX Stack Completeness 50/100 | Structured Data Quality 0/100 | Discovery Readiness 10/100 | Consistency 56%
...
Across the audited set, we see a strong foundations: an impressive SEO score of 87/100 and perfect accessibility at 100/100, with HTTPS, HSTS, and CSP headers consistently applied. These strengths lay the groundwork for the improvements highlighted in the following table.
...
|-----------|-------|------------|
| Performance | Good | Good  -  1036ms average load time |
| SEO (content pages) | 87 | Excellent  -  titles, meta descriptions, canonical URLs in place |
| Security | 3/5 | 3/5 headers present (X-Frame-Options, X-Content-Type-Options absent); 0 of 12 URLs carry all five |
| Heading Quality | 77 | Excellent  -  headings present and machine-parseable |
...
| 3 | Semantic Structure (Naked Containers) 14/100 | Compliance Risk | Medium | Medium | machines lose structural context and infer page regions by position |
| 4 | Security headers absent: X-Frame-Options, X-Content-Type-Options | Cross-cutting | Medium | Low | Missing security headers increase exposure to content injection and clickjacking |
...
**Finding:** 9 of 80 images (11%) on the audited set carry no alt text, so their content is unavailable to assistive technology and to machines reading the page.
...
**Priority 3: Semantic Structure (Naked Containers) 14/100**

**Bucket:** Compliance Risk

**Finding:** Rendered semantic-structure score 14/100: containers carry no role, ARIA landmark, or descriptive class, so machines fall back on positional inference to determine meaning. The worst page ([the home page](https://oxide.computer/)) carries 492 bare divs of 640.
...
**Priority 4: Security headers absent: X-Frame-Options, X-Content-Type-Options**

**Bucket:** Cross-cutting

**Finding:** Security headers absent: X-Frame-Options, X-Content-Type-Options (across the audited set). Missing security headers increase exposure to content injection and clickjacking
...
- **Content-Signal directives**: adding Content-Signal directives in robots.txt declares content-use policy for machines and clarifies how your pages may be reused.  
- **Organisation schema with potentialAction**: embedding an Organisation JSON-LD snippet on the homepage or about page exposes contact capabilities to agents, enabling them to discover outreach options.  
- **SpeakableSpecification on article pages**: defining SpeakableSpecification CSS selectors on article-type pages unlocks voice surfaces for AI agents, allowing them to surface spoken summaries.
...
| Check | Result |
|-------|--------|
| URL probed | https://oxide.computer |
| HTTP status | 200 |
| Content-Type returned | text/html |
...
| Internal navigation links | 5 links to same-site pages |
...
Across the 12 pages we checked, 36 distinct issue patterns reduce what reaches the tree. 2 of these repeat across pages with the same structure, which marks them as template-level: one change in the shared component clears the finding everywhere it appears.

**Clickable element with no semantic role** (WCAG 2.1 4.1.2)

A container is wired to respond to clicks but carries no role and is not a native control, so it does not exist as an interactive element in the accessibility tree. A screen reader user cannot reach it; an agent reading the tree or the markup cannot tell it is actionable. Repeats on 9 of 12 pages with the same structure (a template-level pattern).
...
An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Seen on 4 pages, for example `/blog/a-tool-for-discussion`.
...
An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Seen on 3 pages, for example `/blog/a-tool-for-discussion`.
...
A container is wired to respond to clicks but carries no role and is not a native control, so it does not exist as an interactive element in the accessibility tree. A screen reader user cannot reach it; an agent reading the tree or the markup cannot tell it is actionable. Seen on `/blog`.
...
An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Seen on `/blog`.
...
An input relies on placeholder text as its only label. The placeholder vanishes the moment the visitor types, is not reliably exposed as the accessible name, and disappears entirely in text projections of the page. Repeats on 12 of 12 pages with the same structure (a template-level pattern).
...
An image whose name or description suggests it carries data (a chart, a price list, a menu, a timetable) has no adjacent text equivalent. The data exists in exactly one channel - pixels - which no agent and no screen reader can read. Seen on `/`.
...
An image whose name or description suggests it carries data (a chart, a price list, a menu, a timetable) has no adjacent text equivalent. The data exists in exactly one channel - pixels - which no agent and no screen reader can read. Seen on `/blog/a-tool-for-discussion`.

*The fix:* Publish the underlying data beside the image: a table, a definition list, or descriptive prose in the same template component.

**Link whose accessible name carries no destination meaning** (WCAG 2.1 2.4.4)

The link's accessible name is a generic phrase or a bare URL. In the accessibility tree, and in any links-only projection of the page, every such link reads identically: the destination is unknowable without the surrounding layout, which machines and screen reader users do not have. Seen on `/blog/exploiting-undocumented-hardware-blocks-in-the-lpc55s69`.
...
The full set, one row per pattern with every affected page counted, is recorded in the `oxide-computer-accessibility-tree.csv` sidecar alongside this report.
...
**Slowest.** The slowest page is `https://oxide.computer/blog/dtrace-conf24`. A first-time visitor sees the cold-cache cost: the crawler recorded 1850 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three fresh re-probes that followed returned 789ms, 570ms, 634ms, giving a returning-visitor median of **634 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://oxide.computer/blog/compensation-as-a-reflection-of-values`. A first-time visitor sees the cold-cache cost: the crawler recorded 821 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 1126ms, 929ms, 966ms, giving a returning-visitor median of **966 ms**. **Returning-visitor verdict: Healthy**.
...
Disallow: /careers/logout
Disallow: /careers/upload-token
Disallow: /careers/magic
Disallow: /careers/submit
Disallow: /careers/submit-application
Disallow: /careers/create-application
Disallow: /careers/update-contact-data
```

*Showing the first 10 lines of `robots.txt`; the full 14-line file is preserved alongside this report as `oxide-computer-robots-txt.txt`.*

The robots.txt declares 10 disallow paths; all other paths are open to crawlers and machines. It announces the sitemap, so a machine reading the file can find the URL index directly.
...
The sitemap declares 99 URLs and grades Minimal. Lastmod is absent, so machines have no per-page change signal to schedule re-crawls against. The sitemap omits changefreq and priority. Google dropped both as ranking signals in 2017, but non-Google crawlers and AI agents still read changefreq as a re-crawl cadence hint and priority as a relative-importance signal, so adding them is a low-effort way to broaden machine compatibility.

This was a full crawl: the audit reached every page it could discover, and 6 of them are absent from the sitemap (which lists 99). The unlisted pages: `/about/`, `/blog/feed`, `/img/blog/dtrace-conf24/bi-mon-sci-fi-con.gif`, `/img/blog/dtrace-conf24/dtrace-conf24-shirt.png`, `/img/blog/dtrace-conf24/wiki.jpg`, `/podcasts/oxide-and-friends/1173990`. The full set is recorded in the `oxide-computer-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.
...
This is a clean-slate site with no Schema.org markup. There are no property gaps to report because no typed entities exist yet; every structured data addition is net new capability. The served HTML is machine-readable (served score 96/100); agents can extract content without JSON-LD.
...
|------|----------|-----------|--------------------------|------------------------|----------------------|
| [/blog/benefits-as-a-reflection-of-values](https://oxide.computer/blog/benefits-as-a-reflection-of-values) |  -  |  -  | "We offer the best" | 6 third-party links | no JSON-LD entities |
...
We score a machine's metadata and compare it with how well a machine can read each page. We run eleven reading-resilience checks on every audited page. Each one asks whether a page survives a known agent-reading risk: truncation by the agent's fetch tool, condensing by the relevance layer, JavaScript-only content, tab disclosure, soft 404s, broken code fences, content negotiation drift, cross-host redirects, generic headings, content that begins too far into the document, or overhead-heavy pages where scripts, styles, and images outweigh actual content.
...
- **Truncation Risk** - Pass · 12/12
  - *Means:* Every page is well under the 250 KB hard ceiling at which some server-side agents stop reading, and main content sits within the 50-100 KB offset windows. The largest page is 157 KB ([the home page](https://oxide.computer/)).
  - *Data:* Largest page: 157 KB ([the home page](https://oxide.computer/)). Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows.
...
- **Body Content Ratio** - Fail · N/A
  - *Means:* Prose content averages only 28% of served bytes. Scripts, styles, and images dominate; agents get little signal per byte.
  - *Data:* Average: 28%. Threshold: 30%.
- **Inline Tag Bloat** - Fail · 12/12
  ...
  - *Data:* 48 element(s) > 500 bytes. Largest single-page inline CSS block: 0 B. Largest single-page inline JS block: 84387 B. See oxide-computer-pipeline-inline-tag-bloat-pages.csv (12 pages).
...
Across the audited set, we found that every page shows Inline Tag Bloat, meaning machines may waste time parsing excessive inline markup and could miss content.  
We can refactor inline styles and scripts into external files to cut tag count, speed bot processing, and make the pages more resilient.  
We see this as an opportunity to strengthen machine comprehension without changing the user experience.
...
|--------|--------------|----------------|--------------------|
| Served HTML | 15/100 (high) | 412 bare divs (75% of containers, depth 13) | `div.w-full.px-(--gutter-width)` (74), `div.m-auto.max-w-300` (54), `div.Accordion.text-left` (50), `div.text-mono-sm.text-secondary` (47), `div` (41) |
| Rendered HTML | 14/100 (high) | 492 bare divs (77% of containers, depth 13) | `div.w-full.px-(--gutter-width)` (73), `div.bg-accent-hover.w-1` (57), `div.m-auto.max-w-300` (55), `div.Accordion.text-left` (50), `div.text-mono-sm.text-secondary` (47) |

**Worst page (served):** [the home page](https://oxide.computer/)\
**Worst page (rendered):** [the home page](https://oxide.computer/)

Across the audited set, the worst page-https://oxide.computer/-shows a higher bare-div ratio on the rendered output (77 % of elements are plain `<div>`s) than on the served markup (75 %), meaning machines lose structural context and must rely on positional inference to determine meaning.  
The soup is structural, with a deepest bare chain of 13 nested divs, indicating that the source pipeline likely injects many layers of untyped components or late-stage JavaScript rather than using semantic markup.  
A cost-effective first move is to wrap obvious landmarks-header, nav, main, footer, aside-and give the remaining elements meaningful class names so the bare-div ratio drops without restructuring the layout.
...
2 of the five standard security headers are absent on every audited response: X-Frame-Options, X-Content-Type-Options. Adding them at the origin-server or CDN edge closes the corresponding attack surfaces without touching application code.

**Coverage:** 0 of 12 audited URLs carry all five headers; see the Security Headers appendix for the full exception list.

- **`/`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/bio`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/a-tool-for-discussion`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/another-vulnerability-in-the-lpc55s69-rom`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/benefits-as-a-reflection-of-values`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/building-big-systems-with-remote-hardware-teams`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/compensation-as-a-reflection-of-values`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/cosmo-sp`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/dtrace-conf24`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/engineering-culture`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No
- **`/blog/exploiting-undocumented-hardware-blocks-in-the-lpc55s69`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame No · X-Content-Type No

HTTPS: 12/12 | HSTS: 12/12 | CSP: 12/12 | X-Frame-Options: 0/12 | X-Content-Type-Options: 0/12
...
| Canonical URL | 92% |  -  |
| Exactly 1 H1 | 92% | `/bio` |
| Code examples present | 42% | 7 |
| Self-contained sections | 100% |  -  |
| Error/troubleshooting docs | 8% | 11 |
| Lighthouse heading compliance | 92% | `/bio` |
...
We found 5 identical inline fragment(s) repeated across multiple pages, totalling 37 KB redundant bytes. Extracting these to external CSS or JS files would reduce page weight, improve cacheability, and simplify maintenance.
...
|------|-------------------:|-------------------:|------------------------------------------------------------------|
| js | 1821 | 12 | !function(){if(!window.dreamdata\|\|!window.dreamdata.initiali |
| css | 1059 | 12 | /* This stylesheet is created by 6sense */             .mkto |
| js | 364 | 12 | ((u,h)=>{if(!window.history.state\|\|!window.history.state.key |
| js | 105 | 12 | window.ENV = {"VERCEL_ENV":"production","VERCEL_URL":"oxide- |
| js | 53 | 12 | window.__reactRouterContext.streamController.close(); |

*The full inventory (every fragment with its hash and the page URLs that carry it) is preserved alongside this report as `oxide-computer-inline-code-duplicates.csv`.*
...
We noted that accessibility legislation across major markets converges on ISO 14289-1 (PDF/UA) as the technical baseline; EAA is a highly codified example but illustrates this global alignment.  
We also observed that an untagged PDF remains invisible to machines, whereas a tagged PDF with a proper structure tree becomes machine-readable in the same way semantic HTML is.
...
- https://media.defcon.org/DEF%20CON%2026/DEF%20CON%2026%20presentations/DEFCON-26-Yuwei-Zheng-Shaokun-Cao-Bypass-the-SecureBoot-and-etc-on-NXP-SOCs-Updated.pdf
  Source: https://oxide.computer/blog/exploiting-undocumented-hardware-blocks-in-the-lpc55s69, HTML alternative: Yes
...
Across the audited set, https://oxide.computer demonstrates a perfect Accessibility score of 100/100, showing that your content is fully usable for all visitors. However, Structured Data and Discovery Readiness lag behind with scores of 0/100 and 10/100 respectively, limiting how effectively machines can understand and surface your pages. We invite you to address these opportunities to enhance machine comprehension and broaden discoverability.
...
|-----------|-------|------|
| Served-HTML Structure | 96/100 | Excellent |
...
| Heading Quality | 77/100 | Excellent |
...
- **`/ (nav)`**: SEO 89 · A11y 100 · Back 60 · Served 99 · Rendered 100
- **`/bio`**: SEO 78 · A11y 100 · Back 60 · Served 86 · Rendered 91
- **`/blog`**: SEO 82 · A11y 100 · Back 60 · Served 96 · Rendered 100
- **`/blog/a-tool-for-discussion`**: SEO 90 · A11y 100 · Back 70 · Served 95 · Rendered 100
- **`/blog/another-vulnerability-in-the-lpc55s69-rom`**: SEO 90 · A11y 100 · Back 60 · Served 96 · Rendered 100
- **`/blog/benefits-as-a-reflection-of-values`**: SEO 91 · A11y 100 · Back 60 · Served 96 · Rendered 100
- **`/blog/building-big-systems-with-remote-hardware-teams`**: SEO 79 · A11y 100 · Back 60 · Served 94 · Rendered 100
- **`/blog/compensation-as-a-reflection-of-values`**: SEO 90 · A11y 100 · Back 60 · Served 96 · Rendered 100
- **`/blog/cosmo-sp`**: SEO 88 · A11y 100 · Back 70 · Served 96 · Rendered 100
- **`/blog/dtrace-conf24`**: SEO 92 · A11y 100 · Back 70 · Served 96 · Rendered 100
- **`/blog/engineering-culture`**: SEO 93 · A11y 100 · Back 60 · Served 96 · Rendered 100
- **`/blog/exploiting-undocumented-hardware-blocks-in-the-lpc55s69`**: SEO 83 · A11y 100 · Back 70 · Served 100 · Rendered 100

The page marked (nav) is navigational: it routes visitors to content rather than containing it, and is excluded from the SEO content average. Content-pages SEO average: 87/100.
...
Per page, internal links range from 1 to 47, averaging 15 across 24 pages. That is typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 354 |
...
We reviewed 80 images across the audited set: 1 WebP, 33 SVG and 46 in other or unidentified formats. 71 of 80 (88.8%) carry alt text, leaving 9 without it. Each missing alt attribute is a place where a screen-reader user or a machine reading the page gets no description of what the image shows.

On loading strategy, 3 images are marked `loading="lazy"` and 0 `loading="eager"`, while 77 carry no loading attribute at all. No attribute is not the same as eager: the browser decides for itself when to fetch, which removes the explicit control that lazy and eager give you. Setting an explicit attribute on those images makes the fetch behaviour predictable for browsers and machines alike.
...
**Scope:** 12 pages examined | Platform: Unknown Platform | Frameworks: **Tailwind CSS** -  CSS framework; **Bootstrap** (low confidence) -  CSS framework; **Vercel** (low confidence) -  CDN / delivery | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (12 directives)
