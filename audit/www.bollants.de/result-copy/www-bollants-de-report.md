...
reportId: "www-bollants-de-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://www.bollants.de --pages 10 --date 2026-06-16"
description: "Executive audit report reviewing accessibility, performance, SEO, structured data, and AI agent compatibility for Www Bollants"
tags: [web-audit, accessibility, wcag-aa, ai-agents, seo, performance, mx, executive-report]
performanceScore: 35
accessibilityScore: 100
seoScore: 88
llmSuitabilityScore: 58
totalIssues: 0
pagesAudited: 12
version: "1.0"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/www.bollants.de/www-bollants-de-report.md
  ...
  runbook: "Executive audit report for Www Bollants. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/www.bollants.de/www-bollants-de-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/www.bollants.de/www-bollants-de-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/www.bollants.de/result-copy/www-bollants-de-report.pdf"
...
**Date:** 16 June 2026\
**Report ID:** www-bollants-de-WEB-AUDIT-20260616
...
| SEO | **88**/100 | `################--` |
| Served-HTML Structure | **58**/100 | `##########--------` |
| MX Stack Completeness | **59**/100 | `###########-------` |
| Agent Readability | **40**/100 | `#######-----------` **(!)** |
...
Agent Readability was adjusted down by 30 points for site-wide gaps a machine cannot work around:

- **Real AI agents are blocked from fetching the site** (-10): blocked: ClaudeBot (Anthropic)
- **Origin server is slow for an agent fetch** (-5): origin verdict slow, slowest median 384ms
- **Content appears only after JavaScript runs (site-wide)** (-8): 10 of 12 pages serve a JS shell or far less text than they render
- **Bare-div nesting (div soup) across most pages** (-7, capped at the limit): 11 of 12 pages are heavy with bare-div nesting

Across the audited set, your brand delivers a polished experience for visitors. The strongest human-experience dimension is SEO, with an excellent score of 88/100 that reflects clean URLs, descriptive titles and meta descriptions, and a well-structured content hierarchy. Users find what they need quickly, and search engines reward the site’s clear signals.

The headline opportunity we want to draw attention to is the governance layer that will lift your machine-readiness from Discoverable to Governed. Across the audited set, AI Suitability sits at 58/100 - HTML can be parsed, but machines do not have the context needed for accurate attribution because MX fields such as mx:status, mx:contentType, mx:audience, canonicalUri and provenance markers are absent. Adding these full MX governance metadata will raise MSC above 60 and Discovery Readiness above 40, moving the site into Level 2 where machines can understand who produced the content, when it was published, and its intended audience.
...
We find that across the audited set, the site delivers a strong experience for human visitors, though page load times indicate room for improvement.
...
Across the audited set, machines can discover and parse your content with a Discovery Readiness score of 40/100, yet they lack governance context as indicated by an MX Readiness Level of 1 (Discoverable), while Structured Data Quality sits at 52/100, MX Stack Completeness at 59/100, and Pipeline Survivability remains strong at 91/100.
...
**Evidence:** MX Stack Completeness 59/100 | Structured Data Quality 52/100 | Discovery Readiness 40/100 | Consistency 55%
...
We see a strong foundations across the audited set, with an 88/100 SEO score and perfect accessibility compliance at 100/100. These strengths lay the groundwork for the improvements that follow.
...
| 3 | Semantic Structure (Naked Containers) 39/100 | Compliance Risk | Medium | Medium | machines lose structural context and infer page regions by position |
...
**Finding:** 6 of 287 images (2%) on the audited set carry no alt text, so their content is unavailable to assistive technology and to machines reading the page.
...
**Priority 3: Semantic Structure (Naked Containers) 39/100**

**Bucket:** Compliance Risk

**Finding:** Rendered semantic-structure score 39/100: containers carry no role, ARIA landmark, or descriptive class, so machines fall back on positional inference to determine meaning. The worst page ([/en/living](https://www.bollants.de/en/living)) carries 163 bare divs of 272.
...
**Finding:** The slowest audited page took 40597 ms on a first-time (cold-cache) fetch, well above the healthy origin-response band, so a first-time visitor or an agent arriving on a cold cache waits noticeably for the first byte (returning visitors with a warm cache are served in about 384 ms).
...
**Finding:** Security headers absent: CSP (across the audited set). Missing security headers increase exposure to content injection and clickjacking
...
- **sameAs** links on Organisation: we link the Organisation entity to ORCID, Wikidata and LinkedIn, giving machines a verified identity context; we embed this in the Organisation JSON-LD on the homepage and contact pages.  
- **potentialAction** on Organisation: we add a ContactPoint or Action schema exposing your contact capabilities, enabling agents to surface direct call-to-action links; we place this within the Organisation entity on the main page and any dedicated contact page.  
- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt: we suggest adding a Content-Signal header to declare content-use policy, informing machines how they may reuse your pages; we would add this to the existing robots.txt under a `User-agent: *` block.
...
**Slowest.** The slowest page is `https://www.bollants.de/en/gift-vouchers-extras`. A first-time visitor sees the cold-cache cost: the crawler recorded 40597 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three fresh re-probes that followed returned 418ms, 384ms, 296ms, giving a returning-visitor median of **384 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://www.bollants.de/en/special-offers`. A first-time visitor sees the cold-cache cost: the crawler recorded 2582 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three fresh re-probes that followed returned 348ms, 395ms, 373ms, giving a returning-visitor median of **373 ms**. **Returning-visitor verdict: Healthy**.
...
The sitemap declares 131 URLs and grades Minimal. Lastmod is absent, so machines have no per-page change signal to schedule re-crawls against. The sitemap omits changefreq and priority. Google dropped both as ranking signals in 2017, but non-Google crawlers and AI agents still read changefreq as a re-crawl cadence hint and priority as a relative-importance signal, so adding them is a low-effort way to broaden machine compatibility.

This was a full crawl: the audit reached every page it could discover, and 182 of them are absent from the sitemap (which lists 131). The full set is recorded in the `www-bollants-de-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.
...
|--------------|-------|------------|---------------|------------------------------------------|
| ImageObject | 8 | 100% | 100% |  -  |
| ListItem | 12 | 100% | 100% | Reference |
| PostalAddress | 12 | 100% | 100% |  -  |
| Hotel | 12 | 100% | 100% | PostalAddress, GeoCoordinates, OpeningHoursSpecification |
| WebPage | 12 | 8% | 100% | Website, Organisation, Hotel |
| GeoCoordinates | 12 | 100% | 100% |  -  |
| OpeningHoursSpecification | 12 | 100% | 100% |  -  |
| BreadcrumbList | 12 | 100% | 100% |  -  |
| Page | 12 | 100% | 100% |  -  |
| Schedule | 1 | 100% | 100% |  -  |
| Event | 1 | 100% | 50% | Place, Hotel, Organisation, Audience |
| Audience | 1 | 100% | 100% |  -  |
| Organisation | 1 | 100% | 100% |  -  |
| WebSite | 1 | 100% | 25% | Organisation |
| Place | 1 | 100% | 100% | PostalAddress |
| ItemList | 1 | 100% | 100% |  -  |
...
- **Boilerplate Burial** - Pass · 12/12
  ...
  - *Data:* Highest boilerplate-to-content ratio: 0.10. Threshold: < 10 (and < 80 KB of inline head bytes).
...
- **Body Content Ratio** - Pass · N/A
  - *Means:* Actual prose content averages 74% of served bytes, well above the 30% threshold. Pages are content-heavy, not overhead-heavy.
  - *Data:* Average: 74%. Threshold: 30%.
- **Inline Tag Bloat** - Fail · 12/12
  ...
  - *Data:* 43 element(s) > 500 bytes. Largest single-page inline CSS block: 0 B. Largest single-page inline JS block: 9707 B. See www-bollants-de-pipeline-inline-tag-bloat-pages.csv (12 pages).
...
We found Inline Tag Bloat on every one of the twelve audited pages, which can cause machines to misinterpret or skip content because the markup is overloaded with unnecessary tags. Truncation Risk also appears on some pages, potentially cutting off information that machines rely on for indexing. Removing redundant tags and simplifying the page structure will give machines a clearer view of your content and has the biggest impact on resilience.
...
|--------|--------------|----------------|--------------------|
| Served HTML | 33/100 (high) | 101 bare divs (61% of containers, depth 6) | `div.inside` (80), `div.tooltip` (60), `div` (40), `div.handorgel__content` (26), `div.handorgel__content__inner` (26) |
| Rendered HTML | 39/100 (high) | 163 bare divs (60% of containers, depth 5) | `div.inside` (80), `div.tooltip` (60), `div` (52), `div.spacer` (28), `div.handorgel__content__inner` (26) |

**Worst page (served):** [/en/the-hotel/our-story](https://www.bollants.de/en/the-hotel/our-story)\
**Worst page (rendered):** [/en/living](https://www.bollants.de/en/living)

We found that on the worst-served page (https://www.bollants.de/en/the-hotel/our-story), the served surface has a 61 % bare-div ratio (101 of 165), higher than the rendered surface’s 60 %. This high ratio means machines lose structural context and must rely on positional inference to determine meaning.  

The soup is surface-wide, with many shallow chains of generic divs such as `div.inside` and `div.tooltip`. This pattern indicates that the source pipeline likely injects late-stage JavaScript or uses untyped component frameworks rather than a semantic markup strategy.  

A low-cost first step is to wrap the obvious landmarks-header, nav, main, footer, aside-and assign meaningful class names to the remaining elements so the bare-div ratio falls without restructuring the layout.
...
We found 10 identical inline fragment(s) repeated across multiple pages, totalling 1273 KB redundant bytes. Extracting these to external CSS or JS files would reduce page weight, improve cacheability, and simplify maintenance.
...
We note that across major markets, accessibility legislation has converged on ISO 14289-1 (PDF/UA) as the technical baseline, with the European Accessibility Act-Directive (EU) 2019/882-as a precisely codified example, while Section 508 of the US Rehabilitation Act, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent laws in Australia and Canada all resolve to the same structural artefact.  
We also observe that an untagged PDF remains invisible to machines; only a tagged PDF that includes a proper structure tree can be read by search crawlers, AI systems, and automated pipelines in the same way semantic HTML is understood.
...
We found that https://www.bollants.de achieves a perfect Accessibility score of 100/100, demonstrating strong compliance with WCAG guidelines and ensuring an inclusive experience for all users. Across the audited set, Discovery Readiness scores only 40/100 and Structured Data sits at 52/100, indicating opportunities to improve machine discoverability and richer content representation. We invite you to address these gaps to enhance your site’s visibility and relevance to machines and visitors alike.
...
|-----------|-------|------|
| Served-HTML Structure | 58/100 | Good |
...
| Agent Readability | 40/100 | Could Be Better |
...
- **`/en`**: SEO 90 · A11y 100 · Back 85 · Served 61 · Rendered 18
- **`/en/the-hotel/our-story`**: SEO 100 · A11y 100 · Back 85 · Served 61 · Rendered 27
- **`/en/gift-vouchers-extras`**: SEO 76 · A11y 100 · Back 95 · Served 61 · Rendered 18
- **`/en/before-you-arrive`**: SEO 97 · A11y 100 · Back 85 · Served 71 · Rendered 37
- **`/en/the-hotel/photo-gallery`**: SEO 84 · A11y 100 · Back 85 · Served 71 · Rendered 37
- **`/en/living`**: SEO 87 · A11y 100 · Back 85 · Served 45 · Rendered 18
- **`/en/living/rooms-suites/details`**: SEO 69 · A11y 100 · Back 85 · Served 59 · Rendered 25
- **`/en/living/whats-included`**: SEO 93 · A11y 100 · Back 85 · Served 61 · Rendered 27
- **`/en/living/enquire-about-a-room`**: SEO 95 · A11y 100 · Back 85 · Served 43 · Rendered 18
- **`/en/special-offers`**: SEO 92 · A11y 100 · Back 85 · Served 61 · Rendered 27
- **`/en/special-offers/details`**: SEO 78 · A11y 100 · Back 85 · Served 49 · Rendered 18
- **`/en/spa-im-park`**: SEO 90 · A11y 100 · Back 85 · Served 71 · Rendered 28
...
Per page, internal links range from 24 to 199, averaging 55 across 13 pages. That is denser than typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 710 |
...
We reviewed 286 images across the audited set: 1 SVG, 36 PNG and 249 JPEG. 281 of 286 (98.3%) carry alt text, leaving 5 without it. Each missing alt attribute is a place where a screen-reader user or a machine reading the page gets no description of what the image shows.

On loading strategy, 168 images are marked `loading="lazy"` and 0 `loading="eager"`, while 118 carry no loading attribute at all. No attribute is not the same as eager: the browser decides for itself when to fetch, which removes the explicit control that lazy and eager give you. Setting an explicit attribute on those images makes the fetch behaviour predictable for browsers and machines alike.
...
**Scope:** 12 pages examined | Platform: Unknown Platform | Frameworks: None detected | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (4 directives)
