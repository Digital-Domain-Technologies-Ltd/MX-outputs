...
reportId: "www-contentful-com-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://www.contentful.com --pages 10"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/www.contentful.com/www-contentful-com-report.md
  ...
  runbook: "Executive audit report for Www Contentful. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/www.contentful.com/www-contentful-com-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/www.contentful.com/www-contentful-com-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/www.contentful.com/result-copy/www-contentful-com-report.pdf"
...
**Date:** 16 June 2026\
**Report ID:** www-contentful-com-WEB-AUDIT-20260616
...
- **Origin server is slow for an agent fetch** (-5): origin verdict slow, slowest median 993ms
...
We found that your content delivers a strong human experience. Across the audited set, search visibility is solid with an SEO score of 94 / 100, and the pages showcase clear, engaging copy that reflects Contentful’s brand voice and design standards. The foundation you’ve built for visitors is evident in the consistent layout, intuitive navigation, and well-structured content that users can quickly understand.

The headline opportunity is to elevate machine comprehension by adding MX governance metadata. First, we identified a priority compliance area: accessibility. Across the audited set there are 11 distinct WCAG AA issue types affecting 279 raw instances; addressing each category-many of which cluster in 14 recurring template patterns-will resolve a large portion of these issues with relatively few fixes. Next, while the served HTML is highly suitable for machines (99 / 100), no MX-namespaced governance markers are present. Adding full MX fields, including mx:status, mx:contentType, mx:audience, canonicalUri, and provenance metadata, will give machines the structured context they need for accurate comprehension and lift your MX Readiness Level from Discoverable to Governed, raising MSC above 60 and DR above 40.
...
Across the audited set, we find the site delivers an excellent experience for human visitors, though page load times average 3429 ms and could be improved.
...
We found that across the audited set, machines can discover and parse your content-Discovery Readiness at 39/100-but lack governance context, as shown by Structured Data Quality of 61/100, MX Stack Completeness of 66/100, Pipeline Survivability of 79/100, an overall MX Readiness Level of 1 (Discoverable), and the absence of any governance markers.
...
Across the audited set, we see a strong foundations: SEO scores of 94/100 overall (98/100 on content pages), accessibility at 77/100 with many issues already addressed by template patterns, structured data quality at 61/100, and most security headers present on every page. These strengths provide the groundwork for targeted improvements that will further enhance machine comprehension and user experience.
...
**Finding:** The slowest audited page took 4248 ms on a first-time (cold-cache) fetch, well above the healthy origin-response band, so a first-time visitor or an agent arriving on a cold cache waits noticeably for the first byte (returning visitors with a warm cache are served in about 993 ms).
...
- **sameAs**: linking your Organisation entity to external identifiers such as LinkedIn or ORCID gives machines a reliable reference point for attribution; add these links in the Organisation schema on each page that includes it.  
- **potentialAction**: adding a potentialAction property to the Organisation schema exposes contact methods (e.g., ContactPoint) so agents can discover how to reach you directly; place this on pages where the Organisation entity appears.  
- **Content-Signal directives**: declare content-use policy in robots.txt per contentsignals.org, enabling machines to understand licensing and reuse rights for your content.
...
| Attribute | Value |
|-----------|-------|
| Origin | https://www.contentful.com |
...
|------|----------|----------|----------:|------:|----------------|
| WebSite | recommended | image | 12 | 12 | None of the audited pages declare a logo or hero image in structured data |
...
All audited pages show a truncation risk that can prevent machines from seeing the full content, which limits their ability to index or summarise accurately. Body content ratio and inline tag bloat also pose challenges by slowing parsing and increasing payload size. Addressing truncation risk first will give the biggest lift in machine comprehension.
...
The served surface shows the highest bare-div ratio-52 % (209 of 405) on the worst-scoring page https://www.contentful.com/products/personalization/ai-suggestions/, which is the single worst page in the audited set, meaning machines lose structural context and must fall back on positional inference.  
The soup is structural, with deep chains of seven nested bare divs, indicating that the content was produced by an untyped component framework or drag-and-drop builder that nests elements without semantic wrappers.  
A cost-effective first move is to wrap the primary landmarks-header, navigation, main, footer, aside-with semantic tags and give remaining elements meaningful class names; this lowers the bare-div ratio while preserving layout.
...
Ninetailed (personalisation) is present on https://www.contentful.com/products/ai-actions/, https://www.contentful.com/products/studio/, https://www.contentful.com/contact/sales/, https://www.contentful.com/products/platform/, https://www.contentful.com/products/personalization/ and 7 more, but every visit returned the same content during this audit.

This is a latent risk to note, not a current finding. Because machines arrive with no cookie and no session, if the layer is later configured to act on first-touch or uncohorted traffic, machine visitors could start receiving variant content. The personalisation layer was installed but served identical content in this run; it could begin assigning variants if configuration changes.

- No action is needed today: the layer served machines consistent content throughout this audit.
...
Analysis of text patterns across audited pages found content reaching Hints at AI on the AI-tells scale (1 of 12 pages scored). Machines do not consistently cite or label AI-generated content; this observation describes what the analysis found, not a conclusion about authorship. The full per-page breakdown is in `ai-tells.json` in the results directory.
...
We found that https://www.contentful.com scores an strong 99/100 in AI Suitability, indicating strong readiness for machine consumption across the audited set of 12 pages. However, Discovery Readiness and Structured Data score 39/100 and 61/100 respectively, highlighting key opportunities to improve metadata exposure and discovery signals for machines. We invite you to address these gaps to enhance your content’s machine-readability and overall digital performance.
...
**Scope:** 12 pages examined | Platform: Unknown Platform | Frameworks: **Next.js** (low confidence) -  JS framework; **Gatsby** (low confidence) -  JS framework; **Svelte** (low confidence) -  JS framework; **Tailwind CSS** (medium confidence) -  CSS framework | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (3 directives)
