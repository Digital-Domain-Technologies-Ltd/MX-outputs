...
- Pages audited: 7 of 278 in sitemap - sample run
- Date: 2026-05-27
- Scores: SEO 80/100 | Accessibility 86/100 | Discovery Readiness 25/100 | Structured Data 0/100
...
- Accessibility: 20 distinct WCAG AA issue types (35 raw instances, 35 marked critical by Pa11y) - 17 of these instances trace to 3 recurring template patterns (single theme edit per pattern fixes all instances)
- AI Suitability (served): 85/100
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- Performance: 867ms avg (Excellent)
- Accessibility: 86/100 (Excellent)
- SEO: 80/100 (Excellent)
- Accessibility issues: 20 distinct WCAG codes across 35 raw instances (17 instances trace to 3 template patterns)
-->
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- SEO (all pages): 80/100
- SEO (content pages only): 81/100
- Accessibility: 86/100 (20 distinct WCAG codes, 35 raw instances) - 17 of these instances trace to 3 template-level patterns
...
Div Soup (Semantic Structure): rendered score 23/100, band high. When the rendered score is below 70, include this as a numbered Priority row. The figures 234 bare divs out of 332 total describe the WORST-CASE page in the audited set (https://typo3.com/), not a site-wide average. The finding title and the Finding paragraph must scope to that worst page; do NOT present these numbers as if they hold across every audited page. The title should include the rendered score, the worst-page URL, and the bare-divs-of-total figure (for example, a title naming the score, the page path, and the figure such as "95 of 153 bare divs"). In the Finding paragraph, name the worst page URL explicitly and note that the figures cited come from it specifically; if the band still applies because most pages share the template, say so.

Separate Priority blocks with `---`.

FACTS:
- Pa11y data for this audit:  Of these, 10 (29%) trace to 2 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 7 (20%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- **WCAG 4.1.1**: Duplicate id attribute value "Ebene_1" found on the web page. | selector: `#Ebene_1` | 3 instances on 2 pages | affects: all assistive tech users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.09:1. Recommendation:  change background to #c25700. | selector: `#c491 > div > div > div > div > div > div > div:nth-child(2)…` | 1 instance on 1 page | affects: low-vision users
- **WCAG 3.2.2**: This form does not contain a submit button, which creates issues for those who cannot submit the form using the keyboard. Submit buttons are INPUT elements with type attribute "submit" or "image", or BUTTON elements with type "submit" or omitted/invalid. | selector: `#collapseCaseStudyFilter > div > form` | 1 instance on 1 page | affects: all users
- Div Soup rendered score: 23/100, band high
- Div Soup bare divs: 234 of 332 (figures from the WORST page only)
- Div Soup worst page URL: https://typo3.com/
- Div Soup audited page count: 7

When a Pa11y finding row in  Of these, 10 (29%) trace to 2 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 7 (20%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 10 (29%) trace to 2 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 7 (20%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- **WCAG 4.1.1**: Duplicate id attribute value "Ebene_1" found on the web page. | selector: `#Ebene_1` | 3 instances on 2 pages | affects: all assistive tech users
- **WCAG 1.4.3**: This element has insufficient contrast at this conformance level. Expected a contrast ratio of at least 4.5:1, but text in this element has a contrast ratio of 3.09:1. Recommendation:  change background to #c25700. | selector: `#c491 > div > div > div > div > div > div > div:nth-child(2)…` | 1 instance on 1 page | affects: low-vision users
- **WCAG 3.2.2**: This form does not contain a submit button, which creates issues for those who cannot submit the form using the keyboard. Submit buttons are INPUT elements with type attribute "submit" or "image", or BUTTON elements with type "submit" or omitted/invalid. | selector: `#collapseCaseStudyFilter > div > form` | 1 instance on 1 page | affects: all users above.
...
- Rendered score: 23, band high (worst page: https://typo3.com/)
- Bare divs (rendered): 234 of 332 (70%)
- Deepest bare chain (rendered): 7
- Top bare selectors (rendered): `div.frame-group-container` (85), `div.frame-group-inner` (85), `div.frame-container.frame-container-default` (85), `div.frame-inner` (85), `div.partner-list-item__types` (20)
...
WRITE:
2-3 sentences summarising the audit findings for Typo3 (https://typo3.com). Always refer to the site by its domain (https://typo3.com), never by the consultancy name. The highest-scoring dimension is SEO at 80/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 80/100; Structured Data: 0/100; Discovery Readiness: 25/100
