...
- Date: 2026-05-27
- Scores: SEO 80/100 | Accessibility 87/100 | Discovery Readiness 25/100 | Structured Data 0/100
...
- Accessibility: 18 distinct WCAG AA issue types (33 raw instances, 33 marked critical by Pa11y) - 15 of these instances trace to 3 recurring template patterns (single theme edit per pattern fixes all instances)
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- Performance: 2283ms avg (Excellent)
- Accessibility: 87/100 (Excellent)
- SEO: 80/100 (Excellent)
- Accessibility issues: 18 distinct WCAG codes across 33 raw instances (15 instances trace to 3 template patterns)
-->
...
- Accessibility: 87/100 (18 distinct WCAG codes, 33 raw instances) - 15 of these instances trace to 3 template-level patterns
...
FACTS:
- Pa11y data for this audit:  Of these, 9 (27%) trace to 2 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 6 (18%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.

Pa11y findings for this audit (use ONLY these for specific recommendations):
- **WCAG 2.4.1**: Iframe element requires a non-empty title attribute that identifies the frame. | selector: `#cross-domain-consent-sharing-iframe` | 6 instances on 6 pages | affects: sighted keyboard users | third-party element, vendor: Usercentrics (host: `app.usercentrics.eu`, consent-management), NOT in site template
...
When a Pa11y finding row in  Of these, 9 (27%) trace to 2 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 6 (18%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 9 (27%) trace to 2 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 6 (18%) trace to 1 vendor-injected DOM pattern from Usercentrics; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Accessibility: 87/100 (Excellent) - 33 issues - 15 trace to 3 template pattern(s)
- SEO: 80/100 (Excellent)
- Security headers: 2/5 present (HTTPS, X-Content-Type-Options); 0 of 7 audited URLs carry all five
- Heading quality: 90/100
- Pipeline Survivability: 96/100
- Served HTML: 85/100, Rendered HTML: n/a/100
- Performance: 2283ms average load time
...
| Full Optimization | Catalogue Visibility, Structured Data, Semantic Structure, Discovery Readiness, MX Stack Completeness, Performance, Security headers, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
