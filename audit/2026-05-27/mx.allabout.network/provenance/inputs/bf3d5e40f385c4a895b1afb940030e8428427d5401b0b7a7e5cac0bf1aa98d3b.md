...
- Pages audited: 12 of 107 in sitemap - sample run
- Date: 2026-05-27
- Scores: SEO 94/100 | Accessibility 100/100 | Discovery Readiness 85/100 | Structured Data 94/100
- Scope: 12 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): ListItem (16), WebPage (14), Organization (11), Person (8), BreadcrumbList (7), BlogPosting (5), Offer (3), Service (3), WebSite (2), ContactPoint (2), AboutPage (1), CommunicateAction (1), EntryPoint (1), PropertyValue (1), ContactPage (1), ImageObject (1), Blog (1)
...
- AI Suitability (served): 95/100
- Structured Data Quality: 94/100
- Discovery Readiness: 85/100
- Catalogue Visibility: 20/100
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 12
- Performance: 252ms avg (Excellent)
...
Div Soup (Semantic Structure): rendered score 100/100, band low. When the rendered score is below 70, include this as a numbered Priority row. The figures 0 bare divs out of 23 total describe the WORST-CASE page in the audited set (https://mx.allabout.network/), not a site-wide average. The finding title and the Finding paragraph must scope to that worst page; do NOT present these numbers as if they hold across every audited page. The title should include the rendered score, the worst-page URL, and the bare-divs-of-total figure (for example, a title naming the score, the page path, and the figure such as "95 of 153 bare divs"). In the Finding paragraph, name the worst page URL explicitly and note that the figures cited come from it specifically; if the band still applies because most pages share the template, say so.

Separate Priority blocks with `---`.

FACTS:
- Pa11y data for this audit: 
- Div Soup rendered score: 100/100, band low
- Div Soup bare divs: 0 of 23 (figures from the WORST page only)
- Div Soup worst page URL: https://mx.allabout.network/
...
|-------------|-------|-----------|--------------|-------|
| ListItem | 7 | 100% | 100% | - |
| WebPage | 12 | 100% | 100% | Website, WebSite, Organization, Person |
| Organization | 9 | 100% | 100% | Person, ContactPoint, Reference, Website |
| Person | 8 | 100% | 100% | Organization, PropertyValue |
| BreadcrumbList | 7 | 100% | 100% | - |
| BlogPosting | 5 | 100% | 100% | Person, Organization, Reference, Website |
| Offer | 1 | 100% | 100% | Organization, Service |
| Service | 1 | 100% | 100% | Organization, Service, Website |
...
| Blog | 1 | 100% | 100% | Organization, Person, Reference, Website |
...
- Pages affected by that check: 0 of 12
...
- Rendered score: 100, band low (worst page: https://mx.allabout.network/)
- Bare divs (rendered): 0 of 23 (0%)
- Deepest bare chain (rendered): 0
...
| Full Optimization | Catalogue Visibility, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 94/100; Structured Data: 94/100; Discovery Readiness: 85/100
