...
|---|------|----------|---------|----------|
| 1 | template-voice | template-voice-style | Template voice/style findings: 26 violation(s) | 2026-05-27T13:47:40Z |
...
We audited six pages of dkd.de and found a property that presents well to human visitors across every dimension we measure. SEO foundations are solid, scoring 92/100 in the Excellent band, and accessibility is clean across the audited set, with no distinct WCAG AA issue types recorded and no raw instances flagged as critical by Pa11y. That combination tells us the groundwork is there: content is well-structured, findable, and inclusive, giving human visitors a coherent and navigable experience built on a stable TYPO3 CMS platform.

The headline opportunity is in machine experience. Machines can already discover and parse dkd.de at MX Readiness Level 1 (Discoverable), and the numeric scores for Machine Suitability and Structured Data Quality are moving in the right direction. The concrete next step, however, is not to raise those scores further; it is to add full MX governance fields, including mx:status, mx:contentType, mx:audience, canonicalUri, and provenance markers, across the audited set so that machines can cite dkd.de as an attested source rather than merely read it. Alongside that, we recommend raising Machine Suitability above 60 and Discovery Readiness above 40, since both sit below their respective thresholds. Catalogue Visibility, the lowest machine-readiness score we recorded, signals that the chance to be indexed and surfaced by agent catalogues remains almost entirely untapped.

That said, Discovery Readiness at 25/100 means the discovery artefacts machines rely on to map and prioritise a site are thin across the audited set, so adding governance metadata works best in combination with strengthening the discovery layer. Schema.org JSON-LD is worth noting here as a high-leverage asset: it is readable by every machine regardless of how a page is rendered, meaning improvements made at that layer compound across the full range of crawlers, language models, and structured-data consumers without any dependency on rendering behaviour.
...
Across the audited set, dkd.de delivers a strong human experience, with Accessibility at 100/100 and SEO at 92/100, both in the Excellent band, and performance averaging 2881ms in the Good range.
...
Across the audited set, machines can discover and parse content on dkd.de, though the current MX Readiness Level of 1 (Discoverable) means they cannot yet cite it as an attested source.
...
Across the audited set, dkd.de demonstrates a strong foundations of technical rigour, from an SEO score of 92/100 and a perfect Accessibility score of 100/100 to a full complement of five security headers. These results reflect deliberate engineering choices that give the improvements ahead a strong base to build from.

| Dimension | Score | Highlights |
|-----------|-------|------------|
| Performance | Good | Good - 2881ms average load time |
...
| Agent access | 6/7 | 6 of 7 AI user-agents receive HTTP 200; CCBot (Common Crawl) blocked |
...
We have prioritised the findings below by the impact each opportunity carries for machines reading dkd.de, with discovery gaps leading because they block everything downstream. Catalogue Visibility and MX Stack Completeness follow closely, given their direct bearing on how completely and reliably machines can interpret and act on the content we present.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Duplicate ID Attributes, WCAG 4.1.1 (accessibility, account, alarm, article, bell), 6 pages | Compliance Risk | High | Low | Assistive tech users may miss or misread labelled controls across the audited set |
| 2 | Semantic Structure 53/100, Worst Page https://dkd.de/de/kontakt/ (26 of 48 bare divs) | Cross-cutting | Medium | Medium | Machines and assistive tech are less likely to parse content hierarchy correctly on that page |
| 3 | Discovery Readiness 25/100, Five Artefacts Absent (llms.txt, llms-full.txt, agent-card.json, ai.txt, humans.txt) | AI Opportunity | High | Medium | Machines may miss dkd.de entirely when building answer sets or citation pools |
| 4 | Structured Data Quality 65/100, MX Governance Fields Absent (canonicalUri, contentType, audience, status) | AI Opportunity | Medium | Medium | Machines reading structured data are less likely to treat content as an attested, attributable source |
...
**Finding:** Across the audited set, five ID values (`#accessibility`, `#account`, `#alarm`, `#article`, `#bell`) each appear on all 6 audited pages. Because IDs are required to be unique within a document, assistive technologies that rely on ID references to navigate or announce elements may reach the wrong target or skip it entirely. The pattern is template-level: 356 instances trace to 65 recurring template patterns, meaning a single theme edit per pattern resolves all instances across the audited set.

**What to change and why:**
- **Remove duplicate ID values for `#accessibility`, `#account`, `#alarm`, `#article`, and `#bell` from the shared template.** WCAG 4.1.1 (Parsing) requires each ID to be unique per page; duplicate IDs break the programmatic name and state that screen readers depend on, directly excluding assistive tech users from correct interaction.
- **Audit the 65 template patterns identified in the Pa11y data and resolve each at source.** Because these are template-level patterns, a single change propagates across every page that inherits the template, making the remediation effort disproportionately low relative to the compliance gain.
- **Validate the corrected template against an automated accessibility checker before deployment.** This confirms no new duplicate IDs are introduced by the fix and gives the team an audit trail for WCAG 4.1.1 conformance.
...
**Finding:** We score Semantic Structure at 53/100 across the audited set, placing it in the medium band. The figures for bare-div density are drawn specifically from https://dkd.de/de/kontakt/, where 26 of 48 total elements are bare divs. Where most pages in the audited set share the same template structure, this pattern is likely to recur across them. Machines and assistive technologies infer meaning from semantic elements; a high proportion of bare divs reduces the confidence with which content hierarchy, landmarks, and relationships can be determined.

**What to change and why:**
- **Replace bare divs that serve a structural or landmark role on https://dkd.de/de/kontakt/ with appropriate semantic elements.** Semantic elements give machines an unambiguous signal about the role of each content block, improving both crawlability and screen-reader navigation without requiring any visual change.
- **Review the shared template for bare-div patterns that carry implicit heading, list, or navigation roles.** Because the rendered score of 53/100 applies across the audited set, template-level changes propagate the benefit broadly and address the root cause rather than individual pages.
- **Re-measure Semantic Structure after refactoring.** Raising the score above 70 moves the finding out of the medium band and improves the Pipeline Survivability score, which currently sits at 75/100 partly because content structure is difficult for machines to parse reliably.
...
**Finding:** We score Discovery Readiness at 25/100 (Needs Improvement) across the audited set. None of the five well-known discovery artefacts are reachable on dkd.de: llms.txt, llms-full.txt, agent-card.json, ai.txt, and humans.txt are all absent. Without these artefacts, machines that look for explicit permission signals, content summaries, or agent identity declarations before indexing or citing a source will find nothing to work with, reducing dkd.de's presence in machine-generated answer sets.

**What to change and why:**
- **Add llms.txt to the root of dkd.de.** This file gives machines a structured, human-curated summary of what the site contains and which sections are available for indexing; its absence is the single largest contributor to the 25/100 Discovery Readiness score.
- **Add llms-full.txt to complement llms.txt with the full content layer.** Machines that ingest llms-full.txt can build richer representations of dkd.de's content for citation and answer generation.
- **Add agent-card.json to declare the site's identity and permissions for autonomous agents.** This artefact supports MX Stack Completeness, which currently sits at 62/100; publishing agent-card.json addresses the discovery-artefact gap in that score.
- **Add ai.txt and humans.txt.** ai.txt signals crawl permissions explicitly to machine clients; humans.txt provides team and contact context that supports trustworthiness signals for both machines and human reviewers.

**Effort:** Medium

---

**Priority 4: Structured Data Quality 65/100, MX Governance Fields Absent (canonicalUri, contentType, audience, status)**

**Bucket:** AI Opportunity

**Finding:** We score Structured Data Quality at 65/100 across the audited set. The schema types already in place (ListItem, Question, Answer, BreadcrumbList, WebSite, FAQPage) give machines a reasonable content map, but the absence of MX governance fields in page frontmatter means machines reading those schemas have no canonical URI, no content type declaration, no stated audience, and no publication status to anchor attribution. This reduces the likelihood that machines will treat dkd.de content as a citable, attributable source. MX Stack Completeness sits at 62/100, and the missing governance fields are a direct contributor to that gap.

**What to change and why:**
- **Add `canonicalUri` to page frontmatter.** Without a declared canonical URI, machines resolving duplicate or syndicated content risk attributing claims to the wrong source, reducing citation accuracy for dkd.de.
- **Add `contentType` and `audience` fields to page frontmatter.** These fields allow machines to classify and route content to the right answer context; their absence means content may be served by machines to audiences for whom it was not intended, or omitted from specialist answer pools entirely.
- **Add `status` to page frontmatter.** A declared publication status tells machines whether content is current, archived, or in draft; without it, machines may cite outdated material as current, or decline to cite because provenance is unclear.
- **Review structured data coverage alongside governance fields.** An SDQ of 65/100 with the existing schema types suggests coverage depth and governance signals together move the MX Stack Completeness score; both surfaces need attention to reach Citation-ready status.
...
- **sameAs links on Organisation**: adding `sameAs` properties pointing to dkd.de's Wikidata or LinkedIn entries on the `Organisation` entity would allow machines to cross-reference and confirm the entity's identity across knowledge graphs, strengthening citation confidence.

- **BreadcrumbList on deep pages**: where deep pages currently rely on URL path structure alone, attaching `BreadcrumbList` markup gives machines an explicit navigational context, making it easier to understand page hierarchy and surface the correct entry point in an answer.

- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt: declaring content-use policy for AI agents in robots.txt signals to machines which content they are permitted to use for training or citation, reducing ambiguity about dkd.de's intent.
...
**Slowest.** The slowest page is `https://dkd.de/de`. A first-time visitor sees the cold-cache cost: the crawler recorded 6731 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three cache-busted re-probes that followed returned 307ms (HTTP 503), 659ms (HTTP 503), 369ms (HTTP 503); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: Indeterminate**.

**Median-load control.** The median-load control page is `https://dkd.de/de/typo3-agentur/`. A first-time visitor sees the cold-cache cost: the crawler recorded 1586 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three cache-busted re-probes that followed returned 322ms (HTTP 503), 1848ms (HTTP 503), 280ms (HTTP 503); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: Indeterminate**.
...
We found robots.txt in place, listing 21 disallow paths that restrict machines from crawling those areas of dkd.de, while one sitemap reference is announced to guide discovery of the permitted content.
...
We grade the sitemap at A: it covers 9 URLs and carries lastmod, changefreq, and priority attributes on every entry, giving crawlers the scheduling signals they need to prioritise and time their visits accurately.

Across the audited set, we identified a URL variant cluster where both https://dkd.de/de and https://dkd.de/de/ appear in the crawl seed, producing nine crawled URLs against eight unique canonical pages at a 1.1× inflation factor; machines that skip URL normalisation will fetch the same content multiple times, wasting token budget and potentially generating contradictory or inflated findings. We recommend consolidating sitemap.xml to a single canonical form per resource and adding a canonical link tag to each page so that machines resolve one authoritative URL without ambiguity.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms.txt file reachable on dkd.de, meaning machines that consult this discovery artefact before crawling or citing a source will find no site description, no page inventory, and no content policy to guide them. We recommend adding llms.txt to give machines a structured entry point into the content available across the audited set.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We did not find a reachable llms-full.txt on dkd.de; the endpoint returns a 404 with no sitemap reference and no discovery link in the homepage head. Whether adding one is worthwhile depends on the depth of content across the full site, which our audited sample of six pages does not yet measure, so we recommend treating this as a conditional next step once the team has assessed whether the site carries sufficient content volume to justify a full machine-readable corpus.
...
- **Truncation Risk** - Fail · 5/6
  ...
  - *Data:* Largest page: 223 KB. Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows. See dkd-de-de-pipeline-truncation-risk-pages.csv (5 pages).
- **SPA Shell** - Pass · 6/6
...
- **HTTP Content Negotiation (Vary)** - Fail · 3/6
  ...
  - *Data:* 3 page(s) advertise format negotiation. See dkd-de-de-pipeline-http-content-negotiation-(vary)-pages.csv (3 pages).
- **Cross-Host Redirect** - Fail · 6/6
  - *Means:* 3 page(s) redirect to a different host. Agents that enforce same-origin policy may not follow these chains.
  - *Data:* 3 page(s) cross origin during redirect. See dkd-de-de-pipeline-cross-host-redirect-pages.csv (6 pages).
...
- **Inline Tag Bloat** - Fail · 6/6
  ...
  - *Data:* 6 element(s) > 500 bytes. Largest single-page inline CSS block: 2548 B. Largest single-page inline JS block: 841 B. See dkd-de-de-pipeline-inline-tag-bloat-pages.csv (6 pages).
...
Across the audited set, we flag four resilience checks requiring attention: Truncation Risk, Content Negotiation, Cross-Host Redirect, and Inline Tag Bloat. The most consistent of these is Cross-Host Redirect, which we record on every one of the six audited pages, meaning machines following links may absorb redirect overhead or misattribute content before they reach a canonical response. Resolving the cross-host redirect pattern would produce the largest single improvement to how reliably machines can read and process dkd.de.
...
On the worst-performing page we sampled, https://dkd.de/de/kontakt/, we record a bare-div ratio of 54% across 48 total elements, meaning machines lose structural context and must rely on positional inference to determine meaning. The pattern here is surface-wide rather than deep: a shallow chain of three alongside a high bare ratio points to untyped component frameworks where class names carry visual intent but no structural role, leaving the rendered tree semantically thin. The most direct first move is to wrap the obvious landmarks, header, nav, main, footer, and aside, with their corresponding semantic elements, then give the remaining components meaningful class names so the bare-div ratio falls without requiring any layout restructure.
...
The audited set shows consistent metadata patterns across the six pages, with no organisation-name or canonical-URL divergence flagged by the consistency check.
...
Across the audited set, PDFs sit at an intersection where accessibility legislation and machine readability converge on the same technical requirement: a properly tagged document with a valid structure tree. The EAA (Directive (EU) 2019/882, in force 28 June 2025) is the most precisely codified instance of a global alignment that includes Section 508, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent frameworks in Australia and Canada, all of which resolve to ISO 14289-1 (PDF/UA) as the structural baseline. Independently, an untagged or image-based PDF is opaque to machines: search crawlers, AI systems, and automated pipelines cannot extract text, entities, or structure from it, whereas a tagged PDF with a proper structure tree is machine-readable in the same way that semantic HTML is.
...
Across the audited set, https://dkd.de returns an Accessibility score of 100/100, a strong foundation that serves every visitor who reaches the pages we reviewed. SEO follows closely at 92/100, while Structured Data Quality at 65/100 and Discovery Readiness at 25/100 represent the clearest opportunities to extend that strength toward machines reading, parsing, and eventually citing the content. We look forward to walking through the detail with the team and mapping a practical path forward.
...
Across the audited set, we examined 48 images in total. The format breakdown shows 9 PNG files and 1 JPEG, with 38 images in other or unrecognised formats. Alt-text coverage is complete: all 48 images carry descriptive alt text, leaving no gaps for screen readers or machines parsing the page content.

On loading strategy, every one of the 48 images we sampled carries an explicit loading="lazy" attribute, and none are set to loading="eager" or left without a loading declaration. That last point is worth noting in its own right: an image with no loading attribute is not the same as eager loading, it simply leaves the decision to the browser's heuristics, which vary by position in the document and by browser version. Across the audited set, that ambiguity is absent entirely, a clean result.
...
**Scope:** 6 pages analysed | Platform: TYPO3 CMS | Analysis method: Hybrid (automated + manual verification) | robots.txt: Found
