...
- Site: Dkd (dkd.de)
- Pages audited: 6 (full audit)
- Date: 2026-05-27
- Scores: SEO 92/100 | Accessibility 100/100 | Discovery Readiness 25/100 | Structured Data 65/100
- Scope: 6 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): ListItem (13), Question (12), Answer (12), BreadcrumbList (6), WebSite (2), FAQPage (1)
...
- Accessibility: 0 distinct WCAG AA issue types (0 raw instances, 0 marked critical by Pa11y) - 356 of these instances trace to 65 recurring template patterns (single theme edit per pattern fixes all instances)
- AI Suitability (served): 79/100
- Structured Data Quality: 65/100
- Discovery Readiness: 25/100
- Catalogue Visibility: 10/100
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 6
- Performance: 3462ms avg (Good)
- Accessibility: 100/100 (Excellent)
- SEO: 92/100 (Excellent)
- Accessibility issues: 0 distinct WCAG codes across 0 raw instances (356 instances trace to 65 template patterns)
-->
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 6
- SEO (all pages): 92/100
- SEO (content pages only): 92/100
- Accessibility: 100/100 (0 distinct WCAG codes, 0 raw instances) - 356 of these instances trace to 65 template-level patterns
- Security headers: 5/5 present (HTTPS, HSTS, CSP, X-Frame-Options, X-Content-Type-Options); 0 of 6 audited URLs carry all five
- Structured Data Quality: 65/100
- Consistency: 62%
-->
...
Div Soup (Semantic Structure): rendered score 53/100, band medium. When the rendered score is below 70, include this as a numbered Priority row. The figures 26 bare divs out of 48 total describe the WORST-CASE page in the audited set (https://dkd.de/de/kontakt/), not a site-wide average. The finding title and the Finding paragraph must scope to that worst page; do NOT present these numbers as if they hold across every audited page. The title should include the rendered score, the worst-page URL, and the bare-divs-of-total figure (for example, a title naming the score, the page path, and the figure such as "95 of 153 bare divs"). In the Finding paragraph, name the worst page URL explicitly and note that the figures cited come from it specifically; if the band still applies because most pages share the template, say so.

Separate Priority blocks with `---`.

FACTS:
- Pa11y data for this audit:  Of these, 356 (97%) trace to 65 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.

Pa11y findings for this audit (use ONLY these for specific recommendations):
- **WCAG 4.1.1**: Duplicate id attribute value "accessibility" found on the web page. | selector: `#accessibility` | 6 instances on 6 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "account" found on the web page. | selector: `#account` | 6 instances on 6 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "alarm" found on the web page. | selector: `#alarm` | 6 instances on 6 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "article" found on the web page. | selector: `#article` | 6 instances on 6 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "bell" found on the web page. | selector: `#bell` | 6 instances on 6 pages | affects: all assistive tech users
- Div Soup rendered score: 53/100, band medium
- Div Soup bare divs: 26 of 48 (figures from the WORST page only)
- Div Soup worst page URL: https://dkd.de/de/kontakt/
- Div Soup audited page count: 6

When a Pa11y finding row in  Of these, 356 (97%) trace to 65 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 356 (97%) trace to 65 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- **WCAG 4.1.1**: Duplicate id attribute value "alarm" found on the web page. | selector: `#alarm` | 6 instances on 6 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "article" found on the web page. | selector: `#article` | 6 instances on 6 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "bell" found on the web page. | selector: `#bell` | 6 instances on 6 pages | affects: all assistive tech users above.
...
FACTS (do not change any number, percentage, URL, page count, or name):
- Pages audited: 6 - scope all Priority descriptions to "the audited page" (singular) or "the audited set" (plural); do NOT say "site-wide" or "across the site"
...
FACTS:
- Crawled URLs before deduplication: 9
- Unique canonical pages after normalisation: 8
- Inflation factor: 1.1×
- Variant clusters found:
  - Canonical: https://dkd.de/de; variants also present: https://dkd.de/de/
...
- Pages audited: 6
- Pipeline Survivability score: 75
- Checks flagged on any page: Truncation Risk, Content Negotiation, Cross-Host Redirect, Inline Tag Bloat
- Check with the most pages flagged: Cross-Host Redirect
...
- Rendered score: 53, band medium (worst page: https://dkd.de/de/kontakt/)
- Bare divs (rendered): 26 of 48 (54%)
- Deepest bare chain (rendered): 3
- Top bare selectors (rendered): `div.text-columns__column` (81), `div.text-columns.text-columns--` (26), `div.textpic.textpic--inside` (12), `div.blog-card__info` (8), `div.text-columns.text-columns--above` (6)
- Pages contributing to the Div Soup aggregation: 6
...
| Phase | Scope | Outcome |
|-------|-------|---------|
| Critical Fixes | Priority 1 compliance items and Catalogue Visibility fixes | Priority 1 items resolved, compliance risk removed |
| Full Optimization | Catalogue Visibility, Discovery Readiness, Semantic Structure, MX Stack Completeness, Structured Data, Performance, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
WRITE:
2-3 sentences summarising the audit findings for Dkd (https://dkd.de). Always refer to the site by its domain (https://dkd.de), never by the consultancy name. The highest-scoring dimension is Accessibility at 100/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 92/100; Structured Data: 65/100; Discovery Readiness: 25/100
...
- PNG: 9
- JPEG: 1
- Other / unrecognised formats: 38
- With alt text: 48 (100.0%)
- Missing alt text: 0
- loading="lazy": 48
- loading="eager": 0
