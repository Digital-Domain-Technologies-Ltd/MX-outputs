...
- Site: Dkd (dkd.de)
- Pages audited: 10 (full audit)
- Date: 2026-05-27
- Scores: SEO 94/100 | Accessibility 100/100 | Discovery Readiness 25/100 | Structured Data 68/100
- Scope: 10 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): ListItem (32), Question (18), Answer (18), BreadcrumbList (10), FAQPage (2), WebSite (1)
...
SCOPE: 10 pages were audited. Scope all claims to "the audited set" - do not say "site-wide" unless cross-page consistency is explicitly confirmed in the facts.
...
- Accessibility: 0 distinct WCAG AA issue types (0 raw instances, 0 marked critical by Pa11y) - 560 of these instances trace to 56 recurring template patterns (single theme edit per pattern fixes all instances)
- AI Suitability (served): 76/100
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 10
- Performance: 2367ms avg (Excellent)
- Accessibility: 100/100 (Excellent)
- SEO: 94/100 (Excellent)
- Accessibility issues: 0 distinct WCAG codes across 0 raw instances (560 instances trace to 56 template patterns)
-->
...
- Accessibility: 100/100 (0 distinct WCAG codes, 0 raw instances) - 560 of these instances trace to 56 template-level patterns
- Security headers: 5/5 present (HTTPS, HSTS, CSP, X-Frame-Options, X-Content-Type-Options); 0 of 10 audited URLs carry all five
- Structured Data Quality: 68/100
- Consistency: 63%
-->
...
FACTS:
- Pa11y data for this audit:  Of these, 560 (96%) trace to 56 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.

Pa11y findings for this audit (use ONLY these for specific recommendations):
- **WCAG 4.1.1**: Duplicate id attribute value "accessibility" found on the web page. | selector: `#accessibility` | 10 instances on 10 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "account" found on the web page. | selector: `#account` | 10 instances on 10 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "alarm" found on the web page. | selector: `#alarm` | 10 instances on 10 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "article" found on the web page. | selector: `#article` | 10 instances on 10 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "bell" found on the web page. | selector: `#bell` | 10 instances on 10 pages | affects: all assistive tech users
...
- Div Soup audited page count: 10

When a Pa11y finding row in  Of these, 560 (96%) trace to 56 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 560 (96%) trace to 56 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- **WCAG 4.1.1**: Duplicate id attribute value "alarm" found on the web page. | selector: `#alarm` | 10 instances on 10 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "article" found on the web page. | selector: `#article` | 10 instances on 10 pages | affects: all assistive tech users
- **WCAG 4.1.1**: Duplicate id attribute value "bell" found on the web page. | selector: `#bell` | 10 instances on 10 pages | affects: all assistive tech users above.
...
FACTS (do not change any number, percentage, URL, page count, or name):
- Pages audited: 10 - scope all Priority descriptions to "the audited page" (singular) or "the audited set" (plural); do NOT say "site-wide" or "across the site"
...
- Accessibility: 100/100 (Excellent) - 0 issues - 560 trace to 56 template pattern(s)
- SEO: 94/100 (Excellent)
- Security headers: 5/5 present (HTTPS, HSTS, CSP, X-Frame-Options, X-Content-Type-Options); 0 of 10 audited URLs carry all five
- Heading quality: 88/100
- Pipeline Survivability: 73/100
- Served HTML: 76/100, Rendered HTML: n/a/100
- Performance: 2367ms average load time
...
- Unique canonical pages after normalisation: 13
...
- Pages audited: 10
- Pipeline Survivability score: 73
...
- Pages affected by that check: 10 of 10
...
- Top bare selectors (rendered): `div.text-columns__column` (117), `div.text-columns.text-columns--` (38), `div.textpic.textpic--inside` (18), `div.wrap.wrap--master` (8), `div.input-group.col-xs-12` (6)
- Pages contributing to the Div Soup aggregation: 10
...
FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 94/100; Structured Data: 68/100; Discovery Readiness: 25/100
- Human visitors: served well
- Machine opportunity: structured data, discovery, metadata

DO_NOT:
- Say "site-wide" unless consistency was explicitly confirmed. 10 pages were audited; scope claims to "the audited set".
-->
...
- PNG: 12
- JPEG: 1
- Other / unrecognised formats: 44
- With alt text: 56 (98.2%)
- Missing alt text: 1
- loading="lazy": 57
- loading="eager": 0
