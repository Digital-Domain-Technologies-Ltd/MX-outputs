...
| 2 | template-voice | template-voice-style | Template voice/style findings: 26 violation(s) | 2026-05-27T16:32:31Z |
...
**2. template-voice - Template voice/style findings: 26 violation(s)**
...
We audited five pages of dkd.de against ten dimensions spanning human usability and machine readiness, and the picture for human visitors is genuinely strong. Performance leads the scorecard as the strongest dimension across the audited set, and an SEO score of 92/100 confirms that the technical foundations search engines depend on are firmly in place. Accessibility is equally encouraging: we recorded zero distinct WCAG AA issue types across the audited set, and the 283 raw instances we did surface trace entirely to 57 recurring template patterns in the TYPO3 CMS theme, meaning a single edit per pattern resolves every instance in one pass.

The headline opportunity is in machine readiness. Across the audited set, dkd.de sits at MX Readiness Level 1 (Discoverable), which means machines can discover and parse the content but cannot yet cite it as an attested source. The numeric thresholds for several dimensions are already well-positioned, yet no MX-namespaced governance metadata was detected on the audited pages, and that absence is the specific lever holding the site at Level 1. The path to Level 2 (Citation-ready) runs through adding full MX fields, governance, and provenance metadata so agents can cite as well as discover, alongside raising Machine Suitability above 60 and Discovery Readiness above 40. Catalogue Visibility at 10/100 is the single score most in need of attention and the one that most directly shapes how visible dkd.de is to the catalogues and indices that feed machine-driven discovery.

Because dkd.de runs on TYPO3 CMS, Schema.org JSON-LD is the highest-leverage structured-data asset available: it is read by every agent regardless of how the page is rendered or assembled, making it the natural home for governance and provenance metadata once the MX fields are added.
...
Across the audited set, dkd.de delivers a strong human experience, with Accessibility at 100/100 and SEO at 92/100, both in the Excellent band, and average page load times of 2025ms.
...
Across the audited set, machines can discover and parse dkd.de content, though the combination of a Discovery Readiness score of 25/100 and an MX Readiness Level of 1 (Discoverable) means citation as an attested source is not yet within reach.
...
Across the audited set, dkd.de demonstrates a strong foundations to build from, with strong SEO scores, a clean accessibility record, and all five security headers present. These strengths represent real groundwork for the improvements that follow.
...
We have prioritised the findings below by the degree to which each gap constrains machine access, with discovery and catalogue visibility leading because they block everything downstream before any content-level opportunity can be realised. Structured data depth and MX stack completeness follow as the next tier of readiness across the audited set.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Duplicate ID Attributes, WCAG 4.1.1 (accessibility, account, alarm, article, bell), 5 pages | Compliance Risk | High | Low | Assistive tech users may miss or misread content anchored to duplicated IDs |
| 2 | Semantic Structure 53/100, Worst Page https://dkd.de/de/kontakt/ (26 of 48 bare divs) | Cross-cutting | Medium | Medium | Machines are less likely to parse content hierarchy accurately, reducing agent confidence in page structure |
| 3 | Discovery Readiness 25/100, Five Artefacts Absent (llms.txt, llms-full.txt, agent-card.json, ai.txt, humans.txt) | AI Opportunity | High | Low | Machines may miss dkd.de entirely when building agent indexes and citation pools |
| 4 | Structured Data Quality 66/100, MX Stack Completeness 62/100 (Good) | AI Opportunity | Medium | Medium | Machines risk missing structured signals needed to cite dkd.de as an attested source |

---

**Priority 1: Duplicate ID Attributes, WCAG 4.1.1 (accessibility, account, alarm, article, bell), 5 Pages**

**Bucket:** Compliance Risk

**Finding:** Across the audited set, five ID values ("accessibility", "account", "alarm", "article", "bell") each appear more than once on every audited page. WCAG 4.1.1 requires that all ID values be unique within a document; duplicate IDs break the programmatic contract that assistive technologies rely on to identify, label, and navigate to elements. Because all five IDs recur on all five audited pages, the pattern almost certainly originates in a shared template component, meaning a targeted edit to that component resolves every instance across the audited set in a single pass.

**What to change and why:**

- Ensure the ID values "accessibility", "account", "alarm", "article", and "bell" each appear no more than once per rendered page. This directly satisfies WCAG 4.1.1 and ensures that screen readers and other assistive technologies resolve anchor references, label associations, and ARIA attributes correctly.
- Audit the template component where these IDs are declared. Because 283 issues (93%) in the audited set trace to 57 recurring template-level patterns, fixing the originating template propagates the correction to every page that includes it, compressing remediation effort significantly.
- Where an ID is used purely as a styling hook or repeated icon identifier (e.g. SVG symbol IDs), convert repeated instances to class attributes or unique namespaced IDs. This separates styling concerns from programmatic labelling and prevents the duplication pattern from re-emerging as new pages are added.
- After the template edit, run an automated accessibility check against a representative sample of pages to confirm that no duplicate of these five IDs remains in the rendered DOM.
...
**Finding:** We score Semantic Structure at 53/100, placing it in the medium band across the audited set. The most acute instance we identified is https://dkd.de/de/kontakt/, where a high proportion of structural elements are bare divs rather than semantically meaningful elements. Because this page shares a template with others in the audited set, the band reflects a pattern present beyond this single URL. Bare divs convey no structural meaning to machines or to assistive technologies; a document composed largely of them forces readers, both human and automated, to infer hierarchy from visual styling alone rather than from the markup itself.

**What to change and why:**

- Review the layout regions on https://dkd.de/de/kontakt/ and replace bare container divs with appropriate sectioning elements where a structural role is clear. This directly improves the Semantic Structure score by reducing the bare-div ratio, and it gives machines a clearer content outline that raises Pipeline Survivability (currently 76/100) by making section boundaries unambiguous.
- Where a bare div wraps a list of items, a navigation block, or a standalone piece of content, substitute the element that names the role. Machines use these signals to decide which content regions are primary and which are peripheral; stronger signals improve the fidelity of content extraction.
- Apply the same review to any shared template partials that render the same structural patterns across the audited set. A single partial edit propagates the improvement to every page that includes it, amplifying the score movement per unit of effort.
- Validate the revised markup against the rendered DOM to confirm that visual appearance is unchanged and that the Served HTML score (currently 79/100) is not adversely affected by the restructuring.
...
**Finding:** We score Discovery Readiness at 25/100 (Needs Improvement). None of the five well-known discovery artefacts are reachable on dkd.de: llms.txt, llms-full.txt, agent-card.json, ai.txt, and humans.txt are all absent. These artefacts are the primary mechanism by which machines understand what a domain publishes, who it speaks to, and whether it consents to agent indexing. Without them, dkd.de is navigable only by machines that choose to crawl and interpret individual pages, and it cannot signal intent, scope, or governance to any agent that checks for these files before proceeding.

**What to change and why:**

- Add llms.txt to declare the site's content scope and permissions for machine consumption. We note that our recommendation diverges from the llmstxt.org specification; we recommend serving llms.txt as text/html. This is the file most widely checked by agent pipelines and its presence alone will lift Discovery Readiness materially.
- Add llms-full.txt to provide a richer, expanded content index for agents that request deeper context. Paired with llms.txt, this gives machines a two-tier consumption path aligned with how modern retrieval pipelines operate.
- Add agent-card.json to declare dkd.de's identity, capabilities, and contact surface in a structured, machine-parseable form. This artefact is the foundation of MX Stack Completeness (currently 62/100) and its absence means agents have no verified identity anchor for the domain.
- Add ai.txt and humans.txt to round out the discovery surface. ai.txt communicates crawl and usage permissions to automated agents; humans.txt provides team attribution that supports citation provenance. Together, all five artefacts move Discovery Readiness out of the Needs Improvement band and position dkd.de for agent citation eligibility.

**Effort:** Low

---

**Priority 4: Structured Data Quality 66/100, MX Stack Completeness 62/100 (Good)**

**Bucket:** AI Opportunity

**Finding:** We score Structured Data Quality at 66/100 and MX Stack Completeness at 62/100, both in the Good band. The structured data already present across the audited set (ListItem, Question, Answer, BreadcrumbList, WebSite, FAQPage) gives machines a strong foundations, but the overall MX stack sits short of Citation-ready. Two categories pull MX Stack Completeness down: the discovery artefacts addressed in Priority 3, and the absence of MX governance fields in page frontmatter (specifically canonicalUri, contentType, audience, and status). Without these governance fields, machines reading individual pages lack the metadata to resolve canonical identity, intended audience, and content lifecycle state, all of which bear on whether a machine treats dkd.de as a citable, attested source.

**What to change and why:**

- Add the MX governance fields canonicalUri, contentType, audience, and status to page frontmatter across the audited set. These fields directly address the MX Stack Completeness gap and give machines the contextual signals needed to move from "readable" to "citation-eligible".
- Review structured data coverage to identify content types in the audited set that currently carry no schema annotation. Expanding coverage to additional page types lifts SDQ beyond 66/100 and broadens the surface area machines can interpret with confidence.
- Once Priority 3's discovery artefacts are in place, ensure that the structured data types already present (including BreadcrumbList and FAQPage) are referenced or echoed in llms.txt and llms-full.txt, so agent pipelines can resolve the relationship between the discovery layer and the on-page schema layer.
- Treat MX Stack Completeness improvements as a coordinated effort across discovery artefacts (Priority 3) and governance fields (this priority); partial completion of either category raises the score but only completing both unlocks the full Citation-ready tier.
...
- **sameAs links on Organisation**: adding `sameAs` properties pointing to dkd.de's Wikidata or LinkedIn profile on the `Organisation` entity would allow machines to cross-reference and verify the publisher's identity against external knowledge bases, strengthening citation confidence.

- **BreadcrumbList on deep pages**: where pages currently rely on URL path structure alone, a `BreadcrumbList` entity gives machines an explicit navigational context, improving how agents understand page hierarchy and surface content in structured responses.

- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt: declaring content-use policy via Content-Signal directives would communicate to machines how dkd.de's content may be used, giving the site a clear and parseable stance on AI-agent access and reuse rights.
...
We found the robots.txt file live and well-formed, declaring 21 disallow paths that collectively restrict machine access to a substantial portion of dkd.de's content. One sitemap reference is announced, giving crawlers a clear path to the content that remains open for indexing.
...
We grade the sitemap at A: it covers 9 URLs and carries lastmod, changefreq, and priority attributes on every entry, giving crawlers a complete scheduling and weighting signal.

We found that the sitemap presents both https://dkd.de/de and https://dkd.de/de/ as distinct entries, producing nine crawled URLs against eight unique canonical pages, an inflation factor of 1.1×; machines that skip URL normalisation will fetch the same content more than once, wasting token budget and risking contradictory or inflated findings. We recommend consolidating sitemap.xml to a single canonical form per resource and adding a canonical link tag to each page so that every machine reading dkd.de follows one unambiguous path to each piece of content.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We found no llms.txt file present on dkd.de, meaning machines that query the host for a structured content index receive no site description, no page inventory, and no content policy. We recommend adding llms.txt to give machines a direct, structured entry point into the content available across the audited set.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We find no llms-full.txt on dkd.de, with the endpoint returning a 404 and no reference to it in either the sitemap or the homepage head. Whether adding it would be worthwhile depends on the actual content depth of the full site, which our audited sample of five pages does not yet measure; we recommend treating its introduction as conditional on a broader content inventory.
...
Across the audited set, the resilience checks that most warrant attention are Cross-Host Redirect, which we recorded on every one of the five audited pages, alongside Truncation Risk, Content Negotiation, and Inline Tag Bloat. When machines follow links and encounter cross-host redirects, they may drop context, misattribute content, or abandon the request entirely, reducing the reliability of any automated reading of dkd.de. Resolving the redirect pattern across the audited set would have the largest single effect on pipeline survivability, and addressing Truncation Risk alongside it would further strengthen the signal machines receive.
...
On the worst-performing page in the audited set, https://dkd.de/de/kontakt/, a bare-div ratio of 54% means machines lose structural context and must rely on positional inference to determine meaning. The pattern here is surface-wide rather than deeply chained, with the deepest bare chain reaching only 3 levels, suggesting the soup originates in a component framework that generates typed class names such as `div.text-columns__column` and `div.textpic.textpic--inside` but omits native semantic roles entirely. The most immediate gain comes from wrapping the clear landmarks, header, nav, main, footer, and aside, and where those component wrappers already carry descriptive class names, reviewing whether a semantic element can replace the outer div without touching the visual layout.
...
Accessibility legislation has converged on ISO 14289-1 (PDF/UA) as its shared technical baseline across major markets, with the EAA (Directive (EU) 2019/882, in force 28 June 2025) offering the most precisely codified instance of a requirement that Section 508, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent frameworks in Australia and Canada all reflect in parallel. The machine-readability concern is equally independent: an untagged or image-based PDF is opaque to machines in the same way that unsemantic markup is, and only a properly tagged PDF with a complete structure tree allows search crawlers, AI systems, and automated pipelines to extract text, entities, and structural relationships from the document.
...
Across the audited set, https://dkd.de returns a strong SEO score of 92/100, signalling that human visitors are well served by the foundations already in place. The clearest opportunities lie in Discovery Readiness, which sits at 25/100, and Structured Data at 66/100, both of which limit how effectively machines can find, parse, and act on the content we sampled. We welcome the chance to walk through the detailed findings and agree a prioritised path forward.
...
Across the audited set, we reviewed 37 images in total. The format breakdown shows 8 PNG files and 1 JPEG, with 28 images in other or unrecognised formats. Alt-text coverage is complete: all 37 images carry descriptive alt text, leaving no gaps for screen readers or machines parsing visual content.

Every one of the 37 images we sampled carries a loading="lazy" attribute, and we found no images set to loading="eager" and none left without a loading attribute at all. That last point is worth stating clearly: omitting the attribute is not equivalent to eager loading; it leaves the decision to the browser's own heuristics, which can behave inconsistently across contexts. Across the audited set, that ambiguity is absent, since every image is explicitly instructed to defer until it approaches the viewport.
