...
|---|------|----------|---------|----------|
| 1 | template-voice | template-voice-style | Template voice/style findings: 26 violation(s) | 2026-05-27T11:23:52Z |
...
**Finding:** Across the audited set, every page carries at least one anchor element in the site header that has a valid href attribute but supplies no link content visible to an accessibility API. Because the issue traces to a single recurring template pattern, a single theme edit resolves all seven instances.
...
**Finding:** We score Discovery Readiness at 25/100 (Needs Improvement). While llms.txt is present, the four remaining well-known discovery artefacts, llms-full.txt, agent-card.json, ai.txt, and humans.txt, are absent from dotfusion.com. Machines traversing the host find a partial signal and may reduce confidence in dotfusion.com as a complete or governable source.
...
**Finding:** We score Structured Data Quality at 63/100 (Good). The audited set carries a broad range of schema types already, but the numeric score indicates coverage or depth gaps that reduce how reliably machines can resolve entity relationships when processing dotfusion.com pages.
...
We find no llms-full.txt at dotfusion.com, with the endpoint returning a 404 across every signal we checked: HTTP status, sitemap.xml inclusion, and the homepage head. Whether adding it would deliver meaningful value depends on the content depth of the full site, which our audited sample of 7 pages does not yet measure; we recommend revisiting this artefact once a broader crawl establishes whether the site carries sufficient content volume to justify a full machine-readable digest.
