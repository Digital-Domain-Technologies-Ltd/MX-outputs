...
- Accessibility: 38 distinct WCAG AA issue types (68 raw instances, 68 marked critical by Pa11y) - 44 of these instances trace to 9 recurring template patterns (single theme edit per pattern fixes all instances)
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- Performance: 3114ms avg (Good)
- Accessibility: 73/100 (Good)
- SEO: 86/100 (Excellent)
- Accessibility issues: 38 distinct WCAG codes across 68 raw instances (44 instances trace to 9 template patterns)
-->
...
- Accessibility: 73/100 (38 distinct WCAG codes, 68 raw instances) - 44 of these instances trace to 9 template-level patterns
...
When a Pa11y finding row in  Of these, 40 (59%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 4 (6%) trace to 2 vendor-injected DOM patterns from cdn-ca.aglty.io; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 40 (59%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide. A further 4 (6%) trace to 2 vendor-injected DOM patterns from cdn-ca.aglty.io; these elements are NOT in the site's template and cannot be fixed by a theme edit. Remediation is an SDK upgrade with the vendor, or a DOM-observer patch that adds the missing attribute after injection.
...
- Accessibility: 73/100 (Good) - 68 issues - 44 trace to 9 template pattern(s)
- SEO: 86/100 (Excellent)
- Security headers: 2/5 present (HTTPS, HSTS); 0 of 7 audited URLs carry all five
- Heading quality: 96/100
- Pipeline Survivability: 97/100
- Served HTML: 100/100, Rendered HTML: n/a/100
- Performance: 3114ms average load time
...
- PNG: 27
- JPEG: 0
- Other / unrecognised formats: 19
- With alt text: 62 (71.3%)
- Missing alt text: 25
- loading="lazy": 19
- loading="eager": 0
- no loading attribute: 68
