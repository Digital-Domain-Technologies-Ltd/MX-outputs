...
|---|------|----------|---------|----------|
| 1 | template-voice | template-voice-style | Template voice/style findings: 26 violation(s) | 2026-05-27T19:46:44Z |
...
- Date: 2026-05-27
- Scores: SEO 85/100 | Accessibility 74/100 | Discovery Readiness 20/100 | Structured Data 44/100
...
- Accessibility: 62 distinct WCAG AA issue types (99 raw instances, 99 marked critical by Pa11y) - 65 of these instances trace to 7 recurring template patterns (single theme edit per pattern fixes all instances)
...
- SEO: 85/100 (Excellent)
- Accessibility issues: 62 distinct WCAG codes across 99 raw instances (65 instances trace to 7 template patterns)
-->
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 12
- SEO (all pages): 85/100
- SEO (content pages only): 84/100
- Accessibility: 74/100 (62 distinct WCAG codes, 99 raw instances) - 65 of these instances trace to 7 template-level patterns
...
FACTS:
- Pa11y data for this audit:  Of these, 65 (66%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
When a Pa11y finding row in  Of these, 65 (66%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- Invent Pa11y findings (e.g. keyboard-navigation or focus-order issues) not listed in  Of these, 65 (66%) trace to 7 recurring template-level patterns; a single theme edit per pattern resolves all instances site-wide.
...
- Accessibility: 74/100 (Good) - 99 issues - 65 trace to 7 template pattern(s)
- SEO: 85/100 (Excellent)
- Security headers: 2/5 present (HTTPS, HSTS); 0 of 12 audited URLs carry all five
- Heading quality: 94/100
- Pipeline Survivability: 95/100
- Served HTML: 100/100, Rendered HTML: n/a/100
- Performance: 2457ms average load time
...
1. **Address Priority 1 findings**: address the 99 WCAG 2.1 AA accessibility issues identified (regulatory exposure)
...
WRITE:
2-3 sentences summarising the audit findings for Dotfusion (https://dotfusion.com). Always refer to the site by its domain (https://dotfusion.com), never by the consultancy name. The highest-scoring dimension is SEO at 85/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 85/100; Structured Data: 44/100; Discovery Readiness: 20/100
...
- PNG: 38
- JPEG: 0
- Other / unrecognised formats: 24
- With alt text: 99 (79.2%)
- Missing alt text: 26
- loading="lazy": 24
- loading="eager": 0
- no loading attribute: 101
