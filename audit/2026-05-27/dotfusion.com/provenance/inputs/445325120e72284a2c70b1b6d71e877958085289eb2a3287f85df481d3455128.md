...
We audited seven pages from dotfusion.com's 148-page sitemap and found a platform that serves human visitors well. SEO foundations are solid, scoring 85/100 (Excellent), and the Next.js build delivers an AI Suitability score of 100/100 on served content, which tells us the technical scaffolding is already in strong shape. Content is accessible to search crawlers, metadata is well-formed, and the overall human-facing experience reflects careful, considered work.

Before we turn to machine readiness, we want to name accessibility as a Priority 1 compliance item. Across the audited set we recorded 68 raw instances of WCAG AA issues spanning 38 distinct issue types, with 68 instances marked critical. Encouragingly, 45 of those instances trace to 9 recurring template patterns, meaning a single corrective edit per pattern resolves the majority of instances in one pass; the remediation effort is more concentrated than the raw count suggests. With that noted, the headline opportunity is machine readiness: dotfusion.com currently sits at Level 1 (Discoverable), meaning machines can find and parse the pages but cannot yet cite them as attested sources. The audited pages carry no MX governance markers, and that is the concrete lever. Adding full MX fields, governance, and provenance metadata is what moves the site to Level 2 (Citation-ready); raising MSC above 60 and Discovery Readiness above 40 completes the picture. The groundwork is there, and the path forward is specific.

Discovery Readiness sits at 25/100, the lowest machine-readiness score across the audited set, and it is worth noting that the Schema Maturity level sits at Level 1 (Decoration). Neither of these prevents the move to citation readiness on its own terms, but both represent the chance to deepen the signal machines receive when they arrive at a dotfusion.com page. JSON-LD structured data is the highest-leverage asset in this context because every agent can read it regardless of how the Next.js layer renders the page, and the schema vocabulary already deployed across the audited set gives a solid base to build from.
...
Across the audited set, dotfusion.com delivers strong SEO performance, with accessibility representing the clearest opportunity to bring the human experience fully in line with that standard.
...
Across the audited set, machines can discover and parse dotfusion.com's content reliably, though citation capability as an attested source is not yet unlocked at the current MX Readiness Level 1 (Discoverable).
...
Across the audited set, dotfusion.com demonstrates a strong foundations in search visibility and structured data breadth, with SEO scoring at 85/100 and a rich vocabulary of schema types already deployed across the audited pages. These strengths provide the groundwork for the targeted improvements that follow.

| Dimension | Score | Highlights |
|-----------|-------|------------|
| Performance | Good | Good - 3201ms average load time |
...
We have prioritised the findings below by downstream impact, with discovery and machine-readiness gaps leading because they determine whether machines can reach and interpret dotfusion.com at all before any other signal matters. Schema depth and catalogue visibility follow, reflecting the opportunity to extend the structured data already present across the audited set into richer, more actionable layers.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Unlabelled Header Anchors, WCAG 4.1.2 (7 instances across 7 pages) | Compliance Risk | High | Low | Screen reader users may not understand link purpose and risk missing navigation |
| 2 | reCAPTCHA Textarea Missing Accessible Name and Label, WCAG 4.1.2 and 1.3.1 (7 instances across 7 pages) | Compliance Risk | High | Low | Screen reader users may not perceive or operate the CAPTCHA field |
| 3 | Contact Form Fields Missing Accessible Names, WCAG 4.1.2 (name, company fields, 5 pages) | Compliance Risk | High | Low | Screen reader users may not identify form fields and risk missing submission paths |
| 4 | Div Soup, Semantic Structure 49/100 (https://dotfusion.com/services/contentful-development-agency, 23 of 45 bare divs) | Compliance Risk | Medium | Medium | Screen reader users and machines are less likely to infer content hierarchy correctly |
| 5 | Security Headers Incomplete, 2 of 5 Present | Cross-cutting | Medium | Low | Browsers and security scanners may flag missing protections, reducing trust signals |
| 6 | Discovery Readiness 25/100, Needs Improvement (llms-full.txt, agent-card.json, ai.txt, humans.txt absent) | AI Opportunity | High | Medium | Machines risk missing structured discovery signals and are less likely to cite dotfusion.com as an attested source |
| 7 | MX Stack Completeness 44/100, Could Be Better (discovery artefacts and MX governance fields absent) | AI Opportunity | Medium | Medium | Machines may assign lower confidence to content provenance and audience targeting |
...
**Finding:** Across the audited set, all 7 pages carry an anchor element in the site header that holds a valid href but provides no accessible link content. Screen reader users encounter a link with no announced purpose, a direct violation of WCAG 4.1.2 (Name, Role, Value). Because the selector traces to the global site-header template, a single template-level edit resolves all instances.

**What to change and why:**
- Supply visible text, an aria-label, or an aria-labelledby reference on the header anchor so that its purpose is announced to assistive technology; this directly addresses WCAG 4.1.2 and removes a barrier for screen reader users on every page that shares the template.
- Confirm the fix in a template-wide accessibility test after the edit, because the pattern repeats identically across the audited set; one resolved instance should propagate to all affected pages.

**Effort:** Low

---

**Priority 2: reCAPTCHA Textarea Missing Accessible Name and Label, WCAG 4.1.2 and 1.3.1**

**Bucket:** Compliance Risk

**Finding:** Across the audited set, all 7 pages surface a reCAPTCHA textarea element (selector: `#g-recaptcha-response-100000`) that neither exposes an accessible name to the accessibility API nor carries any form label. This trips two WCAG criteria simultaneously: 4.1.2 (Name, Role, Value) and 1.3.1 (Info and Relationships). The element is injected at runtime by the reCAPTCHA SDK hosted at cdn-ca.aglty.io and does not exist in the site's own template; the fix path is not a theme edit.

**What to change and why:**
- Engage the vendor responsible for the cdn-ca.aglty.io SDK to ship a fix in their next SDK release, as the element is a vendor-injected DOM node and the accessible name must be applied at the point of injection.
- As an interim measure, apply a DOM-observer patch that adds an accessible name to the textarea after the vendor SDK injects it; this restores WCAG 4.1.2 and 1.3.1 conformance without waiting for an upstream SDK release.
- Retest all 7 affected pages after either path is applied, since the element appears consistently across the audited set.

**Effort:** Low

---

**Priority 3: Contact Form Fields Missing Accessible Names, WCAG 4.1.2 (name, company fields)**

**Bucket:** Compliance Risk

**Finding:** On 5 of the 7 audited pages, the contact form fields with selectors `#name` and `#company` each lack an accessible name exposed to the accessibility API, violating WCAG 4.1.2. Screen reader users landing on the contact form receive no announced label for either field and risk bypassing or misidentifying the inputs. Because 41 of the total accessibility issues trace to recurring template-level patterns, and these fields appear consistently across the affected pages, a template-level fix is the appropriate remediation path.

**What to change and why:**
- Associate a visible label element, or supply an aria-label or aria-labelledby attribute, with the `#name` field so that assistive technology announces the field's purpose before or on focus; this resolves the WCAG 4.1.2 violation for that input.
- Apply the same labelling approach to the `#company` field; both inputs share the same shortfall mode and the same resolution path.
- Address both fields in the same template edit pass, since doing so removes the pattern for all 5 affected pages in a single deployment.

**Effort:** Low

---

**Priority 4: Div Soup, Semantic Structure 49/100 (https://dotfusion.com/services/contentful-development-agency, 23 of 45 bare divs)**

**Bucket:** Compliance Risk

**Finding:** Our rendered Semantic Structure score sits at 49/100 across the audited set. The figures for bare-div density are drawn specifically from https://dotfusion.com/services/contentful-development-agency, which represents the worst-performing page in the audited set. Because that page shares its template with other service pages in the audited set, the structural pattern is likely repeated more broadly. A high proportion of layout nodes carry no semantic role, making it harder for screen reader users and machines to infer content relationships and hierarchy.

**What to change and why:**
- Review the template underlying https://dotfusion.com/services/contentful-development-agency and replace purely structural wrapper divs with semantically appropriate elements (section, article, nav, main, aside, header, footer) where the content warrants it; this improves the semantic score, directly supports WCAG 1.3.1 (Info and Relationships), and gives machines a clearer content hierarchy to parse.
- Prioritise regions that wrap primary content blocks, as these are the areas where a semantic element carries the most meaning for both assistive technology and structured parsing agents.
- After updating the template, re-run the semantic structure check across the audited set to confirm the bare-div ratio has improved across all pages sharing that template.
...
**Finding:** Across the audited set, dotfusion.com serves only 2 of the 5 expected security headers (HTTPS and HSTS), and none of the 7 audited URLs carry the full complement. The three absent headers represent protections that browsers consult before rendering page content; their absence reduces the defensive posture of every page in the audited set and may generate warnings in third-party security scanners that clients or procurement teams run independently.

**What to change and why:**
- Add the three absent security headers at the server or CDN layer so that all responses carry the full set; configuring headers at this level means every page benefits from a single change without requiring per-template edits.
- Review each absent header's directive options against the specific content served on dotfusion.com, because overly restrictive values can break embedded third-party assets; testing in a staging environment before deploying to production reduces that risk.

**Effort:** Low

---

**Priority 6: Discovery Readiness 25/100, Needs Improvement (llms-full.txt, agent-card.json, ai.txt, humans.txt absent)**

**Bucket:** AI Opportunity

**Finding:** We score Discovery Readiness at 25/100, placing dotfusion.com in the Needs Improvement band. We confirm that llms.txt is present, but llms-full.txt, agent-card.json, ai.txt, and humans.txt are all absent. Without these artefacts, machines crawling dotfusion.com lack structured signals about content scope, permissions, agent interaction rules, and team attribution. At the current readiness level, agents can discover and parse pages but cannot yet cite dotfusion.com as an attested source.

**What to change and why:**
- Add llms-full.txt to provide machines with a richer content inventory beyond the summary currently available in llms.txt; this directly moves the Discovery Readiness score and expands the range of pages machines can confidently surface in agent answers.
- Add agent-card.json to declare the site's agent interaction policies and capabilities in a structured format; this gives AI agents a machine-readable record of how dotfusion.com expects to be queried and cited.
- Add ai.txt to specify per-agent permissions and crawl preferences; without it, machines apply their own defaults and dotfusion.com loses control over how its content is accessed.
- Add humans.txt to provide team and authorship attribution; this supports the provenance signals that citation-capable agents require before attributing content to a named source.

**Effort:** Medium

---

**Priority 7: MX Stack Completeness 44/100, Could Be Better (discovery artefacts and MX governance fields absent)**

**Bucket:** AI Opportunity

**Finding:** We score MX Stack Completeness at 44/100, placing dotfusion.com in the Could Be Better band. The score reflects gaps across two contributing categories: the discovery artefacts addressed in Priority 6 (llms-full.txt, agent-card.json, ai.txt, humans.txt), and the absence of MX governance fields in page frontmatter, specifically canonicalUri, contentType, audience, and status. Without these frontmatter fields, machines reading dotfusion.com pages receive no explicit signal about canonical identity, content classification, intended audience, or publication status, reducing confidence in content provenance.

**What to change and why:**
- Add the canonicalUri governance field to page frontmatter so that machines can resolve the definitive URL for each piece of content without ambiguity; this is the single highest-leverage frontmatter field for provenance clarity.
- Add contentType and audience fields to frontmatter to give machines a structured classification of what each page is and who it is intended for; agents use these signals to match content to query context more reliably.
- Add the status field to frontmatter to signal whether content is current, in review, or deprecated; machines that encounter an explicit status are less likely to surface outdated pages in agent answers.
- Completing the discovery artefacts in Priority 6 in parallel with these frontmatter additions produces the largest combined lift to the MX Stack Completeness score, as both categories contribute to the 44/100 baseline.
...
- **sameAs links on Organisation**: adding `sameAs` properties pointing to Dotfusion's Wikidata, LinkedIn, or other authoritative directory entries on the existing Organisation entities would allow machines to resolve and corroborate the brand's identity across knowledge graphs, strengthening citation confidence.

- **potentialAction on Organisation**: attaching a `potentialAction` (such as a contact or quote-request action) to the existing Organisation entities would advertise Dotfusion's callable capabilities directly to machines, enabling agents to surface actionable next steps without requiring a user to navigate the site.

- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt: declaring content-use policy via Content-Signal directives would give machines an explicit, machine-readable statement of how Dotfusion's content may be used, a low-effort addition that sits at the host level and requires no changes to page markup.
...
**Slowest.** The slowest page is `https://dotfusion.com/services/answer-engine-optimisation-agency-dotfusion`. A first-time visitor sees the cold-cache cost: the crawler recorded 5296 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three cache-busted re-probes that followed returned 256ms, 105ms, 94ms, giving a returning-visitor median of **105 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://dotfusion.com/services/agility-cms-development-agency`. A first-time visitor sees the cold-cache cost: the crawler recorded 2723 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three cache-busted re-probes that followed returned 135ms, 95ms, 179ms, giving a returning-visitor median of **135 ms**. **Returning-visitor verdict: Healthy**.
...
The robots.txt file is present and declares no disallow paths, meaning machines have unrestricted crawl access across all directories. One sitemap reference is announced, giving crawlers a clear starting point for discovery.
...
The sitemap covers 148 URLs and earns a Partial grade, with lastmod and changefreq values present to guide crawl scheduling, but the absence of priority attributes leaves machines without a signal for relative page importance.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We find llms.txt present and carrying a site description, which gives machines an initial orientation point, though the file currently lacks both a page inventory and a content policy, and adding those two sections would meaningfully extend what machines can learn about the scope and permitted use of the content.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We find no llms-full.txt at dotfusion.com, with the endpoint returning a 404 and no link relation present in the homepage head. Whether adding it would be worthwhile depends on the content depth of the full 148-page set, which the audited sample does not yet measure; we would reassess that recommendation once a broader crawl confirms the volume of substantive, citable content across dotfusion.com.
...
Dotfusion.com is Partially Compatible on the MX Journey, with three of four applicable stages passing and Purchase Confidence not applicable for this site type.
...
Across the audited set, pipeline survivability scores 97/100, and the single resilience check that warrants attention is truncation risk, which we observed on two of the seven pages. When a page's content is cut short before a machine has finished reading it, the machine may form an incomplete picture of the service being described, weakening both indexing and agent comprehension. Addressing truncation on those two pages would round out an already strong result and give machines the full signal they need to represent dotfusion.com accurately.
...
On the worst-performing page we sampled, https://dotfusion.com/services/contentful-development-agency, the rendered surface carries a bare-div ratio of 51%, meaning machines lose structural context and must rely on positional inference to determine meaning. The pattern here is surface-wide rather than deeply nested, a high bare-div ratio alongside a relatively shallow chain depth of 4 points toward an untyped component framework or utility-CSS build pipeline where layout elements are assembled from generic containers rather than semantic ones. The cheapest first move is to wrap the obvious landmarks, header, nav, main, footer, and aside, with their correct HTML counterparts, and to give the remaining generic containers meaningful class names, which would bring the bare-div ratio down without requiring any restructuring of the visual layout.
...
Across the audited set, PDF accessibility sits at a crossroads of two independent concerns: legal and machine readability. Accessibility legislation has converged globally on ISO 14289-1 (PDF/UA) as the shared technical baseline, with the EAA (Directive (EU) 2019/882, in force 28 June 2025) as the most precisely codified instance, alongside Section 508, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent frameworks in Australia and Canada. Separately, an untagged or image-based PDF is invisible to machines in the same way that unsemantic markup is: search crawlers, AI systems, and automated pipelines cannot extract text, entities, or structure from it, whereas a properly tagged PDF with a complete structure tree is as machine-readable as well-formed semantic HTML.
...
Across the audited set, https://dotfusion.com performs well on search fundamentals, with SEO scoring 85/100, a strong foundations for organic visibility. Discovery Readiness at 25/100 and Structured Data at 63/100 represent the clearest opportunities to extend that strength toward machines, giving crawlers, agents, and LLMs the signals they need to find, parse, and act on content. We welcome the chance to walk through the findings and map out a practical path forward.
...
- **`/services/answer-engine-optimisation-agency-dotfusion`**: SEO 76 · A11y 75 · Back 85 · Served 100 · Rendered 100
...
Across the audited set, we reviewed 88 images in total. The format distribution skews toward SVG and PNG, with 38 SVG files and 27 PNG files making up the majority. WebP adoption is limited to 3 images, and 20 images are in other or unrecognised formats. Alt-text coverage stands at 62 images (70.5%), leaving 26 without an alt attribute; for a developer audience, that gap is straightforward to close and carries real weight for both accessibility compliance and machine-readable content signals.

On loading strategy, 19 images carry an explicit `loading="lazy"` attribute, none carry `loading="eager"`, and the remaining 69 have no loading attribute set at all. That last group deserves attention: the absence of a loading attribute is not equivalent to eager loading. Without the attribute, the browser applies its own heuristics to decide when to fetch each image, which can mean above-the-fold images are deferred longer than intended and below-the-fold images are fetched earlier than necessary. Adding explicit loading declarations across the audited set gives the browser clear intent and removes that ambiguity.
