...
Across the audited set, we find that machines can discover and parse content with Discovery Readiness 32/100, Structured Data Quality 52/100, MX Stack Completeness 59/100, and Pipeline Survivability 97/100, yet they lack governance context because MX readiness level is 1 (Discoverable).
...
Across the audited set, your pages demonstrate a strong foundations in SEO and accessibility, with scores of 87/100 and 95/100 respectively. These achievements provide the groundwork for further enhancements.
...
- **sameAs**: link the Organisation entities to external identifiers such as ORCID, Wikidata or LinkedIn, enabling machines to disambiguate and enrich your brand profile.  
- **potentialAction** on Organisation: expose a `Contact` action that lets machines understand how to reach you directly from structured data.  
- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt: declare content-use policy for machines, improving trust and compliance.
...
We found two resilience checks that deserve attention: Inline Tag Bloat, which appears on three of the eleven pages we examined, and SPA Shell, which is present on at least one page. The excess inline code can slow how quickly machines fetch and render content, making it harder for crawlers to understand the page’s structure. Addressing the inline tag bloat-by moving scripts or styles into external files-will give machines a clearer view of the content and provide the biggest improvement.
...
We observed Google Optimise on https://sibotacademy.pl/en/index.html, https://sibotacademy.pl/en/parent/sign-in, https://sibotacademy.pl/en/privacy-policy.html, https://sibotacademy.pl/en, https://sibotacademy.pl/en/forparents.html and six additional URLs.  
No content variance was detected across cache-busted fetches in this audit run.  

Machines arrive cold on every fetch - no cookies or cohort continuity are persisted between visits. Consequently each page may return a different experiment variant. This stateless behaviour means that a bot sweeping the full sitemap for ingestion at training time could ingest contradictory facts from the same URL on successive crawls, embedding inconsistent information into its knowledge graph. The publisher’s canonical content is therefore masked by the experiment layer.

Mitigations: add bypass rules in your personalisation platform to serve canonical content when the User-Agent matches known machine visitors such as ClaudeBot, GPTBot, CCBot, and others; reference experiment URLs in llms.txt with a note that content varies by session so machines are warned not to treat any single fetch as canonical; consider serving a stable, non-personalised version of key pages to bot User-Agents via CDN routing rules.
...
We note that accessibility legislation across major markets converges on ISO 14289-1 (PDF/UA) as the technical baseline, with the EAA Directive (EU) 2019/882 serving as a precisely codified example of this global alignment.  
We also observe that an untagged PDF remains invisible to machines; only a tagged PDF equipped with a proper structure tree is machine-readable in the same way semantic HTML is.
...
Our audit shows that https://sibotacademy.pl excels in SEO, scoring 87/100 and giving machines a clear signal of relevance.  
Key opportunities lie in Discovery Readiness (32/100) and Structured Data (52/100), where improving metadata will enhance machine comprehension.  
We recommend prioritising these enhancements to unlock greater visibility and richer search experiences for your audience.
