...
Across the audited set, we found that your pages deliver a strong human experience. Performance is excellent, keeping visitors engaged with rapid load times and smooth navigation. SEO foundations are solid, reflected in an 87/100 score, and the clear branding helps users locate content quickly.

The headline opportunity we want to draw attention to is machine comprehension. First, accessibility is a Priority 1 compliance item; we identified one distinct WCAG AA issue type that appears in 35 elements, all flagged critical by Pa11y. Second, the site sits at
...
Across the audited set, we find that machines can discover and parse your content but lack governance context, as reflected by a Discovery Readiness score of 32/100, Structured Data Quality of 52/100, MX Stack Completeness of 59/100, and Pipeline Survivability of 97/100.
...
Across the audited set, we see a strong foundations of strong SEO performance and accessibility compliance that sets the groundwork for further enhancements. These strengths give your content a robust platform from which to build even greater machine-readability and user experience.
...
Across the audited set, two resilience checks were flagged: Inline Tag Bloat on three pages and a SPA Shell issue on one page. The inline tag bloat forces machines to parse more HTML than necessary, which can slow rendering or lead to incomplete content extraction. Addressing the inline tag bloat-by moving scripts to external files or trimming inline code-offers the biggest opportunity to strengthen resilience and make the site easier for machines to read.
...
On the worst-page /en/worlds, 78 % of rendered elements are bare divs; machines lose structural context and fall back on positional inference to determine meaning.  
The deep chain of eight bare divs indicates a structural soup produced by late-stage JavaScript injection or an untyped component framework, rather than a surface-wide soup from drag-and-drop builders.  
A cost-effective first step is to wrap the obvious landmarks-header, nav, main, footer, aside-and assign meaningful class names to the remaining elements so that the bare-div ratio drops without restructuring the layout.
...
The A/B testing layer is powered by Google Optimise and is active on https://sibotacademy.pl/en/index.html, https://sibotacademy.pl/en/parent/sign-in, https://sibotacademy.pl/en/privacy-policy.html, https://sibotacademy.pl/en, https://sibotacademy.pl/en/forparents.html and six additional pages.  
Because machines arrive cold on every fetch-no cookies, no cohort data, no session continuity-each request can be served a different experiment variant. This stateless behaviour means that a crawler or knowledge-graph builder may record contradictory facts from successive visits to the same URL.

The consequence for training-data pipelines is significant: a sitemap sweep performed by a bot will ingest varying content on each crawl, embedding inconsistent information into downstream models and potentially distorting the machine’s understanding of what your business actually says. The canonical content that represents your true messaging is therefore obscured behind an experiment layer.

Mitigations we recommend are: add bypass rules in your personalisation platform to serve canonical content when the User-Agent matches known machine visitors such as ClaudeBot, GPTBot, CCBot, and others; reference the experiment URLs in llms.txt with a note that content varies by session so machines are warned not to treat any single fetch as canonical; and consider serving a stable, non-personalised version of key pages to bot User-Agents via CDN routing rules.
...
We recognise that legal frameworks worldwide now converge on ISO 14289-1 (PDF/UA) as the technical baseline for accessible documents; the European Accessibility Act exemplifies this convergence, but it represents just one of several global mandates. From a machine-readability perspective, we note that an untagged PDF is invisible to search crawlers and AI systems, whereas a tagged PDF with a proper structure tree exposes its content in the same way semantic HTML does; because no PDFs were identified across the audited set, this risk remains theoretical for the sampled pages.
...
Our audit shows that https://sibotacademy.pl excels in SEO, scoring 87/100 and giving machines a clear signal of relevance. However, discovery readiness at 32/100 and structured data quality at 52/100 reveal significant opportunities for improving machine discoverability and content understanding. We recommend prioritising enhancements to these areas so that your pages are more readily indexed and interpreted by search crawlers and other automated agents.
