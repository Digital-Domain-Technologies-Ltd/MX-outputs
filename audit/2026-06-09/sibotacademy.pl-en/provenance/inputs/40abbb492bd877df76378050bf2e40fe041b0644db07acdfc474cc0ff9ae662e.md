...
|-----------|---------|------------|
| `<loc>` URLs | 22 entries | 15 audited pages missing from the sitemap |
...
This was a full crawl: the audit reached every page it could discover, and 15 of them are absent from the sitemap (which lists 22). The full set is recorded in the `sibotacademy-pl-en-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.
