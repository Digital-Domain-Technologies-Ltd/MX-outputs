...
reportId: "sibotacademy-pl-en-WEB-AUDIT-20260609"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-09/sibotacademy.pl-en/sibotacademy-pl-en-report.md
  runbook: "Executive audit report for Sibotacademy. Focus on the highest-leverage MX opportunities surfaced by the audit. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-09/sibotacademy.pl-en/sibotacademy-pl-en-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-09/sibotacademy.pl-en/sibotacademy-pl-en-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-09/sibotacademy.pl-en/sibotacademy-pl-en-report.pdf"
...
**Date:** 9 June 2026\
**Report ID:** sibotacademy-pl-en-WEB-AUDIT-20260609
...
Across the audited set, performance leads the scorecard as the strongest human-experience dimension - pages load quickly and feel responsive, giving visitors a smooth learning journey. The site’s clear layout, concise course descriptions, and consistent branding reinforce that experience, while SEO foundations are solid, helping users find the content they need.

Accessibility presents a priority 1 compliance opportunity: addressing the single WCAG AA issue type will resolve 35 critical instances across the audited pages. The headline opportunity we want to draw attention to is adding full MX governance metadata so machines have the structured context they need for accurate comprehension. By embedding mx:status, mx:contentType, mx:audience, canonicalUri and provenance markers, and by raising MSC above 60 and DR above 40, the site can move from Discoverable (Level 1) to Governed (Level 2), unlocking richer machine-readiness for future AI and search applications.
...
We present the Human Experience subtable, summarising key metrics that affect visitors’ interaction with the audited pages. Across the audited set, we find the site delivers an excellent experience for human visitors, with performance at 896 ms average, accessibility scoring 95/100, and SEO at 87/100.
...
Across the audited set, machines can discover and parse the content but currently lack structured governance context, as reflected in Discovery Readiness 32/100, Structured Data Quality 52/100, MX Stack Completeness 59/100, Pipeline Survivability 97/100, and an MX Readiness Level of 1 (Discoverable).
...
We assess that this page is aimed at a child audience (inferred from the page content; no audience signal was declared). We describe who the page appears to be for; we do not judge the content, and we do not block anything.
...
We inferred this audience rather than reading a declared signal, so we recommend making it machine-readable: declare the intended audience in your structured data, a schema.org Audience or typicalAgeRange entry in your JSON-LD, and in MX metadata, mx:intendedAgeRange, so an agent reads who your page is for instead of inferring it. The age-awareness signals an agent looks for were not observed either: where you operate an age-assurance step or a consent mechanism, make it machine-detectable, and declare the age-appropriateness of the content for the bands above. We carry these forward as Machine Readability findings; they make the signals legible to machines, and they are not compliance requirements.

<div class="page-break"></div>

## What's Working Well

Across the audited set, your pages show a strong foundations in SEO (87/100) and accessibility (95/100), with a respectable Structured Data Quality score of 52/100 and two of five key security headers already in place.
...
- **Content-Signal directives**: Adding Content-Signal directives in `robots.txt` clarifies the content-use policy for machines.
...
We found that a few of the audited pages contain excessive inline script and style tags - an Inline Tag Bloat issue that can slow or block machines from quickly understanding the page content. A smaller number of pages also use a SPA Shell, which may delay initial rendering for bots. Removing or moving those inline resources would give the biggest boost to machine readability and overall resilience.
...
We find that the site uses semantic HTML effectively, giving machines reliable structural cues.
...
We observed No content variance was detected across cache-busted fetches in this audit run. Google Optimise was identified on six key pages: https://sibotacademy.pl/en/index.html, https://sibotacademy.pl/en/parent/sign-in, https://sibotacademy.pl/en/privacy-policy.html, https://sibotacademy.pl/en, https://sibotacademy.pl/en/forparents.html and six additional URLs. Because machines arrive cold on every fetch-no cookies or session continuity-each request may be served a different experiment variant. This stateless behaviour means that crawlers building knowledge graphs or training corpora can ingest contradictory facts from the same URL during separate sweeps, obscuring your canonical content for automated agents. To mitigate, we recommend adding bypass rules in your personalisation platform to serve canonical content when the User-Agent matches known machine visitors such as ClaudeBot, GPTBot, CCBot, etc.; reference experiment URLs in llms.txt with a note that content varies by session; and consider routing key pages to a stable, non-personalised version for bot agents via CDN rules.

---

## Content Consistency

We observed consistent metadata patterns across the audited set, with no brand-name or canonical-URL divergence flagged by the consistency check.
...
2. **Review Priority 2-3 findings**: declaring the intended child audience in machine-readable metadata (the audit had to infer it) and making any age-assurance and consent steps machine-detectable, alongside Semantic Structure improvements and metadata tuning that compound over time
...
We found that https://sibotacademy.pl excels in SEO, scoring 87/100 across the audited set, indicating strong search visibility and optimisation. However, structured data (52/100) and discovery readiness (32/100) present significant opportunities for improving machine comprehension and discoverability. We recommend prioritising enhancements to these areas to unlock greater reach and engagement.
