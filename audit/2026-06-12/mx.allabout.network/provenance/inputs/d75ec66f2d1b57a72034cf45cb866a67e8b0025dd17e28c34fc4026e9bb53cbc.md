...
- Pages audited: 7 of 138 in sitemap - sample run
- Date: 2026-06-12
- Scores: SEO 93/100 | Accessibility 100/100 | Discovery Readiness 94/100 | Structured Data 95/100
- Scope: 7 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): ListItem (16), WebPage (8), BreadcrumbList (7), Organization (6), Offer (4), Service (4), Person (3), WebSite (2), ContactPoint (2), AboutPage (1), CommunicateAction (1), EntryPoint (1), PropertyValue (1), ContactPage (1), ImageObject (1), PriceSpecification (1)
...
- Discovery Readiness: 94/100
- Commerce Visibility: 20/100
- MX Readiness Level: 3 (Comparable / Attested)
- MX governance markers present on audited pages: yes
- Next-level action: Register in a discovery registry with SLA and aliveness signals so agents understand pricing and can transact.
- Schema Maturity: Level 4 (Verified linked data)
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 7
- Performance: 803ms avg (Excellent)
...
FACTS (do not change any number, percentage, URL, page count, or name):
- Schema.org types detected: 16 type(s) present
...
|--------------|-------|------------|---------------|------------------------------------------|
| ListItem | 7 | 100% | 100% | - |
| WebPage | 7 | 100% | 100% | Website, WebSite, Organization, Person |
| BreadcrumbList | 7 | 100% | 100% | - |
| Organization | 4 | 100% | 100% | Person, ContactPoint, Reference, Website |
| Offer | 2 | 100% | 100% | Organization, Service, PriceSpecification |
| Service | 2 | 100% | 100% | Organization, Service, Website, Reference |
| Person | 3 | 100% | 100% | Organization, PropertyValue |
...
- Checks flagged on any page: none
- Check with the most pages flagged: none
- Pages affected by that check: 0 of 7
...
- Score (both surfaces, identical): 100, band low (worst page: [the home page](https://mx.allabout.network/))
- Bare divs (both surfaces, identical): 0 of 25 (0%)
- Deepest bare chain (both surfaces, identical): 0
- Top bare selectors (served): -
- Top bare selectors (rendered): -
...
- Dynamic Yield, Unbounce Smart Traffic (personalisation) was detected on pages in the audited set
...
- Open by naming what was found and what was not: "Dynamic Yield, Unbounce Smart Traffic (personalisation) is present on pages in the audited set, but every visit returned the same content during this audit."
...
FACTS:
- PDF count: 1
- Section to render: determined by infill (1 == 0 → PDF_EAA_EMPTY; > 0 → PDF_EAA_SAMPLE)
...
| Full Implementation | Commerce Visibility, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
WRITE:
2-3 sentences summarising the audit findings for Mx Allabout (https://mx.allabout.network). Always refer to the site by its domain (https://mx.allabout.network), never by the consultancy name. The highest-scoring dimension is AI Suitability at 100/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: 100/100; Accessibility: 100/100; SEO: 93/100; Structured Data: 95/100; Discovery Readiness: 94/100
