...
SCOPE: 150 pages were audited. Scope all claims to "the audited set" - do not say "site-wide" unless cross-page consistency is explicitly confirmed in the facts.
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 150
- Performance: 374ms avg (Excellent)
...
- Security headers: 5/5 present (HTTPS, HSTS, CSP, X-Frame-Options, X-Content-Type-Options); 160 of 161 audited URLs carry all five
- Structured Data Quality: 91/100
- Consistency: 98%
-->
...
FACTS (do not change any number, percentage, URL, page count, or name):
- Schema.org types detected: 47 type(s) present
...
| WebPage | 121 | 100% | 100% | Website, WebSite, Organization, Person |
| BreadcrumbList | 98 | 100% | 100% | - |
| BlogPosting | 86 | 100% | 100% | Person, Organization, Reference, Website |
| Offer | 17 | 100% | 100% | Organization, Service, PriceSpecification, Country |
| Organization | 66 | 100% | 100% | Person, ContactPoint, Reference, Website |
| Person | 64 | 100% | 100% | Organization, PropertyValue |
...
WRITE:
1-2 short sentences stating what this means for https://mx.allabout.network specifically. Take the edge-capture state from the FACTS below; it is derived from the data, do not infer it. When it is yes, say the site has an active edge-capture pattern and AI visits ARE being separated from organic direct traffic; when no, say it has none yet. Never state the site has no edge-capture pattern when the Live capture table below shows AI-attributable visits. Use plain language: use "the browser does not tell your server which AI sent them" or similar.

FACTS:
- Site: https://mx.allabout.network
- Edge capture pattern active: yes
...
- Pages affected by that check: 1 of 150
...
- Score (both surfaces, identical): 43, band high (worst page: [/reginald/mx-machine-readiness.html](https://mx.allabout.network/reginald/mx-machine-readiness.html))
- Bare divs (both surfaces, identical): 75 of 121 (62%)
- Deepest bare chain (both surfaces, identical): 4
- Top bare selectors (served): `div` (17), `div.icon` (6), `div.k` (4), `div.v` (4), `div.role` (4)
- Top bare selectors (rendered): `div` (17), `div.icon` (6), `div.k` (4), `div.v` (4), `div.role` (4)
- Pages contributing to the Div Soup aggregation: 161
...
FACTS:
- Scores: AI Suitability: 99/100; Accessibility: 100/100; SEO: 91/100; Structured Data: 91/100; Discovery Readiness: 84/100
- Human visitors: served well
- Machine opportunity: structured data, discovery, metadata

DO_NOT:
- Say "site-wide" unless consistency was explicitly confirmed. 150 pages were audited; scope claims to "the audited set".
-->
