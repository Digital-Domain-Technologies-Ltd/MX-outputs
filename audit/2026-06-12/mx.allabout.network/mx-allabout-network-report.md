...
reportId: "mx-allabout-network-WEB-AUDIT-20260612"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-12"
auditCommand: "node scripts/audit-pipeline.js https://mx.allabout.network --pages -1 --date 2026-06-12"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-12/mx.allabout.network/mx-allabout-network-report.md
  ...
  runbook: "Executive audit report for Mx Allabout. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/mx.allabout.network/mx-allabout-network-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/mx.allabout.network/mx-allabout-network-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-12/mx.allabout.network/mx-allabout-network-report.pdf"
...
**Date:** 12 June 2026\
**Report ID:** mx-allabout-network-WEB-AUDIT-20260612
...
**About sample scope.** Findings throughout this report describe what we observed on the 150 pages we crawled. Verdicts scoped to the sample should not be extrapolated to the full estate without a wider audit; where a finding is structural (a missing security header, a soft 404 pattern, an llms.txt transport problem) we say so.
...
Across the audited set of 150 pages, we see a user experience built on strong foundations. The design delivers fast, responsive interactions and clear content hierarchy, making it easy for visitors to find what they need. Performance is excellent, with page loads consistently under the target thresholds, giving users a smooth journey from first click to final interaction.

The headline opportunity we want to draw attention to is strengthening discovery readiness and governance metadata so that machines have the context they need for accurate attribution. While AI Suitability sits at 99/100-meaning machines can parse the served HTML-the lowest machine-readiness score of 84/100 highlights opportunities in canonical URIs, status markers, audience tags, and provenance data. Enhancing these MX fields will lift the site into a higher level of machine comprehension and open the door for agents to reliably surface your content at purchase points.

\clearpage

<!-- AUDIT-DELTA:START -->
## Change Since Our 7 June 2026 Audit

We last audited mx.allabout.network on 7 June 2026. The table compares that audit with the current one across the headline measures. Some scores improved, the rest held steady, and none declined; the table shows each change.

| Measure | 7 June 2026 | 12 June 2026 | Change |
|---------|------:|------:|:-------|
| Performance | 97 | 97 | No change |
| Accessibility | 100 | 100 | No change |
| SEO | 90 | 91 | +1 (improved) |
| Machine Suitability | 98 | 99 | +1 (improved) |
| WCAG AA issues | 0 | 0 | No change |
| Pages audited | 146 | 150 | +4 |
...
Across the audited set, the pages deliver an excellent experience for human visitors, with fast load times, consistent accessibility and strong SEO.
...
Across the audited set, machines can discover and parse content with a Discovery Readiness of 84/100, yet until Level 2 they lack governance context; at Level 2 our MX Stack Completeness of 94/100 and Structured Data Quality of 91/100 give them full governance for accurate comprehension, while Pipeline Survivability stays at 100/100 and across the audited set the MX Readiness Level is 4 (Transactable).
...
| Structured Data Quality | 91/100 | Excellent | A | B (median) |
| MX Stack Completeness | 94/100 | Excellent | A | B (median) |
...
**Evidence:** MX Stack Completeness 94/100 | Structured Data Quality 91/100 | Discovery Readiness 84/100 | Consistency 98%
...
We found that across the 150 pages we audited, your site demonstrates a strong foundations in SEO (91/100), accessibility (100/100), security headers (5/5 present on 160 of 161 URLs), structured data quality (91/100) and consistency (98%). These strengths provide the groundwork for the enhancements outlined below.
...
|-----------|-------|------------|
| Performance | Excellent | Excellent - 374ms average load time |
| SEO (content pages) | 91 | Excellent - titles, meta descriptions, canonical URLs in place |
| Security | 5/5 | 5/5 headers present; 160 of 161 URLs carry all five |
| Structured Data | 91 | Excellent - JSON-LD on every page with valid Schema.org vocabulary |
...
- Accessibility is compliant across the audited set: Pa11y reports 100/100 with zero WCAG 2.1 AA errors on 150 pages.
...
- Structured Data Quality of 91/100 (Good schema): the schema is valid, required properties are complete, and the vocabulary is in good order.
...
**Finding:** 84 of 501 images (17%) on the audited set carry no alt text, so their content is unavailable to assistive technology and to machines reading the page.
...
- **sameAs**: Adding ORCID, LinkedIn or Wikidata URLs to Person and Organisation entities gives machines richer identity context, enabling more accurate attribution across the audited set.  
- **SpeakableSpecification**: Defining CSS selectors for voice surfaces on Article, BlogPosting and TechArticle pages lets conversational agents surface content directly in spoken interfaces throughout the audited set.  
- **potentialAction**: Attaching a ContactAction to Organisation entities exposes contact capabilities to machines, allowing them to suggest direct outreach options across the audited set.
...
Across the 149 pages we checked, 6 distinct issue patterns reduce what reaches the tree.
...
The page declares no main landmark. Landmark navigation is how tree consumers skip repeated chrome and land on the content; without it, every visit starts from the top of everything. Seen on `/books/download-intro`.
...
**Slowest.** The slowest page is `https://mx.allabout.network/blog/your-site-is-already-training-ai.html`. A first-time visitor sees the cold-cache cost: the crawler recorded 1057 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 469ms, 192ms, 408ms, giving a returning-visitor median of **408 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://mx.allabout.network/llms.txt`. A first-time visitor sees the cold-cache cost: the crawler recorded 330 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 443ms, 314ms, 311ms, giving a returning-visitor median of **314 ms**. **Returning-visitor verdict: Healthy**.
...
The sitemap declares 139 URLs and grades Complete. Lastmod dates vary across entries, which tells machines which pages changed and when.

This was a full crawl: the audit reached every page it could discover, and 27 of them are absent from the sitemap (which lists 139). The full set is recorded in the `mx-allabout-network-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.
...
| WebPage | 121 | 100% | 100% | Website, WebSite, Organisation, Person |
| BreadcrumbList | 98 | 100% | 100% | - |
| BlogPosting | 86 | 100% | 100% | Person, Organisation, Reference, Website |
| Offer | 17 | 100% | 100% | Organisation, Service, PriceSpecification, Country |
| Organisation | 66 | 100% | 100% | Person, ContactPoint, Reference, Website |
| Person | 64 | 100% | 100% | Organisation, PropertyValue |
| ImageObject | 41 | 100% | 100% | - |
| SpeakableSpecification | 32 | 100% | 100% | - |
| Article | 30 | 100% | 100% | ImageObject, Person, Organisation, Reference |
| PriceSpecification | 11 | 100% | 100% | - |
| Book | 6 | 100% | 100% | Person, Organisation, Offer, Reference |
| Service | 10 | 100% | 100% | Organisation, Service, Website, Reference |
...
| WebSite | 3 | 100% | 100% | Organisation, Person, Website, Reference |
...
We have an active edge-capture pattern on https://mx.allabout.network, meaning AI visits are being separated from organic direct traffic. The browser does not inform your server which AI generated those requests.
...
| Metric | Count |
|--------|-------|
| AI crawler hits (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, …) | 1906 |
| AI referral hits (human visits from chat surfaces) | 0 |
| Total AI-attributable visits | 1906 |
...
|-------|-----------|------|
| amazonbot | crawler | 617 |
| meta-ai | crawler | 360 |
| bytespider | crawler | 270 |
| claude | crawler | 243 |
| chatgpt | crawler | 201 |
| ccbot | crawler | 159 |
| perplexity | crawler | 39 |
| gemini | crawler | 16 |
| you | crawler | 1 |
...
- **Boilerplate Burial** - Pass · 150/150
...
- **Tabbed Disclosure** - Pass · 150/150
...
- **Broken Code Fences** - Pass · 150/150
...
- **HTTP Content Negotiation (Vary)** - Pass · 150/150
...
- **Cross-Host Redirect** - Pass · 150/150
...
We found one page with Inline Tag Bloat, a resilience check that can cause machines to misinterpret the structure or waste resources when parsing the content. Addressing this by moving inline styles and scripts to external files will give machines a cleaner view of the page and improve reliability. This small fix is an opportunity to strengthen overall resilience across the audited set.
...
On the worst-page, /reginald/mx-machine-readiness.html, we recorded 75 of 121 elements as bare divs (62 %), meaning that machines lose reliable structural context and must rely on positional inference to interpret content.  
The soup is surface-wide, with a shallow deepest chain of four, signalling that the source pipeline likely relies on drag-and-drop builders or untyped component frameworks that generate many generic divs.  
A low-cost first step is to wrap the primary landmarks-header, nav, main, footer, aside-and assign meaningful class names to the remaining elements so the bare-div ratio falls without altering layout.
...
| HSTS | Yes (160/161) | Forces HTTPS, prevents downgrade attacks |
| Content-Security-Policy | Yes (160/161) | Prevents XSS and injection attacks |
| X-Frame-Options | Yes (160/161) | Prevents clickjacking |
| X-Content-Type-Options | Yes (160/161) | Prevents MIME-type sniffing |

Header coverage is uneven across audited responses: HSTS (Strict-Transport-Security) on 160/161 audited responses, Content-Security-Policy (CSP) on 160/161 audited responses, X-Frame-Options on 160/161 audited responses, X-Content-Type-Options on 160/161 audited responses. The header set is configured per route or per virtual host rather than uniformly at the origin or CDN edge; a single server-config change brings the missing responses in line with the strongest baseline already in place.

**Coverage:** 160 of 161 audited URLs carry all five headers (`/about` is the exception).
...
- **`/canon/fields-data.yaml`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
- **`/canon/fields-data-carriers.yaml`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
- **`/canon/cognovamx-fields.yaml`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
- **`/blog/governance/`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
- **`/blog/profiles/`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
- **`/blog/use-cases/`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
...
HTTPS: 161/161 | HSTS: 160/161 | CSP: 160/161 | X-Frame-Options: 160/161 | X-Content-Type-Options: 160/161
...
| Canonical URL | 98% | 2 |
| Exactly 1 H1 | 99% | 2 |
| Code examples present | 17% | 125 |
| Self-contained sections | 100% | - |
| Error/troubleshooting docs | 5% | 142 |
| Lighthouse heading compliance | 98% | 3 |

**Overall Consistency:** 98%

Some pages in the 150-page sample are missing metadata patterns that others carry. Machines hitting different pages get different quality data. The Missing Pages column shows where to focus on the sampled pages.
...
None of the declared `expires` dates is in the past; the declared content is current as of the audit run on 2026-06-12.
...
Content consistency examines how metadata, titles, and structured data align across pages. We evaluated these elements across all 150 audited pages to ensure that machines interpret each page with a unified context. This analysis helps prevent duplicate content signals and supports accurate search engine understanding.
...
| Check                            | Result | Notes                    |
|----------------------------------|--------|--------------------------|
| Brand-name parity | Pass | Brand name appears consistently across all 150 audited pages |
| Canonical URL duplicates | Pass | No duplicate canonical URLs detected across the 150-page audited set |
...
Legal and technical convergence around ISO 14289-1 (PDF/UA) underpins accessibility law worldwide; the EAA Directive (EU 2019/882), Section 508, UK Public Sector Bodies Accessibility Regulations 2018, and comparable Australian and Canadian statutes all codify this standard, with the EAA being the most precisely defined example. From a machine-readability perspective, an untagged PDF remains invisible to search crawlers, machines, and automated pipelines; only a tagged PDF that includes a proper structure tree can be parsed by machines in the same way semantic HTML is.
...
Accessibility at 100/100 is a clear strength, ensuring all users can navigate the content on https://mx.allabout.network.  
Opportunities exist in Discovery Readiness (84/100) and Structured Data (91/100), where improving metadata visibility and schema implementation will enhance machine comprehension and search discoverability.  
We invite you to act on these opportunities to further strengthen your digital presence.
...
| Structured Data Quality | 91/100 | Excellent |
...
*Showing the first 25 of 150 audited pages; the remaining 125 are in `mx-allabout-network-pages-audited.csv` next to this report.*
...
Per page, internal links range from 1 to 76, averaging 8 across 330 pages. That is sparser than typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 2735 |
...
We reviewed 501 images across the audited set: 496 WebP, 4 SVG and 1 JPEG. 417 of 501 (83.2%) carry alt text, leaving 84 without it. Each missing alt attribute is a place where a screen-reader user or a machine reading the page gets no description of what the image shows.

On loading strategy, 341 images are marked `loading="lazy"` and 159 `loading="eager"`, while 1 carry no loading attribute at all. No attribute is not the same as eager: the browser decides for itself when to fetch, which removes the explicit control that lazy and eager give you. Setting an explicit attribute on those images makes the fetch behaviour predictable for browsers and machines alike.
...
**Platform detection:** We fingerprint the hosting platform from HTTP response headers, HTML signatures, asset paths, and class patterns. Platform identification is probabilistic -- a site can obscure or mimic platform signals. We report the result as: Not determined (the fingerprint probe could not fetch the origin on this run). The platform fingerprint probe could not fetch the origin on this run, so the platform was not determined; the audit used conservative default rate limits, paced slowly enough to stay below typical shared-host thresholds, with exponential backoff and retry (up to 4 attempts) on rate-limit responses.
