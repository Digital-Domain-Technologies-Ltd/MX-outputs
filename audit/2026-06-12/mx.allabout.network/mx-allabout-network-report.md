...
auditCommand: "node scripts/audit-pipeline.js https://mx.allabout.network --pages 5"
...
| MX Stack Completeness | **97**/100 | `#################-` |
| Agent Readability | **99**/100 | `##################` |
...
Across the audited set we see a site that delivers a smooth and engaging experience for visitors. The performance foundation is solid, with page loads consistently swift, allowing users to access content quickly and enjoy interactive elements without delay. Design choices such as clear navigation, responsive imagery, and concise copy reinforce your brand’s credibility and keep readers focused on the information they seek.

The headline opportunity we want to draw attention to is commerce visibility. Across the audited set this dimension sits at 20/100, indicating that machines can parse the HTML but lack the signals needed for agents to discover pricing or transact. By enriching your pages with MX governance markers-status tags, audience identifiers and provenance metadata-and registering in a discovery registry with SLA and aliveness signals, you give agents the context they need to understand and act on your offerings.

We already have AI Suitability at 100/100, meaning machines can read the served HTML, and Structured Data Quality at 95/100 with Schema Maturity Level 4 (Verified linked data). Building on this strong foundation by improving commerce visibility will elevate machine comprehension and open new avenues for automated agents to engage with your content.
...
We last audited mx.allabout.network on 7 June 2026. The table compares that audit with the current one across the headline measures. Some scores improved and others slipped since that audit; the table shows each change.

| Measure | 7 June 2026 | 12 June 2026 | Change |
|---------|------:|------:|:-------|
| Performance | 97 | 85 | -12 (declined) |
| Accessibility | 100 | 100 | No change |
| SEO | 90 | 93 | +3 (improved) |
| Machine Suitability | 98 | 100 | +2 (improved) |
| WCAG AA issues | 0 | 0 | No change |
| Pages audited | 146 | 7 | -139 |
...
We find that across the audited set, the pages deliver a strong experience for human visitors, with excellent performance, accessibility and SEO.
...
Across the audited set, machines can discover and parse content with a readiness score of 94/100 yet still lack full governance context, while structured data quality (95/100), MX stack completeness (97/100) and pipeline survivability (100/100) collectively give them a robust foundation for accurate comprehension at Level 3 (Comparable / Attested).
...
|-----------|-------|--------|-------|----------|
| Discovery Readiness | 94/100 | Excellent | A | C (median) |
| Structured Data Quality | 95/100 | Excellent | A | B (median) |
| MX Stack Completeness | 97/100 | Excellent | A | B (median) |
...
**Current Level:** 3: Comparable / Attested

**Evidence:** MX Stack Completeness 97/100 | Structured Data Quality 95/100 | Discovery Readiness 94/100 | Consistency 93%

**To reach the next level:** Register in a discovery registry with SLA and aliveness signals so agents understand pricing and can transact.
...
Across the audited set, we find that your seven pages demonstrate a strong foundations in SEO (93/100), accessibility (100/100), security headers (5/5 present on six URLs), and structured data quality (95/100). These strengths provide the groundwork for the targeted improvements outlined below.
...
|-----------|-------|------------|
| Performance | Excellent | Excellent - 803ms average load time |
| SEO (content pages) | 94 | Excellent - titles, meta descriptions, canonical URLs in place |
| Security | 5/5 | 5/5 headers present; 6 of 7 URLs carry all five |
| Structured Data | 95 | Excellent - JSON-LD on every page with valid Schema.org vocabulary |
| Heading Quality | 97 | Excellent - single H1 per page, no level jumps, Lighthouse-compliant |
| Consistency | 93% | 93% - same metadata patterns across every page |
| Agent access | 8/8 | every tested agent receives HTTP 200 |

**Positive patterns observed:**

- Accessibility is compliant across the audited set: Pa11y reports 100/100 with zero WCAG 2.1 AA errors on 7 pages.
...
- Structured Data Quality of 95/100 (Verified linked data): the schema is valid, required properties are complete, and the vocabulary is in good order.
- JSON-LD is present in the served HTML of every page: every agent that fetches the raw HTML gets the structured data.
- Body content ratio averages 54%: pages are content-heavy, not overhead-heavy, which helps agents reach real prose inside a cheap fetch.
...
**Finding:** 1 of 19 images (5%) on the audited set carry no alt text, so their content is unavailable to assistive technology and to machines reading the page.
...
- **sameAs links**: linking the Person and Organisation entities to ORCID, Wikidata or LinkedIn gives machines richer identity context and improves cross-source attribution.  
- **PotentialAction on Organisation**: adding a contact `potentialAction` exposes machine-readable ways for agents to reach your organisation, enhancing discoverability of communication channels.  
...
**Slowest.** The slowest page is `https://mx.allabout.network/about/contact.html`. A first-time visitor sees the cold-cache cost: the crawler recorded 1051 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 182ms, 202ms, 364ms, giving a returning-visitor median of **202 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://mx.allabout.network/audit`. A first-time visitor sees the cold-cache cost: the crawler recorded 943 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 515ms, 482ms, 333ms, giving a returning-visitor median of **482 ms**. **Returning-visitor verdict: Healthy**.
...
The sitemap declares 138 URLs and grades Complete. Lastmod dates vary across entries, which tells machines which pages changed and when.

This was a full crawl: the audit reached every page it could discover, and 2 of them are absent from the sitemap (which lists 138). The unlisted pages: `/AI-USAGE.pdf`, `/about/index.html`. The full set is recorded in the `mx-allabout-network-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.
...
|--------------|-------|------------|---------------|------------------------------------------|
| ListItem | 7 | 100% | 100% | - |
| WebPage | 7 | 100% | 100% | Website, WebSite, Organisation, Person |
| BreadcrumbList | 7 | 100% | 100% | - |
| Organisation | 4 | 100% | 100% | Person, ContactPoint, Reference, Website |
| Offer | 2 | 100% | 100% | Organisation, Service, PriceSpecification |
| Service | 2 | 100% | 100% | Organisation, Service, Website, Reference |
| Person | 3 | 100% | 100% | Organisation, PropertyValue |
...
**Structured Data Quality:** 95/100\
...
| Entity richness | 11 | 15 | Entities are described with enough properties to be useful |
...
We have an active edge-capture pattern on https://mx.allabout.network, so AI visits are being separated from organic direct traffic. The browser does not inform the server which AI generated the request.
...
| Metric | Count |
|--------|-------|
| AI crawler hits (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, …) | 1894 |
| AI referral hits (human visits from chat surfaces) | 0 |
| Total AI-attributable visits | 1894 |
...
- **Truncation Risk** - Pass · 7/7
  - *Means:* Every page is well under the 250 KB hard ceiling at which some server-side agents stop reading, and main content sits within the 50-100 KB offset windows. The largest page is 25 KB ([/about/about.html](https://mx.allabout.network/about/about.html)).
  - *Data:* Largest page: 25 KB ([/about/about.html](https://mx.allabout.network/about/about.html)). Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows.
- **SPA Shell** - Pass · 7/7
...
- **Body Content Ratio** - Pass · N/A
  - *Means:* Actual prose content averages 54% of served bytes, well above the 30% threshold. Pages are content-heavy, not overhead-heavy.
  - *Data:* Average: 54%. Threshold: 30%.
...
We found that every resilience check we ran on the seven audited pages passed, meaning machines can fetch and render your content without interruption. Because there are no flagged issues, there is currently nothing to fix; the site already delivers a fully reliable experience for automated agents. This presents an opportunity to maintain this level of stability by continuing routine checks as new pages are added.
...
We find that the site uses semantic HTML well, giving machines reliable structural cues.
...
| HSTS | Yes (6/7) | Forces HTTPS, prevents downgrade attacks |
| Content-Security-Policy | Yes (6/7) | Prevents XSS and injection attacks |
| X-Frame-Options | Yes (6/7) | Prevents clickjacking |
| X-Content-Type-Options | Yes (6/7) | Prevents MIME-type sniffing |

Header coverage is uneven across audited responses: HSTS (Strict-Transport-Security) on 6/7 audited responses, Content-Security-Policy (CSP) on 6/7 audited responses, X-Frame-Options on 6/7 audited responses, X-Content-Type-Options on 6/7 audited responses. The header set is configured per route or per virtual host rather than uniformly at the origin or CDN edge; a single server-config change brings the missing responses in line with the strongest baseline already in place.

**Coverage:** 6 of 7 audited URLs carry all five headers (`/about` is the exception).
...
HTTPS: 7/7 | HSTS: 6/7 | CSP: 6/7 | X-Frame-Options: 6/7 | X-Content-Type-Options: 6/7
...
| Open Graph tags | 86% | - |
| Twitter Card tags | 86% | - |
| Skip link | 86% | - |
| llms.txt link tag | 86% | - |
| Canonical URL | 86% | - |
| Exactly 1 H1 | 100% | - |
...
Some pages in the 7-page sample are missing metadata patterns that others carry. Machines hitting different pages get different quality data. The Missing Pages column shows where to focus on the sampled pages.

## Content Tailoring and A/B Testing: Machine Visitor Impact

Dynamic Yield and Unbounce Smart Traffic (personalisation) are present on pages in the audited set, but every visit returned the same content during this audit.  

The layer is installed, yet it served machines consistent content throughout the run. Because machines arrive with no cookie and no session, the layer could begin assigning variant content if it is later configured to act on first-touch or uncohorted traffic. This is a latent risk worth noting, not a problem observed in this run.

No action is needed today: the layer served machines consistent content throughout this audit. If you later configure the personalisation layer to act on uncohorted or first-touch traffic, add bypass rules so machine visitors (ClaudeBot, GPTBot, CCBot, and similar) keep receiving canonical content. Keep key pages available as a stable, non-personalised variant for machine User-Agents, so a future configuration change cannot silently start feeding crawlers inconsistent content.
...
We examined the metadata and structured data across the seven pages to assess how consistently your content is presented to machines. Across the audited set, we found consistent brand naming, canonical URLs, meta descriptions, and entity values, with no duplication or out-of-range values flagged by our consistency check. No brand-name or canonical-URL divergence was detected in the sampled pages.
...
| Check                            | Result | Notes                    |
|----------------------------------|--------|--------------------------|
| Brand-name parity | Pass | Brand name appears consistently across all 7 audited pages |
...
We note that accessibility legislation worldwide converges on ISO 14289-1 (PDF/UA) as the technical baseline, with the European Accessibility Act-Directive (EU) 2019/882, in force 28 June 2025-as a particularly precise codification, while Section 508 of the US Rehabilitation Act, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent statutes in Australia and Canada all resolve to the same structural artefact.  
We also observed that an untagged PDF is invisible to machines; only a tagged PDF with a proper structure tree can be read by search crawlers, AI systems, and automated pipelines in the same way semantic HTML is understood.
...
Across the audited set of seven pages on https://mx.allabout.network, we found AI Suitability at a perfect 100/100, indicating that machines can immediately understand and use your content. Opportunities for improvement lie in SEO (93/100) and Discovery Readiness (94/100), where enhancing keyword relevance and discovery artefacts will further boost machine visibility. We invite you to act on these insights to strengthen your digital presence.
...
| MX Stack Completeness | 97/100 | Excellent |
| Structured Data Quality | 95/100 | Excellent |
| Commerce Visibility | 20/100 | Needs Improvement |
| Discovery Readiness | 94/100 | Excellent |
| Heading Quality | 97/100 | Excellent |
| Agent Readability | 99/100 | Excellent |
| Pipeline Survivability | 100/100 | Excellent |
| Cross-Page Consistency | 93% | Excellent |
...
Per page, internal links range from 1 to 25, averaging 6 across 20 pages. That is sparser than typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 118 |
...
We reviewed 19 images across the audited set: 19 WebP. 18 of 19 (94.7%) carry alt text, leaving 1 without it. Each missing alt attribute is a place where a screen-reader user or a machine reading the page gets no description of what the image shows.

On loading strategy, 11 images are marked `loading="lazy"` and 8 `loading="eager"`, so every image on the audited pages declares its fetch behaviour explicitly rather than leaving the browser to guess.
