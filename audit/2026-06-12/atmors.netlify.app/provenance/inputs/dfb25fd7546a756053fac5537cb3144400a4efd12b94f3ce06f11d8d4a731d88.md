...
- Site: Atmors Netlify (atmors.netlify.app)
- Pages audited: 1 (full audit)
- Date: 2026-06-12
- Scores: SEO 84/100 | Accessibility 50/100 | Discovery Readiness 10/100 | Structured Data 0/100
...
- Accessibility: 10 distinct WCAG AA issue types (43 raw instances, 43 marked critical by Pa11y)
- AI Suitability (served): 23/100
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 1
- Performance: 1403ms avg (Good)
- Accessibility: 50/100 (Could Be Better)
- SEO: 84/100 (Excellent)
- Accessibility issues: 10 distinct WCAG codes across 43 raw instances
-->
...
- SEO (content pages only): n/a/100
- Accessibility: 50/100 (10 distinct WCAG codes, 43 raw instances)
- Security headers: 1/5 present (HTTPS); 0 of 1 audited URL carry all five
- Structured Data Quality: 0/100
- Consistency: 20%
-->
...
|-----------|-------|------------|
| Performance | Good | Good - 1403ms average load time |
| SEO (content pages) | - | Excellent - titles, meta descriptions, canonical URLs in place |
| Security | 1/5 | 1/5 headers present (HSTS, CSP, X-Frame-Options, X-Content-Type-Options absent); 0 of 1 URL carry all five |
...
| 9 | Security headers absent: HSTS, CSP, X-Frame-Options, X-Content-Type-Options | Cross-cutting | Medium | Low | Missing security headers increase exposure to content injection and clickjacking |
...
**Priority 9: Security headers absent: HSTS, CSP, X-Frame-Options, X-Content-Type-Options**
...
Across the 0 pages we checked, every pattern in the catalogue came back clean: the page semantics that reach the tree match what the page presents visually. This is the state to protect when templates change.
...
**Slowest.** The slowest page is `https://atmors.netlify.app/`. A first-time visitor sees the cold-cache cost: the crawler recorded 1403 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 203ms, 435ms, 766ms, giving a returning-visitor median of **435 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://atmors.netlify.app`. A first-time visitor sees the cold-cache cost: the crawler recorded 1403 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 305ms, 691ms, 783ms, giving a returning-visitor median of **691 ms**. **Returning-visitor verdict: Healthy**.
...
FACTS:
- Crawled URLs before deduplication: 14
- Unique canonical pages after normalisation: 2
- Inflation factor: 7×
- Variant clusters found:
  - Canonical: https://atmors.netlify.app; variants also present: https://atmors.netlify.app/#top, https://atmors.netlify.app/#about, https://atmors.netlify.app/#services, https://atmors.netlify.app/#team, https://atmors.netlify.app/#clients, https://atmors.netlify.app/#impact, https://atmors.netlify.app/#about, https://atmors.netlify.app/#services, https://atmors.netlify.app/#team, https://atmors.netlify.app/#clients, https://atmors.netlify.app/#impact, https://atmors.netlify.app/#services
...
This is a clean-slate site with no Schema.org markup. There are no property gaps to report because no typed entities exist yet; every structured data addition is net new capability. The served HTML score is 23/100; structured markup would significantly improve agent comprehension.
...
- Pages audited: 1
- Pipeline Survivability score: 71
- Checks flagged on any page: Truncation Risk, Boilerplate Burial, Inline Tag Bloat
- Check with the most pages flagged: Truncation Risk
- Pages affected by that check: 1 of 1
...
- Score (both surfaces, identical): 38, band high (worst page: [the home page](https://atmors.netlify.app/))
- Bare divs (both surfaces, identical): 166 of 266 (62%)
- Deepest bare chain (both surfaces, identical): 5
- Top bare selectors (served): `div.c-name` (12), `div.c-meta` (12), `div.meta` (10), `div.row` (10), `div.n` (8)
- Top bare selectors (rendered): `div.c-name` (12), `div.c-meta` (12), `div.meta` (10), `div.row` (10), `div.n` (8)
- Pages contributing to the Div Soup aggregation: 1
...
HTTPS: 1/1 | HSTS: 0/1 | CSP: 0/1 | X-Frame-Options: 0/1 | X-Content-Type-Options: 0/1
...
| Full Implementation | Commerce Visibility, Structured Data, Discovery Readiness, Semantic Structure, MX Stack Completeness, Security headers, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
WRITE:
2-3 sentences summarising the audit findings for Atmors Netlify (https://atmors.netlify.app). Always refer to the site by its domain (https://atmors.netlify.app), never by the consultancy name. The highest-scoring dimension is SEO at 84/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: 23/100; Accessibility: 50/100; SEO: 84/100; Structured Data: 0/100; Discovery Readiness: 10/100
- Human visitors: served well
- Machine opportunity: structured data, discovery, metadata

DO_NOT:
- Write "site-wide", "across the site", or any cross-page generalisation. This is a single-page audit; scope every claim to "the audited page".
-->
...
- **`/ (nav)`**: SEO 84 · A11y 50 · Back 30 · Served 23 · Rendered 23
