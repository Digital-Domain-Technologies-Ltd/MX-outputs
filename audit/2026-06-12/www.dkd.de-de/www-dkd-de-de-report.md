...
|---------|------:|------:|:-------|
| Performance | 55 | 55 | No change |
| Accessibility | 89 | 100 | +11 (improved) |
| SEO | 92 | 93 | +1 (improved) |
| Machine Suitability | 79 | 75 | -4 (declined) |
| WCAG AA issues | 368 | 0 | -368 (improved) |
| Pages audited | 6 | 11 | +5 |
...
Across the audited set, the pages deliver a strong experience for human visitors, with excellent performance, accessibility and SEO scores.
...
We find that across the audited set, machines can discover and parse your content but lack governance context, as shown by a Discovery Readiness score of 25/100, Structured Data Quality of 68/100, MX Stack Completeness of 62/100, Pipeline Survivability of 76/100, and an overall MX Readiness Level of 1 (Discoverable).
...
Across the audited set, we see a strong foundations: SEO scores 93/100, accessibility is perfect at 100/100, structured data quality sits at 68/100, consistency reaches 64%, and four of five key security headers are present on every page. These strengths give you a strong base to build upon as we outline the next steps.
...
- **sameAs**: add external identifiers (e.g., ORCID, Wikidata, LinkedIn) to the Person entity on author pages and to the WebSite entity on the homepage; this gives machines authoritative references for attribution and cross-site discovery.  
- **SpeakableSpecification**: define CSS selectors for spoken content on BlogPosting pages; this allows LLMs to surface voice-ready excerpts of articles.  
- **Content-Signal directives**: add Content-Signal directives in robots.txt to declare the site’s content-use policy, giving machines a clear understanding of how content may be reused.
...
We found that every audited page has a content-negotiation issue, so machines may receive an incorrect MIME type or language variant and cannot parse the content reliably. Correcting these headers across all pages will give machines clear signals for indexing and rendering, giving your content the best chance of being understood. While truncation risk and inline tag bloat were flagged on fewer pages, addressing them too would further strengthen resilience and improve the overall machine experience.
...
We observed that the worst page (/de/kontakt) has a bare-div ratio of 54 % (26 of 48), which forces machines to rely on positional inference rather than semantic structure.  
The soup is surface-wide, with a shallow deepest chain of three, indicating the content was likely assembled by a drag-and-drop builder or an untyped component framework that injects markup late in the rendering pipeline.  
Our cheapest first move is to wrap the primary landmarks-header, nav, main, footer, aside-and give remaining elements descriptive class names, which will lower the bare-div ratio without restructuring the layout.
...
We detected Google Optimise (A/B testing) on https://www.dkd.de/de/kontakt, https://www.dkd.de/de/leistungen/consulting/dkd-experten-webprojekte-it-loesungen, https://www.dkd.de/de/typo3-agentur, https://www.dkd.de/de/referenzen/digitalisierungsprojekte-aco, https://www.dkd.de/de/referenzen/typo3-website-launch-hessen-mice and six additional URLs, but every visit returned the same content during this audit. This indicates a latent risk: because machines arrive with no cookie and no session, if the optimisation layer is later configured to act on first-touch or uncohorted traffic, machine visitors could start receiving variant content. No action is required today; however, should you adjust the personalisation settings to target uncohorted or first-touch traffic, add bypass rules so that ClaudeBot, GPTBot, CCBot and similar agents continue to receive canonical content, and keep key pages available as a stable, non-personalised variant for machine user-agents.
...
We note that accessibility legislation worldwide has converged on ISO 14289-1 (PDF/UA) as the technical baseline for document accessibility, with the European Accessibility Act-Directive (EU) 2019/882, in force 28 June 2025-providing the most precisely codified example of this alignment.  
We also recognise that an untagged PDF remains invisible to machines; a PDF that includes a proper structure tree is machine-readable in the same way semantic HTML is, enabling search crawlers and AI systems to extract text, entities, and structure.
