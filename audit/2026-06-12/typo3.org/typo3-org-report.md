...
reportId: "typo3-org-WEB-AUDIT-20260612"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-12"
auditCommand: "node scripts/audit-pipeline.js https://typo3.org --pages 10 --date 2026-06-12 --client typo3-org"
description: "Executive audit report reviewing accessibility, performance, SEO, structured data, and AI agent compatibility for Typo3"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-12/typo3.org/typo3-org-report.md
  ...
  runbook: "Executive audit report for Typo3. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/typo3.org/typo3-org-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/typo3.org/typo3-org-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-12/typo3.org/typo3-org-report.pdf"
...
**Date:** 12 June 2026\
**Report ID:** typo3-org-WEB-AUDIT-20260612
...
| SEO | **75**/100 | `##############----` |
| Served-HTML Structure | **91**/100 | `################--` |
| MX Stack Completeness | **52**/100 | `#########---------` |
| Agent Readability | **59**/100 | `###########-------` |
...
Across the audited set your pages deliver a smooth and engaging experience for visitors. Performance is the strongest human-experience dimension, with fast load times that keep users on track. Accessibility is consistent - we recorded zero distinct WCAG AA issue types across all 12 pages, giving every visitor an inclusive journey.

The headline opportunity we want to draw attention to is adding full MX governance metadata. While machines can parse your served HTML (AI Suitability 91/100), the absence of MX fields means they lack the context needed for accurate comprehension. Adding mx:status, mx:contentType, mx:audience, canonicalUri and provenance markers will lift MSC above 60 and Discovery Readiness above 40, moving the site from Discoverable to Governed.

Embedding Schema.org JSON-LD lets agents understand page content without rendering. Introducing even a single structured data block will give machines the context they need for accurate attribution and unlock further machine-experience benefits.
...
We last audited typo3.org on 28 May 2026. The table compares that audit with the current one across the headline measures. Some scores improved and others slipped since that audit; the table shows each change.

| Measure | 28 May 2026 | 12 June 2026 | Change |
|---------|------:|------:|:-------|
| Performance | 85 | 85 | No change |
| Accessibility | 91 | 100 | +9 (improved) |
| SEO | 77 | 75 | -2 (declined) |
| Machine Suitability | 91 | 91 | No change |
| WCAG AA issues | 29 | 0 | -29 (improved) |
...
Across the audited set, we find that the pages deliver a strong experience for human visitors, with excellent performance and accessibility scores, while SEO presents an area for improvement.
...
Across the audited set, machines can discover and parse your content with a Discovery Readiness of 25/100 and Structured Data Quality of 0/100, yet they lack governance context because MX Readiness Level is 1 (Discoverable) and no governance markers are present; however, the MX stack completeness at 52/100 and Pipeline Survivability at 96/100 give them a strong foundations for future enhancements.
...
| MX Stack Completeness | 52/100 | Good | B | B (median) |
| Pipeline Survivability | 96/100 | Excellent | A | A (median) |
...
**Evidence:** MX Stack Completeness 52/100 | Structured Data Quality 0/100 | Discovery Readiness 25/100 | Consistency 43%
...
|-----------|-------|------------|
| Performance | Excellent | Excellent - 882ms average load time |
...
| Heading Quality | 88 | Excellent - headings present and machine-parseable |
...
- Body content ratio averages 84%: pages are content-heavy, not overhead-heavy, which helps agents reach real prose inside a cheap fetch.
...
| 4 | Security headers absent: HSTS, CSP, X-Frame-Options | Cross-cutting | Medium | Low | Missing security headers increase exposure to content injection and clickjacking |
...
**Finding:** 26 of 98 images (27%) on the audited set carry no alt text, so their content is unavailable to assistive technology and to machines reading the page.
...
**Finding:** Rendered semantic-structure score 21/100: containers carry no role, ARIA landmark, or descriptive class, so machines fall back on positional inference to determine meaning. The worst page ([/association](https://typo3.org/association)) carries 228 bare divs of 310.
...
**Priority 4: Security headers absent: HSTS, CSP, X-Frame-Options**

**Bucket:** Cross-cutting

**Finding:** Security headers absent: HSTS, CSP, X-Frame-Options (across the audited pages). Missing security headers increase exposure to content injection and clickjacking

**What to change and why:**

- Add the missing response headers at the server or CDN edge; each is a one-line directive that applies across the audited pages
...
- **Organisation JSON-LD**: Add a minimal Organisation entity with name and URL on the homepage or About page to give machines basic context about your brand.  
- **Content-Signal directives**: Include Content-Signal directives in robots.txt to declare content-use policy for machines, enabling agents to understand how they may reuse your content.
...
Across the 12 pages we checked, 4 distinct issue patterns reduce what reaches the tree. 3 of these repeat across pages with the same structure, which marks them as template-level: one change in the shared component clears the finding everywhere it appears.

**Clickable element with no semantic role** (WCAG 2.1 4.1.2)

A container is wired to respond to clicks but carries no role and is not a native control, so it does not exist as an interactive element in the accessibility tree. A screen reader user cannot reach it; an agent reading the tree or the markup cannot tell it is actionable. Repeats on 11 of 12 pages with the same structure (a template-level pattern).
...
An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Seen on `/association/structure/board`.

*The fix:* Add an aria-label (or a title element inside the SVG) in the component, or mark it decorative with aria-hidden when it carries no meaning.

**Link whose accessible name carries no destination meaning** (WCAG 2.1 2.4.4)

The link's accessible name is a generic phrase or a bare URL. In the accessibility tree, and in any links-only projection of the page, every such link reads identically: the destination is unknowable without the surrounding layout, which machines and screen reader users do not have. Repeats on 5 of 12 pages with the same structure (a template-level pattern).

*The fix:* Name the destination in the link text itself in the template ("View pricing plans" rather than "click here").

The full set, one row per pattern with every affected page counted, is recorded in the `typo3-org-accessibility-tree.csv` sidecar alongside this report.
...
**Slowest.** The slowest page is `https://typo3.org/association/contact`. A first-time visitor sees the cold-cache cost: the crawler recorded 1508 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three fresh re-probes that followed returned 313ms (HTTP 404), 397ms (HTTP 404), 202ms (HTTP 404); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: Indeterminate**.

**Median-load control.** The median-load control page is `https://typo3.org/association/structure`. A first-time visitor sees the cold-cache cost: the crawler recorded 837 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 224ms (HTTP 404), 314ms (HTTP 404), 316ms (HTTP 404); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: Indeterminate**.

**Verdict:** First-visit response time is within healthy bounds. The returning-visitor view is Indeterminate for both pages because the fresh re-probes did not return usable timing data (network errors or non-2xx non-retry-status responses), so we cannot characterise the warmed-cache experience from this audit.
...
The sitemap declares 75 URLs and grades Partial. Lastmod dates vary across entries, which tells machines which pages changed and when. The sitemap omits changefreq. Google dropped this as ranking signals in 2017, but non-Google crawlers and AI agents still read changefreq as a re-crawl cadence hint and priority as a relative-importance signal, so adding it is a low-effort way to broaden machine compatibility.

This was a full crawl: the audit reached every page it could discover, and 1 of them is absent from the sitemap (which lists 75). The unlisted pages: `/community/dataprotection-corner`. The full set is recorded in the `typo3-org-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.
...
- **Truncation Risk** - Fail · 1/12
  ...
  - *Data:* The flagged page is 80 KB with main content starting at 30 KB. Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows. Page: https://typo3.org/association/contact
...
- **Delayed Content Start** - Pass · 4/4
  ...
  - *Data:* Content starts at up to 25% of the document on some pages. Check applied to 4 of 12 audited pages; the remaining 8 pages were skipped by a size or eligibility gate.
...
- **Body Content Ratio** - Pass · 4/4
  - *Means:* Actual prose content averages 84% of served bytes, well above the 30% threshold. Pages are content-heavy, not overhead-heavy.
  - *Data:* Average: 84%. Threshold: 30%. Check applied to 4 of 12 audited pages; the remaining 8 pages were skipped by a size or eligibility gate.
- **Inline Tag Bloat** - Fail · 5/12
...
- **Head Weight** - Pass · 4/4
  ...
  - *Data:* Max ratio: 0.01. Average: 0.00. Threshold: 0.50. Check applied to 4 of 12 audited pages; the remaining 8 pages were skipped by a size or eligibility gate.

**Pipeline Survivability score:** 96/100

We found that Inline Tag Bloat and Truncation Risk are the resilience checks that need attention across the audited set. Because inline tags inflate page size, machines can truncate content before fully parsing it, which means they may miss key information or metadata on those five pages. The biggest opportunity is to move scripts and styles out of the HTML so the pages load more efficiently, giving agents a complete view of the content.
...
|--------|--------------|----------------|--------------------|
| Served and rendered | 20/100 (high) | 216 bare divs (75% of containers, depth 7) | `div.frame-group-container` (195), `div.frame-group-inner` (183), `div.frame-container.frame-container-default` (183), `div.frame-inner` (183), `div.contentcontainer-column` (88) |

**Worst page (served and rendered are identical):** [/association](https://typo3.org/association)

Across the audited set, the worst-page at https://typo3.org/association shows a higher bare-div ratio on the served surface (216 of 288, 75 %) than on the rendered surface (228 of 310, 74 %), meaning machines lose structural context and must rely on positional inference to determine meaning.  
The soup is structural, with deep chains reaching seven levels; this pattern indicates that the source pipeline likely relies on drag-and-drop builders or untyped component frameworks that generate nested divs without semantic tags.  
A cost-effective first step is to wrap the obvious landmarks-header, nav, main, footer, aside-with proper elements and assign meaningful class names to remaining wrappers so the bare-div ratio drops without restructuring the layout.
...
- **`/governance-values/data-privacy`**: HTTPS Yes · HSTS No · CSP No · X-Frame No · X-Content-Type Yes
- **`/governance-values/security`**: HTTPS Yes · HSTS No · CSP No · X-Frame No · X-Content-Type Yes
- **`/governance-values/rules-guidelines-policies`**: HTTPS Yes · HSTS No · CSP No · X-Frame No · X-Content-Type Yes
- **`/association/structure`**: HTTPS Yes · HSTS No · CSP No · X-Frame No · X-Content-Type Yes
- **`/association/structure/board`**: HTTPS Yes · HSTS No · CSP No · X-Frame No · X-Content-Type Yes
...
We examined metadata and structured-data consistency across the 12 audited pages to assess how well your content aligns with machine expectations. Consistency in brand naming, canonical URLs, meta descriptions, and entity values is critical for reliable indexing and accurate attribution by machines. Across the audited set, we found no brand-name or canonical-URL divergence flagged by the consistency check.
...
We found 5 identical inline fragment(s) repeated across multiple pages, totalling 2156 KB redundant bytes. Extracting these to external CSS or JS files would reduce page weight, improve cacheability, and simplify maintenance.
...
| css | 218748 | 11 | @keyframes ucOpacity{0%{opacity:0}100%{opacity:1}}.usercentr |
| css | 773 | 11 | .usercentrics-button .uc-corner-modal .uc-corner-modal-conte |
| css | 652 | 11 | #usercentrics-button .uc-banner-content {       background-c |
| css | 151 | 11 | #uc-corner-modal a[href], #uc-centre-modal a[href], #uc-bann |
...
We note that across major markets, accessibility legislation has converged on ISO 14289-1 (PDF/UA) as the technical baseline for document accessibility, with the European Accessibility Act-Directive (EU) 2019/882, in force 28 June 2025-as a precisely codified example of that global alignment; similar convergence is reflected in Section 508 of the US Rehabilitation Act, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent legislation in Australia and Canada. Because an untagged PDF is invisible to machines, we find that search crawlers and AI systems cannot extract text, entities or structure from a scanned or image-based document; a tagged PDF with a proper structure tree becomes machine-readable in the same way semantic HTML is.
...
Accessibility leads the scorecard as a strength, scoring 100/100 across the audited set of pages on https://typo3.org.  
Key opportunities lie in structured data, which scored 0/100, and discovery readiness at 25/100, limiting machine comprehension and discoverability.  
Prioritising these areas improves the site's machine readability, helping agents surface its content more effectively.
...
|-----------|-------|------|
| Served-HTML Structure | 91/100 | Excellent |
| Accessibility | 100/100 | Excellent |
| SEO (all pages) | 75/100 | Good |
...
- **`/ (nav)`**: SEO 84 · A11y 100 · Back 45 · Served 80 · Rendered 85
- **`/association`**: SEO 85 · A11y 100 · Back 45 · Served 95 · Rendered 100
- **`/company`**: SEO 75 · A11y 100 · Back 45 · Served 94 · Rendered 99
- **`/association/membership`**: SEO 85 · A11y 100 · Back 45 · Served 95 · Rendered 100
- **`/governance-values`**: SEO 85 · A11y 100 · Back 45 · Served 95 · Rendered 100
- **`/association/funding-finances`**: SEO 69 · A11y 100 · Back 45 · Served 94 · Rendered 99
- **`/association/contact`**: SEO 59 · A11y 100 · Back 45 · Served 84 · Rendered 89
- **`/governance-values/data-privacy`**: SEO 84 · A11y 100 · Back 45 · Served 85 · Rendered 90
- **`/governance-values/security`**: SEO 73 · A11y 100 · Back 45 · Served 95 · Rendered 100
- **`/governance-values/rules-guidelines-policies`**: SEO 67 · A11y 100 · Back 45 · Served 94 · Rendered 99
- **`/association/structure`**: SEO 76 · A11y 100 · Back 45 · Served 95 · Rendered 100
- **`/association/structure/board`**: SEO 58 · A11y 100 · Back 45 · Served 84 · Rendered 89

The page marked (nav) is navigational: it routes visitors to content rather than containing it, and is excluded from the SEO content average. Content-pages SEO average: 74/100.
...
Per page, internal links range from 14 to 22, averaging 16 across 12 pages. That is typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 191 |
...
We reviewed 98 images across the audited set: 31 WebP and 67 SVG. 72 of 98 (73.5%) carry alt text, leaving 26 without it. Each missing alt attribute is a place where a screen-reader user or a machine reading the page gets no description of what the image shows.

On loading strategy, 22 images are marked `loading="lazy"` and 0 `loading="eager"`, while 76 carry no loading attribute at all. No attribute is not the same as eager: the browser decides for itself when to fetch, which removes the explicit control that lazy and eager give you. Setting an explicit attribute on those images makes the fetch behaviour predictable for browsers and machines alike.
...
**Scope:** 12 pages examined | Platform: Netlify | Frameworks: **Bootstrap** (low confidence) - CSS framework; **Netlify** (low confidence) - CDN / delivery | Analysis method: Hybrid (automated + manual verification) | robots.txt: Absent (HTTP 404)
