---
title: "Specification: Website Analysis & Machine Readiness"
author: "Tom Cranstoun"
created: "2026-06-12"
modified: "2026-06-12"
client: "Specification"
clientSlug: "specification-website"
clientUrl: "https://specification.website"
reportId: "specification-website-WEB-AUDIT-20260612"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-12"
auditCommand: "node scripts/audit-pipeline.js https://specification.website --pages 10 --date 2026-06-12 --client spec-website"
description: "Executive audit report reviewing accessibility, performance, SEO, structured data, and AI agent compatibility for Specification"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-12/specification.website/specification-website-report.md
  maintainer: info@cognovamx.com
  stability: stable
  partOf: mx-audit
  purpose: "Executive machine-readiness audit for Specification covering accessibility, performance, SEO, structured data, and AI agent compatibility."
  x-mx-contextProvides: ["web audit findings for Specification", "WCAG accessibility assessment", "AI agent compatibility scores", "SEO and structured data analysis", "machine readiness recommendations"]
  ...
  runbook: "Executive audit report for Specification. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/specification.website/specification-website-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/specification.website/specification-website-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-12/specification.website/specification-website-report.pdf"
    description: "Generate PDF audit report for Specification"
  ...
  x-mx-provenance:
    sidecar: "specification-website-report.provenance.ai.json"
    frameworks: [EU-AI-Act, UK-ICO-AI-guidance, NIST-AI-RMF, Colorado-AI-Act]
    companion: "specification-website-report.provenance.deterministic.json"
...
**Date:** 12 June 2026\
**Report ID:** specification-website-WEB-AUDIT-20260612
...
We found that the audited pages deliver a strong user experience. Performance is excellent, with consistently fast load times across all 11 pages. Accessibility is consistent; we recorded zero WCAG AA issues. Across the 11 audited pages, the clear navigation and consistent branding reinforce trust for visitors.

Adding MX governance metadata will improve machine comprehension. Score thresholds for Governed are met (MSC 76, SDQ 75, DR 76), but no MX-namespaced governance fields were detected across the audited set. Adding mx:status, mx:contentType, mx:audience, canonicalUri and provenance markers to at least one published page will unlock Governed level, giving machines the context they need for accurate attribution.
...
The Human Experience subtable summarises how the audited pages perform from a visitor perspective.  
We find that across the audited set, the site delivers a strong experience for human visitors, with excellent performance, accessibility and SEO scores.
...
Across the audited set, we find that machines can discover and parse your content with a Discovery Readiness of 76/100, Structured Data Quality of 75/100, MX Stack Completeness of 76/100, and Pipeline Survivability of 90/100, yet they lack governance context as indicated by an MX Readiness Level of 1 (Discoverable).
...
|-----------|-------|--------|-------|----------|
| Discovery Readiness | 76/100 | Excellent | A | C (median) |
| Structured Data Quality | 75/100 | Good | B | B (median) |
| MX Stack Completeness | 76/100 | Excellent | A | B (median) |
...
**Evidence:** MX Stack Completeness 76/100 | Structured Data Quality 75/100 | Discovery Readiness 76/100 | Consistency 85%

Score thresholds for Governed are met (MSC 76, SDQ 75, DR 76), but no MX-namespaced governance metadata was detected across the audited set.ected on the audited pages. Add MX governance fields (mx:status, mx:contentType, mx:audience, canonicalUri, and provenance markers) to at least one published page to unlock Governed level. Without those fields a machine can discover the page but lacks the governance context for accurate comprehension.
...
Across the audited set, we see a strong foundations in SEO (94/100), accessibility (100/100), security headers (5/5), structured data quality (75/100) and consistency (85 %). These strengths position your content well for the enhancements outlined later.
...
|-----------|-------|------------|
| Performance | Excellent | Excellent - 412ms average load time |
| SEO (content pages) | 94 | Excellent - titles, meta descriptions, canonical URLs in place |
| Security | 5/5 | HTTPS, HSTS, CSP, X-Frame-Options, X-Content-Type-Options on every audited URL |
| Structured Data | 75 | Good - JSON-LD on every page with valid Schema.org vocabulary |
| Heading Quality | 91 | Excellent - single H1 on every page |
...
- Structured Data Quality of 75/100 (Decoration): the schema is valid, required properties are complete, and the vocabulary is in good order.
...
The audit identified two findings on the audited set, ordered by regulatory exposure first and then by priority within each category.
...
|---|---------|--------|----------|--------|--------|
| 1 | Heading Hierarchy Skips Levels | Compliance Risk | Medium | Low | screen-reader and machine outline-builders may misread the page structure |
| 2 | Structured Data Property Gaps | Machine Readability Opportunity | Medium | Medium | machines may extract these entities incompletely or skip them |

---

**Priority 1: Heading Hierarchy Skips Levels**
...
**Priority 2: Structured Data Property Gaps**

**Bucket:** Machine Readability Opportunity

**Finding:** 44 Schema.org property gap(s) on the audited set across WebSite: required or recommended properties are missing, so machines extract these entities less reliably.
...
- **sameAs** links on Person and Organisation entities: adds authoritative external identifiers, enabling machines to disambiguate authors and corporate identities wherever these entities appear in the audited pages.  
- **potentialAction** on Organisation entities: exposes contact or booking actions, allowing agents to surface direct interaction options for users interacting with your organisation across the audited set.  
- **SpeakableSpecification** on TechArticle pages: defines CSS selectors for voice surfaces, letting conversational agents present article content in spoken form on each TechArticle page.
...
| Internal navigation links | 56 links to same-site pages |
...
| Measure | Result |
|---------|--------|
| Accessibility-tree score | 90/100 |
| Pages checked | 3 |

Across the 3 pages we checked, 1 distinct issue pattern reduce what reaches the tree. 1 of these repeat across pages with the same structure, which marks them as template-level: one change in the shared component clears the finding everywhere it appears.

**Element exposed as an image with no accessible name** (WCAG 2.1 1.1.1)

An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Repeats on 2 of 3 pages with the same structure (a template-level pattern).
...
The full set, one row per pattern with every affected page counted, is recorded in the `specification-website-accessibility-tree.csv` sidecar alongside this report.
...
**Slowest.** The slowest page is `https://specification.website/contribute`. A first-time visitor sees the cold-cache cost: the crawler recorded 712 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 511ms, 469ms, 414ms, giving a returning-visitor median of **469 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://specification.website/spec/foundations`. A first-time visitor sees the cold-cache cost: the crawler recorded 393 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 401ms, 463ms, 481ms, giving a returning-visitor median of **463 ms**. **Returning-visitor verdict: Healthy**.
...
*The full `robots.txt` (4 lines) is preserved alongside this report as `specification-website-robots-txt.txt`.*
...
The sitemap declares 155 URLs and grades Partial. Lastmod dates vary across entries, which tells machines which pages changed and when. The sitemap omits changefreq and priority. Google dropped both as ranking signals in 2017, but non-Google crawlers and AI agents still read changefreq as a re-crawl cadence hint and priority as a relative-importance signal, so adding them is a low-effort way to broaden machine compatibility.

This was a full crawl: the audit reached every page it could discover, and 7 of them are absent from the sitemap (which lists 155). The unlisted pages: `/.well-known/agent-skills/specification-website/SKILL.md`, `/.well-known/security.txt`, `/changelog/`, `/llms-full.txt`, `/robots.txt`, `/rss.xml`, `/sitemap-index.xml`. The full set is recorded in the `specification-website-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.

We found that the sitemap contains multiple URL variants for the same canonical page, such as trailing-slash and hash-fragment forms, causing AI agents and LLM pipelines that skip URL normalisation to fetch identical content repeatedly and inflate token usage by roughly 1.1×. The fix is to publish one canonical URL per resource in sitemap.xml and add a <link rel="canonical"> tag to each affected URL.
...
4 additional registered `/.well-known/` paths were probed; none returned an identifiable discovery file. The per-path breakdown is preserved alongside this report as a sidecar JSON.
...
We probed 51 addresses that should answer 404 when they are absent; 1 returned a soft 404 instead. Among the well-known discovery paths, 1 of 43 were soft 404s. Severity: schemaOnError.
<!-- END:SOFT_404 -->
...
| CreativeWork | 4 | 100% | 100% | - |
| SearchAction | 11 | 100% | 100% | EntryPoint |
| EntryPoint | 11 | 100% | 100% | - |
| TechArticle | 4 | 100% | 100% | WebSite, Person, Organisation |
| Person | 4 | 100% | 100% | - |
| Organisation | 4 | 100% | 100% | - |
| BreadcrumbList | 4 | 100% | 100% | - |

**Structured Data Quality:** 75/100\
...
| Linked-data signals | 4 | 10 | Linked-data properties present (sameAs, mainEntityOfPage, isPartOf, about, mentions) |
...
The full per-entity list is delivered alongside this report as a sidecar CSV: [`specification-website-structured-data-findings.csv`](specification-website-structured-data-findings.csv). The 44 rows describe individual Schema.org property gaps on specific entities; most of them share a small number of underlying patterns, shown below ranked by instance count.
...
Stage 2 (Citation) is the bottleneck in the agent journey. Because each stage builds on the one before it, the stages after it cannot improve until this one does. It is the highest-priority place to start; the stage's key metric is Schema.org: WebSite, SearchAction, EntryPoint (100% required properties).
...
- **Truncation Risk** - Pass · 11/11
  - *Means:* Every page is well under the 250 KB hard ceiling at which some server-side agents stop reading, and main content sits within the 50-100 KB offset windows. The largest page is 129 KB ([/checklist](https://specification.website/checklist)).
  - *Data:* Largest page: 129 KB ([/checklist](https://specification.website/checklist)). Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows.
...
- **Boilerplate Burial** - Pass · 11/11
  ...
  - *Data:* Highest boilerplate-to-content ratio: 0.74. Threshold: < 10 (and < 80 KB of inline head bytes).
...
- **Delayed Content Start** - Pass · 3/3
  ...
  - *Data:* Content starts at up to 11% of the document on some pages. Check applied to 3 of 11 audited pages; the remaining 8 pages were skipped by a size or eligibility gate.
...
- **HTTP Content Negotiation (Vary)** - Fail · 5/11
  ...
  - *Data:* 5 page(s) advertise format negotiation. See specification-website-pipeline-http-content-negotiation-(vary)-pages.csv (5 pages).
...
- **Body Content Ratio** - Pass · 3/3
  - *Means:* Actual prose content averages 76% of served bytes, well above the 30% threshold. Pages are content-heavy, not overhead-heavy.
  - *Data:* Average: 76%. Threshold: 30%. Check applied to 3 of 11 audited pages; the remaining 8 pages were skipped by a size or eligibility gate.
- **Inline Tag Bloat** - Fail · 11/11
  ...
  - *Data:* 11 element(s) > 500 bytes. Largest single-page inline CSS block: 0 B. Largest single-page inline JS block: 1070 B. See specification-website-pipeline-inline-tag-bloat-pages.csv (11 pages).
- **Head Weight** - Pass · 3/3
  - *Means:* Head bytes are a small fraction of each page. Agents reach body content quickly.
  - *Data:* Max ratio: 0.03. Average: 0.01. Threshold: 0.50. Check applied to 3 of 11 audited pages; the remaining 8 pages were skipped by a size or eligibility gate.

**Pipeline Survivability score:** 90/100

Across the audited set, two resilience checks need attention: Content Negotiation and Inline Tag Bloat. The inline tag bloat, present on every page, inflates markup and can cause machines to misinterpret structure or miss key content; removing unnecessary tags would give the biggest improvement. Addressing content negotiation will also help machines reliably request the correct representation of each page, strengthening overall machine comprehension.
...
|--------|--------------|----------------|--------------------|
| Served and rendered | 79/100 (low) | 3 bare divs (19% of containers, depth 2) | `div.mx-auto.max-w-6xl` (11), `div.grid.gap-10` (11), `#search-overlay-mount` (11), `#toc` (4), `div.mx-auto.grid` (1) |

**Worst page (served and rendered are identical):** [/about](https://specification.website/about)

Across the audited set we confirm that your pages use semantic HTML effectively, giving machines reliable structural cues.
...
- **`/spec/foundations/meta-charset`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
- **`/spec/foundations/meta-viewport`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
- **`/spec/`**: HTTPS Yes · HSTS Yes · CSP Yes · X-Frame Yes · X-Content-Type Yes
...
| Skip link | 91% | - |
| llms.txt link tag | 91% | - |
| Canonical URL | 91% | - |
| Exactly 1 H1 | 100% | - |
| Code examples present | 45% | 6 |
| Self-contained sections | 100% | - |
| Error/troubleshooting docs | 18% | 9 |
...
We found 2 identical inline fragment(s) repeated across multiple pages, totalling 10 KB redundant bytes. Extracting these to external CSS or JS files would reduce page weight, improve cacheability, and simplify maintenance.
...
| js | 368 | 11 | (function(){try{var t=localStorage.getItem("theme");if(t!==" |

*The full inventory (every fragment with its hash and the page URLs that carry it) is preserved alongside this report as `specification-website-inline-code-duplicates.csv`.*
...
We observe that no PDFs were identified in the audited set, yet accessibility legislation worldwide converges on ISO 14289-1 (PDF/UA) as the technical baseline, with the European Accessibility Act-Directive (EU) 2019/882-as a precisely codified example alongside Section 508 of the US Rehabilitation Act, UK Public Sector Bodies Accessibility Regulations 2018, and equivalent statutes in Australia and Canada. Because an untagged PDF is invisible to machines, search crawlers and automated pipelines cannot extract text, entities or structure from a scanned or image-based PDF; a tagged PDF with a proper structure tree becomes machine-readable in the same way semantic HTML is.
...
We found AI Suitability at a perfect 100/100 for https://specification.website, confirming the content’s readiness for machine consumption. Across the audited set, Structured Data and Discovery Readiness scored 75/100 and 76/100 respectively, indicating key opportunities to enhance machine discoverability and metadata richness. We invite you to address these areas to further strengthen your digital presence.
...
| SEO (content pages) | 94/100 | Excellent |
| MX Stack Completeness | 76/100 | Excellent |
| Structured Data Quality | 75/100 | Good |
| Commerce Visibility | 10/100 | Needs Improvement |
| Discovery Readiness | 76/100 | Excellent |
...
- **`/spec`**: SEO 94 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/checklist`**: SEO 98 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/about`**: SEO 91 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/contribute`**: SEO 88 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/spec/foundations`**: SEO 90 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/spec/foundations/doctype`**: SEO 96 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/spec/foundations/html-lang`**: SEO 97 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/spec/foundations/meta-charset`**: SEO 95 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/spec/foundations/meta-viewport`**: SEO 97 · A11y 100 · Back 100 · Served 100 · Rendered 100
- **`/spec/ (nav)`**: SEO 94 · A11y 100 · Back 100 · Served 100 · Rendered 100

Pages marked (nav) are navigational: they route visitors to content rather than containing it, and are excluded from the SEO content average. Content-pages SEO average: 94/100.

**URL deduplication note:** 12 crawled URLs resolved to 11 unique pages after canonicalisation (inflation factor 1.09×). The following URL clusters were treated as the same page: https://specification.website/spec (2 variants: https://specification.website/spec, https://specification.website/spec/). Fragment suffixes (such as `#` and `#/`) and trailing-slash variants are treated as identical resources by HTTP servers and search engines; this audit deduplicated them before scoring to avoid inflating per-page counts.
...
Per page, internal links range from 22 to 159, averaging 62 across 11 pages. That is denser than typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 690 |
...
We found no images on the audited pages, so there is nothing to report on format efficiency or loading strategy here.
...
**Scope:** 11 pages examined | Platform: Netlify | Frameworks: **Astro** (low confidence) - JS framework; **Tailwind CSS** - CSS framework; **Cloudflare CDN** (low confidence) - CDN / delivery; **Netlify** (low confidence) - CDN / delivery | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (20 directives)
