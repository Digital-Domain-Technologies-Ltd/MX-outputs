...
- Site: Specification (specification.website)
- Pages audited: 11 of 155 in sitemap - sample run
- Date: 2026-06-12
- Scores: SEO 94/100 | Accessibility 100/100 | Discovery Readiness 76/100 | Structured Data 75/100
- Scope: 11 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): ListItem (16), WebSite (15), CreativeWork (14), SearchAction (11), EntryPoint (11), TechArticle (4), Person (4), Organization (4), BreadcrumbList (4)
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 11
- Performance: 412ms avg (Excellent)
...
- Security headers: 5/5 on every one of the 12 audited URLs
- Structured Data Quality: 75/100
- Consistency: 85%
-->
...
FACTS (do not change any number, percentage, URL, page count, or name):
- Schema.org types detected: 9 type(s) present
- robots.txt Content-Signal directives: present
-->
...
- Variant clusters found:
  - Canonical: https://specification.website/llms.txt; variants also present: https://specification.website/llms.txt
...
| TechArticle | 4 | 100% | 100% | WebSite, Person, Organization |
| Person | 4 | 100% | 100% | - |
...
- Pages audited: 11
- Pipeline Survivability score: 90
- Checks flagged on any page: Content Negotiation, Inline Tag Bloat
...
- Score (both surfaces, identical): 79, band low (worst page: [/about](https://specification.website/about))
- Bare divs (both surfaces, identical): 3 of 16 (19%)
- Deepest bare chain (both surfaces, identical): 2
- Top bare selectors (served): `div.mx-auto.max-w-6xl` (11), `div.grid.gap-10` (11), `#search-overlay-mount` (11), `#toc` (4), `div.mx-auto.grid` (1)
- Top bare selectors (rendered): `div.mx-auto.max-w-6xl` (11), `div.grid.gap-10` (11), `#search-overlay-mount` (11), `#toc` (4), `div.mx-auto.grid` (1)
...
| Full Implementation | Commerce Visibility, Structured Data, Discovery Readiness, MX Stack Completeness, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
WRITE:
2-3 sentences summarising the audit findings for Specification (https://specification.website). Always refer to the site by its domain (https://specification.website), never by the consultancy name. The highest-scoring dimension is AI Suitability at 100/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: 100/100; Accessibility: 100/100; SEO: 94/100; Structured Data: 75/100; Discovery Readiness: 76/100
