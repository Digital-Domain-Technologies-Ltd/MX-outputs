...
reportId: "dkd-de-de-WEB-AUDIT-20260612"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-12"
auditCommand: "node scripts/audit-pipeline.js https://dkd.de/de --pages 10 --date 2026-06-12 --client dkd-de-de"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-12/dkd.de-de/dkd-de-de-report.md
  ...
  runbook: "Executive audit report for Dkd. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/dkd.de-de/dkd-de-de-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/dkd.de-de/dkd-de-de-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-12/dkd.de-de/dkd-de-de-report.pdf"
...
**Date:** 12 June 2026\
**Report ID:** dkd-de-de-WEB-AUDIT-20260612
...
- **Real AI agents are blocked from fetching the site** (-10): blocked: GoogleOther (Google AI), Google-Extended
...
We found that visitors experience a polished and engaging site. The design is clean, navigation intuitive, and brand messaging consistent across pages, all of which support the strongest human-experience dimension-SEO foundations scoring 94/100. These elements keep users on page and help them find what they need quickly.

The headline opportunity is to strengthen machine comprehension by adding full MX governance fields, provenance metadata, and canonical URIs across the audited set. While AI Suitability at 75/100 shows that machines can parse the served HTML, the absence of MX markers means they lack the context required for accurate attribution. Raising MSC above 60 and Discovery Readiness above 40 will move the site from Discoverable to Governed, giving machines the structured context they need for precise understanding.

\clearpage

<!-- AUDIT-DELTA:START -->
## Change Since Our 28 May 2026 Audit

We last audited www.dkd.de on 28 May 2026. The table compares that audit with the current one across the headline measures. Some scores improved and others slipped since that audit; the table shows each change.

| Measure | 28 May 2026 | 12 June 2026 | Change |
|---------|------:|------:|:-------|
| Performance | 70 | 55 | -15 (declined) |
| Accessibility | 89 | 100 | +11 (improved) |
| SEO | 92 | 94 | +2 (improved) |
| Machine Suitability | 78 | 75 | -3 (declined) |
| WCAG AA issues | 520 | 0 | -520 (improved) |
| Pages audited | 7 | 11 | +4 |
...
The Human Experience subtable summarizes how the audited pages perform for human visitors. Across the audited set, the pages deliver a strong experience, with performance averaging 2099 ms (Good) and both accessibility and SEO scoring excellent.
...
Across the audited set, machines can discover and parse your content but lack governance context, as shown by a Discovery Readiness score of 25/100, Structured Data Quality of 68/100, MX Stack Completeness of 62/100, Pipeline Survivability of 76/100, and an overall MX Readiness Level of 1 (Discoverable).
...
We find that across the audited set, your pages demonstrate a strong foundations in SEO and accessibility, with a 94/100 score and perfect compliance respectively. The strong implementation of key security headers and robust structured-data quality further reinforce this groundwork for future enhancements.
...
|-----------|-------|------------|
| Performance | Good | Good - 2099ms average load time |
...
| Agent access | 6/8 | 6 of 8 AI user-agents receive HTTP 200; GoogleOther (Google AI), Google-Extended blocked |
...
- **sameAs**: linking Person entities to ORCID, Wikidata and LinkedIn gives machines richer identity context for accurate attribution on author pages.  
- **SpeakableSpecification**: adding CSS selectors to BlogPosting pages enables voice assistants to surface key content snippets from articles.  
- **Content-Signal directives**: adding Content-Signal directives in robots.txt declares a content-use policy that informs machines how the material may be reused.
...
**Summary:** 6 of 8 tested agents can access the site. 2 agent(s) returned non-200 responses: GoogleOther (Google AI), Google-Extended.
...
**Slowest.** The slowest page is `https://www.dkd.de/de`. A first-time visitor sees the cold-cache cost: the crawler recorded 4239 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three fresh re-probes that followed returned 293ms (HTTP 503), 257ms (HTTP 503), 216ms (HTTP 503); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: rate_limited_after_pacing**.

**Median-load control.** The median-load control page is `https://www.dkd.de/de/referenzen/typo3-website-launch-hessen-mice`. A first-time visitor sees the cold-cache cost: the crawler recorded 2088 ms on its initial fetch. **First-visit verdict: Acceptable but elevated**. Three fresh re-probes that followed returned 209ms (HTTP 503), 339ms (HTTP 503), 333ms (HTTP 503); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: rate_limited_after_pacing**.
...
This was a full crawl: the audit reached every page it could discover, and 18 of them are absent from the sitemap (which lists 667). The full set is recorded in the `dkd-de-de-pages-not-in-sitemap.csv` sidecar alongside this report. Adding them to the sitemap lets search engines and machines discover all content.
...
- **Truncation Risk** - Fail · 10/11
  ...
  - *Data:* Largest page: 238 KB ([/de/referenzen](https://www.dkd.de/de/referenzen)). Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows. See dkd-de-de-pipeline-truncation-risk-pages.csv (10 pages).
...
- **HTTP Content Negotiation (Vary)** - Fail · 11/11
  ...
  - *Data:* 11 page(s) advertise format negotiation. See dkd-de-de-pipeline-http-content-negotiation-(vary)-pages.csv (11 pages).
...
- **Inline Tag Bloat** - Fail · 11/11
  ...
  - *Data:* 11 element(s) > 500 bytes. Largest single-page inline CSS block: 9252 B. Largest single-page inline JS block: 841 B. See dkd-de-de-pipeline-inline-tag-bloat-pages.csv (11 pages).
...
We found that all 11 audited pages trigger a Content Negotiation issue, meaning machines may receive the wrong representation of resources and miss key content. Truncation Risk causes some pages to cut off data before bots can read it, while Inline Tag Bloat slows parsing for automated agents; both are opportunities to strengthen resilience. Fixing the server’s content negotiation headers first will give machines reliable access to page assets and has the greatest impact.
...
On the worst-page (https://www.dkd.de/de/kontakt) the bare-div ratio reaches 54 % (26 of 48), meaning machines lose structural context and must rely on positional inference to determine meaning; these figures refer to the single worst-page in the audited set.  
The soup is surface-wide, with a high proportion of bare divs but only shallow chains (deepest chain length 3), indicating that the source pipeline likely relies on drag-and-drop builders or untyped component frameworks rather than a well-structured markup hierarchy.  
A cost-effective first step is to wrap obvious landmarks-header, nav, main, footer, aside-and assign meaningful class names to remaining elements so the bare-div ratio drops without restructuring the layout.
...
Google Optimise (A/B testing) is present on https://www.dkd.de/de/kontakt, https://www.dkd.de/de/leistungen/consulting/dkd-experten-webprojekte-it-loesungen, https://www.dkd.de/de/typo3-agentur, https://www.dkd.de/de/referenzen/digitalisierungsprojekte-aco, https://www.dkd.de/de/referenzen/typo3-website-launch-hessen-mice and six additional pages, but every visit returned the same content during this audit.  

The layer served machines consistent content throughout the sample run; however, because machines arrive with no cookie and no session, the system could begin assigning variant content if it is later configured to act on first-touch or uncohorted traffic. This represents a latent risk that we note for future consideration rather than an issue observed in this audit.

No action is required today: the layer served consistent content during our crawl. If you later configure the personalisation layer to target uncohorted or first-touch traffic, add bypass rules so machine visitors (ClaudeBot, GPTBot, CCBot, and similar) continue receiving canonical content. Keep key pages available as a stable, non-personalised variant for machine User-Agents so that future configuration changes cannot silently feed crawlers inconsistent content.
...
We note that accessibility legislation has converged on ISO 14289-1 (PDF/UA) as the technical baseline across major markets; the EAA Directive (EU) 2019/882 in force 28 June 2025 is a precisely codified example of this global convergence.  
We also recognise that an untagged PDF remains invisible to machines-only a tagged PDF with a proper structure tree can be read by search crawlers, AI systems and automated pipelines in the same way semantic HTML is understood.
...
Accessibility is a clear strength on https://www.dkd.de, scoring 100/100 and ensuring all users can interact with content without barriers. Across the audited set, Discovery Readiness sits at 25/100 and Structured Data scores 68/100, limiting how effectively machines can discover and understand your pages. We recommend prioritising improvements in discovery signals and structured data to unlock greater machine visibility and engagement.
