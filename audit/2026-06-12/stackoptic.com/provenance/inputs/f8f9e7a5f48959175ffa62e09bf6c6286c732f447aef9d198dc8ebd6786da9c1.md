...
- Pages audited: 6 of 34026 in sitemap - sample run
- Date: 2026-06-12
- Scores: SEO 86/100 | Accessibility 93/100 | Discovery Readiness 38/100 | Structured Data 89/100
- Scope: 6 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): Person (32), BlogPosting (24), Organization (19), ContactPoint (18), PostalAddress (16), SoftwareApplication (15), WebSite (14), Question (14), Answer (14), Offer (11), ImageObject (9), WebPage (8), Place (7), SearchAction (7), EntryPoint (7), ListItem (6), HowToStep (3), BreadcrumbList (3), FAQPage (2), VideoObject (1), HowTo (1), HowToTool (1), ContactPage (1), CollectionPage (1), Blog (1)
...
SCOPE: 6 pages were audited. Scope all claims to "the audited set" - do not say "site-wide" unless cross-page consistency is explicitly confirmed in the facts.
ACCESSIBILITY NOTE: There are 3 distinct WCAG AA issue types across 34 raw instances. If Accessibility is not grade A, do NOT call it the "strongest" dimension in Paragraph 1. Acknowledge the accessibility opportunity in Paragraph 2 as a Priority 1 compliance item before the MX opportunity. Reference the DISTINCT count when discussing remediation effort (one fix per category clears many instances); reference the RAW count when discussing how many elements need touching.
...
- Accessibility: 3 distinct WCAG AA issue types (34 raw instances, 34 marked critical by Pa11y) - 19 of these instances trace to 6 recurring template patterns (single theme edit per pattern fixes all instances)
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 6
- Performance: 3085ms avg (Could Be Better)
- Accessibility: 93/100 (Excellent)
- SEO: 86/100 (Excellent)
- Accessibility issues: 3 distinct WCAG codes across 34 raw instances (19 instances trace to 6 template patterns)
-->
...
- Accessibility: 93/100 (3 distinct WCAG codes, 34 raw instances) - 19 of these instances trace to 6 template-level patterns
...
FACTS (do not change any number, percentage, URL, page count, or name):
- Schema.org types detected: 25 type(s) present
...
- Variant clusters found:
  - Canonical: https://stackoptic.com/llms.txt; variants also present: https://stackoptic.com/llms.txt
...
| Organization | 6 | 100% | 100% | PostalAddress, Place, Person, ImageObject |
| ContactPoint | 6 | 100% | 100% | - |
| PostalAddress | 6 | 100% | 100% | - |
| SoftwareApplication | 6 | 100% | 100% | Offer, Organization |
| WebSite | 6 | 100% | 25% | SearchAction, Organization |
...
| 2 | Citation | Partial | 59 | Schema.org: Organization, PostalAddress, Place (100% required properties) |
...
Stage 2 (Citation) is the bottleneck in the agent journey. Because each stage builds on the one before it, the stages after it cannot improve until this one does. It is the highest-priority place to start; the stage's key metric is Schema.org: Organization, PostalAddress, Place (100% required properties).
...
- Pages affected by that check: 6 of 6
...
- Score (both surfaces, identical): 27, band high (worst page: [the home page](https://stackoptic.com/))
- Bare divs (both surfaces, identical): 206 of 325 (63%)
- Deepest bare chain (both surfaces, identical): 7
- Top bare selectors (served): `div` (62), `div.flex.items-center` (44), `div.w-10.h-10` (30), `div.mt-auto.pt-4` (24), `div.container.mx-auto` (21)
- Top bare selectors (rendered): `div` (62), `div.flex.items-center` (44), `div.w-10.h-10` (30), `div.mt-auto.pt-4` (24), `div.container.mx-auto` (21)
...
| Full Implementation | Commerce Visibility, Semantic Structure, Discovery Readiness, MX Stack Completeness, Performance, and optional enhancements | Full machine readiness: every agent, search engine, and structured-data consumer can read, trust, and act on the site |
...
WRITE:
2-3 sentences summarising the audit findings for Stackoptic (https://stackoptic.com). Always refer to the site by its domain (https://stackoptic.com), never by the consultancy name. The highest-scoring dimension is AI Suitability at 100/100 - open with this as a strength. Open with the dimension that scored HIGHEST on the scorecard as a genuine STRENGTH. "Leads" means highest score, not highest priority finding. A dimension scoring 10/100 is never "the strongest" - it is an opportunity. High priority ≠ high score; do not confuse the two. Then name 1-2 lowest-scoring dimensions as the key OPPORTUNITIES. Close with an invitation to act.

FACTS:
- Scores: AI Suitability: 100/100; Accessibility: 93/100; SEO: 86/100; Structured Data: 89/100; Discovery Readiness: 38/100
- Human visitors: served well
- Machine opportunity: structured data, discovery, metadata

DO_NOT:
- Say "site-wide" unless consistency was explicitly confirmed. 6 pages were audited; scope claims to "the audited set".
-->
