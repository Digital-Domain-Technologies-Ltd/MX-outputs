...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-12/atmors.netlify.app.d/atmors-netlify-app-report.md
  ...
  runbook: "Executive audit report for Atmors Netlify. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/atmors.netlify.app.d/atmors-netlify-app-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-12/atmors.netlify.app.d/atmors-netlify-app-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-12/atmors.netlify.app.d/atmors-netlify-app-report.pdf"
...
We find that the audited page has a strong SEO footing, scoring 84 out of 100, and it already benefits from core security headers such as HTTPS and HSTS. These elements provide a strong foundations for the enhancements outlined in the following sections.
...
- **Organisation Schema**: Add a JSON-LD block defining the site’s organisation (name, URL) in the page header so machines can identify ownership and context.  
- **SameAs Links on Organisation**: Include sameAs URLs for LinkedIn or other social profiles within that Organisation schema to enable agents to cross-reference authoritative identities.  
...
We discovered that internal links contain multiple URL variants-trailing-slash and hash-fragment forms-that point to the same canonical resource, inflating crawlable URLs by a factor of twelve. Machines that skip URL normalisation will treat each variant as a distinct page, wasting token budget and risking contradictory or inflated findings; we recommend adding a <link rel="canonical"> tag to every affected URL and, when publishing a sitemap.xml, listing only one canonical form per resource.
...
We found that on the audited page, the Truncation Risk check is triggered, meaning machines may receive incomplete HTML and miss key information. Boilerplate Burial and Inline Tag Bloat also appear but affect fewer pages; they limit how quickly agents can parse the content. Addressing the truncation risk-by reducing inline payload or moving heavy resources to separate files-will give machines full visibility and has the greatest impact on resilience.
...
On the worst page (https://atmors.netlify.app/) we counted 166 of 266 elements as plain divs, a 62 % bare-div ratio that forces machines to abandon semantic structure and rely on positional inference.  
The soup is structural: a deepest chain of five nested divs indicates a drag-and-drop builder or untyped component framework feeding the markup.  
A low-cost first move is to wrap key landmarks-header, nav, main, footer, aside-in semantic tags and give remaining divs descriptive class names, which will reduce the bare-div ratio without redesigning the layout.
...
We recognise that accessibility legislation worldwide has converged on ISO 14289-1 (PDF/UA) as the technical baseline, with the European Accessibility Act-Directive (EU) 2019/882-as a highly detailed example alongside Section 508 of the US Rehabilitation Act, the UK Public Sector Bodies Accessibility Regulations 2018, and equivalent statutes in Australia and Canada.  
We also observe that an untagged PDF remains invisible to machines; search crawlers, AI systems, and automated pipelines cannot extract text, entities or structure from a scanned or image-based PDF, whereas a tagged PDF with a proper structure tree is machine-readable in the same way semantic HTML is.
