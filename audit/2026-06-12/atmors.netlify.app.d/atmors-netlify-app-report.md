...
**Finding:** Heading levels skip on the audited page `/` (for example an h2 followed by an h4), so the document outline a machine or screen reader builds does not match the visible structure.
...
The site’s SEO score of 84/100 indicates it is easily discoverable by search engines. However, opportunities exist in structured data-scoring 0/100-and discovery readiness-10/100-where adding rich metadata and discovery artefacts would give machines better context. We recommend addressing these gaps to enhance machine comprehension and unlock further visibility.
