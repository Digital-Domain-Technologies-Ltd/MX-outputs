...
reportId: "mx-allabout-network-WEB-AUDIT-20260530"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-05-30/mx.allabout.network/mx-allabout-network-report.md
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-05-30/mx.allabout.network/mx-allabout-network-report.pdf"
...
- Pages audited: 127 (full audit)
- Date: 2026-05-30
- Scores: SEO 91/100 | Accessibility 100/100 | Discovery Readiness 87/100 | Structured Data 92/100
- Scope: 127 pages audited - write "across the audited set", NEVER "site-wide"
- Schema types already present (do NOT recommend adding these): ListItem (311), WebPage (125), BreadcrumbList (98), Offer (69), ImageObject (64), BlogPosting (64), Organization (47), Person (46), SpeakableSpecification (32), Article (30), Book (26), Service (23), Question (23), Answer (23), PriceSpecification (22), CollectionPage (12), Product (12), Brand (12), AggregateRating (12), ItemList (9), Blog (6), LearningResource (6), EducationalAudience (6), PropertyValue (4), Review (4), OfferShippingDetails (4), MonetaryAmount (4), MerchantReturnPolicy (4), ProfessionalService (4), WebSite (3), AboutPage (3), AggregateOffer (3), Country (3), Occupation (3), ContactPoint (2), EntryPoint (2), FAQPage (2), DefinedRegion (2), TechArticle (2), BusinessAudience (2), CommunicateAction (1), ContactPage (1), Thing (1), AskAction (1), ProfilePage (1), OfferCatalog (1), WebApplication (1)
...
SCOPE: 127 pages were audited. Scope all claims to "the audited set" - do not say "site-wide" unless cross-page consistency is explicitly confirmed in the facts.
...
- Lowest machine-readiness score: 87/100
-->
...
Facts (do not change any number, percentage, URL, page count, or name):
- Pages audited: 127
- Performance: 284ms avg (Excellent)
...
|-----------|-------|--------|-------|----------|
| Discovery Readiness | 87/100 | Excellent | A | D (median) |
| Structured Data Quality | 92/100 | Excellent | A | C (median) |
| MX Stack Completeness | 95/100 | Excellent | A | C (median) |
...
**Evidence:** MX Stack Completeness 95/100 | Structured Data Quality 92/100 | Discovery Readiness 87/100 | Consistency 99%
...
- Security headers: 5/5 present (HTTPS, HSTS, CSP, X-Frame-Options, X-Content-Type-Options); 132 of 133 audited URLs carry all five
- Structured Data Quality: 92/100
- Consistency: 99%
-->
...
|-----------|-------|------------|
| Performance | Excellent | Excellent - 284ms average load time |
| SEO (content pages) | 91 | Excellent - titles, meta descriptions, canonical URLs in place |
| Security | 5/5 | 5/5 headers present; 132 of 133 URLs carry all five |
...
- Accessibility is compliant across the audited set: Pa11y reports 100/100 with zero WCAG 2.1 AA errors on 127 pages.
...
- Div Soup audited page count: 133
...
FACTS (do not change any number, percentage, URL, page count, or name):
- Pages audited: 127 - scope all Priority descriptions to "the audited page" (singular) or "the audited set" (plural); do NOT say "site-wide" or "across the site"
...
- Performance: 284ms average load time
...
**Slowest.** The slowest page is `https://mx.allabout.network/blog/the-new-web-agentic-era-infrastructure.html`. A first-time visitor sees the cold-cache cost: the crawler recorded 3802 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three cache-busted re-probes that followed returned 451ms, 238ms, 261ms, giving a returning-visitor median of **261 ms**. **Returning-visitor verdict: Healthy**.

**Median-load control.** The median-load control page is `https://mx.allabout.network/blog/profiles/about.claude.code.html`. A first-time visitor sees the cold-cache cost: the crawler recorded 227 ms on its initial fetch. **First-visit verdict: Healthy**. Three cache-busted re-probes that followed returned 287ms, 402ms, 324ms, giving a returning-visitor median of **324 ms**. **Returning-visitor verdict: Healthy**.
...
|-----------|---------|------------|
| `<loc>` URLs | 115 entries | Matches crawl count |
...
| Probed at | 2026-05-30T00:14:18.420Z |
...
| WebPage | 99 | 100% | 100% | Website, WebSite, Organization, Person |
| BreadcrumbList | 98 | 100% | 100% | - |
| Offer | 13 | 100% | 91% | Organization, Service, Country, PriceSpecification |
| ImageObject | 41 | 100% | 100% | - |
| BlogPosting | 64 | 100% | 100% | Person, Organization, Reference, Website |
| Organization | 43 | 100% | 100% | Person, ContactPoint, Reference, Website |
| Person | 41 | 100% | 100% | Organization, PropertyValue |
| SpeakableSpecification | 32 | 100% | 100% | - |
| Article | 30 | 98% | 93% | ImageObject, Person, Organization, Reference |
| Book | 6 | 100% | 100% | Person, Organization, Offer, Reference |
| Service | 10 | 100% | 84% | Organization, Service, Website, Reference |
...
We identified 31 specific Schema.org property gaps. Each row names a single missing property on a single entity with a short note on why it matters to machines.

The full per-entity list is delivered alongside this report as a sidecar CSV: [`mx-allabout-network-structured-data-findings.csv`](mx-allabout-network-structured-data-findings.csv). The 31 rows describe individual Schema.org property gaps on specific entities; most of them share a small number of underlying patterns, shown below ranked by instance count.

| Type | Severity | Property | Instances | Pages | Why it matters |
|------|----------|----------|----------:|------:|----------------|
| Service | recommended | offers | 5 | 5 | Service has no Offer block; pricing structure invisible |
| Service | recommended | image | 4 | 4 | Service has no representative image |
...
| Article | required | author | 2 | 2 | No attribution surface - AI agents cannot identify who wrote the article |
| Article | recommended | image | 2 | 2 | Search snippets and social shares show no thumbnail |
| Article | recommended | publisher | 2 | 2 | AI agents cannot identify the publishing organisation; brand attribution lost |
| Article | recommended | dateModified | 2 | 2 | Crawlers cannot tell when the article was last updated; freshness signals stale |
...
| Metric | Count |
|--------|-------|
| AI crawler hits (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, …) | 1347 |
| AI referral hits (human visits from chat surfaces) | 1 |
| Total AI-attributable visits | 1348 |

**Top referring AI surfaces:**

| Agent | Event Type | Hits |
|-------|-----------|------|
| amazonbot | crawler | 507 |
| chatgpt | crawler | 264 |
| claude | crawler | 211 |
| meta-ai | crawler | 193 |
| ccbot | crawler | 82 |
| bytespider | crawler | 40 |
| perplexity | crawler | 33 |
| gemini | crawler | 16 |
| claude | referral | 1 |
| you | crawler | 1 |
...
- **Truncation Risk** - Pass · 125/125
  - *Means:* Every page is well under the 250 KB hard ceiling at which some server-side agents stop reading, and main content sits within the 50-100 KB offset windows. The largest page is 92 KB.
  - *Data:* Largest page: 92 KB. Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows.
- **SPA Shell** - Pass · 125/125
...
- **Boilerplate Burial** - Pass · 125/125
...
- **Tabbed Disclosure** - Pass · 125/125
...
- **Broken Code Fences** - Pass · 125/125
...
- **HTTP Content Negotiation (Vary)** - Pass · 125/125
...
- **Cross-Host Redirect** - Pass · 125/125
...
- Pages affected by that check: 1 of 125
...
- Pages contributing to the Div Soup aggregation: 133
...
| HSTS | Yes (132/133) | Forces HTTPS, prevents downgrade attacks |
| Content-Security-Policy | Yes (132/133) | Prevents XSS and injection attacks |
| X-Frame-Options | Yes (132/133) | Prevents clickjacking |
| X-Content-Type-Options | Yes (132/133) | Prevents MIME-type sniffing |

Header coverage is uneven across audited responses: HSTS (Strict-Transport-Security) on 132/133 audited responses, Content-Security-Policy (CSP) on 132/133 audited responses, X-Frame-Options on 132/133 audited responses, X-Content-Type-Options on 132/133 audited responses. The header set is configured per route or per virtual host rather than uniformly at the origin or CDN edge; a single server-config change brings the missing responses in line with the strongest baseline already in place.

**Coverage:** 132 of 133 audited URLs carry all five headers (`/about` is the exception).
...
HTTPS: 133/133 | HSTS: 132/133 | CSP: 132/133 | X-Frame-Options: 132/133 | X-Content-Type-Options: 132/133
...
| Canonical URL | 100% | - |
| Exactly 1 H1 | 100% | - |
| Code examples present | 17% | 104 |
| Self-contained sections | 100% | - |
| Error/troubleshooting docs | 6% | 117 |
...
| Check | Result | Notes |
|-------|--------|-------|
| Organisation name parity | Pass | Organisation name appears consistently across all 125 audited pages |
| Canonical URL duplicates | Pass | No duplicate canonical URLs detected across the 125-page audited set |
...
FACTS:
- Scores: AI Suitability: -/100; Accessibility: -/100; SEO: 91/100; Structured Data: 92/100; Discovery Readiness: 87/100
- Human visitors: served well
- Machine opportunity: structured data, discovery, metadata

DO_NOT:
- Say "site-wide" unless consistency was explicitly confirmed. 127 pages were audited; scope claims to "the audited set".
-->
...
- **`/blog`**: SEO 98 · A11y 100 · Back 100 · Served 37 · Rendered 37
...
*Showing the first 25 of 127 audited pages; the remaining 102 are in `mx-allabout-network-pages-audited.csv` next to this report.*
...
**Scope:** 127 pages analysed | Platform: GitHub Pages | Analysis method: Hybrid (automated + manual verification) | robots.txt: Found
