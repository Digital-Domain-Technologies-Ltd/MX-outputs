...
- **Origin server is slow for an agent fetch** (-5): origin verdict slow, slowest median 251ms
- **Bare-div nesting (div soup) across most pages** (-8): 5 of 12 pages are heavy with bare-div nesting

Across the audited set, your website delivers a strong foundations for human visitors. The SEO score of 84/100 reflects well-structured content, clear navigation and keyword optimisation that help users find what they need quickly.  

We identified six distinct WCAG AA issue types affecting 100 elements across the audited set; each can be resolved by a single template adjustment. Machines can already parse the served HTML (AI suitability 100/100), but without governance metadata they lack the context to interpret it fully. Adding these tags will raise the MX Stack Completeness score from 43/100 to above 60, meeting the threshold for Level 2. With these fields in place, machines will have the structured context required for accurate comprehension.
...
Across the audited set, we find the site delivers a strong experience for human visitors, with good performance and accessibility scores and an excellent SEO score.
...
We found that across the audited set, your pages demonstrate a strong foundations in key areas-SEO scores of 84/100 and content-page performance at 83/100, accessibility at 73/100 with consistent template patterns, and a baseline of two essential security headers. These strengths provide a strong groundwork for the targeted improvements outlined below.
...
**Finding:** Heading levels skip on `/about` (for example an h2 followed by an h4), so the document outline a machine or screen reader builds does not match the visible structure.
...
**Finding:** The slowest audited page took 5072 ms on a first-time (cold-cache) fetch, well above the healthy origin-response band, so a first-time visitor or an agent arriving on a cold cache waits noticeably for the first byte (returning visitors with a warm cache are served in about 251 ms).
...
- **sameAs** links on Organisation entities: linking ORCID, Wikidata, and LinkedIn via sameAs gives machines richer identity context and improves discoverability of your organisation.  
- **BreadcrumbList** markup on deep pages that currently rely only on URL path: adding explicit breadcrumb JSON-LD provides agents with clear navigational structure and enhances breadcrumb extraction for deeper content.  
- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)) in robots.txt to declare content-use policy for machines: this signals to bots how the content may be reused, improving compliance and trust.
...
Across the audited set, five of the twelve pages trigger a **Truncation Risk** check.  
Because the page size or offset exceeds what a machine can fetch safely, those agents may miss part of the content and cannot fully understand the page’s information.  
Fixing this by reducing the page size or adjusting the safe-fetch offset will give machines complete access to the content, making it the most impactful improvement for strengthening your site’s resilience.
...
Across the audited set, the served surface of the worst-page at https://dotfusion.com/about shows a bare-div ratio of 73 %, meaning machines lose structural context and must rely on positional inference to determine meaning.  
The soup is surface-wide - a high bare-div ratio with shallow chains (deepest chain length 5) - indicating the source pipeline likely relies on drag-and-drop builders or untyped component frameworks that inject many generic divs.  
Our cheapest first move is to wrap obvious landmarks such as header, nav, main, footer and aside in semantic elements and assign meaningful class names to remaining divs so the bare-div ratio drops without restructuring the layout.
...
We note that accessibility legislation worldwide has converged on ISO 14289-1 (PDF/UA) as the technical baseline for PDFs, with the European Accessibility Act serving as a precisely codified example of this global alignment. We also recognise that an untagged PDF remains invisible to machines-search crawlers and AI systems cannot extract text or structure from scanned or image-based files-whereas a tagged PDF equipped with a proper structure tree becomes machine-readable in the same way semantic HTML is.
...
AI suitability is 100/100, showing that agents can fully understand the content.eract with your content. The lowest-scoring dimensions are Structured Data (44/100) and Discovery Readiness (20/100), which limit how effectively search crawlers and other bots discover and interpret your pages. We recommend prioritising improvements in these areas to unlock greater machine visibility and engagement.
