...
reportId: "crowdfavorite-com-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://crowdfavorite.com --pages 10"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/crowdfavorite.com/crowdfavorite-com-report.md
  ...
  runbook: "Executive audit report for Crowdfavorite. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/crowdfavorite.com/crowdfavorite-com-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/crowdfavorite.com/crowdfavorite-com-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/2026-06-16/crowdfavorite.com/crowdfavorite-com-report.pdf"
...
**Date:** 16 June 2026\
**Report ID:** crowdfavorite-com-WEB-AUDIT-20260616
...
- **Origin server is slow for an agent fetch** (-5): origin verdict slow, slowest median 1200ms
...
Across the audited set, your brand delivers a polished experience that resonates with visitors. The SEO foundation is solid-an excellent score of 95/100 demonstrates that search engines can easily find and understand your content. Coupled with clear navigation, engaging copy, and consistent visual identity, users are guided effortlessly to the information they seek.

The headline opportunity for machine experience lies in governance metadata. First, accessibility requires attention; we identified 11 distinct WCAG AA issue types affecting 540 instances-each category can be addressed by a single template change that would resolve many occurrences. Second, the AI suitability score of 29/100 shows that machines currently struggle to parse the served HTML, highlighting an opportunity for markup refinement. Finally, while structured data quality is strong (90/100), the site lacks MX-namespaced governance fields. Score thresholds for Governed are met (MSC 73, SDQ 90, DR 40), but without mx:status, mx:contentType, mx:audience, canonicalUri, and provenance markers a machine can discover your pages yet cannot fully comprehend their context. Adding these fields to at least one published page will elevate the site from Discoverable to Governed.
...
Across the audited set, we find the site delivers a strong overall human experience, with excellent SEO and good accessibility, but page load times could be improved.
...
We find that across the audited set, machines can discover and parse content with a Discovery Readiness score of 40/100, yet lack governance context as MX Readiness Level is 1; structured data quality stands at 90/100, MX stack completeness at 73/100, and pipeline survivability at 63/100.
...
Across the audited set your pages exhibit a strong foundations in SEO (95/100), accessibility (75/100), security headers (5/5 present on 11 of 12 URLs), and structured data quality (90/100). These strengths provide the groundwork for targeted improvements that will further elevate machine comprehension and user experience.
...
**Finding:** The slowest audited page took 7140 ms on a first-time (cold-cache) fetch, well above the healthy origin-response band, so a first-time visitor or an agent arriving on a cold cache waits noticeably for the first byte (returning visitors with a warm cache are served in about 1200 ms).
...
- Add the missing response headers at the server or CDN edge; each is a one-line directive that applies across the audited set once configured.onfigured.
...
- **sameAs**: link Person and Organisation entities to ORCID, Wikidata, LinkedIn; gives machines richer identity context and cross-reference capability across your content.  
- **SpeakableSpecification**: add CSS selectors on Article pages; enables voice assistants to surface article text directly from the page markup.  
- **Content-Signal directives** ([contentsignals.org](https://contentsignals.org)): declare content-use policy in robots.txt; informs machines how they may reuse or transform your content.
...
The sitemap lists multiple variants for the same canonical page-trailing-slash and hash-fragment forms-which means AI agents that do not normalise URLs will fetch the identical content repeatedly. To resolve this, we recommend publishing one canonical URL per resource in sitemap.xml and adding a <link rel='canonical'> tag to each affected URL.
...
| Attribute | Value |
|-----------|-------|
| Origin | https://crowdfavorite.com |
...
We found that across the audited set every page suffers from truncation risk, meaning machines may not retrieve full content and could miss key information. The most impactful fix is to adjust server responses so the entire page fits within the safe-fetch offset, eliminating truncation. Additionally, addressing boilerplate burial, inline tag bloat, and delayed content start will further strengthen resilience by ensuring complete, fast, and clean parsing for machines.
...
We found that the worst page, https://crowdfavorite.com/campaign/digital-summit-atlanta-2022, contains 1024 of 1812 elements as bare divs, a 57 % ratio; this high proportion removes structural cues and forces machines to rely on positional inference.  
The soup is surface-wide with a high bare-div ratio but only a shallow deepest chain of five levels, indicating that the page was likely assembled by a drag-and-drop builder or an untyped component framework that injects many divs without semantic markup.  
A low-cost first step is to wrap the main landmarks-header, nav, main, footer, aside-with their semantic tags and assign meaningful class names to remaining elements; this reduces the bare-div ratio while preserving layout.
...
We note that legal frameworks worldwide-from the EU’s EAA to Section 508 in the US and equivalent rules in the UK, Australia and Canada-converge on ISO 14289-1 (PDF/UA) as the technical baseline, with the EAA serving as a precisely codified example of this global alignment.  
We also observe that an untagged PDF remains invisible to machines such as search crawlers and AI systems, whereas a PDF that includes a proper structure tree becomes machine-readable in the same way semantic HTML is.
...
The site scores 95/100 in SEO, signalling high relevance to agents.d discoverability. However, opportunities remain in discovery readiness (40/100) and AI suitability (29/100), where structured data and metadata are under-utilised for machine comprehension. We invite you to address these gaps to unlock greater machine engagement.
...
**Scope:** 11 pages examined | Platform: Unknown Platform | Frameworks: **React** (medium confidence) -  JS framework; **Tailwind CSS** (medium confidence) -  CSS framework; **Foundation** (medium confidence) -  CSS framework; **Gutenberg (WordPress block editor)** -  CMS plugin; **WooCommerce** (low confidence) -  CMS plugin; **Yoast SEO** (low confidence) -  CMS plugin; **Cloudflare CDN** (medium confidence) -  CDN / delivery | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (5 directives)
