...
reportId: "dkd-de-de-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://dkd.de/de --pages 10"
description: "Executive audit report reviewing accessibility, performance, SEO, structured data, and AI agent compatibility for Dkd"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/dkd.de-de/dkd-de-de-report.md
  ...
  runbook: "Executive audit report for Dkd. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/dkd.de-de/dkd-de-de-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/dkd.de-de/dkd-de-de-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/dkd.de-de/result-copy/dkd-de-de-report.pdf"
...
**Date:** 16 June 2026\
**Report ID:** dkd-de-de-WEB-AUDIT-20260616
...
**About sample scope.** Findings throughout this report describe what we observed on the 8 pages we crawled. Verdicts scoped to the sample should not be extrapolated to the full estate without a wider audit; where a finding is structural (a missing security header, a soft 404 pattern, an llms.txt transport problem) we say so.
...
Agent Readability was adjusted down by 5 points for site-wide gaps a machine cannot work around:

- **Origin server is slow for an agent fetch** (-5): origin verdict slow

Across the audited set, your brand delivers a polished experience for visitors. The SEO foundations are solid, reflected in an excellent score of 92/100, and the clean, accessible design ensures that users can find and consume content effortlessly.  

The headline opportunity is to elevate machine comprehension by embedding governance metadata. Across the audited set we found no MX governance markers; adding full MX fields-mx:status, mx:contentType, mx:audience, canonicalUri, provenance markers-will give machines the structured context they need for accurate attribution. AI Suitability at 77/100 shows that machines can parse the served HTML, but without these governance layers the site remains only Discoverable. Raising MSC above 60 and Discovery Readiness above 40 will move the set from Level 1 (Discoverable) to Level 2 (Governed).
...
Across the audited set, we find that the pages deliver a strong experience for human visitors, with excellent accessibility and SEO scores and good average performance.
...
Across the audited set, we find that machines can discover and parse your content but lack governance context, as reflected by a Discovery Readiness score of 25/100, Structured Data Quality of 64/100, MX Stack Completeness of 62/100, Pipeline Survivability of 75/100, and an overall MX Readiness Level of 1 (Discoverable).
...
| Structured Data Quality | 64/100 | Good | B | B (median) |
| MX Stack Completeness | 62/100 | Good | B | B (median) |
| Pipeline Survivability | 75/100 | Good | B | A (median) |
...
**Evidence:** MX Stack Completeness 62/100 | Structured Data Quality 64/100 | Discovery Readiness 25/100 | Consistency 63%
...
Across the audited set, your pages show a strong foundations with an impressive 92/100 SEO score, perfect accessibility compliance, and strong structured-data quality at 64/100. These strengths provide the groundwork for further enhancements.
...
|-----------|-------|------------|
| Performance | Good | Good  -  1853ms average load time |
| SEO (content pages) | 92 | Excellent  -  titles, meta descriptions, canonical URLs in place |
| Security | 4/5 | 4/5 headers present (X-Frame-Options absent); 0 of 8 URLs carry all five |
| Structured Data | 64 | Good  -  JSON-LD on every page with valid Schema.org vocabulary |
| Heading Quality | 90 | Excellent  -  single H1 on every page |
| Consistency | 63% | 63%  -  same metadata patterns across every page |
| Agent access | 8/8 | every tested agent receives HTTP 200 |

**Positive patterns observed:**

- Accessibility is compliant across the audited set: Pa11y reports 100/100 with zero WCAG 2.1 AA errors on 8 pages.
...
| 7 | Schema.org coverage is partial: Decoration (SDQ 64/100) | Machine Readability Opportunity | Medium | Medium | Agents can partially parse structured facts but key properties may be missing |
...
| 9 | No sitemap.xml | Machine Readability Opportunity | Medium | Low | Crawlers rely on link discovery only; unlisted pages may not be indexed |
...
**Finding:** 1 of 96 images (1%) on the audited set carry no alt text, so their content is unavailable to assistive technology and to machines reading the page.
...
**Finding:** Heading levels skip on 6 audited page(s) (for example an h2 followed by an h4), so the document outline a machine or screen reader builds does not match the visible structure.
...
**Finding:** The slowest audited page took 4239 ms on a first-time (cold-cache) fetch, well above the healthy origin-response band, so a first-time visitor or an agent arriving on a cold cache waits noticeably for the first byte.
...
**Finding:** 8 Schema.org property gap(s) on the audited set across WebSite: required or recommended properties are missing, so machines extract these entities less reliably.
...
**Priority 7: Schema.org coverage is partial: Decoration (SDQ 64/100)**

**Bucket:** Machine Readability Opportunity

**Finding:** Schema.org coverage is partial: Decoration (SDQ 64/100) (Homepage). Agents can partially parse structured facts but key properties may be missing
...
- **potentialAction on WebSite**: add a potentialAction property to the WebSite entity to expose contact or support capabilities, allowing agents to identify how users can reach out directly from the structured data.
...
| Measure | Result |
|---------|--------|
| Accessibility-tree score | 42/100 |
| Pages checked | 5 |

Across the 5 pages we checked, 7 distinct issue patterns reduce what reaches the tree. 4 of these repeat across pages with the same structure, which marks them as template-level: one change in the shared component clears the finding everywhere it appears.

**Element exposed as an image with no accessible name** (WCAG 2.1 1.1.1)

An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Repeats on 4 of 5 pages with the same structure (a template-level pattern).
...
An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Seen on `/de/leistungen/`.

*The fix:* Add an aria-label (or a title element inside the SVG) in the component, or mark it decorative with aria-hidden when it carries no meaning.

**Link whose accessible name carries no destination meaning** (WCAG 2.1 2.4.4)

The link's accessible name is a generic phrase or a bare URL. In the accessibility tree, and in any links-only projection of the page, every such link reads identically: the destination is unknowable without the surrounding layout, which machines and screen reader users do not have. Seen on `/en/`.
...
An image whose name or description suggests it carries data (a chart, a price list, a menu, a timetable) has no adjacent text equivalent. The data exists in exactly one channel - pixels - which no agent and no screen reader can read. Seen on `/en/`.
...
The full set, one row per pattern with every affected page counted, is recorded in the `dkd-de-de-accessibility-tree.csv` sidecar alongside this report.
...
**Slowest.** The slowest page is `https://www.dkd.de/de`. A first-time visitor sees the cold-cache cost: the crawler recorded 4239 ms on its initial fetch. **First-visit verdict: Slow: investigate origin**. Three fresh re-probes that followed returned 175ms (HTTP 503), 242ms (HTTP 503), 228ms (HTTP 503); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: rate_limited_after_pacing**.

**Median-load control.** The median-load control page is `https://www.dkd.de/de/leistungen`. A first-time visitor sees the cold-cache cost: the crawler recorded 1115 ms on its initial fetch. **First-visit verdict: Healthy**. Three fresh re-probes that followed returned 138ms (HTTP 503), 138ms (HTTP 503), 124ms (HTTP 503); no median is reported because no sample returned a usable timing. **Returning-visitor verdict: rate_limited_after_pacing**.

**Verdict:** Returning-visitor cost not measurable for both pages: every fresh re-probe returned 429/503 despite spaced pacing and rotated User-Agents (Chrome 120, Firefox 122, Safari 17). The origin enforces a rate-limit window on follow-up requests independent of cache state. The first-visit cost on https://www.dkd.de/de (cold-cache baseline 4239 ms) sits in the Slow band; that signal is real and worth investigating at the origin.
...
|--------------|-------|------------|---------------|------------------------------------------|
| ListItem | 8 | 100% | 100% | Reference |
| BreadcrumbList | 8 | 100% | 100% |  -  |
| WebSite | 2 | 100% | 0% |  -  |

**Structured Data Quality:** 64/100\
**Coverage:** 8 pages with JSON-LD out of 8 total (100%)\
**Unique types:** 3

Across the 8 pages we audited, structured data is solid. Adding recommended properties and increasing type diversity on the sampled pages gives machines more to work with.
...
| Cross-entity references | 5 | 15 | Entities reference each other (nested types and @id links) |
...
- **Truncation Risk** - Fail · 8/8
  - *Means:* 8 page(s) flag for truncation risk because the combination of total bytes and main-content byte offset (worst case: offset at 63 KB) crosses one of the size/offset thresholds (50 / 75 / 100 KB content-offset windows; 250 KB hard ceiling). Agents with limited fetch windows may stop reading before they reach prose.
  - *Data:* Largest page: 238 KB ([/de/referenzen](https://www.dkd.de/de/referenzen)). Thresholds: 250 KB hard ceiling; 50/75/100 KB content-offset windows. See dkd-de-de-pipeline-truncation-risk-pages.csv (8 pages).
- **SPA Shell** - Pass · 8/8
...
- **Boilerplate Burial** - Pass · 8/8
  - *Means:* Navigation and chrome do not dominate the page; main content is reachable without wading through overhead.
  - *Data:* Highest boilerplate-to-content ratio: 0.50. Threshold: < 10 (and < 80 KB of inline head bytes).
- **Tabbed Disclosure** - Pass · 8/8
...
- **Delayed Content Start** - Pass · 1/1
  ...
  - *Data:* Content starts at up to 26% of the document on some pages. Check applied to 1 of 8 audited pages; the remaining 7 pages were skipped by a size or eligibility gate.
- **Broken Code Fences** - Pass · 8/8
...
- **HTTP Content Negotiation (Vary)** - Fail · 8/8
  ...
  - *Data:* 8 page(s) advertise format negotiation. See dkd-de-de-pipeline-http-content-negotiation-(vary)-pages.csv (8 pages).
- **Cross-Host Redirect** - Pass · 8/8
...
- **Body Content Ratio** - Pass · 1/1
  ...
  - *Data:* Average: 44%. Threshold: 30%. Check applied to 1 of 8 audited pages; the remaining 7 pages were skipped by a size or eligibility gate.
- **Inline Tag Bloat** - Fail · 8/8
  - *Means:* 8 page(s) carry inline `<style>` or executable `<script>` blocks over 500 bytes. Externalising these to separate .css/.js files lets agents skip them during cheap fetches.
  - *Data:* 8 element(s) > 500 bytes. Largest single-page inline CSS block: 9252 B. Largest single-page inline JS block: 841 B. See dkd-de-de-pipeline-inline-tag-bloat-pages.csv (8 pages).
- **Head Weight** - Pass · 1/1
  - *Means:* Head bytes are a small fraction of each page. Agents reach body content quickly.
  - *Data:* Max ratio: 0.06. Average: 0.01. Threshold: 0.50. Check applied to 1 of 8 audited pages; the remaining 7 pages were skipped by a size or eligibility gate.

**Pipeline Survivability score:** 75/100

We found that every audited page has a truncation-risk issue, meaning machines may receive only part of the content or metadata, which can prevent them from correctly interpreting the page’s purpose. Content negotiation and inline tag bloat also appear on some pages, potentially confusing agents about the correct format or inflating payloads. Fixing the truncation risk-by ensuring full responses are served-will give machines the most immediate benefit and strengthen overall resilience.
...
|--------|--------------|----------------|--------------------|
| Served and rendered | 53/100 (medium) | 26 bare divs (54% of containers, depth 3) | `div.text-columns__column` (92), `div.blog-card__info` (59), `div.text-columns.text-columns--` (33), `div.wrap.wrap--master` (8), `div.input-group.col-xs-12` (6) |

**Worst page (served and rendered are identical):** [/de/kontakt](https://www.dkd.de/de/kontakt)

Across the audited set, the worst-page at https://www.dkd.de/de/kontakt shows a bare-div ratio of 26 of 48 (54 %); these figures refer to that single page and do not represent a site-wide average, forcing machines to abandon structural cues and rely on positional inference to infer meaning.  
The soup is surface-wide, with a high bare-div ratio and shallow chains; this pattern points to a pipeline that relies on drag-and-drop builders or untyped component frameworks that emit many generic div elements.  
The most cost-effective first step is to wrap the obvious landmarks-header, nav, main, footer, aside-and assign meaningful class names to the remaining divs so that the bare-div ratio drops without restructuring the layout.
...
**Coverage:** 0 of 8 audited URLs carry all five headers; see the Security Headers appendix for the full exception list.
...
HTTPS: 8/8 | HSTS: 8/8 | CSP: 8/8 | X-Frame-Options: 0/8 | X-Content-Type-Options: 8/8
...
| MX governance tags | 0% | 8 |
| Open Graph tags | 100% |  -  |
| Twitter Card tags | 100% |  -  |
| Skip link | 0% | 8 |
| llms.txt link tag | 0% | 8 |
| Canonical URL | 100% |  -  |
| Exactly 1 H1 | 100% |  -  |
| Code examples present | 0% | 8 |
| Self-contained sections | 100% |  -  |
| Error/troubleshooting docs | 0% | 8 |
| Lighthouse heading compliance | 25% | 6 |

**Overall Consistency:** 63%

Some pages in the 8-page sample are missing metadata patterns that others carry. Machines hitting different pages get different quality data. The Missing Pages column shows where to focus on the sampled pages.
...
| Check                            | Result | Notes                    |
|----------------------------------|--------|--------------------------|
| Brand-name parity | Pass | Brand name appears consistently across all 8 audited pages |
| Canonical URL duplicates | Pass | No duplicate canonical URLs detected across the 8-page audited set |
...
We found 5 identical inline fragment(s) repeated across multiple pages, totalling 68 KB redundant bytes. Extracting these to external CSS or JS files would reduce page weight, improve cacheability, and simplify maintenance.
...
|------|-------------------:|-------------------:|------------------------------------------------------------------|
| css | 19 | 16 | .st0{fill:#B6D644;} |
| css | 9085 | 8 | /*InlineDefaultCss*/ /* default styles for extension "tx_for |
...
Across the audited set, accessibility legislation in the EU, US, UK, Australia and Canada converges on ISO 14289-1 (PDF/UA) as the technical baseline; the EAA is a precisely codified instance of this global alignment. In parallel, an untagged PDF remains invisible to machines-search crawlers, AI systems and automated pipelines cannot extract text or structure from scanned or image-based documents-whereas a tagged PDF with a proper structure tree becomes machine-readable in the same way semantic HTML is.

We linked no PDFs from the 8-page sample we crawled, and the sitemap declares no `.pdf` URLs either. This is a statement about what we sampled and what the sitemap reports, not a verdict about the wider document estate: PDFs do not appear in this count if they sit behind login forms, are linked only from uncrawled pages, are stored in unlinked directories, are kept out of the sitemap, or are hosted on third-party domains.
...
Accessibility scores 100/100 across the audited set of eight pages on https://www.dkd.de, demonstrating that your content is fully usable by all users. Key opportunities lie in Discovery Readiness (25/100) and Structured Data (64/100), where improving metadata and discovery artefacts will give machines better context for indexing and presentation. We invite you to address these gaps to enhance machine comprehension and unlock further reach.
...
| Structured Data Quality | 64/100 | Good |
...
- **`/de`**: SEO 92 · A11y 100 · Back 95 · Served 83 · Rendered 77
...
- **`/de/kontakt/ (nav)`**: SEO 88 · A11y 100 · Back 95 · Served 68 · Rendered 68
- **`/en/ (nav)`**: SEO 93 · A11y 100 · Back 95 · Served 83 · Rendered 83
...
**URL deduplication note:** 8 crawled URLs resolved to 7 unique pages after canonicalisation (inflation factor 1.14×). The following URL clusters were treated as the same page: https://www.dkd.de/de/kontakt (2 variants: https://www.dkd.de/de/kontakt, https://www.dkd.de/de/kontakt/). Fragment suffixes (such as `#` and `#/`) and trailing-slash variants are treated as identical resources by HTTP servers and search engines; this audit deduplicated them before scoring to avoid inflating per-page counts.
...
Per page, internal links range from 1 to 106, averaging 6 across 124 pages. That is sparser than typical (benchmark median 20 per page).
...
| Link class | Count |
| --- | ---: |
| Same-host internal links (all pages) | 1515 |
...
We reviewed 96 images across the audited set: 38 PNG, 33 JPEG and 25 in other or unidentified formats. 95 of 96 (99.0%) carry alt text, leaving 1 without it. Each missing alt attribute is a place where a screen-reader user or a machine reading the page gets no description of what the image shows.

On loading strategy, 96 images are marked `loading="lazy"` and 0 `loading="eager"`, so every image on the audited pages declares its fetch behaviour explicitly rather than leaving the browser to guess.
...
**Scope:** 8 pages examined | Platform: Unknown Platform | Frameworks: **Bootstrap** (medium confidence) -  CSS framework; **Foundation** (low confidence) -  CSS framework | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (39 directives)
