...
reportId: "www-dkd-de-de-WEB-AUDIT-20260520"
...
mx:
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-05-20/www.dkd.de-de/www-dkd-de-de-report.pdf"
...
**Date:** 20 May 2026\
**Report ID:** www-dkd-de-de-WEB-AUDIT-20260520
...
We audited six pages of www.dkd.de and found a well-constructed digital presence that serves human visitors with confidence. SEO foundations across the audited set are solid, recording 92/100, which places them firmly in the Excellent band. The work already done on content structure, metadata, and crawlability gives www.dkd.de a strong platform from which to grow.

Before turning to machine readiness, we want to name accessibility as a Priority 1 compliance item. We recorded 303 critical WCAG AA issues across the audited set, and we want to be clear that the remediation picture is more manageable than that number first suggests: 280 of those issues trace to 56 recurring template patterns, meaning a single theme-level correction per pattern resolves all related instances. The headline opportunity on the machine-readiness side is Discovery Readiness, which scores 25/100, with Catalogue Visibility at 10/100. These scores reflect how legible www.dkd.de currently is to machines, including search crawlers, AI agents, and automated bots, and they represent the area where targeted investment will move the needle most decisively.

Structured Data Quality sits at 65/100, Schema Maturity at Level 1 (Decoration), and overall MX Readiness at Level 1 (Basic). Schema.org JSON-LD is the highest-leverage asset available here, because every machine can read it regardless of how content is rendered or delivered. The groundwork is there; what follows in this report maps the concrete steps to build on it.
...
Across the audited set, www.dkd.de performs strongly for human visitors on performance and SEO, with accessibility representing the primary area for improvement.
...
Across the audited set, machines can reliably process content once they reach a page (Pipeline Survivability 79/100) and interpret partial structured context (Structured Data Quality 65/100), though limited discovery signals (Discovery Readiness 25/100) and an incomplete metadata stack (49/100) constrain how consistently they can locate and attribute that content without guidance.
...
Across the audited set, www.dkd.de demonstrates real strengths that provide a solid foundation for the improvements detailed below. An SEO score of 92/100, a Structured Data Quality score of 65/100, and a cross-page consistency rate of 87% reflect careful, considered work that we can build on directly.
...
| Agent access | 0/8 | all 8 tested AI user-agents received HTTP 429 (Blocked) |
...
We present the findings below as prioritised opportunities, ordered by the downstream impact each gap has on how machines read and route www.dkd.de. Discovery gaps lead the sequence because they sit at the top of every machine's ingestion pipeline: a Discovery Readiness score of 25/100 means that structured data quality at 65/100, catalogue visibility at 10/100, and metadata stack completeness at 49/100 are all harder to act on until the foundation is in place.

| # | Finding | Bucket | Priority | Effort | Impact |
|---|---------|--------|----------|--------|--------|
| 1 | Duplicate ID Attributes, WCAG 4.1.1 (5 IDs repeated across 5 pages, Accessibility 15/100) | Compliance Risk | High | Low | Assistive tech users may receive incorrect or ambiguous element references |
| 2 | Semantic Structure 53/100, worst page https://www.dkd.de/de/kontakt (26 of 48 bare divs) | Compliance Risk | Medium | Medium | Screen reader users are less likely to navigate content structure accurately |
| 3 | Security Headers 0/5 Present | Cross-cutting | High | Low | Visitors and machines risk missing expected trust signals on every response |
| 4 | Discovery Readiness 25/100, Metadata Stack 49/100 | AI Opportunity | High | Medium | Machines may miss or misrepresent www.dkd.de in agent-generated answers |
| 5 | Structured Data Quality 65/100, Schema Types Thin | AI Opportunity | Medium | Medium | Machines are less likely to attribute rich entity context to www.dkd.de |

---

**Priority 1: Duplicate ID Attributes, WCAG 4.1.1 (5 IDs on 5 Pages, Accessibility 15/100)**

**Bucket:** Compliance Risk

**Finding:** Across the audited set we recorded 5 duplicate ID values ("accessibility", "account", "alarm", "article", "bell"), each appearing 5 times across 5 pages. WCAG 4.1.1 requires that every ID attribute value be unique within a document. When the same ID appears more than once, assistive technologies may resolve references to the first matching element only, leaving subsequent elements unreachable or mislabelled. With an overall Accessibility score of 15/100, this pattern is a significant contributor to the compliance exposure we identified.

**What to change and why:**
- Remove duplicate ID values for "accessibility", "account", "alarm", "article", and "bell" so that each ID is unique within its document. This directly addresses WCAG 4.1.1 and ensures assistive technologies can resolve element references correctly, reducing the risk of mislabelled or unreachable controls for all assistive tech users.
- Because 280 of the 303 total issues (92%) trace to 56 recurring template-level patterns, resolving these duplicate IDs at the template level will propagate the fix across the audited set in a single edit per pattern, rather than requiring page-by-page remediation.
- After each template edit, re-run an automated accessibility check against the audited pages to confirm the duplicate ID instances have been cleared and no new ID collisions have been introduced.
...
**Finding:** The rendered Semantic Structure score across the audited set sits at 53/100, placing it in the medium band. The figures cited here come specifically from the worst-performing page in the audited set, https://www.dkd.de/de/kontakt, where 26 of 48 total elements are bare divs. Because the pages in this audit share a common template, the structural pattern observed on that page is likely representative of the broader template. Where content regions are composed entirely of generic container elements rather than semantic equivalents, screen readers and machines encounter a flat, undifferentiated document outline, making accurate navigation and content interpretation less reliable.

**What to change and why:**
- Audit the template regions responsible for the layout on https://www.dkd.de/de/kontakt and identify which bare div elements represent semantically meaningful document regions (navigation, main content, complementary content, footers). Replacing these with appropriate semantic elements moves the Semantic Structure score above the medium band and directly improves the document outline available to assistive technologies.
- Improving semantic structure addresses the underlying conditions that contribute to the 15/100 Accessibility score by giving screen reader users a navigable landmark hierarchy, reducing the risk that users are required to traverse undifferentiated content blocks.
- Because the same template governs multiple pages in the audited set, a single template-level edit will produce structural improvements across all pages that inherit it, improving the score for the audited set rather than for the contact page alone.

**Effort:** Medium

---

**Priority 3: Security Headers 0/5 Present**

**Bucket:** Cross-cutting

**Finding:** Across the audited set we recorded zero of the five standard security response headers present. The absence of these headers means that browsers receive no server-declared policy for content sources, framing permissions, or transport enforcement. This affects both human visitors (who lose browser-enforced protections) and machines (which may treat the absence of trust-signal headers as a confidence-reducing factor when evaluating the reliability of responses from www.dkd.de).

**What to change and why:**
- Add the five standard security response headers at the server or CDN configuration layer. Doing so at that layer means the fix applies to every response served from the audited URLs without requiring per-page edits, and it removes the 0/5 gap in a single deployment.
- Transport and content-policy headers also contribute to the trustworthiness signals that machines evaluate when indexing or citing a domain. Resolving this gap supports the broader Discovery Readiness and Pipeline Survivability scores alongside the direct user-protection benefit.
- Verify header delivery after deployment by re-running a header inspection against the audited URLs to confirm all five are present and correctly formed.

**Effort:** Low

---

**Priority 4: Discovery Readiness 25/100, Metadata Stack Completeness 49/100**

**Bucket:** AI Opportunity

**Finding:** Discovery Readiness sits at 25/100 (Needs Improvement) and Metadata Stack Completeness at 49/100 (Could Be Better). Together these scores indicate that www.dkd.de presents a thin surface to machines attempting to understand, index, or cite its content. When discovery artefacts and metadata are absent or incomplete, machines risk missing the structured signals they rely on to attribute content accurately to www.dkd.de in agent-generated answers and knowledge panels.

**What to change and why:**
- Review and complete the metadata stack across the audited set, focusing on the four WebSite properties identified as missing in the structured data findings: image, datePublished, author, and publisher. A fuller metadata surface gives machines the unambiguous signals they need to associate pages with the correct entity, improving citation eligibility for www.dkd.de.
- Address the discovery-layer gaps driving the 25/100 Discovery Readiness score by publishing /llms.txt at the site root as the first addition: this single file can move the Discovery Readiness score by up to 35 points (presence, reference, and content signals combined). Well-formed discovery artefacts are the primary mechanism by which machines enumerate and prioritise content; a score in the Needs Improvement band means machines may process fewer pages or with lower confidence than the content warrants.
- Improvements to Discovery Readiness and Metadata Stack Completeness also support Pipeline Survivability (currently 79/100) by providing machines with more complete context at each stage of content ingestion, reducing the risk of misattribution or omission in agent outputs.

**Effort:** Medium

---

**Priority 5: Structured Data Quality 65/100, Schema Coverage Thin Beyond Core Types**

**Bucket:** AI Opportunity

**Finding:** Structured Data Quality sits at 65/100 (Good). The audited set carries ListItem (11 instances), BreadcrumbList (6 instances), and WebSite (1 instance). While these core types are present and contribute to the current score, the coverage beyond navigation and site-identity types is thin. Machines rely on richer schema types to build confident entity models for an organisation and its services; a score in the Good band with limited type diversity means machines are less likely to attribute detailed, accurate entity context to www.dkd.de in knowledge representations and agent answers.

**What to change and why:**
- Identify the content types present across the audited set (services, team members, articles, events, or similar) that are not currently represented in the schema graph, and introduce appropriate schema types for those content categories. Expanding type coverage moves the Structured Data Quality score towards the Excellent band and gives machines the structured signals needed to build a more complete entity model for www.dkd.de.
- Ensure that newly added schema types include the properties machines use most heavily for entity disambiguation (such as name, url, and description equivalents for the relevant type). This reduces the risk that machines conflate www.dkd.de with other entities and improves the accuracy of citations in agent-generated answers.
- After adding new schema types, validate the markup against the relevant specification to confirm there are no property errors that would reduce machine confidence in the structured data signals.
...
- **sameAs links on Organization**: adding sameAs properties to the WebSite and BreadcrumbList-anchoring Organization entity would let machines resolve www.dkd.de to its authoritative profiles on Wikidata, LinkedIn, or similar directories, increasing citation confidence in agent-generated answers.

- **potentialAction on Organization**: declaring a potentialAction (such as a contact or search action) on the Organization entity would surface callable entry points that machines can advertise when recommending the team's services in conversational interfaces.
...
We found the robots.txt to be present and configured with 21 disallow paths, giving machines a clear map of which areas are off-limits. One sitemap reference is declared within the file, ensuring crawlers can locate the primary index.

### sitemap.xml

We did not surface data sufficient to assess the sitemap attributes in detail; a deeper review would extend that lens.

### [llms.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We did not find a llms.txt file present on www.dkd.de, meaning machines have no structured entry point for a site description, page inventory, or content policy. We recommend adding one to give machines the context they need to represent this site accurately in agent-driven responses.

### [llms-full.txt](https://mx.allabout.network/blog/llms-txt-guide.html)

We did not detect an llms-full.txt file at www.dkd.de; the endpoint returns a 404 with no corresponding entry in the sitemap or homepage discovery headers. For a content-heavy site, we recommend adding one to give machines a complete, structured view of available content without requiring them to crawl individual pages.
...
Www Dkd (www.dkd.de) is **Not Compatible** with the MX Journey; both assessed stages fail, and Search & Compare, Price Understanding, and Purchase Confidence are N/A for this site type.
...
| ---------------- | ------ | ----- | ------------- | ---- |
| Truncation Risk | Pass | 6/6 | All pages fall within the 250 KB threshold. Agents with standard fetch windows can read each page to completion. | Largest page: 231 KB. Threshold: 250 KB. See www-dkd-de-de-pipeline-truncation-risk-pages.csv (6 pages). |
...
Across the audited set, one resilience check warrants attention: Inline Tag Bloat, which we recorded on all six pages. When machines consume pages carrying large inline script and style blocks, they must parse overhead that could otherwise be skipped during a cheap fetch. Addressing Inline Tag Bloat across the full set of pages represents a clear opportunity to strengthen how www.dkd.de is read and represented by automated pipelines.
...
On the audited page at https://www.dkd.de/de/kontakt, we recorded 26 of 48 divs as bare (54%), meaning machines lose structural context and must rely on positional inference to determine meaning. The pattern here is surface-wide rather than structurally deep: a bare-div ratio of 54% against a deepest chain of only 3 suggests the source pipeline is producing components with named wrapper classes but no semantic element beneath them, a behaviour pattern consistent with untyped component frameworks or drag-and-drop builders. The most immediate gain comes from wrapping the obvious landmarks (header, nav, main, footer, aside), which would reduce the bare-div count without any restructuring of the visual layout.
...
4 of the five standard security headers are absent from every audited response: Content-Security-Policy (CSP), X-Frame-Options, X-Content-Type-Options, HSTS (Strict-Transport-Security). Adding these at the origin-server or CDN edge closes the corresponding attack surfaces without touching application code.
...
1. **Address Priority 1 findings**: we recommend addressing the 303 WCAG 2.1 AA accessibility issues identified across the 6 audited pages (regulatory exposure)
2. **Review Priority 2-3 findings**: we recommend Discovery Readiness improvements and metadata tuning that compound over time
3. **Consider optional enhancements**: we note optional patterns that give a first-mover advantage in AI search
...
Across the audited set, https://www.dkd.de performs well for human visitors, with SEO reaching 92/100, a strong foundation that machines and people alike benefit from. The clearest opportunities lie in Discovery Readiness, which sits at 25/100, and Accessibility, where the score signals meaningful room to grow, particularly in structured data and metadata that help machines interpret and surface content accurately. We welcome the opportunity to walk through these findings together and chart a practical path forward.
...
| Link class                      | Count |
| ------------------------------- | ----: |
| Same-host internal links        | - |
| External links                  | - |
| Anchor-only (`#fragment`) links | - |
| mailto / tel links              | - |
