...
reportId: "crowdfavorite-com-WEB-AUDIT-20260604"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-04/crowdfavorite.com/crowdfavorite-com-report.md
  ...
  generate:
    ...
    format: "pdf"
    output: "mx-outputs/audit/2026-06-04/crowdfavorite.com/crowdfavorite-com-report.pdf"
...
**Date:** 4 June 2026\
**Report ID:** crowdfavorite-com-WEB-AUDIT-20260604
...
PATTERN: Describe the findings as opportunities ordered by impact, not as a list of problems. State what the priority order was based on (e.g. "commerce schema gaps lead because they directly affect shopping agents", "discovery gaps lead because they block everything downstream").
...
PREFERRED: "opportunities", "areas to strengthen", "findings", "ordered by impact".
...
WRITE:
Assess the sitemap ONLY when one was found (379 is greater than 0). Name the grade and call out the most significant attribute (e.g. varied lastmod dates, missing changefreq, missing priority). If no sitemap.xml was found (379 is 0), output nothing at all (an empty string): the deterministic prose immediately below already states the absence and the recommendation, so a sentence here would duplicate it.
...
WRITE:
Assess the llms.txt ONLY when it is present (Yes is Yes): describe the structure (site description, page inventory, content policy) and affirm what is well-formed. If Yes is No, output nothing at all (an empty string): the deterministic prose immediately below already states the absence and the recommendation, so a sentence here would duplicate it.
...
WRITE:
Assess the llms-full.txt ONLY when it is present. If No is No, output nothing at all (an empty string): the deterministic prose immediately below already states the absence and the recommendation, so any sentence here would duplicate it. When present, write 1-2 sentences describing coverage (how many pages, total size, whether each page carries a canonical URL header, transport). Scope language to "the audited sample" - never extrapolate to whether the full site is content-heavy.
