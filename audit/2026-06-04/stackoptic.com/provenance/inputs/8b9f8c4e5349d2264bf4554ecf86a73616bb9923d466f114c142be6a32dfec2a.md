...
This pattern works against machine readability rather than for it. A small genuine content set (22,361 pages, much of it itself templated) is wrapped in a very large listing of templated pages about other entities, served to crawlers but withheld from human readers. A sitemap dominated by pages about third parties tells a machine little about the site itself, and breadth of this kind reads to an agent as volume without substance. It is also the sort of scaled, low-substance content that search engines discount. We assess the site's own content set in this report; the bulk listing is recorded as a structural finding, not scored, and we would steer a site away from building it.

**Review the sitemap.** It declares roughly 222,361 pages, far more than the site's genuine content. List in the sitemap only the pages the site wants machines to treat as its own content, and move or exclude the bulk listings, so a crawler is not told the site is orders of magnitude larger than it is. (This recommendation reflects the sitemap's declared composition, not the number of pages this audit sampled.)
