...
reportId: "stackoptic-com-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://stackoptic.com --pages 10"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/stackoptic.com/stackoptic-com-report.md
  ...
  runbook: "Executive audit report for Stackoptic. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/stackoptic.com/stackoptic-com-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/stackoptic.com/stackoptic-com-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/stackoptic.com/result-copy/stackoptic-com-report.pdf"
...
**Date:** 16 June 2026\
**Report ID:** stackoptic-com-WEB-AUDIT-20260616
...
- **Origin server is slow for an agent fetch** (-5): origin verdict slow, slowest median 268ms
- **Bare-div nesting (div soup) across most pages** (-8): 4 of 7 pages are heavy with bare-div nesting

Your site delivers a clear, engaging experience for visitors, with well-structured content and a polished design that supports intuitive navigation. The strongest human-experience dimension we observed is SEO, scoring 86/100 - an excellent foundation that ensures your pages are easily discoverable by users.

The headline opportunity we want to draw attention to is the governance layer that will enable machines to understand and use your content accurately. Across the audited set, accessibility presents a Priority 1 compliance item with three distinct WCAG AA issue types affecting 34 elements; addressing these will improve user experience for all visitors. Beyond accessibility, adding full MX fields - mx:status, mx:contentType, mx:audience, canonicalUri and provenance markers - will lift your pages from Level 1 (Discoverable) to Level 2 (Governed). This lever also raises the MSC above 60 and Discovery Readiness above 40, giving machines the structured context they need for accurate comprehension.
...
Across the audited set, we find the site delivers a strong experience for human visitors, with excellent accessibility and SEO scores, while page load times indicate an opportunity to improve performance.
...
Across the audited set, machines can discover and parse your content with a Discovery Readiness of 38/100, Structured Data Quality of 89/100, MX Stack Completeness of 69/100, Pipeline Survivability of 92/100, yet lack governance context as indicated by an MX Readiness Level of 1 (Discoverable).
...
The “What is Working Well” table highlights a strong foundations your team has built across the audited set, showcasing strong SEO (86/100 overall, 84/100 on content pages), high accessibility (93/100 with only three WCAG codes and 34 instances), consistent security headers, excellent structured-data quality, and consistent implementation.
...
**Finding:** The slowest audited page took 11670 ms on a first-time (cold-cache) fetch, well above the healthy origin-response band, so a first-time visitor or an agent arriving on a cold cache waits noticeably for the first byte (returning visitors with a warm cache are served in about 268 ms).
...
- **sameAs links**: linking Person and Organisation entities to ORCID, Wikidata or LinkedIn gives machines richer identity context; add to all Person and Organisation JSON-LD blocks.  
- **SpeakableSpecification**: defining CSS selectors for spoken content on BlogPosting pages lets voice assistants surface the article text; include in each BlogPosting JSON-LD.  
- **Content-Signal directives**: adding Content-Signal directives to robots.txt declares how machines may reuse your content; place them at the top of the robots.txt file.
...
Across the 6 pages we checked, 25 distinct issue patterns reduce what reaches the tree. 2 of these repeat across pages with the same structure, which marks them as template-level: one change in the shared component clears the finding everywhere it appears.

**Element exposed as an image with no accessible name** (WCAG 2.1 1.1.1)

An element declares role="img" (or is an inline SVG acting as content) with no accessible name. The tree exposes an image-shaped hole: consumers know something pictorial is there and nothing else. Repeats on 3 of 6 pages with the same structure (a template-level pattern).
...
The sitemap lists multiple variants for the same canonical page-trailing-slash and hash-fragment forms-which means AI agents and LLM pipelines that do not normalise URLs will treat each variant as a separate resource, wasting token budget and potentially generating contradictory or inflated findings. We recommend publishing one canonical URL per resource in sitemap.xml and adding a <link rel="canonical"> tag to each affected URL.
...
Inline Tag Bloat is flagged on every page we examined, which can make it harder for machines to parse the content and risk missing important information. The biggest improvement comes from moving inline styles and scripts into external files, giving machines a cleaner structure to read. Addressing this will strengthen your site’s resilience and help machines understand your pages more reliably.
...
We found that 206 of 325 elements on the worst-page (https://stackoptic.com/) are bare divs, a ratio of 63 %. This high proportion removes structural context and forces machines to rely on positional inference.  
The soup is surface-wide rather than deeply nested; the shallow chains combined with a high bare-div ratio suggest that the page was assembled using a drag-and-drop builder or an untyped component framework, which injects many generic div wrappers late in the build process.  
A low-cost first step is to replace the obvious landmarks-header, nav, main, footer, aside-with their semantic counterparts and to give remaining elements descriptive class names; this reduces the bare-div ratio without altering the visual layout.
...
We noted that accessibility legislation worldwide converges on ISO 14289-1 (PDF/UA) as the technical baseline, with the EAA in the EU being the most precisely codified example. We also observed that an untagged PDF remains invisible to machines; only a tagged PDF with a proper structure tree can be read by search crawlers, AI systems and automated pipelines just like semantic HTML.
...
Across the audited set of https://stackoptic.com, we found an AI Suitability score of 100/100, indicating that the site’s content is fully prepared for machine consumption.achine consumption by LLMs and search crawlers. However, Discovery Readiness sits at 38/100, indicating gaps in discovery artefacts and structured data exposure that limit how quickly machines can find and understand your pages; even though Structured Data scores 89/100, there remains room to enrich schema implementation. We recommend addressing these opportunities to unlock greater visibility for machines and strengthen your digital presence.
...
**Scope:** 6 pages examined | Platform: Unknown Platform | Frameworks: **Next.js** (low confidence) -  JS framework; **Tailwind CSS** -  CSS framework; **WooCommerce** (low confidence) -  CMS plugin; **Cloudflare CDN** (medium confidence) -  CDN / delivery | Analysis method: Hybrid (automated + manual verification) | robots.txt: Present (124 directives)
