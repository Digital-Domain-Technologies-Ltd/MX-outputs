...
reportId: "atmors-netlify-app-WEB-AUDIT-20260616"
reportType: "executive-sales-report"
auditTool: "web-audit-suite"
auditDate: "2026-06-16"
auditCommand: "node scripts/audit-pipeline.js https://atmors.netlify.app --pages 10"
...
mx:
  ...
  canonicalUri: https://raw.githubusercontent.com/Digital-Domain-Technologies-Ltd/MX-outputs/main/audit/2026-06-16/atmors.netlify.app.d/atmors-netlify-app-report.md
  ...
  runbook: "Executive audit report for Atmors Netlify. Focus on the highest-leverage MX opportunities surfaced by the audit. To re-run the audit from scratch (re-crawl and re-analyse), use the command in the top-level auditCommand field. Regenerate the tagged PDF with 'node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/atmors.netlify.app.d/atmors-netlify-app-report.md', which validates the report then renders it through scripts/bin/mx.pdf.sh."
  generate:
    command: "node scripts/audit-pipeline.js --gates mx-outputs/audit/2026-06-16/atmors.netlify.app.d/atmors-netlify-app-report.md"
    script: "scripts/bin/mx.pdf.sh"
    format: "pdf"
    output: "mx-outputs/audit/atmors.netlify.app.d/result-copy/atmors-netlify-app-report.pdf"
...
**Date:** 16 June 2026\
**Report ID:** atmors-netlify-app-WEB-AUDIT-20260616
...
On the audited page, we see a strong foundations for human visitors. The SEO score of 84/100 demonstrates that your content is well-structured and keyword-rich, helping users find what they need quickly. The design delivers clear navigation and engaging visuals that support a pleasant browsing experience.

The headline opportunity is to strengthen machine comprehension by adding MX governance fields-mx:status, mx:contentType, mx:audience, canonicalUri, provenance markers-to move from Level 0 to Level 2. Before that, we recommend addressing accessibility; the audit uncovered ten distinct WCAG AA issue types across forty-three critical instances, a Priority 1 compliance item that will improve usability for all users.
...
We find that the audited page delivers a solid user experience with fast loading times and excellent search visibility, though its accessibility could benefit from further refinement.
...
Machines accessing the audited page can discover and parse content yet lack governance context; discovery readiness sits at 10/100, structured data quality is 0/100, MX stack completeness 41/100, pipeline survivability 71/100, and MX readiness level indicates Not Ready.
...
On the audited page, we find a strong foundations in SEO with an 84/100 score and a baseline of two essential security headers. These strengths provide the groundwork for focused enhancements across accessibility and structured data.
...
**Finding:** Heading levels skip on `/` (for example an h2 followed by an h4), so the document outline a machine or screen reader builds does not match the visible structure.
...
**Finding:** Security headers absent: CSP, X-Frame-Options, X-Content-Type-Options (across the audited page). Missing security headers increase exposure to content injection and clickjacking

**What to change and why:**

- Add the missing response headers at the server or CDN edge; each is a one-line directive that applies across the audited page once configured.
...
- **potentialAction on Organisation**: add a simple Organisation JSON-LD with a `potentialAction` (e.g., ContactPoint) to advertise contact capabilities and give agents a clear way to interact on the audited page.
...
| Attribute | Value |
|-----------|-------|
| Origin | https://atmors.netlify.app |
...
We found a truncation risk on the audited page that stops machines from seeing the full content because the main section is cut off before the fetch completes. As a result, search crawlers and LLMs may miss key information, lowering discoverability and relevance. Fixing this by ensuring the entire page loads-such as moving critical content earlier or increasing the safe-fetch offset-offers the biggest improvement, while addressing boilerplate burial and inline tag bloat will further strengthen performance.
...
On the audited page (https://atmors.netlify.app/) we recorded a bare-div ratio of 166 of 266 elements, or 62 %; this high proportion means machines lose structural context and must rely on positional inference to determine meaning.  
The soup is surface-wide - a high bare-div ratio combined with shallow chains (deepest chain length 5) - which indicates the page was likely assembled by a drag-and-drop builder or an untyped component framework that injects markup late in rendering.  
A cost-effective first step is to wrap the obvious landmarks-header, nav, main, footer, aside-and assign meaningful class names to remaining elements so the bare-div ratio falls without restructuring the layout.
...
We note that legal frameworks worldwide now converge on ISO 14289-1 (PDF/UA) as the technical baseline for accessible PDFs, with the European Accessibility Act-an example of that convergence-codifying the requirement in the EU, while Section 508, UK Public Sector Bodies Accessibility Regulations 2018, and equivalent laws in Australia and Canada echo the same standard.  
We also recognise that machines cannot read an untagged PDF; search crawlers, AI systems, and automated pipelines fail to extract text, entities or structure from a scanned or image-based file, whereas a tagged PDF with a proper structure tree is machine-readable just as semantic HTML is.
...
On the audited page at https://atmors.netlify.app, we found a strong SEO foundation with an 84/100 score, indicating that search engines can readily discover and index its content. However, opportunities exist in structured data-scoring 0/100-and discovery readiness-10/100-where machines lack the metadata needed to understand and surface the page effectively. We recommend addressing these gaps to enhance machine comprehension and unlock further visibility.
...
**Scope:** 1 page examined | Platform: Unknown Platform | Frameworks: **Bootstrap** (low confidence) -  CSS framework | Analysis method: Hybrid (automated + manual verification) | robots.txt: Absent (HTTP 404)
