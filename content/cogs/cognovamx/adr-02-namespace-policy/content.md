...
The Gathering governs the MX open standard. CogNovaMX builds one implementation (MX OS). To prevent namespace pollution and ensure the standard remains vendor-neutral, The Gathering must clearly define which namespaces belong to the standard and which belong to implementations.
...
- `x-mx-` — MX OS-specific fields (vendor: CogNovaMX)
...
2. **Implementations use vendor prefixes** — CogNovaMX uses `x-mx-` for extensions, others use `x-{vendor}-`
...
- Cog unified spec: `specifications/cog-unified-spec.cog.md`
- Vendor extensions policy: `https://github.com/cognovamx/mx-hub/blob/main/mx-canon/mx-maxine-lives/registers/ADR/vendor-extensions-policy.cog.md` (CogNovaMX implementation)
