...
CogNovaMX uses `x-mx-` (public) and `x-mx-p-` (private) as its vendor prefixes.
...
When CogNovaMX needs fields that are not part of the open standard, they use the `x-mx-` prefix. The `x-` signals "extension, not the standard." The `mx-` signals whose extension.
...
When CogNovaMX needs fields whose values are obfuscated — only decodable by registry holders — they use `x-mx-p-`.
...
- `mx-` — this extension belongs to CogNovaMX
...
- [ADR: Vendor extensions](../../mx-maxine-lives/registers/ADR/vendor-extensions-policy.cog.md) — CogNovaMX vendor prefixes
