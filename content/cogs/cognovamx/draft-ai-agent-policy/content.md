...
mx:
  ...
  tags: [draft, ai, agent, policy, governance, specification]
...
**Version:** 1.0-draft
**Status:** Deferred draft by Tom Cranstoun
**Date:** 27 April 2026
**Author:** Tom Cranstoun
**License:** MIT
...
This note defines three conformance levels for AI/agent governance metadata (modelled on the [WCAG 2.1](https://www.w3.org/TR/WCAG21/) Level A/AA/AAA pattern):
...
This note is a draft authored by Tom Cranstoun and offered to The Gathering for review. It is currently **deferred** — the `ai.*` namespace is being incubated as a vendor extension pending concrete adoption evidence. Once adoption emerges, this note will be revised or withdrawn.
...
All field names in this note use camelCase (matching the [Schema.org Style Guide](https://schema.org/docs/styleguide.html)) and avoid regional spelling variants. The two-zone frontmatter model used throughout — Zone 1 (top-level identity fields) and Zone 2 (the `mx:` object for operational fields) — is the conventional pattern for MX-aware markdown documents. AI training policy fields complement [robots.txt](https://www.rfc-editor.org/rfc/rfc9309) for crawler-facing controls.
...
- **Profile** — A named set of fields applicable to a specific document type (e.g., `core`, `folder`, `code-file`, `media-image`, `database`).
...
- `contextProvides` declares the knowledge or capabilities an agent gains by reading this document. Agents SHOULD use it to determine whether a document is relevant to their current task before reading the full content.
...
- Agents SHOULD use `readingLevel` to select content appropriate to the user's expertise when multiple documents cover the same topic.
...
- [RFC 9309](https://www.rfc-editor.org/rfc/rfc9309) — Robots Exclusion Protocol (referenced for crawler-facing access controls)
...
| 1.0-draft | 2026-04-27 | Renamed from "MX AI/Agent Policy Standard" to "MX AI/Agent Policy note" to clarify this is a draft by Tom Cranstoun, not a ratified standard. Made the note standalone — removed cross-references to other Gathering drafts and inlined required material. |
