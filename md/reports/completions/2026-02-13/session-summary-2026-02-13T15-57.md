...
Consolidated all four MX publication repositories from git submodules into a single datalake location. Each was copied, sanitised (removed internal repo artifacts, renamed "MX-Bible" to "MX: The Corpus", cleaned GitHub URLs), had its submodule removed, and its GitHub repo archived. The result is a clean, self-contained publications directory at `datalake/manuscripts/mx-books/` with no external dependencies.
...
Copied `packages/mx-code-examples/` to `datalake/manuscripts/mx-books/mx-code-examples/`. Removed .git. Renamed "MX-Bible" to "MX: The Corpus" across all files. Rewrote README.md for public edition.
...
1. **mx-corpus/** — 82 files (manuscripts, illustrations, web appendices). Sanitised README, LICENSE, chapter-00.
2. **mx-appendices/** — 71 files (19 appendices, web/ HTML pages, site/ demo pages). All "MX-Bible" renamed to "MX: The Corpus". All GitHub URLs cleaned.
...
- **Rename:** All "MX-Bible" → "MX: The Corpus" (book title evolution)
...
- `manuscripts/bible/` directory name in Corpus (physical directory, renaming would break structure)
...
├── mx-corpus/          # The Corpus (16 chapters, ~78K words)
